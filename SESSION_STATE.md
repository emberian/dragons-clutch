...
| CORESTATE | `a1151a64b6bfa9895` | phase A: Lean spec + regen storing market + realm bumps in CoreState, zero-means-search, founding writes them; reader conversions held for phase B. Worktree. |
| CARRY | `a59254695c09d8c61` | doc §2 caller-authority suffix + digest-width pin test + §4 own-account bumps (+ seal bump, + activation-cache own-account hypothesis). Owns `hot_v3.rs` + margin gate. Worktree, Claims before Custody. |
