...
/// Exact account-rent debit of the first collateral transaction for this
/// Market input. The campaign owns its fee quote and retained payer floor;
/// this helper owns only the Token-2022 accounts that this module creates.
pub(crate) fn initial_collateral_account_rent_v1(
    rpc: &mut Rpc,
    input: &MarketRunInput,
) -> Result<u64> {
    validate_market_input(input)?;
    let wallet_count = if input.local_participant_fixture_liquidity_atoms == 0 {
        1_u64
    } else {
        2_u64
    };
    let wallets = rpc
        .minimum_balance(ACCOUNT_BYTES)?
        .checked_mul(wallet_count)
        .ok_or_else(|| Error::new("initial collateral wallet rent overflow"))?;
    rpc.minimum_balance(MINT_BYTES)?
        .checked_add(wallets)
        .ok_or_else(|| Error::new("initial collateral account rent overflow"))
}
