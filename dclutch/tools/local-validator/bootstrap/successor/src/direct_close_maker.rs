...
use crate::terminal_lifecycle::finalized_snapshot;
...
pub(crate) fn usage() -> &'static str {
    "dclutch-local-successor-bootstrap local-private-validator-direct-close-maker-v1 --rpc-url http://127.0.0.1:PORT --market MARKET --maker MAKER --market-plan ABSOLUTE_JSON --evidence ABSOLUTE_JSON [--entry-index N] [--maker-replay ADDRESS] [--execute --fee-payer-keypair ABSOLUTE_JSON]\n\
     dclutch-local-successor-bootstrap devnet-direct-close-maker-v1 --rpc-url URL --i-mean-devnet GENESIS_HASH --market MARKET --maker MAKER --market-plan ABSOLUTE_JSON --evidence ABSOLUTE_JSON [--entry-index N] [--maker-replay ADDRESS] [--execute --fee-payer-keypair ABSOLUTE_JSON]\n\
...
fn close_v1(
    ...
    // outcome of losing a permissionless race, and saying so costs one account
    // read rather than a whole authenticated snapshot.
    ...
    let Some(replay_data) = replay_account.as_ref().map(|account| account.data.clone()) else {
        ...
        write_evidence(&arguments.evidence, None, &coordinates, None, label)?;
        return Ok(());
    };
    let replay = MakerReplayRootV1::decode(&replay_data)
        .map_err(|error| Error::new(format!("maker replay {}: {error:?}", coordinates.maker_replay)))?;
    let rent_owner = Pubkey::new_from_array(replay.rent_owner());

    let snapshot = gather(&mut rpc, arguments, &coordinates, rent_owner, cluster, genesis)?;
...
fn record_pair(registry: Pubkey, schema: [u8; 32], digest: [u8; 32]) -> RecordPairV1 {
    RecordPairV1 {
        raw: record_raw(registry, schema, digest),
...
fn derive_coordinates(rpc: &mut Rpc, arguments: &ArgumentsV1) -> Result<CoordinatesV1> {
    ...
    let entry = manifest
        .entry(arguments.entry_index)
        .map_err(|error| Error::new(format!("manifest entry {}: {error:?}", arguments.entry_index)))?;
    ...
    let ordinary_witness = ordinary_witness(&arguments.market_plan)?;
    let release = build_direct_inline_ordinary_lifecycle_program_set_v1(
        ordinary_witness,
        entry.capacity_profile_id().to_bytes(),
    )
    .map_err(|error| Error::new(format!("regenerate the Direct lifecycle release: {error:?}")))?;
...
fn parse(arguments: Vec<String>) -> Result<ArgumentsV1> {
    ...
    while let Some(flag) = cursor.next() {
        ...
        match flag.as_str() {
            ...
            "--market-plan" => market_plan = Some(PathBuf::from(value()?)),
            "--entry-index" => {
                entry_index = value()?
                    .parse::<u16>()
    ...
    Ok(ArgumentsV1 {
        ...
        market_plan: market_plan.ok_or_else(|| Error::new("--market-plan is required"))?,
        entry_index,
        maker_replay,
        fee_payer_keypair,
...
mod tests {
    ...
    fn the_market_and_maker_are_required_and_the_entry_index_defaults() {
        ...
        for missing in ["--market", "--maker", "--market-plan", "--evidence"] {
