...
| Journey | `journey/` | localhost validator | JRNY: one Market's WHOLE life on a validator the tier stands up itself. Since 2026-09-06 it takes a `--checked-release-gate` instead of a `--market` nobody could compile for it: it links the relayed vertical's `substrate.rs` (as `ladder/` does), brings up the checked-mutable substrate, compiles the Market against the LIVE deployment, founds it with `campaign --founding-only`, and keeps the validator for distribution, a holder ring, two strangers admitted, a Direct Hot fill, the fee settlement, the Resolution funding ladder, the Pyth transport to Terminal, a wallet-signed redemption, CloseFund/BeginRetiring and the four checkpointed retirement packets — every one of the last five through the SHIPPED command a host runs, under one conservation ledger. **Timing and reach, 2026-09-06, hbox, into a fresh `--work` at gate revision `932edc83f`, run `20260906T110501Z`: 1m of campaign build on a warm cargo, then 16m24s of chain, and the campaign WALKED EVERY STAGE to rent recovery for the first time — 198 founding transactions and 6,184,510 CU to an Open RECOVERY-BEARING Market, the collateral distribution (4 tx, 14,624 CU), the holder ring (4 tx, 8,080 CU) and the rent sweep (2 tx, 7,517 CU), 223 transactions and 6,634,011 CU in total, all ten conservation censuses evaluated and the verdict `conserved` with no violated law.** A stage whose helper hard-errors now WRITES THE TRANSCRIPT anyway — the stage it was inside, the helper's own sentence, the finalized slot and the whole conservation aperture account by account — so a run names every wall it meets rather than the first wall costing a run to see. **Five stand, and the run named all five at once**: the admission's `PacketTooLarge` at message compilation; the fill, which cannot read the report the refused admission never wrote; the Pyth submit's `custom program error 0x8004` (`ResolutionError::FinalizedRecord`); the redemption, which needs a terminal receipt the Pyth leg never minted; and the terminal sequence, which refuses evidence carrying `direct_trading_funding_ledger` without `direct_capability_root` because the Direct first-use accounts are created together by the first trade. The Resolution funding stage is `not-driven` and says why: **the atomic founding creates the Source state, writes the activation receipt and leaves all three selected rows Active**, so a second `CreateFund` refuses to be BUILT, by name |
