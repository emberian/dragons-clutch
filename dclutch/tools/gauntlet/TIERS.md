...
| Journey | `journey/` | localhost validator | JRNY: one Market's WHOLE life on a validator the tier stands up itself. Since 2026-09-06 it takes a `--checked-release-gate` instead of a `--market` nobody could compile for it: it links the relayed vertical's `substrate.rs` (as `ladder/` does), brings up the checked-mutable substrate, compiles the Market against the LIVE deployment, founds it with `campaign --founding-only`, and keeps the validator for distribution, a holder ring, two strangers admitted, a Direct Hot fill, the fee settlement, the Resolution funding ladder, the Pyth transport to Terminal, a wallet-signed redemption, CloseFund/BeginRetiring and the four checkpointed retirement packets — every one of the last five through the SHIPPED command a host runs, under one conservation ledger. **Timing and reach, 2026-09-06, hbox, into a fresh `--work` at gate revision `932edc83f`, run `20260906T104204Z`: 1m of campaign build on a warm cargo, then ~21m of chain — 198 founding transactions and 6,265,614 CU to an Open RECOVERY-BEARING Market, the collateral distribution (4 tx, 14,624 CU) and the holder ring (4 tx, 8,080 CU), 207 transactions and 6,288,318 CU in total, conservation `conserved` at every boundary the run reached.** A stage whose helper hard-errors now WRITES THE TRANSCRIPT anyway — the stage it was inside, the helper's own sentence, the finalized slot and the whole conservation aperture account by account — so one run names every wall it met rather than the first wall costing a run to see. That run named three: the admission's missing buyer wallet, the fill that could not read the report the refused admission never wrote, and `CreateFund: FundingConjunct(SourceDestinationNotVacant …)`, which is the founding having already created the Source state this stage was trying to create. Two runs before it stopped inside the founding on a loopback transient (`did not reach finalized transaction history`), whose sentence now says whether the validator was still answering |
