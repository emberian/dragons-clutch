...
fn campaign(request: &JourneyRequestV1, progress: &mut Progress) -> Result<()> {
    ...
    progress.stages.push(json!({"stage": progress.stage, "outcome": "executed"}));
    ...
    let founding = crate::substrate::found_market(&checked, &mut rpc, &market_path, &founding_path)?;
    ...
    crate::structured_campaign::run_owned_loopback_v1(vec![
        "--rpc-url".into(), checked.rpc_url.clone(),
        "--plan".into(), checked.plan_path.display().to_string(),
        "--market-input".into(), market_path.display().to_string(),
        "--campaign-report".into(), founding_path.display().to_string(),
        "--payer-keypair".into(), payer_path.clone(),
        "--output".into(), publication_path.display().to_string(),
        "--execute".into(),
    ])?;
    ...
    let published_transactions: Vec<TransactionEvidence> = serde_json::from_value(
        publication.get("transactions").cloned().ok_or_else(|| Error::new("Structured publication command omitted transactions"))?,
    )?;
    if published_transactions.is_empty() {
        return Err(Error::new("Structured publication command reported no transactions"));
    }
