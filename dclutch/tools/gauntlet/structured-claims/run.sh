#!/usr/bin/env bash
# Execute the Structured Claims lifecycle on one fresh, checked local validator.
#
# The journey binary owns the validator child and every signer it creates under
# --work.  This wrapper deliberately owns no account prestate: a supplied
# account image would turn a validator run back into a fixture.
set -euo pipefail
...
usage() {
    echo "usage: tools/gauntlet/structured-claims/run.sh --checked-release-gate ABS_JSON --work ABS_NEW_DIR --rpc-port PORT"
}
...
[ -z "$(git status --porcelain --untracked-files=normal)" ] || {
    echo "campaign host must be a clean committed checkout" >&2
    exit 2
}
if command -v swarm-build >/dev/null 2>&1 && [ "${SWARM_BUILD_INNER:-}" != 1 ]; then
    echo "run this campaign through swarm-build" >&2
    exit 2
fi
...
seed="$(printf '%s' 'dclutch/gauntlet/structured-claims/seed/v1' | shasum -a 256 | cut -d' ' -f1)"

jq -n --arg host "$host_revision" --arg release "$revision" --arg gate "$gate_sha" \
    '{schema:"dclutch-structured-claims-campaign-source-v1",host_source_revision:$host,release_source_revision:$release,checked_release_gate_sha256:$gate,evidence_level:"local-validator"}' \
    > "$work/source.json"
...
if grep -q 'Unit run-u.*scope was already loaded' "$work/build.log"; then
    echo "scheduler executed no build" >&2
    exit 1
fi

status=0
"$repo/target/release/dclutch-journey-campaign" structured-claims \
