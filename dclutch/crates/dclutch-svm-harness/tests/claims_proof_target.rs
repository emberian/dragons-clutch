//! Real-ELF adversarial campaign for the Lean-generated claim-only executor.
//!
//! This is claim/replay projection evidence, not Direct admission or SPL
//! custody evidence. No native processor or mock adapter is registered.
...
fn projection(authority: Pubkey) -> Vec<u8> {
    ...
    for (index, value) in [0, 0, 5_000, 200].into_iter().enumerate() {
...
#[tokio::test]
async fn generated_claim_elf_executes_and_refuses_hostile_space() {
    require_sbf();
    ...
    let mut seller_nonce_overflow = projection(authority.pubkey());
    put_u64(&mut seller_nonce_overflow, 0, u64::MAX);
    push_case(
        "seller replay overflow",
        seller_nonce_overflow,
        PROGRAM_ID,
        plan.clone(),
        false,
        true,
    ...
    let (success, success_cu) = submit(
        &mut context,
        instruction(authority.pubkey(), success_state, plan.clone(), false, true),
        &authority,
    )
    .await;
    assert!(success, "claim executor success");
    eprintln!("generated claim executor success CU: {success_cu}");
    let post = context
        .banks_client
