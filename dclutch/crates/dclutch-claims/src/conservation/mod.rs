...
//! onto `dclutch_product::economic_slice::BasketAction`, and the slice kernel
//! credits or debits the aggregate's `hoard` scalar by the complete-set count
//! (`crates/dclutch-product/src/economic_slice/mod.rs:741-758`). Two distinct
...
//! (`dclutch_product::economic_slice::market_hoard`'s own doc, verified against
...
//! (`crates/dclutch-claims/src/founding_v5.rs:279-282`) and its own vector
...
//! (`crates/dclutch-source/src/principal_capacity_v1.rs:542-565`).
...
//! (`crates/dclutch-claims/src/product_basis_terminal_v3.rs:400-444`), so the
...
//! (`crates/dclutch-claims/src/composition/exposure.rs:485-535`),
...
/// exactly as `dclutch-claims-sbf` maps `dclutch_custody::Error`
...
/// (`crates/dclutch-claims/src/founding_v5.rs:276-282`), which is what
...
impl ClaimsConservationRequestV1 {
    ...
    /// (`crates/dclutch-custody/src/lib.rs:337-345`), and its meaning,
    ...
    /// over it" (`crates/dclutch-custody/src/lib.rs:348-363`). Two
    ...
    /// `crates/dclutch-claims/src/rational/replay.rs:24-28`),
    ...
    /// (`crates/dclutch-product/src/economic_slice/mod.rs:663`, and
    /// `crates/dclutch-claims/src/lib.rs:480-486` for the plan wire).
