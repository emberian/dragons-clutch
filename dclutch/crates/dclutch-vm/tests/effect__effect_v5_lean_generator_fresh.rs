//! Exact Lean-generator freshness and schema pin checks for DCE6.
...
use dclutch_vm::effect::v5::{SCHEMA_RELEASE_ID_V5, SCHEMA_RELEASE_PREIMAGE_V5};

#[test]
fn checked_in_effect_v5_abi_is_exact_lean_output() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let formal = manifest.join("../../formal/dclutch-semantics");
    let build = Command::new("lake")
        .args(["build", "DClutchSemantics.EffectProgramV5Abi"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean build: {error}"));
    assert!(
        build.status.success(),
        "DCE6 ABI build failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let generated_output = Command::new("lake")
        .args(["env", "lean", "--run", "EmitEffectProgramV5AbiRust.lean"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean generator: {error}"));
    assert!(
        generated_output.status.success(),
        "DCE6 generator failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let checked_in = std::fs::read(manifest.join("src/effect/generated_v5_abi.rs"))
        .unwrap_or_else(|error| panic!("read checked-in generated Rust: {error}"));
    assert_eq!(formatted, checked_in);
}

#[test]
fn effect_v5_schema_id_is_the_exact_sha256_preimage() {
    let temporary =
        std::env::temp_dir().join(format!("dclutch-effect-v4-schema-{}", std::process::id()));
    std::fs::write(&temporary, SCHEMA_RELEASE_PREIMAGE_V5)
    ...
    let observed = String::from_utf8(digest.stdout)
        ...
        .to_owned();
    let expected = SCHEMA_RELEASE_ID_V5
        .iter()
