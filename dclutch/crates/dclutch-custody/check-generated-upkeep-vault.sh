...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/dclutch-upkeep-vault-generated.XXXXXX")
...
if ! (cd "$formal" && lake build DClutchSemantics.UpkeepVaultV1) \
    >"$build_stdout" 2>"$build_stderr"; then
  cat "$build_stdout" >&2
  cat "$build_stderr" >&2
  exit 1
fi
if ! (cd "$formal" && lake env lean --run EmitUpkeepVaultV1Rust.lean) \
...
if ! diff -u "$repo_root/crates/dclutch-custody/src/generated_upkeep_vault_v1.rs" \
    "$emit_stdout"; then
  cat "$build_stdout" >&2
  cat "$build_stderr" >&2
  cat "$emit_stderr" >&2
