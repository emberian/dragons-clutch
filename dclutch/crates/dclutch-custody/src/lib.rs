...
pub use dclutch_registry::release_set::ExecutionRoleV1 as CallerRoleV1;
...
/// Exact token/replay facts which Custody commits after one accepted request.
/// This small host-safe mirror has the same owner and digest framing as the
/// SBF adapter, so planners never invent replay commitments for future
/// accounts.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CustodyPoststateProjectionV1 {
    /// SHA-256 of the accepted Custody request.
    pub request_digest: [u8; 32],
    /// Token or replay account debited by the request.
    pub source: [u8; 32],
    /// Token or replay account credited by the request.
    pub destination: [u8; 32],
    /// Source token balance before the request.
    pub source_before: u64,
    /// Source token balance after the request.
    pub source_after: u64,
    /// Destination token balance before the request.
    pub destination_before: u64,
    /// Destination token balance after the request.
    pub destination_after: u64,
    /// Rent principal created or recovered by the request.
    pub rent_lamports: u64,
}

/// Commit the exact poststate tuple returned in Custody's receipt evidence.
pub fn custody_poststate_commitment_v1(value: CustodyPoststateProjectionV1) -> [u8; 32] {
    dclutch_sha256_adapter::digestv(&[
        CUSTODY_POSTSTATE_DOMAIN_V1,
        &value.request_digest,
        &value.source,
        &value.destination,
        &value.source_before.to_le_bytes(),
        &value.source_after.to_le_bytes(),
        &value.destination_before.to_le_bytes(),
        &value.destination_after.to_le_bytes(),
        &value.rent_lamports.to_le_bytes(),
    ])
}

// These three said "generated by the Lean ABI" and were not: `DCLCUSR1`,
// `DCLCUSS1` and `DCLCUSC1` were typed here while `DClutch.CustodyAbi` held its
// own copy that no backend printed. The comment is now true.
/// Exact receipt magic generated by the Lean ABI.
pub use generated::CUSTODY_RECEIPT_MAGIC_V1;
/// Exact replay-state magic generated by the Lean ABI.
pub use generated::CUSTODY_REPLAY_MAGIC_V1;
/// Exact request magic generated by the Lean ABI.
pub use generated::CUSTODY_REQUEST_MAGIC_V1;

/// Public fixed-layout coordinates for one canonical [`CustodyReplayV1`].
///
/// Account-profile emitters consume these semantic-owner coordinates instead
/// of copying private generated offsets into each Trading family.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CustodyReplayLayoutV1;

impl CustodyReplayLayoutV1 {
    /// Exact replay account width.
    pub const BYTES: usize = CUSTODY_REPLAY_BYTES_V1;
    /// Two-byte schema version offset.
    pub const VERSION_OFFSET: usize = REPLAY_VERSION_OFFSET;
    /// One-byte live-status offset.
    pub const STATUS_OFFSET: usize = REPLAY_STATUS_OFFSET;
    /// One-byte caller-role offset.
    pub const CALLER_ROLE_OFFSET: usize = REPLAY_CALLER_ROLE_OFFSET;
    /// Four-byte open-Vault count offset.
    pub const OPEN_VAULT_COUNT_OFFSET: usize = REPLAY_OPEN_VAULT_COUNT_OFFSET;
    /// Thirty-two-byte release-set offset.
    pub const RELEASE_SET_OFFSET: usize = REPLAY_RELEASE_SET_OFFSET;
    /// Thirty-two-byte Market offset.
    pub const MARKET_OFFSET: usize = REPLAY_MARKET_OFFSET;
    /// Thirty-two-byte Realm offset.
    pub const REALM_OFFSET: usize = REPLAY_REALM_OFFSET;
    /// Thirty-two-byte replay-context offset.
    pub const CONTEXT_OFFSET: usize = REPLAY_CONTEXT_OFFSET;
    /// Thirty-two-byte caller-program offset.
    pub const CALLER_PROGRAM_OFFSET: usize = REPLAY_CALLER_PROGRAM_OFFSET;
    /// Thirty-two-byte immutable rent-refund offset.
    pub const RENT_REFUND_OFFSET: usize = REPLAY_RENT_REFUND_OFFSET;
    /// Eight-byte next-revision offset.
    pub const NEXT_REVISION_OFFSET: usize = REPLAY_NEXT_REVISION_OFFSET;
    /// Eight-byte generation offset.
    pub const GENERATION_OFFSET: usize = REPLAY_GENERATION_OFFSET;
    /// Thirty-two-byte last-request digest offset.
    pub const LAST_REQUEST_DIGEST_OFFSET: usize = REPLAY_LAST_REQUEST_DIGEST_OFFSET;
    /// Thirty-two-byte last-poststate commitment offset.
    pub const LAST_POSTSTATE_COMMITMENT_OFFSET: usize = REPLAY_LAST_POSTSTATE_COMMITMENT_OFFSET;
}
...
pub enum CompartmentV1 {
    ...
    None = generated::CUSTODY_COMPARTMENT_NONE_TAG_V1,
    /// Externally owned depositor, recipient, executor, or beneficiary.
    External = generated::CUSTODY_COMPARTMENT_EXTERNAL_TAG_V1,
    /// General settlement inventory.
    Settlement = generated::CUSTODY_COMPARTMENT_SETTLEMENT_TAG_V1,
    /// Market Hoard principal, never fee or liveness capital.
    HoardPrincipal = generated::CUSTODY_COMPARTMENT_HOARD_PRINCIPAL_TAG_V1,
    /// Direct or Dealer trading principal.
    TradingPrincipal = generated::CUSTODY_COMPARTMENT_TRADING_PRINCIPAL_TAG_V1,
    /// Physically segregated realized fees.
    FeeVault = generated::CUSTODY_COMPARTMENT_FEE_VAULT_TAG_V1,
    /// Present funded-liveness capital.
    LivenessVault = generated::CUSTODY_COMPARTMENT_LIVENESS_VAULT_TAG_V1,
    /// Series Ticket principal before Market creation.
    SeriesEscrow = generated::CUSTODY_COMPARTMENT_SERIES_ESCROW_TAG_V1,
    /// Present recovery-reserve capital.
    RecoveryReserve = generated::CUSTODY_COMPARTMENT_RECOVERY_RESERVE_TAG_V1,
}

impl CompartmentV1 {
    fn decode(value: u8) -> Result<Self> {
        match value {
            generated::CUSTODY_COMPARTMENT_NONE_TAG_V1 => Ok(Self::None),
            generated::CUSTODY_COMPARTMENT_EXTERNAL_TAG_V1 => Ok(Self::External),
            generated::CUSTODY_COMPARTMENT_SETTLEMENT_TAG_V1 => Ok(Self::Settlement),
            generated::CUSTODY_COMPARTMENT_HOARD_PRINCIPAL_TAG_V1 => Ok(Self::HoardPrincipal),
            generated::CUSTODY_COMPARTMENT_TRADING_PRINCIPAL_TAG_V1 => Ok(Self::TradingPrincipal),
            generated::CUSTODY_COMPARTMENT_FEE_VAULT_TAG_V1 => Ok(Self::FeeVault),
            generated::CUSTODY_COMPARTMENT_LIVENESS_VAULT_TAG_V1 => Ok(Self::LivenessVault),
            generated::CUSTODY_COMPARTMENT_SERIES_ESCROW_TAG_V1 => Ok(Self::SeriesEscrow),
            generated::CUSTODY_COMPARTMENT_RECOVERY_RESERVE_TAG_V1 => Ok(Self::RecoveryReserve),
...
mod tests {
    ...
    fn replay_is_bound_to_role_release_market_realm_context_and_generation() {
        ...
        assert_eq!(CustodyReplayLayoutV1::BYTES, bytes.len());
        assert_eq!(
            bytes.get(
                CustodyReplayLayoutV1::NEXT_REVISION_OFFSET
                    ..CustodyReplayLayoutV1::NEXT_REVISION_OFFSET + 8
            ),
            Some(state.next_revision.to_le_bytes().as_slice())
        );
        assert_eq!(
            bytes.get(
                CustodyReplayLayoutV1::GENERATION_OFFSET
                    ..CustodyReplayLayoutV1::GENERATION_OFFSET + 8
            ),
            Some(state.generation.to_le_bytes().as_slice())
        );
        assert_eq!(
            bytes.get(
                CustodyReplayLayoutV1::LAST_POSTSTATE_COMMITMENT_OFFSET
                    ..CustodyReplayLayoutV1::LAST_POSTSTATE_COMMITMENT_OFFSET + 32
            ),
            Some(state.last_poststate_commitment.as_slice())
        );
...
/// Formerly the `dclutch-custody::token_svm` crate.
pub mod token_svm;
