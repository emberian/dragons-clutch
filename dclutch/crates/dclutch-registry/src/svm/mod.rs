...
/// `crate::record::RECORD_FIRST_ACTION_V1` is where the record family
...
pub enum Error {
    ...
    ReleaseSet(crate::release_set::Error),
    /// A typed content identity refused hostile bytes.
    Content(dclutch_core_contract::Error),
}

impl From<crate::release_set::Error> for Error {
    fn from(value: crate::release_set::Error) -> Self {
...
/// (`crate::record::RECORD_INSTRUCTION_MAGIC_V1`), which owns every
