...
`dclutch-market::rent` is an SDK-free, `no_std`, `no_alloc`, safe, fixed-layout semantic contract. It has no account-memory access, PDA implementation, Rent deserialization, CPI, wallet inspection, or transaction construction.
...
Both readers are gone now. `dclutch-trading` pinned `RENT_CREDIT_BYTES_V1`
