//! Exact Lean-generator freshness check for Capability Program Set V2.
...
#[test]
fn checked_in_program_set_v2_is_exact_lean_output() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let formal = manifest.join("../../formal/dclutch-semantics");
    let build = Command::new("lake")
        .args(["build", "DClutchSemantics.CapabilityProgramSetV2"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean build: {error}"));
    assert!(
        build.status.success(),
        "Capability Program Set V2 build failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let generated = Command::new("lake")
        .args([
            ...
            "lean",
            "--run",
            "EmitCapabilityProgramSetV2Rust.lean",
        ])
        .current_dir(&formal)
        .output()
    ...
    assert!(
        ...
        "Capability Program Set V2 generator failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let temporary = std::env::temp_dir().join(format!(
        "dclutch-capability-program-set-v2-{}.rs",
        std::process::id()
    ));
    ...
    let checked_in = std::fs::read(manifest.join("src/capability_program/generated_set_v2.rs"))
