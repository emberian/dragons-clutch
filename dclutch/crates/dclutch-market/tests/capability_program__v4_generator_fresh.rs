//! Exact Lean-generator freshness check for the fixed V4 descriptor.
...
#[test]
fn checked_in_v4_offsets_are_exact_lean_output() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let formal = manifest.join("../../formal/dclutch-semantics");
    let build = Command::new("lake")
        .args(["build", "DClutchSemantics.CapabilityProgramV4Abi"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean build: {error}"));
    assert!(
        build.status.success(),
        "Capability Program V4 ABI build failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let generated = Command::new("lake")
        .args([
            ...
            "lean",
            "--run",
            "EmitCapabilityProgramV4AbiRust.lean",
        ])
        .current_dir(&formal)
        .output()
    ...
    assert!(
        ...
        "Capability Program V4 generator failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let temporary = std::env::temp_dir().join(format!(
        "dclutch-capability-program-v4-{}.rs",
        std::process::id()
    ));
    ...
    let checked_in = std::fs::read(manifest.join("src/capability_program/generated_v4.rs"))
