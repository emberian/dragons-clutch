//! Exact Lean-generator freshness check for the fixed V3 descriptor.
...
#[test]
fn checked_in_v3_offsets_are_exact_lean_output() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let formal = manifest.join("../../formal/dclutch-semantics");
    let build = Command::new("lake")
        .args(["build", "DClutchSemantics.CapabilityProgramV3Abi"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean build: {error}"));
    assert!(
        build.status.success(),
        "Capability Program V3 ABI build failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let generated = Command::new("lake")
        .args([
            ...
            "lean",
            "--run",
            "EmitCapabilityProgramV3AbiRust.lean",
        ])
        .current_dir(&formal)
        .output()
    ...
    assert!(
        ...
        "Capability Program V3 generator failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let temporary = std::env::temp_dir().join(format!(
        "dclutch-capability-program-v3-{}.rs",
        std::process::id()
    ));
    ...
    let checked_in = std::fs::read(manifest.join("src/capability_program/generated_v3.rs"))
