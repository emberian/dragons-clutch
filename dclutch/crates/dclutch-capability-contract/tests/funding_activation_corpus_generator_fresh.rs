//! Exact Lean-generator freshness check for the funding activation corpus.
//!
//! The corpus is the only thing checking `FundingLedgerV2::activate_in_place`
//! against `CapabilityFundingLedgerV2.lean`, so it is byte-gated like every
...
#[test]
fn checked_in_funding_activation_corpus_is_exact_lean_output() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let formal = manifest.join("../../formal/dclutch-semantics");
    let build = Command::new("lake")
        .args(["build", "DClutchSemantics.CapabilityFundingLedgerV2"])
        .current_dir(&formal)
        .output()
        .unwrap_or_else(|error| panic!("launch Lean build: {error}"));
    assert!(
        build.status.success(),
        "CapabilityFundingLedgerV2 build failed\nstdout:\n{}\nstderr:\n{}",
    ...
    let generated = Command::new("lake")
        .args([
            ...
            "lean",
            "--run",
            "EmitCapabilityFundingActivationCorpusRust.lean",
        ])
        .current_dir(&formal)
        .output()
    ...
    assert!(
        ...
        "funding activation generator failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&generated.stdout),
        String::from_utf8_lossy(&generated.stderr)
    );

    let temporary = std::env::temp_dir().join(format!(
        "dclutch-funding-activation-corpus-{}.rs",
        std::process::id()
    ));
    ...
    let checked_in = std::fs::read(manifest.join("src/generated_funding_activation_corpus.rs"))
