...
use crate::{
    ...
    close_maker_bundle_v1::{
        DirectCloseMakerBundleInputV1, DirectCloseMakerBundleV1,
        build_direct_close_maker_bundle_v1, validate_direct_close_maker_bundle_v1,
    },
...
pub enum DirectProgramSetErrorV4 {
    ...
    Activation,
    /// The canonical maker-replay close bundle refused.
    CloseMaker,
}
...
/// Canonical ordinary execution plus every ordered lifecycle action and the
/// capability activation that creates the root they all execute against.
///
/// Five entries since cohort-9: the maker-replay close (wall 22's missing
/// decrement) is a lifecycle action of the same standing as begin-retiring,
/// and a market founded without its entry is permanently unretirable once
/// filled -- exactly what happened to every market founded on the four-entry
/// set.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DirectInlineOrdinaryLifecycleProgramSetV1 {
    ...
    /// Complete reserved-high-selector maker-replay close V1 artifacts.
    pub close_maker: DirectCloseMakerBundleV1,
    /// Exact heterogeneous five-entry CapabilityProgramSetV2 bytes.
    pub program_set: Vec<u8>,
    /// SHA-256 identity of `program_set` selected by the Market manifest.
    pub program_set_id: [u8; 32],
}
...
pub fn build_direct_inline_ordinary_lifecycle_program_set_v1(
    ...
    let close_maker = build_direct_close_maker_bundle_v1(DirectCloseMakerBundleInputV1 {
        ordinary: &ordinary,
        capacity_profile,
    })
    .map_err(|_| DirectProgramSetErrorV4::CloseMaker)?;
    let entries = ordinary_lifecycle_entries(
        &ordinary,
        &begin_retiring,
        &native_close,
        &activation,
...
/// Hostile-decode and bind the exact five-entry Direct lifecycle release.
...
fn ordinary_lifecycle_entries(
    ...
    close_maker: &DirectCloseMakerBundleV1,
) -> Result<[CapabilityProgramSetEntryV2; 5], DirectProgramSetErrorV4> {
    Ok([
        CapabilityProgramSetEntryV2::new(
            DirectExecutionActionV3::InlineOrdinary as u32,
            CapabilityDescriptorReferenceV2::new(
        ...
        CapabilityProgramSetEntryV2::new(
            ...
            CapabilityDescriptorReferenceV2::new(
                ...
                content(close_maker.descriptor_id)?,
            ),
        ),
    ])
}
...
mod tests {
    ...
    #[test]
    fn ordinary_retirement_close_activation_and_maker_close_are_five_distinct_selectors() {
        let ordinary = crate::ordinary_bundle_v4::tests::canonical_bundle_for_cross_module_tests();
        ...
        assert_eq!(
            set.entry(4).expect("maker close").selector(),
            DIRECT_CLOSE_MAKER_SELECTOR_V1
        );
        assert_eq!(
            set.entry(3)
