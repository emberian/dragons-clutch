...
mod tests {
    ...
    fn registered_logical_lengths(sell: bool) -> Vec<u32> {
        ...
        output[13] = u32::try_from(dclutch_market::STATE_BYTES).expect("Core width");
        output[14] =
    ...
    fn signed_request_profile(requirements: &[(u16, u16, u32)]) -> Vec<u8> {
        ...
        let header = dclutch_vm::request_profile::v2::REQUEST_PROFILE_V2_HEADER_BYTES;
        ...
        put(
            &mut output,
            0,
            &dclutch_vm::request_profile::v2::REQUEST_PROFILE_V2_MAGIC,
        );
        put(
            &mut output,
            8,
            &dclutch_vm::request_profile::v2::REQUEST_PROFILE_V2_SCHEMA_VERSION.to_le_bytes(),
        );
        put(
            &mut output,
            10,
    ...
    fn transition() -> Vec<u8> {
        ...
        *output.get_mut(4).expect("transition version") = dclutch_vm::v3::VERSION;
    ...
    fn effect() -> Vec<u8> {
        ...
        put(&mut base, 0, &dclutch_vm::effect::v3::MAGIC);
        *base.get_mut(4).expect("effect version") = dclutch_vm::effect::v3::VERSION;
        put(&mut base, 12, &1_u16.to_le_bytes());
        put(&mut base, 16, &1_u16.to_le_bytes());
        put(&mut base, 20, &1_u16.to_le_bytes());
        let bytes = dclutch_vm::effect::v4::HEADER_BYTES_V4 + base.len();
        let mut scratch = vec![0_u8; bytes];
        let mut output = vec![0_u8; bytes];
        dclutch_vm::effect::v4::encode_program_v4_atomic(
            &base,
    ...
    fn program_set_descriptor_config_and_profile_substitution_refuse() {
        ...
        *wrong_lifecycle
            .get_mut(dclutch_vm::account_profile::lifecycle_v3::HEADER_BYTES)
