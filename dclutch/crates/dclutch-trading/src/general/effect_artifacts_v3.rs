...
/// schema -- `dclutch_vm::effect::v4::SCHEMA_RELEASE_ID_V4` -- and refuses
...
///
/// The values live in [`crate::general::escrow_v1`]. An action that reaches a template
...
fn action_template_compartments(action: Action) -> Result<(CompartmentV1, CompartmentV1)> {
    crate::general::escrow_v1::general_action_template_compartments_v1(action)
        .ok_or(GeneralEffectArtifactErrorV3::Geometry)
}
        ...
        for count in [1_u32, 258] {
            ...
            for operation in 0..54_u16 {
                ...
                if (38..=51).contains(&operation) {
                    assert_eq!(resolved, dclutch_vm::effect::v3::ResolvedEffectV3::Noop);
                }
            }
            ...
            for (operation, tail, value) in [
                ...
                assert_eq!(
                    ...
                    Ok(dclutch_vm::effect::v3::ResolvedEffectV3::WriteScalar {
