...
fn general_child_frame_end_v3(action: Action) -> Result<u16> {
    ...
    crate::general::effect_artifacts_v3::general_effect_account_count_before_system_v3(action)
...
///
/// Every one of the eight seed orders in [`crate::general::state_seeds_v3`] opens with
...
const fn general_system_program_operation_index_v3(action: Action) -> Option<u16> {
    if crate::general::state_artifacts_v3::general_declares_system_program_v3(action) {
...
fn close_candidate_batch_account() -> Result<AccountCoordinateV2> {
    let selected =
        crate::general::state_artifacts_v3::general_readonly_evidence_v3(Action::CloseCandidate, 0)
            .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?;
...
fn order_terms_account(action: Action) -> Result<AccountCoordinateV2> {
    let selected = crate::general::state_artifacts_v3::general_readonly_evidence_v3(action, 0)
        .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?;
...
fn evidence_account(
    ...
    let selected = crate::general::state_artifacts_v3::general_readonly_evidence_v3(action, index)
        .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?;
        ...
        0 => Ok(exact_rule(
            ...
            u32::try_from(
                dclutch_market::capability_program::CAPABILITY_ROOT_HEADER_BYTES_V1
                    .checked_add(crate::general_config::GENERAL_ROOT_BYTES_V2)
                    .ok_or(GeneralAccountRuleErrorV3::Geometry)?,
            )
            .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?,
            0,
...
fn local_state_rule(action: Action, coordinate: u16) -> Result<AccountRuleWithPrestateInputV2> {
    ...
    {
        // The batch record's fixed span; the per-outcome `(price, residual)`
        // cells of the clearing tail are the stride.
        GENERAL_BATCH_ROW_BASE_V2
        ...
        rule: AccountRuleInputV2 {
                ...
                || matches!(action, Action::Consider | Action::Freeze)
            {
                ...
                || (matches!(action, Action::PlaceOrder | Action::CancelOrder)
                    && !cancel_order_state)
            {
                u32::try_from(GENERAL_BATCH_ROW_STRIDE_V2)
                    .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?
            } else if action == Action::ReleaseOrder || cancel_order_state {
...
fn evidence_rule(kind: GeneralReadonlyEvidenceKindV3) -> Result<AccountRuleWithPrestateInputV2> {
    match kind {
        ...
        // `(price, residual)` cell pair per runtime outcome: the clearing tail
        // the joint arm gave the record.
        GeneralReadonlyEvidenceKindV3::ClosedBatch => Ok(exact_rule(
            false,
            false,
            false,
            u32::try_from(GENERAL_LOCAL_STATE_HEADER_BYTES_V3 + GENERAL_BATCH_ROW_BASE_V2)
                .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?,
            u32::try_from(GENERAL_BATCH_ROW_STRIDE_V2)
                .map_err(|_| GeneralAccountRuleErrorV3::Geometry)?,
            no_effects(),
...
fn prior_role_coordinate(
    ...
    let mut prior = crate::general::state_artifacts_v3::general_child_account_start_v3(action);
    while prior < coordinate {
        let (frame, relative) = child_coordinate(action, prior)?;
...
mod tests {
    ...
    use dclutch_vm::account_profile::lifecycle_v3::encode::LifecycleSeedInputV3;
    ...
    /// THE PHYSICAL RULE AND THE ENVELOPE ARE THE SAME WIDTH, AT EVERY WIDTH.
    ///
    /// A local-state rule declares `data_length + outcome_count x
    /// data_item_stride`; `general_local_state_len_v3` is what
    /// `encode_general_local_state_v3_atomic` actually writes. Nothing joined
    /// the two, and the joint arm proved why that matters: it made the Batch
    /// envelope Product-width and five physical sites went on declaring the
    /// 224-byte V1 prefix with a zero stride. Every one of their tests stayed
    /// green -- they were comparing a rule against itself -- and a batch
    /// account created at the rule's width would have been too small for the
    /// record the envelope writes into it.
    ///
    /// One row per (action, coordinate) that names a local state, and the
    /// widths span the runtime range the profile is encoded at.
    #[test]
    fn every_local_state_rule_declares_the_width_its_own_envelope_encodes() {
        use crate::general::local_state_v3::{GeneralLocalStateKindV3, general_local_state_len_v3};
        let rows = [
            (
                Action::OpenBatch,
                GENERAL_PRIMARY_STATE_ACCOUNT_V3,
        ...
        for width in [1_u32, 2, 13, 258] {
            for (action, coordinate, kind) in rows {
                let rule = general_account_profile_rule_v3(action, coordinate, WIDTHS)
                    .expect("local state rule");
                let declared = u64::from(rule.rule.data_length)
                    + u64::from(rule.rule.data_item_stride) * u64::from(width);
                let encoded = general_local_state_len_v3(kind, width).expect("envelope width");
                assert_eq!(
                    declared, encoded as u64,
                    "{action:?} coordinate {coordinate} at width {width}: the rule declares a \
                     width the envelope does not encode",
                );
            }
        }
    }
    ...
    fn the_window_gated_actions_declare_the_current_slot_in_their_bank() {
        for action in ACTIONS {
            ...
            let declared = matches!(
                ...
                dclutch_vm::account_profile::v2::TrustedEnvironmentV2::CurrentSlot { .. }
            );
            assert_eq!(
                declared,
                matches!(
    ...
    /// `input_identities` (`crates/dclutch-vm/account_profile/src/v2.rs:3178`),
    ...
    fn submit_candidate_profile_owns_one_fixed_candidate_and_three_exact_evidence_records() {
        ...
        for (index, kind, base, stride) in [
            (
                ...
                u32::try_from(GENERAL_BATCH_ROW_STRIDE_V2).expect("closed batch stride"),
            ),
            (
                1,
                GeneralReadonlyEvidenceKindV3::CandidateImage,
        ...
        let profile = dclutch_vm::account_profile::v2::AccountProfileV2::decode(&output)
    ...
    fn every_child_role_the_effect_routes_to_has_an_invocable_program_coordinate() {
        for action in [
            ...
            let effect = dclutch_vm::effect::v3::ProgramV3::decode(&bytes).expect("effect");
    ...
    fn child_frames_reuse_semantic_roles_through_authenticated_aliases() {
        for action in [Action::InitializeSettlement, Action::Close] {
            let child = crate::general::state_artifacts_v3::general_child_account_start_v3(action);
