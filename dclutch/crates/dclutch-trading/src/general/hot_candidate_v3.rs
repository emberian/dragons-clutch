...
const fn settlement_phase_tag(phase: crate::general::runtime_width::SettlementPhaseV2) -> u64 {
    match phase {
        crate::general::runtime_width::SettlementPhaseV2::Collecting => 4,
        crate::general::runtime_width::SettlementPhaseV2::Materializing => 5,
        crate::general::runtime_width::SettlementPhaseV2::Distributing => 6,
        crate::general::runtime_width::SettlementPhaseV2::ReadyToClose => 7,
        crate::general::runtime_width::SettlementPhaseV2::Terminal => 8,
    }
}
...
fn position_geometry(
    ...
    claims_active: bool,
    environment: GeneralHotEnvironmentV3,
    header: crate::general::runtime_settlement::RuntimeSettlementEffectHeaderV2,
    ...
    match action {
        ...
        // A CLOSE REACHES HERE ONLY IF IT STRANDS. `claims_active` is false
        // for every close whose clearing left no residual, and the early
        // return above answers those with an empty geometry. So this arm is
        // the strand and nothing else, and it refuses by that name.
        RuntimeSettlementActionV2::Close => Err(GeneralHotCandidateErrorV3::SettlementCoordinate(
            SettlementClauseV3::PositionCloseStrandUnseated,
        )),
    }
}
...
mod tests {
    ...
    /// THE STRAND HAS NO SEAT, AND THE CLOSE SAYS SO BY NAME.
    ///
    /// `runtime_settlement::close` sets `claims_active` on exactly the closes
    /// whose clearing left a residual at a zero-priced outcome
    /// (`runtime_settlement::tests` walks a real one at width 16 and asserts
    /// that flag). This is the other half of that sentence: the geometry the
    /// flag asks for does not exist, because `Action::Close` declares four
    /// child frames and none of them mutates a ProtocolPosition.
    ///
    /// The positive control is the same call with the flag clear -- the close
    /// that strands nothing projects an empty geometry and is executable
    /// today. Without it this test would pass on a `position_geometry` that
    /// refused every Close.
    #[test]
    fn a_close_that_strands_is_refused_by_name_and_one_that_does_not_projects() {
        let environment = GeneralHotEnvironmentV3 {
            settlement_position_present: true,
            close_settlement_position: true,
            rent_refund: [0x41; 32],
            ..environment()
        };
        let header = crate::general::runtime_settlement::RuntimeSettlementEffectHeaderV2 {
            outcome_count: 3,
            action: RuntimeSettlementActionV2::Close,
            ...
            terminal_coordinate: 9,
        };

        let clean = position_geometry(RuntimeSettlementActionV2::Close, false, environment, header)
            .expect("a close that strands nothing has an empty position geometry");
        assert_eq!(clean.count, 0);
        assert!(!clean.source_present);
        assert!(!clean.destination_present);
        ...
        assert_eq!(
            position_geometry(
                ...
                true,
                environment,
                crate::general::runtime_settlement::RuntimeSettlementEffectHeaderV2 {
                    claims_active: true,
                    ..header
                },
            )
            .err(),
            Some(GeneralHotCandidateErrorV3::SettlementCoordinate(
                SettlementClauseV3::PositionCloseStrandUnseated
            )),
            "a stranding close must refuse by the clause that names the missing frame",
        );
    }
