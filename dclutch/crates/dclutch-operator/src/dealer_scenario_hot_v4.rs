...
use dclutch_capability_program_contract::hot_v3::{
    HOT_ACCOUNT_PROFILE_RAW_ACCOUNT_V3, HOT_ACTIVATION_CACHE_ACCOUNT_V3, HOT_CONFIG_RAW_ACCOUNT_V3,
    HOT_CORE_PROGRAM_ACCOUNT_V3, HOT_FIXED_ACCOUNT_COUNT_V3, HOT_LINKED_BASIS_RAW_ACCOUNT_V3,
    HOT_MARKET_ACCOUNT_V3, HOT_PORTFOLIO_RAW_ACCOUNT_V3, HOT_PRODUCT_RAW_ACCOUNT_V3,
    HOT_ROOT_ACCOUNT_V3, HOT_TRADING_PROGRAM_ACCOUNT_V3, HotBumpHintsV1, HotExecutionEnvelopeV3,
};
...
pub fn project_dealer_scenario_unsplit_topology_v4(
    ...
    .map_err(|_| DealerScenarioHotMetaErrorV4::Semantics)?
    .with_bump_hints(dealer_scenario_hot_bump_hints_v4(
        state,
        &Pubkey::new_from_array(semantic.chain.trading_program),
        semantic.chain.release_set,
    )?);
...
/// coordinate of the Dealer scenario Hot frame is the Market, which is the root, and which account
...
fn dealer_scenario_hot_bump_hints_v4(
    state: DealerScenarioHotMetaStateV4<'_>,
    trading_program: &Pubkey,
    release_set: [u8; 32],
) -> Result<HotBumpHintsV1, DealerScenarioHotMetaErrorV4> {
...
mod tests {
    ...
    fn the_mined_corpus_reads_this_frames_market_root_and_custody_deployment() {
        ...
        let frame = corpus::fixed_frame();
        let state = DealerScenarioHotMetaStateV4 {
            fixed_accounts: &frame,
            strategy_accounts: &[],
            runtime_suffix_accounts: &[],
        };
        assert_eq!(
            dealer_scenario_hot_bump_hints_v4(
                state,
