...
// Four writes left with physical ABI v3: the shard Mint, the Structured
// custody Account, the Claims custody owner and the derived asset count are on
// neither the parent nor the child wire.
const EFFECT_INSTRUCTION_COUNT: usize = 26;
...
fn child_template() -> Result<[u8; RATIONAL_TERMINAL_HOT_REQUEST_BYTES_V3]> {
    ...
    put(&mut output, 8, &PHYSICAL_ABI_VERSION_V3.to_le_bytes())?;
    put(
        &mut output,
...
fn request_instructions() -> Result<[EffectInstructionV3; EFFECT_INSTRUCTION_COUNT]> {
    Ok([
        ...
        write_identity(
            ...
            RATIONAL_TERMINAL_IDENTITY_PARENT_DIGEST_V3,
        )?,
        write_identity(
            RATIONAL_TERMINAL_HOT_ACTOR_OFFSET_V3,
...
mod tests {
    ...
    fn effect_reconstructs_exact_terminal_claims_child() {
        ...
        assert_eq!(request.asset_row(0).expect("asset").coefficient, 10);
    }
}
