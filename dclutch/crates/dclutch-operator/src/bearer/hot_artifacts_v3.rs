...
use dclutch_claims::rational::{
    ...
    RATIONAL_TERMINAL_IDENTITY_ACTOR_V3, RATIONAL_TERMINAL_IDENTITY_COLLATERAL_RECIPIENT_V3,
    RATIONAL_TERMINAL_IDENTITY_DESCRIPTOR_V3, RATIONAL_TERMINAL_IDENTITY_GRAPH_V3,
    RATIONAL_TERMINAL_IDENTITY_MARKET_V3, RATIONAL_TERMINAL_IDENTITY_REALM_V3,
...
// Seven projections left with physical ABI v3: a zero receipt Account and an
// asset count of one became unrepresentable, three derived coordinate keys
// stopped riding the wire, and the actor-Position revision is absent for every
// terminal redemption. Both counts are the DECLARED LENGTH of the arrays below
// rather than a number standing beside them, so a projection added or removed
// without revisiting this line is a compile error. It was not always so: this
// count said 34 over a 33-entry array for one revision, and the whole crate
// failed at run time with `RequestProfileArtifact(InvalidLength)` -- a width
// complaint naming no field, from a buffer sized for an instruction nobody
// wrote. The effect twin next door never drifted, because its array length
// was in its return type all along.
const REQUEST_INSTRUCTION_COUNT: usize = 32;
...
-> Result<[u8; RATIONAL_TERMINAL_REQUEST_PROFILE_BYTES_V3]> {
    let fixed: [RequestInstructionV1; REQUEST_INSTRUCTION_COUNT] = [
...
mod tests {
    ...
    fn family_bytes() -> [u8; RATIONAL_TERMINAL_HOT_REQUEST_BYTES_V3] {
        let mut asset_bytes = [0_u8; ASSET_BYTES_V3];
        AssetV2 {
            shard_mint: id(20),
