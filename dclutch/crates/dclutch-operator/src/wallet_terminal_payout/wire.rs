//! The wallet terminal payout derivation, extracted from the local successor
//! bootstrap binary so a browser can run it.
//!
//! WHY THIS CRATE EXISTS. `RedeemFlow` says "This browser never creates or
//! completes a payout plan" and asks a reader to import JSON from
//! `dclutch-local-successor-bootstrap wallet-terminal-payout-input`. That made
//! redemption unreachable for a stranger holding only a wallet: the producer is
//! a Rust binary under `tools/local-validator/`.
//!
//! The producer is not pure -- it reads two files, connects its own RPC, and
//! applies a cluster-origin policy -- but its impurity is NOT AUTHORITY. What
//! it takes from `--plan` is six coordinates: five program ids the browser
//! holds from its deployment, and the release-set id the Market's own Core
//! state carries. What it takes from `--evidence` is a routing table of which
//! addresses to observe, plus a `plan_sha256` that binds the CLI's two files to
//! each other. A deployment table and an address book, and the browser derives
//! both.
//!
//! So the derivation moved here unchanged and the binary keeps its shell. This
//! crate does no file I/O, opens no socket, and knows nothing about clusters:
//! a caller hands it observed accounts and it hands back an authenticated plan.
//! It also declares no SBF program crate, which is what lets it reach wasm32 --
//! `dclutch-trading-sbf` and its siblings pin layouts for a 64-bit target, and
//! those assertions are right.
...
use crate::wallet_terminal_payout::{Error, ObservedAccountValueV1, Result, hex, hex32, pubkey};

use dclutch_versioned_message_operator::EXTEND_ADDRESSES_PER_TRANSACTION_V1;

pub const INPUT_FORMAT: &str = "dclutch-wallet-terminal-payout-plan-input-v1";
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct RecordSelectorsV1 {
    pub realm: String,
    pub product: String,
    pub result_domain: String,
    pub portfolio: String,
    pub product_basis: String,
    pub composition_descriptor: String,
    pub composition_graph: String,
    pub composition_translation: String,
    pub composition_exposure: String,
}
...
pub struct PlanInputV1 {
    ...
    pub programs: ProgramSelectorsV1,
    pub records: RecordSelectorsV1,
}
...
pub struct SelectedInputV1 {
    ...
    custody: Pubkey,
    resolution: Pubkey,
    pub realm: RecordPairV1,
    pub product: RecordPairV1,
    pub result_domain: RecordPairV1,
    pub portfolio: RecordPairV1,
    pub product_basis: RecordPairV1,
    pub composition_descriptor: RecordPairV1,
    pub composition_graph: RecordPairV1,
...
#[derive(Clone, Copy)]
pub enum LookupTableRequirementV1 {
    Present,
    Absent,
}
...
pub struct FinalizedSnapshotV1 {
    pub observation: Observation,
    pub accounts: BTreeMap<Pubkey, ObservedAccount>,
}

impl FinalizedSnapshotV1 {
    /// Build one snapshot from observed values a caller read however it reads.
    pub fn from_observed(
        slot: u64,
        unix_timestamp: i64,
        keys: &[Pubkey],
        values: Vec<Option<ObservedAccountValueV1>>,
    ) -> Result<Self> {
    ...
    pub fn account(&self, key: Pubkey) -> Result<&ObservedAccount> {
        self.accounts
            .get(&key)
            .ok_or_else(|| Error::new(format!("wallet payout snapshot omitted {key}")))
    }

    pub fn required(&self, key: Pubkey, label: &str) -> Result<&ObservedAccount> {
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct WalletTerminalPayoutManifestV3 {
    format: &'static str,
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct WalletTerminalPayoutAltPlanV1 {
    format: &'static str,
...
pub fn record_pair(registry: Pubkey, schema: [u8; 32], digest: [u8; 32]) -> RecordPairV1 {
    RecordPairV1 {
        schema,
        digest,
...
// The binary this derivation was extracted from shares one fixture with it
// (`terminal_lifecycle.rs` builds on `input()`), so the module is reachable
// downstream under a feature rather than copied into two places. A copy is
...
pub mod tests {
    ...
    fn recipient_mode_keeps_wallet_and_claim_check_routes_disjoint() {
        ...
        assert!(
            SelectedInputV1::parse_claim_check_compaction(
                &compaction,
