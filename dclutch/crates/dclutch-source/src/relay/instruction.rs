...
//! `dclutch-source`'s own instruction wire is.
//!
...
impl CreateRecordInstructionV1 {
    ...
    pub fn new(
        ...
        crate::relay::record::relayed_observation_record_bytes_v1(set_count)?;
        if seal_threshold == 0 || seal_threshold > crate::relay::MAX_RELAYER_KEYS_V1_U8 {
            return Err(Error::NonCanonicalKeySet);
        }
        crate::relay::require_nonzero(&source_material_id)?;
        crate::relay::require_nonzero(&source_spec_id)?;
        crate::relay::require_nonzero(&rent_beneficiary)?;
        Ok(Self {
            generation,
            observed_slot,
            set_count,
...
impl ConsumeRecordInstructionV1 {
    ...
    pub fn new(
        ...
        if entry_count == 0 || usize::from(entry_count) > crate::relay::MAX_RELAYED_ACCOUNTS_V1 {
        ...
        match RelayActionV1::decode(one(bytes, ACTION_OFFSET)?)? {
            RelayActionV1::CreateRecord => {
                ...
                Ok(Self::CreateRecord(CreateRecordInstructionV1::new(
                    ...
                    one(bytes, 34)?,
                    one(bytes, 35)?,
                    crate::relay::array(bytes, 40)?,
                    crate::relay::array(bytes, 72)?,
                    crate::relay::array(bytes, 104)?,
                )?))
            }
