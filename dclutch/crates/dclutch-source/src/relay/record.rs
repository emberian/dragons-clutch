...
use crate::relay::{
    ...
    RELAYED_RECORD_MAGIC_OFFSET, RELAYED_RECORD_MARKET_OFFSET,
    RELAYED_RECORD_OBSERVED_CLUSTER_ID_OFFSET, RELAYED_RECORD_OBSERVED_SLOT_OFFSET,
    RELAYED_RECORD_PHASE_OFFSET, RELAYED_RECORD_PROVIDER_RELEASE_ID_OFFSET,
    RELAYED_RECORD_RELAYER_KEY_SET_ID_OFFSET, RELAYED_RECORD_RENT_CREDIT_BENEFICIARY_OFFSET,
    RELAYED_RECORD_RESERVED_OFFSET, RELAYED_RECORD_RESERVED_TAIL_OFFSET,
    RELAYED_RECORD_SEAL_COUNT_OFFSET, RELAYED_RECORD_SEAL_THRESHOLD_OFFSET,
    RELAYED_RECORD_SEALED_BY_BITMAP_OFFSET, RELAYED_RECORD_SEALED_UNIX_SECONDS_OFFSET,
    RELAYED_RECORD_SET_COUNT_OFFSET, RELAYED_RECORD_SET_DIGEST_OFFSET, RELAYED_RECORD_SLOT_BYTES,
    RELAYED_RECORD_SOURCE_MATERIAL_ID_OFFSET, RELAYED_RECORD_VERSION_OFFSET, Result, array, i64_at,
    one, put, require_nonzero, require_zero, slice, u16_at, u64_at, variable_header,
...
impl RelayedRecordPdaSeedsV1 {
    ...
    pub const fn domain(self) -> &'static [u8] {
        crate::relay::RELAYED_RECORD_PDA_DOMAIN_V1
    }
    /// The owning Market.
...
pub fn validate_relayed_observation_record_bytes_v1(bytes: &[u8]) -> Result<()> {
    ...
    if created <= 0
        || filled > set_count
        || threshold == 0
        || threshold > crate::relay::MAX_RELAYER_KEYS_V1_U8
    {
        return Err(Error::NonCanonicalRecord);
    }
...
///
/// `seed_digest` is `SHA-256` of [`crate::relay::release::encode_set_digest_seed_preimage_v1`],
...
pub fn create_relayed_observation_record_into_v1(
    ...
    if seal_threshold == 0 || seal_threshold > crate::relay::MAX_RELAYER_KEYS_V1_U8 {
    ...
    output.fill(0);
    put(output, RELAYED_RECORD_MAGIC_OFFSET, &RELAYED_RECORD_MAGIC)?;
    put(
        output,
        RELAYED_RECORD_VERSION_OFFSET,
        &crate::relay::RELAYED_SCHEMA_VERSION.to_le_bytes(),
    )?;
...
pub fn seal_relayed_observation_in_place_v1(
    ...
    let (next_seals, next_bitmap, threshold, reached) = {
        ...
        if usize::from(member_index) >= crate::relay::MAX_RELAYER_KEYS_V1 {
            return Err(Error::NonCanonicalKeySet);
        }
        let bit = 1u8
...
mod tests {
    ...
    fn the_record_is_runtime_width_in_its_set_count() {
        ...
        assert_eq!(
            ...
            Ok(crate::relay::RELAYED_RECORD_MAX_BYTES)
        );
        assert_eq!(
