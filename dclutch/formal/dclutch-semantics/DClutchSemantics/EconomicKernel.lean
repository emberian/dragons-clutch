...
/-! ## The strand: a partial burn from the settlement Position

Decision 0032 §2a rules that a joint clearing's residual -- the claims minted
at an outcome the batch priced at zero and handed to nobody -- is STRANDED:
burned without releasing collateral. It is the first pre-terminal non-uniform
supply vector in the tree, and the note that ruled it says where the
exception must live: beside `uniformSupply`, because uniformity is a property
of the actions that move supply. This is that action, stated the way the
refunding law above is stated -- a post-state and its laws, executable
against the kernel's own vocabulary -- and the Claims adapter arm
`StrandResidual` is its physical twin (`BUILD_JOINT-CLEARING.md`).

The burn comes out of the SOURCE holder, which is the candidate's own
settlement Position: the strand can only remove what the clearing's own mint
put there, never a trader's claim. -/

def strandPost (pre : State) (outcome quantity : Nat) : State :=
  { pre with
    supply := subAt pre.supply outcome quantity
    nativeSupply := subAt pre.nativeSupply outcome quantity
    sourceNative := subAt pre.sourceNative outcome quantity }

/-- Admission: an open Market, a positive burn inside the width, and the
source holder, the native partition and the aggregate all able to lose it. -/
def strandAccepts (count : Nat) (pre : State) (outcome quantity : Nat) : Bool :=
  pre.phase = .open && 0 < quantity && outcome < count &&
    quantity ≤ valueAt pre.sourceNative outcome &&
    quantity ≤ valueAt pre.nativeSupply outcome &&
    quantity ≤ valueAt pre.supply outcome

/-- THE HOARD DOES NOT MOVE. The collateral behind a stranded claim stays in
the Hoard until the Market terminates; that is the half of the ruling its
record calls uncomfortable, and it is what keeps L4 at zero excess. -/
theorem strand_conserves_hoard (pre : State) (outcome quantity : Nat) :
    (strandPost pre outcome quantity).hoard = pre.hoard := rfl

theorem strand_burns_exactly_the_residual
    (pre : State) (outcome quantity : Nat) (inBounds : outcome < pre.supply.length) :
    valueAt (strandPost pre outcome quantity).supply outcome =
      valueAt pre.supply outcome - quantity := by
  simp only [strandPost]
  exact valueAt_subAt_eq pre.supply outcome quantity inBounds

theorem strand_touches_no_other_outcome
    (pre : State) (outcome quantity other : Nat) (distinct : other ≠ outcome) :
    valueAt (strandPost pre outcome quantity).supply other = valueAt pre.supply other := by
  simp [strandPost, subAt, setAt, valueAt, List.getElem?_set_ne distinct.symm]

/-- FULL BACKING SURVIVES THE STRAND. Every outcome's supply is at or under
the Hoard afterwards if it was before: the burned outcome fell and the
Hoard did not. -/
theorem strand_keeps_full_backing
    (count : Nat) (pre : State) (outcome quantity : Nat)
    (backed : ∀ i, i < count → valueAt pre.supply i ≤ pre.hoard)
    (inBounds : outcome < pre.supply.length) :
    ∀ i, i < count → valueAt (strandPost pre outcome quantity).supply i ≤
      (strandPost pre outcome quantity).hoard := by
  intro i bound
  rw [strand_conserves_hoard]
  by_cases same : i = outcome
  · subst same
    rw [strand_burns_exactly_the_residual pre i quantity inBounds]
    have := backed i bound
    omega
  · rw [strand_touches_no_other_outcome pre outcome quantity i same]
    exact backed i bound

/-- THE EXCEPTION TO UNIFORMITY, stated exactly: a uniform supply becomes
uniform everywhere but at the stranded outcome, which falls by exactly the
residual. This is the pre-terminal non-uniform supply vector decision 0032
§6 names, and it is the only shape a strand can produce. -/
theorem the_strand_breaks_uniformity_by_exactly_the_residual
    (ordinaryCount : Nat) (pre : State) (outcome quantity supply : Nat)
    (uniform : uniformSupply ordinaryCount pre supply)
    (inWidth : outcome < outcomeWidth ordinaryCount)
    (inBounds : outcome < pre.supply.length) :
    valueAt (strandPost pre outcome quantity).supply outcome = supply - quantity ∧
      ∀ other, other < outcomeWidth ordinaryCount → other ≠ outcome →
        valueAt (strandPost pre outcome quantity).supply other = supply := by
  refine ⟨?_, ?_⟩
  · rw [strand_burns_exactly_the_residual pre outcome quantity inBounds, uniform outcome inWidth]
  · intro other inWidth distinct
    rw [strand_touches_no_other_outcome pre outcome quantity other distinct]
    exact uniform other inWidth

/-- Executable witness: the design note's `zeroPriced` batch after its mint of
ten and the strand of eight at outcome 2, on a three-outcome aggregate that
held nothing before. -/
def zeroPricedAfterMint : State := {
  phase := .open, hoard := 10, supply := [10, 10, 10], nativeSupply := [10, 10, 10],
  materializedSupply := [0, 0, 0], sourceNative := [10, 10, 10], sourceMaterialized := [0, 0, 0],
  destinationNative := [0, 0, 0], destinationMaterialized := [0, 0, 0] }

example : strandAccepts 3 zeroPricedAfterMint 2 8 = true := by native_decide
example : (strandPost zeroPricedAfterMint 2 8).supply = [10, 10, 2] ∧
    (strandPost zeroPricedAfterMint 2 8).hoard = 10 := by native_decide
/-- HOSTILE: a strand past what the source holds refuses. -/
example : strandAccepts 3 zeroPricedAfterMint 2 11 = false := by native_decide
/-- HOSTILE: a strand of nothing is not a strand. -/
example : strandAccepts 3 zeroPricedAfterMint 2 0 = false := by native_decide
