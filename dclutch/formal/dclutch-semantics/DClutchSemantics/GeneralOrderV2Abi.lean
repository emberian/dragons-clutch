...
This module is the single author of the V2 layout (`DCGSORD2`). Four shape
...
/-- `DCGSORD2` ("signed order", V2), and NOT `DCGORD02`.

`DCGORD02` is already the settlement manifest's per-order row header
(`runtime_manifest.rs`), which a deployed ELF writes. Same adjudication as the
batch record's magic: the discriminant that has not shipped is the one that
moves. -/
def orderMagic : List Nat := [0x44, 0x43, 0x47, 0x53, 0x4f, 0x52, 0x44, 0x32]
