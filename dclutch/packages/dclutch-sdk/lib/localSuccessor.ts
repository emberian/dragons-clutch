...
async function boundedRpc(endpoint: string, method: string, params: ReadonlyArray<unknown>, fetcher: typeof fetch): Promise<unknown> {
  ...
  // The init is bound with `cache` widened locally: undici's RequestInit (the
  // node fetch types) omits it, browsers honor it, node ignores it at runtime.
  const init: RequestInit & { cache?: 'no-store' } = { method: 'POST', mode: 'cors', credentials: 'omit', redirect: 'error', cache: 'no-store', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', id: 1, method, params }), signal: controller.signal };
  try { const response = await fetcher(endpoint, init); if (!response.ok) throw new Error(`${method} returned HTTP ${response.status}`); const bytes = new Uint8Array(await response.arrayBuffer()); if (bytes.length > MAX_RPC_BYTES) throw new Error(`${method} response exceeds ${MAX_RPC_BYTES} bytes`); const payload: unknown = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)); if (!plain(payload) || payload.jsonrpc !== '2.0' || payload.error !== undefined || !('result' in payload)) throw new Error(`${method} returned an invalid JSON-RPC envelope`); return payload.result; } finally { clearTimeout(timeout); }
}
