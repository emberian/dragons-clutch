 ...
 */
export const PLAIN_POSITION_PAYOUT_BLOCK_V1 = 'The winning balance and its exact payout are chain-derived facts, but the payout instruction for a plain Claims Position is not wallet-constructible yet: claims/terminal_settlement_v3 admits caller role Core or Trading only and no campaign drives it (ADR-0008 §7.6). The Claims-role Custody replay this button creates is the standing precondition every payout plan opens with; the payout route itself is a named protocol gap, not a wallet or browser failure.';
