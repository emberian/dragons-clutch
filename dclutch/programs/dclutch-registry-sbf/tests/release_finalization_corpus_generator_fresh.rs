...
//!
//! What it guards is load-bearing twice over: this route is where decision
//! 0012's bound ELF digest becomes a chain-observed fact, and every hot reader
//! in the protocol now spends a slot compare instead of a complete-ELF hash
//! because it did.
...
#[test]
fn checked_in_release_finalization_corpus_is_exact_lean_output() {
    ...
    let generated = Command::new("lake")
        .args([
            ...
            "lean",
            "--run",
            "EmitArtifactReleaseFinalizationCorpusRust.lean",
        ])
        .current_dir(&formal)
        .output()
    ...
    assert!(
        ...
        "release-finalization corpus generator failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&generated.stdout),
        String::from_utf8_lossy(&generated.stderr)
    );

    let temporary = std::env::temp_dir().join(format!(
        "dclutch-release-finalization-corpus-{}.rs",
        std::process::id()
    ));
    ...
    let checked_in = std::fs::read(manifest.join("src/generated_release_finalization_corpus.rs"))
