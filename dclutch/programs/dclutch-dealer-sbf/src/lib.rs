...
impl DealerSbfError {
    ...
    /// [`DealerSbfError::ordinal`], whose match is exhaustive: a variant added to the
    ...
    /// This refusal's position in [`DealerSbfError::ALL`].
    ///
    /// The match is exhaustive on purpose, and that is the whole mechanism:
    /// a twelfth variant is a COMPILE ERROR here rather than a discriminant no
    /// assertion ever looks at.
    const fn ordinal(self) -> usize {
        match self {
            Self::Instruction => 0,
            Self::AccountFrame => 1,
            Self::AccountIdentity => 2,
            Self::Signature => 3,
            Self::Clock => 4,
            Self::Release => 5,
            Self::Semantic => 6,
...
const _: () = {
    assert!(
        DealerSbfError::ALL[0] as u32 == dclutch_refusal_registry::DEALER_REFUSAL_BASE,
        "DealerSbfError must start at its registered refusal band base"
    );
    let mut index = 0;
    while index < DealerSbfError::ALL.len() {
        let variant = DealerSbfError::ALL[index];
        assert!(
            variant.ordinal() == index,
            "DealerSbfError::ALL repeats a variant, skips one, or is out of discriminant order"
        );
        assert!(
            variant as u32 == dclutch_refusal_registry::DEALER_REFUSAL_BASE + index as u32,
            "DealerSbfError discriminants are not the contiguous run from the band base that ALL claims"
        );
        assert!(
            (variant as u32)
                < dclutch_refusal_registry::DEALER_REFUSAL_BASE
