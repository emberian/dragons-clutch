...
//!
//! This adapter reads logical observations normalized under the decoded
//! AccountProfile geometry, authenticated adapter context, and native owners.
//! The caller accepts the resulting candidate only after AccountProfile
//! projection succeeds; this reader does not replace privilege admission.
//! No request buffer, receipt digest, or principal cap is accepted from a caller.

use super::runtime_source_v1::{
...
/// Selected executable role identities authenticated by the calling adapter.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesRuntimeChildProgramsV1 {
...
/// Authenticated physical context shared by Trading and its Shadow evaluator.
///
/// Callers authenticate the selected config record and executable role evidence
/// before constructing this adapter value. It is not a persisted authority or
/// a caller instruction DTO. The shared reader independently joins all record,
/// root, Ticket, physical-state, and native construction facts below.
pub struct SeriesRuntimeBankContextV1<'a> {
    /// Current authenticated Trading program, also the root owner.
    pub trading_program: Pubkey,
    /// Selected Core program.
    pub core_program: Pubkey,
    /// Current authenticated Registry program.
    pub registry_program: Pubkey,
    /// Authenticated controller capability root, distinct from each child Market.
    pub parent_root: Pubkey,
    /// Exact authenticated Template config body.
    pub config_body: &'a [u8],
    /// Selected child roles; absent only for actions that do not use this bank.
    pub child_programs: Option<SeriesRuntimeChildProgramsV1>,
}

/// Authenticate native Series facts and fill the exact private scalar regions.
///
/// `now_slot` is the calling adapter's authenticated Clock observation. No
/// request buffer, receipt digest, principal ceiling or candidate bank is an
/// input. Non-Series and terminal ticket/root actions return `false`.
#[allow(clippy::too_many_arguments)]
#[inline(never)]
pub fn seed_series_derived_scalars_v1(
    context: &SeriesRuntimeBankContextV1<'_>,
    selected_kind: [u8; 32],
    action: u32,
    family_request: &[u8],
    ...
    let program_id = &context.trading_program;
    ...
    let programs = context.child_programs.ok_or(TradingSbfError::Content)?;
    ...
    require(root.key() == context.parent_root.to_bytes() && root.owner() == program_id.to_bytes())?;
    ...
    require(context.config_body == template_bytes)?;
    // Terminal routes carry the actual Template raw/staging pair in addition
    // to the common logical config projection. Registry uses raw SHA256; the
    // native action separately commits the domain-separated Template identity.
    let template_coordinate = match request.action() {
        SeriesActionV3::Consume => Some(60),
        SeriesActionV3::Expire => Some(dclutch_trading::series::generated_expire_frame_v5::SERIES_EXPIRE_TEMPLATE_RAW_COORDINATE_V5 as usize),
        _ => None,
    };
    if let Some(coordinate) = template_coordinate {
        let (_, raw_template) = record(
            &context.registry_program,
            observations,
            coordinate,
            dclutch_trading::series::SERIES_TEMPLATE_SCHEMA_RELEASE_ID_V3,
        )?;
        require(raw_template == template_bytes)?;
    }
    ...
    let escrow = derive_prefounding_escrow(
        ...
        ticket_bytes,
        projection,
        account_key(context.registry_program.to_bytes())?,
    )
    .map_err(content)?;
...
fn seed_prepare(
    ...
    product: &AuthenticatedProductRuntimeV3<'_, '_>,
    rent: &Rent,
    programs: SeriesRuntimeChildProgramsV1,
    observations: &[AccountObservationV1<'_>],
    scalars: &mut [u64],
) -> Result<()> {
    ...
    let projected = SeriesProjectedCustodyPhysicalV3 {
        ...
        parent_capability_root: context.parent_root.to_bytes(),
    ...
    let bank = derive_series_prepare_requests_v1(SeriesPrepareDerivedRequestInputV1 {
        ...
        snapshot,
        product: projection,
        registry_program: account_key(context.registry_program.to_bytes())?,
        parent_root: context.parent_root.to_bytes(),
        custody,
...
fn seed_terminal(
    ...
    require(
        projected.caller_program == program_id.to_bytes()
            && projected.core_program == context.core_program.to_bytes()
            && projected.parent_capability_root == context.parent_root.to_bytes(),
    )?;
    authenticate_custody_addresses(
        escrow,
        programs.custody,
