...
//!
//! The request wire and chain authentication remain owned by `v3_operator`.
...
pub fn build_close_lp_v4(
    ...
    let bytes = chain
        .lp_position_bytes
        .ok_or(MultiLpOperatorErrorV3::InvalidProjection)?;
    if super::v3_multi_lp::DealerLpPositionV3::decode(bytes) != Ok(position) {
...
mod tests {
    ...
    fn open_selects_the_schema_bound_global_entry_and_refuses_selector_drift() {
        ...
        let lp_position_address = Pubkey::find_program_address(
            &[
                super::super::v3_multi_lp::DEALER_LP_POSITION_PDA_DOMAIN_V3,
                &child_root,
                &lp_owner,
            ],
            &trading,
