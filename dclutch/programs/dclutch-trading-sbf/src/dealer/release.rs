...
pub const DEALER_GLOBAL_SELECTOR_MAX_V3: u16 = 8;
/// Exact canonical selector count.
pub const DEALER_GLOBAL_SELECTOR_COUNT_V3: usize = 8;
/// Exact encoded width of the eight-entry ProgramSet.
...
pub fn finalize_dealer_equity_descriptor_v3(
    ...
    let descriptor = CapabilityProgramV3::new(
        ...
        content_id(hash(DEALER_CONFIG_SCHEMA_PREIMAGE_V4).to_bytes())?,
...
pub fn finalize_dealer_lp_descriptor_v3(
    ...
    // the other. See `lp_release.rs` for the full account.
    lifecycle
...
/// Encode the exact global eight-entry Dealer CapabilityProgramSet.
///
/// `descriptors[0]` is selector 1 and `descriptors[7]` is selector 8. Every
...
pub fn encode_dealer_global_program_set_v3(
    ...
    let expected_config = content_id(hash(DEALER_CONFIG_SCHEMA_PREIMAGE_V4).to_bytes())?;
...
mod tests {
    ...
    fn descriptor(selector: u16) -> [u8; CAPABILITY_PROGRAM_V3_BYTES] {
        CapabilityProgramV3::new(
            ...
            content_id(hash(DEALER_CONFIG_SCHEMA_PREIMAGE_V4).to_bytes()).expect("config"),
    ...
    fn lp_profile(action: MultiLpRequestActionV3) -> Vec<u8> {
        ...
        assert_eq!(
            usize::from(super::super::lp_artifacts::DEALER_LP_LINKED_BASIS_ACCOUNT_V3),
            4,
    ...
    fn a_substituted_policy_surfaces_the_join_cause_and_not_geometry() {
        ...
        // A policy written for another Dealer action is a plausible thing to
        // hand the LP finalizer by mistake, and the join says which conjunct
        // caught it.
        let substituted = policy_for_action(u32::from(DEALER_EQUITY_CONTRIBUTE_P0_SELECTOR_V3));
        assert_eq!(
    ...
    fn frozen_selector_assignments_do_not_collide() {
        assert_eq!(
            super::super::lp_request::MultiLpRequestActionV3::Open.selector(),
            7
        );
        assert_eq!(
            super::super::lp_request::MultiLpRequestActionV3::Close.selector(),
            8
        );
        assert_eq!(DEALER_GLOBAL_SELECTOR_MIN_V3, 1);
        assert_eq!(DEALER_GLOBAL_SELECTOR_MAX_V3, 8);
        ...
        assert_eq!(
            super::super::equity_artifacts::DEALER_EQUITY_REQUEST_PROFILE_MAX_BYTES_V3,
            216
        );
    }
}
