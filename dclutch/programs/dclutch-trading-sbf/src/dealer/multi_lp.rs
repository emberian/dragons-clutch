...
pub struct MultiLpContextV3 {
    ...
    /// (`equity_accelerator.rs` joins the Claims aggregate's
...
pub fn prepare_multi_lp_v3(
    ...
    let expected_obligation_bytes = super::obligation::DEALER_OBLIGATION_HEADER_BYTES_V3
    ...
    .map_err(|error| match error {
        super::equity::PoolEquityErrorV3::Insolvent => MultiLpErrorV3::Insolvent,
        super::equity::PoolEquityErrorV3::WidthMismatch => MultiLpErrorV3::WidthMismatch,
...
fn validate_context(
    ...
    .to_bytes();
    let expected_obligation = super::obligation::obligation_address_hinted_v3(
        &Pubkey::new_from_array(context.trading_program),
        context.child_root,
...
mod tests {
    ...
    fn lp_bytes(shares: u64, trading: [u8; 32]) -> ([u8; 32], [u8; DEALER_LP_POSITION_BYTES_V3]) {
        ...
        let obligation = Pubkey::find_program_address(
            &[
                super::super::obligation::DEALER_OBLIGATION_PDA_DOMAIN_V3,
                &[5; 32],
            ],
            &Pubkey::new_from_array(trading),
        )
