...
fn authenticated_prepare_observations_seed_the_native_bank_and_substitutions_refuse() {
    ...
    assert_eq!(
        derive_series_prepare_principal_cap_v1(
            fixture.frame.registry.key,
            OccurrenceV3::decode(&fixture.occurrence)
                .expect("Occurrence")
                .resolution_policy()
                .to_bytes(),
            fixture.projection.product_record().to_bytes(),
            fixture.custody.mint,
            fixture.product.payout_scale,
            &fixture.observations,
        ),
        Ok(fixture.cap)
    );
    let admitted = admit_occurrence(&fixture.template, &fixture.occurrence, &fixture.siblings)
        .expect("occurrence");
    let escrow = crate::series::derived_prepare_v1::derive_prefounding_escrow(
        admitted,
        &fixture.ticket,
    ...
    assert_eq!(
        ...
        .expect("Realm record")
        .0,
        escrow.realm().to_bytes()
    );
    assert_eq!(
        authenticate_custody_addresses(
            &escrow,
            fixture.programs.custody,
            fixture.projected,
            fixture.observations[7].key(),
            fixture.observations[76].key(),
        ),
        Ok(())
    );
    assert_eq!(
        authenticate_credit(
            &escrow,
            fixture.observations[12],
            fixture.projected.rent_program,
        ),
        Ok(())
    );
    for coordinate in prepare::SERIES_PREPARE_CHILD_CREATED_COORDINATES_V5 {
        let vacancy = fixture.observations[coordinate as usize];
        assert_eq!(vacancy.owner(), system_program::ID.to_bytes());
        assert!(vacancy.data().is_empty());
    }
    assert_eq!(fixture.observations[106].key(), fixture.custody.mint);
    assert_eq!(
        fixture.observations[110].key(),
        fixture.custody.token_program
    );
    assert_eq!(
    ...
    let mut direct = vec![0_u64; prepare::SERIES_PREPARE_COMMON_SCALAR_COUNT_V5 as usize];
    assert_eq!(
        seed_prepare(
            &fixture.family,
            fixture.snapshot(),
