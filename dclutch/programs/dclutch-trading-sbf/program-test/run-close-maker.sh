#!/usr/bin/env bash
# `DCLTDMC1` executed on a real bank, on the five REAL role ELFs.
#
# Wall 22's missing decrement had three on-chain tests and no runner, which is
# the same gap `run-program-test.sh` was written to close: a suite nobody can
# invoke is a suite no gate runs, and a route nobody runs is a route that can
# rot green. The close is the LAST act of a market's life -- a market whose
# replays cannot close is permanently unretirable -- so it gets a runner of its
# own rather than a line in somebody else's.
#
# It runs three tests. Two are the route's own: the wall-22 sequence (begin
# retiring over standing makers, close inside Retiring, the physical-close gate
# opening) and the hostile frames. The third drives the OPERATOR's plan builder
# against the same real ELFs, which is the only place the client-side
# projection and the on-chain poststate are compared to each other; the
# builder's unit tests can only prove it agrees with itself.
#
...
#
# Usage: run-close-maker.sh [<elf-dir>]
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
ELF_DIR="${1:-$ROOT/target/close-maker-elves}"
OUT="$ROOT/target/close-maker.log"
...
for package in dclutch-registry-sbf dclutch-trading-sbf dclutch-core-sbf \
    ...
    build_log="$ROOT/target/close-maker-build-$package.log"
    ...
    if [ "${count:-0}" != "0" ]; then
        ...
        echo "run-close-maker.sh: refusing $package -- the toolchain says these calls may" \
...
SBF_OUT_DIR="$ELF_DIR" \
    ...
    --test direct_close_maker_on_chain -- --nocapture --test-threads=1 \
    2>&1 | tee "$OUT" | grep -E "CLOSEMAKER|BEGINRETIRING|test result|panicked|assertion|error"
