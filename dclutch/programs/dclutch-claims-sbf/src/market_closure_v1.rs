...
impl ClaimsMarketClosureSbfErrorV1 {
    ...
    /// [`ClaimsMarketClosureSbfErrorV1::ordinal`], whose match is exhaustive: a variant added to
    ...
    /// This refusal's position in [`ClaimsMarketClosureSbfErrorV1::ALL`].
    ///
...
const _: () = {
    const SUB_BAND: u32 = dclutch_refusal_registry::CLAIMS_REFUSAL_BASE + 0x500;
    assert!(
        ClaimsMarketClosureSbfErrorV1::ALL[0] as u32 == SUB_BAND,
        "ClaimsMarketClosureSbfErrorV1 must start at its registered sub-band offset"
    );
    let mut index = 0;
    while index < ClaimsMarketClosureSbfErrorV1::ALL.len() {
        let variant = ClaimsMarketClosureSbfErrorV1::ALL[index];
        assert!(
            variant.ordinal() == index,
            "ClaimsMarketClosureSbfErrorV1::ALL repeats a variant, skips one, or is out of discriminant order"
        );
        assert!(
            variant as u32 == SUB_BAND + index as u32,
            "ClaimsMarketClosureSbfErrorV1 discriminants are not the contiguous run from the sub-band offset that ALL claims"
        );
        assert!(
            (variant as u32)
