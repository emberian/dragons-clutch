...
impl RationalLifecycleSbfErrorV2 {
    ...
    /// [`RationalLifecycleSbfErrorV2::ordinal`], whose match is exhaustive: a variant added to the
    ...
    /// This refusal's position in [`RationalLifecycleSbfErrorV2::ALL`].
    ///
...
const _: () = {
    const SUB_BAND: u32 = dclutch_refusal_registry::CLAIMS_REFUSAL_BASE + 0x210;
    assert!(
        RationalLifecycleSbfErrorV2::ALL[0] as u32 == SUB_BAND,
        "RationalLifecycleSbfErrorV2 must start at its registered sub-band offset"
    );
    let mut index = 0;
    while index < RationalLifecycleSbfErrorV2::ALL.len() {
        let variant = RationalLifecycleSbfErrorV2::ALL[index];
        assert!(
            variant.ordinal() == index,
            "RationalLifecycleSbfErrorV2::ALL repeats a variant, skips one, or is out of discriminant order"
        );
        assert!(
            variant as u32 == SUB_BAND + index as u32,
            "RationalLifecycleSbfErrorV2 discriminants are not the contiguous run from the sub-band offset that ALL claims"
        );
        assert!(
            (variant as u32)
