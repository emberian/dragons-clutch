...
pub struct SeriesShadowBundleSourceV4<'a> {
    ...
    /// the Template's canonical proof is nonempty, so the emitted Effect's
    /// exact width -- which this bundle's manifest pins -- is a function of
    /// it. See `series::effect_v4::series_consume_borrowed_range_count_v4`.
