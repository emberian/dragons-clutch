...
//! Small SVM refinement of the canonical lifecycle RentCredit contract.
//!
//! The SDK-free `dclutch-market::rent` remains the sole owner of instruction
...
//! the System create CPI, and commit/post-observation checks.
//!
//! Every route here is `LifecycleRentInstructionV2` — Create, Sweep, Close —
//! behind `LIFECYCLE_RENT_INSTRUCTION_MAGIC_V2`. The permanent per-authority
//! RentCredit V1 Create and Withdraw routes were deleted on 2026-08-27 as
//! superseded by the Market-generation-scoped lifecycle credit that tier 1
//! actually exercises. Anything that is not a V2 request is now refused as
//! `Instruction`, where it used to fall through to a V1 decode.
...
dclutch_refusal_registry::pin_refusal_band!(
    RentSbfError,
    dclutch_refusal_registry::RENT_REFUSAL_BASE,
    [
        Instruction,
        AccountFrame,
        RuntimeAccount,
...
impl From<dclutch_registry::activation_auth_v1::ActivationAuthErrorV1> for RentSbfError {
...
impl<'a, 'info> CreateAccounts<'a, 'info> {
    fn parse(accounts: &'a [AccountInfo<'info>]) -> Result<Self, ProgramError> {
        if accounts.len() != CREATE_ACCOUNT_COUNT_V2 {
        ...
        validate_create_frame_v2(
            meta_v2(frame.payer),
            meta_v2(frame.credit),
            meta_v2(frame.system),
            meta_v2(frame.rent),
        )
...
const CREATE_ACCOUNT_COUNT_V2: usize = 4;
...
mod tests {
    ...
    // The V1 Create and Withdraw routes are deleted. What used to be a
    // fall-through decode of `RENT_CREDIT_INSTRUCTION_MAGIC_V1` ("DCLTRCI1") is
    // now a refusal, and so is every other non-V2 magic.
    #[test]
    fn only_the_lifecycle_v2_magic_reaches_a_route() {
        ...
        *retired.get_mut(10).expect("retired offset in bounds") = 1;
        ...
        let mut truncated = LIFECYCLE_RENT_INSTRUCTION_MAGIC_V2.to_vec();
        truncated.push(0);
        assert_eq!(
            process_instruction(&program_id, &[], &truncated),
            Err(RentSbfError::Instruction.into())
        );
    }
