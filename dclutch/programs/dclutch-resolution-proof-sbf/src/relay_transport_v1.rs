...
use dclutch_relay_contract::{
    ...
    RELAYED_RECORD_TRANSPORT_PROFILE_ID_V1, RELAYER_KEY_SET_BYTES,
...
use dclutch_source_contract::{
    ...
    SOURCE_MATERIAL_SCHEMA_RELEASE_ID_V2, SOURCE_MATERIAL_V2_BYTES, SOURCE_SPEC_BYTES,
    SOURCE_SPEC_SCHEMA_ID_V1, SourceAccessProfile, SourceMaterialV2, SourceSpecV1,
...
fn account<'a, 'info>(
    ...
    accounts.get(index).ok_or(ResolutionError::AccountFrame.into())
}
...
fn process_append(
    ...
    let running = {
        let view = RelayedObservationRecordViewV1::decode(&data)
            .map_err(|_| ResolutionError::OutputState)?;
        view.set_digest().map_err(|_| ResolutionError::OutputState)?
    };
...
fn create_prefunded_pda<'info>(
    ...
    if created.owner != owner
        ...
        || created.lamports() != before.checked_add(top_up).ok_or(ResolutionError::Arithmetic)?
