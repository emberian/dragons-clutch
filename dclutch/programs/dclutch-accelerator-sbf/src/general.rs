...
dclutch_refusal_registry::pin_refusal_band!(
    GeneralAcceleratorSbfErrorV3,
    dclutch_refusal_registry::ACCELERATOR_REFUSAL_BASE,
    [
        InvalidRequest,
        InvalidFrame,
        InvalidTopLevelInstruction,
        InvalidScratchBank,
        InvalidAcknowledgement,
        ForeignInstructionBeforeTrading,
        HeapFrameNotRequested,
        TopLevelProgramNotTrading,
        InvalidHotEnvelope,
        InvalidFamilyRequest,
        InstructionsSysvarAccount,
        CurrentInstructionIndexUnreadable,
        PrecedingInstructionUnreadable,
        ScratchPagePrivileges,
        ScratchPageDecode,
        ScratchPageRequestBinding,
        ScratchPageOrder,
        ScratchBankDigest,
        ScratchBankLength,
...
///
/// Both readings come from the allocator's own heap header, so the probe
/// allocates nothing and moves nothing it measures. The three logged words
/// are: bytes outstanding, the ceiling actually enforced, and the frame the
/// transport requests; the gap between the second and third is what a
/// program that never lifted its ceiling would throw away.
...
pub fn process(
    ...
    // THE GEOMETRY IS VALIDATED AFTER THE ACTION IS KNOWN: the bank width this
    // instruction assembles is per-action, so `validate_request_geometry`
    // cannot run before `authenticate_top_level` names the action.
...
fn validate_request_geometry(
    ...
    // BOTH INPUT TRANSPORTS. The scratch-page transport has no producer that
    // can exist (the bank carries the current slot, a full page does not fit
    // in a transaction, and every reader wants the page read-only); the inline
    // bank rides CPI instruction data (10,240 bytes) and every bank Trading
    // can build is at most 8,192. `AdmittedAcceleratorRequestV2` already
    // refuses a non-empty inline bank under `ScratchPages` and one whose width
    // is not `total_bank_bytes` under `Inline`, so the two conjuncts below are
    // the whole of what this function owes on the transport.
...
fn validate_frame(
    ...
    // the output page is the one account this program owns, and it sits at the
    // one coordinate `admitted_v3` names for it.
...
fn authenticate_top_level(
    ...
    // so a hostile testing the heap conjunct can tell it from the other seven.
    if !heap_frame_requested {
        return Err(GeneralAcceleratorSbfErrorV3::HeapFrameNotRequested.into());
    }
    // The same instruction that satisfies the conjunct above raises this
    // program's own ceiling to what it asks for, and it happens HERE because
    // everything expensive -- `validate_frame`, `assemble_input_bank`, the
    // transition -- runs after `authenticate_top_level` returns, while the
    // decode above it fits the default with room to spare.
    //
...
mod tests {
    ...
    /// The inline transport is admitted at the declared width (General
    /// arrives inline since the page transport lost its producer), and a zero
    /// tail still refuses under either transport.
    #[test]
    fn inline_at_the_declared_width_accepts_and_zero_width_refuses() {
        assert_eq!(
