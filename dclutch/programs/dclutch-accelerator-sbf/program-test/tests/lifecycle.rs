...
use dclutch_trading::general::{
    ...
    runtime_verify::{
        AuthenticatedOrderTermsV2, OrderSideV2, RuntimeCandidateVerifierV2, RuntimeVerifyErrorV2,
...
fn verified_candidate(
    ...
    VerifiedCandidateV2::encode_into(
        ...
        // sum to exactly `price_scale`, which is the runtime width here, so
        // one unit per outcome is the uniform vector at that scale. These
        // certificates are selection fixtures and are never streamed through
        // the verifier, so the vector must be admissible, not forced.
        &vec![1; count],
        &vec![1; count],
        &vec![1; count],
        &mut output,
...
fn submitted_candidate_for_close(width: u32) -> (GeneralBatchV2, GeneralCandidateV1) {
    ...
    let mut header = CandidateHeaderV2 {
        ...
        // orders are live, and this batch closed with none admitted.
...
/// One maker's signed single-outcome order, as it would be placed into the
/// batch.
///
/// Cohort-18's joint arm makes an order an INTERVAL on one side: it moves
/// `claims_per_lot` claims at each outcome of `[outcome_lo, outcome_hi]` and
/// nothing off it. `runtime_verify::current_shape` refuses a row that moves
/// claims at more than one outcome (`ShapeNotInterval`), so every order in
/// this file is one outcome wide and its per-lot vectors are DERIVED from the
/// shape rather than carried beside it.
struct OrderSpec {
    nonce: u64,
    lots: u64,
    side: OrderSideV2,
...
fn order_record(width: u32, batch_id: [u8; 32], spec: &OrderSpec) -> Vec<u8> {
    ...
    let header = GeneralOrderHeaderV2 {
        ...
        batch_id,
        generation: GENERATION,
        // EVERY ORDER IS FILLED TO ITS OWN MAXIMUM, so the marginal conjunct
        // is satisfied twice over. `RationedInsideLimit` refuses an order left
        // short of `max_lots` while the clearing price is strictly inside its
        // limit; here every maker is BOTH at their limit and filled to the
        // brim, which is two reasons rather than one.
        max_lots: spec.lots,
    ...
    let derived: Vec<(u64, u64)> = (0..width)
        ...
        .collect();
    let receive: Vec<u64> = derived.iter().map(|row| row.0).collect();
    let deliver: Vec<u64> = derived.iter().map(|row| row.1).collect();
...
/// seller: it delivers ONE CLAIM AT OUTCOME ZERO per lot and nothing anywhere
/// else, the clearing prices outcome zero at the whole scale, and a lot is
/// therefore paid `scale x 1 / scale = 1` at EVERY runtime width this file
...
/// clears.
///
/// It is also what FORCES the price. A filled seller raises its outcome's
/// price floor to `ceil(floor x scale / claims_per_lot)`, which is the whole
/// scale here, so decision 0032's lexicographic minimum has exactly one vector
/// to choose and the fixture's prices are derived rather than picked.
...
fn walk_at_seller_floor(width: u32, seller_floor: u64) -> Result<TerminalFixture, RefusedRowV1> {
    ...
    //
    // THE BOOK BALANCES AT OUTCOME ZERO, AND THAT IS NOT A CONVENIENCE. The
    // joint arm mints complete sets inside the clearing whenever the net claim
    // flow is positive, which creates claims at EVERY outcome and hands them
    // to takers at only the ones that have any; every other outcome is then a
    // residual the close must STRAND. The strand's Claims burn has no child
    // frame in `Action::Close` yet, so `hot_candidate_v3::position_geometry`
    // refuses every Close whose plan says `claims_active` -- fail-closed, and
    // deliberately. A book with fewer orders than outcomes cannot mint and
    // still reach a terminal close, and this file walks to a terminal close on
    // a real ELF at width 258. So the two buyers take exactly the four lots
    // the seller offers: `M = 0`, no residual anywhere, and the close is
    // reachable at every runtime width.
    ...
    let specs = [
        OrderSpec {
            ...
            outcome: 0,
            claims_per_lot: 1,
            // ONE, WHICH IS THE CLEARING PRICE, not two. The buyer's cap
            // induces a price ceiling of `floor(cap x scale / claims_per_lot)`
            // and the clearing charges `scale x 1 / scale = 1` per lot, so a
            // cap of one is the cap exactly AT the fill -- the same boundary
            // the seller's floor sits on, from the other side.
            debit_limit: 1,
            credit_floor: 0,
        },
        OrderSpec {
    ...
    //
    // THE PRICES ARE FORCED, not chosen. Outcome zero's floor is the whole
    // scale (the filled seller's), its ceiling is the whole scale (the filled
    // buyers' caps), and no other outcome carries a residual or a bound, so
    // decision 0032's lexicographic minimum has one vector: the scale at
    // outcome zero and nothing anywhere else. A vector inside the box but off
    // its minimum refuses `NonMinimalPriceVector`.
    let mut prices = vec![0_u64; count];
    prices[0] = u64::from(width);
    ...
    let header = CandidateHeaderV2 {
        ...
        // admitted and what the certificate enumerates.
        live_order_count: batch.live_order_count(),
    };
    CandidateV2::encode_into(header, &prices, &mut candidate).expect("draft candidate");
    ...
    for (index, (row, terms)) in rows.iter().enumerate() {
        ...
        // The row's own lots, read off the row: an order the certificate fills
        // with nothing emits no settlement row, so the count is a function of
        // the fill as well as the cursor.
        let row_lots = ExecutionV2::decode(row)
...
async fn real_sbf_place_order_admits_canonical_signed_terms_at_runtime_widths() {
    for width in [1_u32, 258] {
        ...
        // A SINGLE-OUTCOME SELL OF TWO CLAIMS PER LOT. This record is admitted
        // and never streamed through a verifier -- the walk below measures the
        // escrow the admission moves -- but it must still be a shape the joint
        // arm admits, and `GeneralOrderV2::decode` refuses rows that disagree
        // with the header's interval (`RowsDisagreeWithShape`). Two claims per
        // lot at two lots is exactly the four-claim funding this walk grants.
...
async fn real_sbf_submit_candidate_creates_exactly_funded_candidate_at_runtime_widths() {
    for width in [1_u32, 258] {
        ...
        let mut header = CandidateHeaderV2 {
            ...
            product_id: product_id(),
            batch_id: batch.batch_id(),
            // The batch's own count: this one closed with nothing admitted,
            // and `authenticate_batch_candidate_v1` refuses a candidate that
            // says otherwise.
            live_order_count: batch.live_order_count(),
        };
...
/// THE FIXTURE'S OWN BOOK IS A JOINT CLEARING, AND ITS CLOSE IS REACHABLE.
///
/// Every other test in this file needs the real ELF and therefore an
/// `SBF_OUT_DIR`. This one needs neither: it drives the protocol's own
/// evaluators over the same fixture the ELF walks, so a book that stopped
/// being a valid clearing is caught by `cargo test` rather than by a program
/// test nobody can run here.
///
/// Three facts, and the third is why the book is shaped the way it is:
///
/// 1. the certificate exists at all, which means every row streamed through
///    `runtime_verify` and satisfied all seven conjuncts -- the simplex, the
///    completeness count, complementary slackness, both limits, the marginal
///    conjunct and decision 0032's lexicographic tie-break;
/// 2. its price vector is the ONE the box forces: the whole scale at outcome
///    zero, zero everywhere else;
/// 3. the terminal close's plan does NOT strand, so `claims_active` is false
///    and `hot_candidate_v3::position_geometry` admits it. A minting clearing
///    at width 258 would leave a residual at 257 outcomes and its close would
///    be refused fail-closed until `Action::Close` gains a ProtocolPosition
///    mutation child frame. That refusal is correct and this fixture must not
///    depend on it being lifted.
#[test]
fn the_fixture_book_clears_natively_and_its_close_strands_nothing() {
    for width in [1_u32, 258] {
        let fixture = terminal_fixture(width);
        let verified = VerifiedCandidateV2::decode(&fixture.verified).expect("certificate");
        assert_eq!(
            verified.header().price_scale,
            u64::from(width),
            "width {width}: the certificate carries the batch's scale",
        );
        for outcome in 0..width {
            let expected = if outcome == 0 { u64::from(width) } else { 0 };
            assert_eq!(
                verified.price(outcome).expect("price"),
                expected,
                "width {width}: outcome {outcome} is off the forced clearing vector",
            );
        }

        let first =
            SettlementManifestV2::decode(fixture.manifests.first().expect("first manifest"))
                .expect("first manifest");
        let last = SettlementManifestV2::decode(fixture.manifests.get(1).expect("final manifest"))
            .expect("final manifest");
        let plan = [
            (first.as_bytes(), 0_u32),
            (last.as_bytes(), 0),
            (last.as_bytes(), 1),
        ];
        let mut cursor = initialized_cursor(&fixture);
        for (manifest, order_index) in plan {
            let (next, _) = settle_native(
                &fixture,
                &cursor,
                RuntimeSettlementActionV2::Collect,
        ...
        let (closed, effect) =
            settle_native(&fixture, &cursor, RuntimeSettlementActionV2::Close, None, 0);
        let close_plan = RuntimeSettlementEffectPlanV2::decode(&effect).expect("close effect");
        assert!(
            !close_plan.header().claims_active,
            "width {width}: the close strands, and a stranding close has no frame to happen in",
        );
        assert_eq!(
            SettlementCursorV2::decode(&closed)
                .expect("closed cursor")
                .header()
                .quote_inventory,
            0,
            "width {width}: the settlement closed holding quote",
        );
    }
}

/// The seller's floor, one step past the fill, refused NATIVELY and by name.
///
/// This is control 1 of `real_sbf_verify_candidate_refuses_a_fill_below_the_sellers_floor_at_runtime_widths`
/// standing on its own, because that test cannot run without an ELF and this
/// one can. Without it the fixture above would be a book that clears with
/// nothing saying the conjunct it clears past can still fire.
#[test]
fn a_fill_below_the_sellers_floor_refuses_natively_at_both_runtime_widths() {
    for width in [1_u32, 258] {
        let refused = walk_at_seller_floor(width, SELLER_CREDIT_FLOOR_REFUSES)
            .err()
            .expect("a floor one step above the fill must refuse this walk");
