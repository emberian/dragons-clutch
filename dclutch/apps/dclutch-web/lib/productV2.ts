...
function decodeCapability(bytes: Uint8Array): CapabilityView { if (bytes.length < 16 || ascii(bytes, 0, 8) !== 'DCLTCAP1' || u16(bytes, 8) !== 1 || u16(bytes, 10) !== 1) throw new Error('capability manifest has the wrong exact header'); const count = u16(bytes, 12); requireZero(bytes, 14, 2, 'capability manifest header'); if (count > 16 || bytes.length !== 16 + count * 528) throw new Error('capability manifest width does not match its entry count'); let found: CapabilityView | null = null; let prior = ''; for (let index = 0; index < count; index += 1) { const offset = 16 + index * 528; const kind = hex(slice(bytes, offset, 32)); if (prior !== '' && prior >= kind) throw new Error('capability manifest entries are not strictly ordered'); prior = kind; requireZero(bytes, offset + 194, 6, 'capability entry'); const dependencyCount = bytes[offset + 193]; if (dependencyCount > 16) throw new Error('capability dependency count exceeds 16'); const dependencies = slice(bytes, offset + 208, 16); for (let position = 0; position < 16; position += 1) { if (position < dependencyCount) { if (dependencies[position] >= count || dependencies[position] === index || (position > 0 && dependencies[position - 1] >= dependencies[position])) throw new Error('capability dependency is noncanonical'); } else if (dependencies[position] !== 0) throw new Error('inactive capability dependency bytes are nonzero'); } if (kind === PRODUCT_PAYOFF_ADMISSION_KIND) { if (found !== null) throw new Error('capability manifest repeats Product payoff admission'); const release = hex(slice(bytes, offset + 32, 32)); const config = slice(bytes, offset + 64, 32); const capacity = slice(bytes, offset + 96, 32); const childSchema = hex(slice(bytes, offset + 128, 32)); const childDerivation = hex(slice(bytes, offset + 160, 32)); [config, capacity].forEach((value) => requireNonzero(value, 'Product capability identity')); if (release !== PRODUCT_PAYOFF_ADMISSION_RELEASE || childSchema !== PRODUCT_PAYOFF_RECEIPT_SCHEMA || childDerivation !== PRODUCT_PAYOFF_RECEIPT_DERIVATION || bytes[offset + 192] !== 0 || u64(bytes, offset + 200) !== BigInt(0)) throw new Error('Product payoff capability does not select the founding admission release and receipt authority'); found = Object.freeze({ bindingDigest: config, capacityProfileId: capacity }); } }
  if (found === null) throw new Error('capability manifest has no Product payoff admission entry'); return found;
}
