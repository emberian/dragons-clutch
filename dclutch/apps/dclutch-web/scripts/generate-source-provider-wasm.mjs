...
// The facts module has one home, the SDK; the web imports it as `@dclutch/sdk/generated/sourceProviderWasmV1`.
const facts = join(root, 'packages/dclutch-sdk/lib/generated/sourceProviderWasmV1.ts');
...
function rustNumber(source, name) {
  const match = source.match(new RegExp(`const ${name}: usize\\s*=\\s*(?:dclutch_source::resolution::PROVIDER_UPDATE_LIFECYCLE_BYTES_V4|([0-9_]+));`));
  if (match === null) throw new Error(`Rust Source-provider owner does not expose ${name}`);
  ...
  const value = codec.match(/pub const PROVIDER_UPDATE_LIFECYCLE_BYTES_V4: usize = ([0-9_]+);/);
