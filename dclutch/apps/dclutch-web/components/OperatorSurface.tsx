...
export default function OperatorSurface() {
  ...
  return <main className="product-shell operator-shell">
    <ConsoleHeader path="/operate" title="Operations" purpose="An operator of a running deployment sees every action the deployed programs accept and exports unsigned bytes for the ones a browser can build." />
    <section className="operator-hero"><div><h1>Operations.</h1><p>You name the deployed programs; the console reads what actually exists on that chain, shows which actions can be constructed right now, and exports unsigned bytes for them. Where the chain has no answer, the console refuses instead of inventing one.</p></div><div className="operator-counts"><article><strong>{counts.constructible}</strong><span>browser-constructible actions</span></article><article><strong>{counts.request}</strong><span>exact request / Rust routes</span></article><article><strong>{counts.blocked}</strong><span>honestly waiting on ABI seams</span></article></div></section>
