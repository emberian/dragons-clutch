...
export default function LocalSuccessorWorkspace() {
  ...
  return <main className="product-shell direct-workspace local-successor-shell">
    <ConsoleHeader path="/local" title="Local successor" purpose="A developer with the checkpointed validator running reads its finalized state and confirms it matches the published evidence." />
    <section className="market-heading local-heading"><div><h1>The local chain.</h1><p>This page reads the fixed localhost validator and compares every account it finds against the hash-pinned checkpoint. Records prepared at genesis are labeled as prepared inputs; only what a transaction actually created is described that way.</p></div><aside className="local-endpoint"><span>Fixed profile</span><strong>{LOCAL_SUCCESSOR_CHECKPOINT.network.rpc_url}</strong><code>{LOCAL_SUCCESSOR_CHECKPOINT.network.genesis_hash}</code><button type="button" onClick={() => void refresh()} disabled={discovery.kind === 'loading'}>{discovery.kind === 'loading' ? 'Reading finalized state…' : 'Refresh exact state'}</button></aside></section>
