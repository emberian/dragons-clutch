...
export default function RationalTerminalPanel() {
  ...
  return <>
    <section className="trade-v3-card">
      <header><span>07</span><div><h2>Preview terminal payout</h2><p>Select a claim and quantity to calculate its collateral payout from the resolved Market result. A losing claim pays zero.</p></div></header>
      <div className="trade-v3-evidence"><article><span>Result width</span><strong>N</strong><small>Product-owned terminal partition</small></article><article><span>Claim width</span><strong>K</strong><small>representation basis; need not equal N</small></article><article><span>Losing claim</span><strong>Zero collateral</strong><small>No collateral is paid for a losing claim</small></article><article><span>Submission</span><strong>Preview available</strong><small>Redemption signing unavailable</small></article></div>
    </section>

    <form className="trade-v3-card route-card" onSubmit={(event) => void inspect(event)}>
      <header><span>08</span><div><h2>Load a resolved claim</h2><p>Enter the claim route and quantity to read the Market result and calculate the payout in collateral base units.</p></div></header>
      ...
      <div className="direct-actions"><button disabled={state.kind === 'loading'}>{state.kind === 'loading' ? 'Reading terminal semantics…' : 'Calculate terminal payout'}</button></div>
      <p className="direct-status">{walletStatus}</p><p className="direct-status" aria-live="polite">{state.message}</p>
      {inspection && summary && <div className="trade-v3-evidence"><article><span>Independent widths</span><strong>K={inspection.representationWidth} · N={inspection.resultOutcomeCount}</strong><small>winner {inspection.terminalWinner} · claim {inspection.selectedOutcome}</small></article><article><span>Resolution certificate</span><strong>{inspection.terminalCertificate.kind}</strong><small>{inspection.terminalCertificate.resultNumerator.toString()}/{inspection.terminalCertificate.resultDenominator.toString()} · native i128/u64</small></article><article><span>Exact payout</span><strong>{inspection.payout.rawPayout.toString()} collateral atoms</strong><small>{inspection.payout.losing ? 'No collateral payout' : `${inspection.payout.payoutPerShard.toString()} per native claim`}</small></article><article><span>Scenario</span><strong>{inspection.payout.scenario}</strong><small>{summary.basis.slice(0, 16)}… ProductBasisV3</small></article></div>}
      {inspection && <div className="direct-output"><dl><div><dt>Execution status</dt><dd>{inspection.refusal}</dd></div><div><dt>Redemption</dt><dd>This route currently supports payout calculation. Transaction signing is unavailable.</dd></div></dl></div>}
    </form>
  </>;
}
