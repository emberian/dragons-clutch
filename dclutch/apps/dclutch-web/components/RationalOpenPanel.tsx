...
export default function RationalOpenPanel() {
  ...
  async function inspect(event: FormEvent<HTMLFormElement>) {
    ...
    setState({ kind: 'loading', message: 'Reacquiring fixed38, SetV2/CapabilityV4, six artifacts, representation K, Product N, descriptor graph, Claims, Token-2022, replay rent, account profile, and ALT…' });
    try {
      ...
      setState({ kind: 'ready', inspection: next, message: `Exact ${next.action} family joined at finalized slot ${next.observedSlot}; representation K=${next.representationWidth}, terminal N=${next.resultOutcomeCount}.` });
  ...
  function download() {
    ...
    link.download = `dclutch-${inspection.action}-v4-unsigned-k${inspection.representationWidth}-n${inspection.resultOutcomeCount}-${candidate.wireBytes.length}.bin`;
    link.click(); URL.revokeObjectURL(link.href);
  }

  return <>
    <section className="trade-v3-card">
      <header><span>04</span><div><h2>Open native shards or a Structured receipt</h2><p>Choose denominate, reconstitute, issue, or unwrap, then enter a quantity in base units. Review the claims and collateral required for that action.</p></div></header>
      <div className="trade-v3-evidence"><article><span>Selected routes</span><strong>36 logical</strong><small>one representation coordinate</small></article><article><span>Structured routes</span><strong>32 + 4K</strong><small>zero coefficients remain zero-delta rows</small></article><article><span>Economics</span><strong>raw u64</strong><small>display decimals never scale values</small></article><article><span>Execution</span><strong>Unsigned export</strong><small>Wallet signing unavailable</small></article></div>
    </section>

    <form className="trade-v3-card route-card" onSubmit={(event) => void inspect(event)}>
      <header><span>05</span><div><h2>Load an opening route</h2><p>Enter the representation descriptor and route accounts to load the available action and current balances.</p></div></header>
      ...
      {inspection && summary && <div className="trade-v3-evidence"><article><span>Action / widths</span><strong>{inspection.action} · K={inspection.representationWidth} / N={inspection.resultOutcomeCount}</strong><small>{summary.quantity}</small></article><article><span>Claims geometry</span><strong>{summary.claims}</strong><small>Profile11 canonical representatives</small></article><article><span>Display metadata</span><strong>{summary.decimals}</strong><small>quantities remain in base units</small></article><article><span>Descriptor</span><strong>{summary.descriptor.slice(0, 16)}…</strong><small>Capability {summary.capability.slice(0, 16)}…</small></article></div>}
    </form>

    <section className="trade-v3-card signing-card">
      <header><span>06</span><div><h2>Build and download an unsigned transaction</h2><p>Build the transaction to check its size and required accounts. Wallet signing is unavailable for this opening route.</p></div></header>
      <button type="button" disabled={inspection === null} onClick={() => void build()}>Build unsigned opening transaction</button><p className="direct-status" aria-live="polite">{buildStatus}</p>
      <div className="direct-actions"><button type="button" disabled title={candidate?.refusal ?? 'Choose a deployment with opening enabled to sign.'}>Wallet signing unavailable</button><button type="button" disabled={candidate === null} onClick={download}>Download unsigned transaction</button></div>
