...
describe('operator command runbook', () => {
  it('renders the exact command with a utility action and quiet initial status', () => {
    const command = 'dclutch-terminal route direct \\\n+  --output "$DIRECT_ROUTE"';
    ...
    expect(html).toContain('dclutch-terminal route direct');
  ...
  it('copies exactly once and does not imply execution', async () => {
    ...
    await expect(copyCommandV1('dclutch-terminal join --execute', write)).resolves.toBe(
      'Copied the exact displayed commands. Nothing was executed.',
    );
    expect(write).toHaveBeenCalledOnce();
    expect(write).toHaveBeenCalledWith('dclutch-terminal join --execute');
  });
