...
export default function RationalRepresentationWorkspace() {
  ...
  return <PageShell className="product-shell trade-v3-shell" header={<ConsoleHeader path="/representation" title="Representation" purpose="Transfer bearer claims and inspect representation lifecycle routes on a local or compatible custom chain." />}>
    ...
    <section className="trade-v3-hero"><div><h1>Claims &amp;<br /><em>redemption.</em></h1><p>Transfer bearer claims, prepare representation transactions, retire empty receipts, and preview resolved payouts.</p></div><aside><span>Choose a Market</span><strong>Claims &amp; receipts</strong><p>Connect to a compatible deployment and load the route for your action.</p></aside></section>

    <section className="trade-v3-card">
      <header><span>00</span><div><h2>Choose a claim action</h2><p>Transfer existing bearer claims, open a representation, calculate a resolved payout, or close an empty receipt.</p></div></header>
      <div className="trade-v3-evidence"><article><span>Bearer transfer</span><strong>Sign and send</strong><small>Transfer between token accounts</small></article><article><span>Rational open</span><strong>Unsigned export</strong><small>Denominate, reconstitute, issue, or unwrap</small></article><article><span>Terminal redeem</span><strong>Calculate payout</strong><small>Redemption signing unavailable</small></article><article><span>Receipt retirement</span><strong>Review and sign</strong><small>Close a zero-supply receipt</small></article></div>
    </section>

    <form className="trade-v3-card route-card" onSubmit={(event) => void inspect(event)}>
      <header><span>01</span><div><h2>Load a bearer transfer</h2><p>Enter the Market, claim mint, and source and destination token accounts to check their balances and transfer requirements.</p></div></header>
      ...
      <button disabled={state.kind === 'loading'}>{state.kind === 'loading' ? 'Reading finalized representation state…' : 'Load transfer route'}</button><p className="direct-status" aria-live="polite">{state.message}</p>
      {inspection && summary && <div className="trade-v3-evidence"><article><span>Market / generation</span><strong>{short(inspection.market)} · {inspection.generation.toString()}</strong><small>{inspection.marketPhase}</small></article><article><span>Behavior selection</span><strong>{summary.selectionDigest.slice(0, 16)}…</strong><small>Registry record {short(inspection.selectionRecord)}</small></article><article><span>Raw source balance</span><strong>{inspection.source.rawAmount.toString()} atoms</strong><small>base units</small></article><article><span>Display metadata</span><strong>{inspection.mint.displayDecimals} decimals</strong><small>Mint supply {inspection.mint.rawSupply.toString()} raw atoms</small></article></div>}
    </form>

    <section className="trade-v3-card">
      <header><span>02</span><div><h2>Set the transfer amount</h2><p>Enter a whole-number amount in the mint&apos;s base units. Review the source and destination balances before signing.</p></div></header>
      ...
      <button type="button" disabled={inspection === null} onClick={() => void build()}>Build unsigned transfer</button><p className="direct-status" aria-live="polite">{buildStatus}</p>
    ...
    <footer className="product-footer"><span>Amounts use whole-number base units</span><span>Saved transfers resume from their transaction signature</span></footer>
  </PageShell>;
}
