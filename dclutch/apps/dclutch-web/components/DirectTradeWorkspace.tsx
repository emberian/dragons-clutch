...
export default function DirectTradeWorkspace() {
  ...
  return <PageShell className="product-shell trade-v3-shell" header={<ConsoleHeader path="/trade" title="Direct trade" purpose="Check a route against live chain state and review one fill without signing or building a transaction." />}>
    ...
    <form className="trade-v3-card route-card" onSubmit={inspectRoute}>
      <header><span>01</span><div><h2>Acquire the action-selected route</h2><p>Every account in the address map is reacquired and joined against current Registry, Core, and Trading state.</p></div></header>
      <div className="trade-v3-route-grid"><label><span>Finalized RPC endpoint</span><input type="url" required value={endpoint} onChange={(event) => setEndpoint(event.target.value.trim())} /></label><label><span>Checked infrastructure manifest · base64, optional — <code>infrastructure.checked</code> from the release pipeline, {CHECKED_INFRASTRUCTURE_BYTES_V1.toLocaleString()} bytes</span><textarea value={infrastructureText} onChange={(event) => setInfrastructureText(event.target.value.trim())} /></label><label className="route-json"><span>Route manifest · JSON — every address in it is reacquired from the chain before use</span><textarea required spellCheck={false} value={routeText} onChange={(event) => setRouteText(event.target.value)} /></label></div>
    ...
    <form className="trade-v3-card" onSubmit={reviewFill}>
      <header><span>02</span><div><h2>Review one possible atomic fill</h2><p>The Market’s authenticated outcome width, price scale, fee rate, and finalized observation slot. No signed intent or transaction material is created.</p></div></header>
      {inspection === null ? <><p className="trade-v3-refusal">Acquire a route first. No Market, width, price scale, fee, revision, or authority is supplied here.</p><button disabled>Review exact arithmetic</button></> : <>
        ...
        {preview && <div className="trade-v3-preview"><div><span>Gross collateral</span><strong>{preview.grossCollateral.toString()}</strong></div><div><span>Seller credit</span><strong>{preview.sellerNetCollateralCredit.toString()}</strong></div><div><span>Buyer debit</span><strong>{preview.buyerCollateralDebit.toString()}</strong></div><div><span>Total fees</span><strong>{preview.totalFeeTransfer.toString()}</strong></div><p>One exact fill×price ÷ scale boundary at the finalized observation slot. Onchain Clock and interpreter execution remain authoritative.</p></div>}
        <p className="direct-status" aria-live="polite">{tradeStatus}</p>
      </>}
    </form>

    <section className="trade-v3-card signing-card">
      <header><span>03</span><div><h2>Execution remains closed</h2><p>No wallet connection, signature request, packet download, or submission control.</p></div></header>
      <p className="trade-v3-refusal">Direct execution reopens here once the caller persists the exact signed bytes before one send, authenticates Trading’s final acknowledgement, and verifies all ten ordered writable poststates from finalized history.</p>
    </section>
