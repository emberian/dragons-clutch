...
export default function DirectTradeWorkspace() {
  ...
  const [routeState, setRouteState] = useState<RouteState>({ kind: 'idle', message: 'Load a market route to begin.' });
  const [form, setForm] = useState<FormFields>(INITIAL_FORM);
  const [tradeStatus, setTradeStatus] = useState('Load a route before reviewing a trade.');
  ...
  async function inspectRoute(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setRouteState({ kind: 'loading', message: 'Reading finalized state…' });
    try {
  ...
  function reviewFill(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTradeStatus(`Trade calculation at slot ${inspection.observedSlot}: buyer pays ${next.buyerCollateralDebit}, seller receives ${next.sellerNetCollateralCredit}, fees ${next.totalFeeTransfer}.`);
    } catch (error) { setTradeStatus(`Refused: ${errorMessage(error)}`); }
  }

  return <PageShell className="product-shell trade-v3-shell" header={<ConsoleHeader path="/trade" title="Direct trade" purpose="Calculate the collateral and fees for a proposed Direct trade." />}>

    <section className="trade-v3-hero"><div><h1>Direct<br /><em>trade.</em></h1><p>Load a market route and enter a proposed trade to calculate the buyer’s payment, seller’s proceeds and fees.</p></div><aside><span>Where trading happens</span><strong>Markets</strong><p>To place a trade, open the market on <Anchor href="/markets">Markets</Anchor> and use its trade panel. Review the offer and payment before signing.</p></aside></section>

    <form className="trade-v3-card route-card" onSubmit={inspectRoute}>
      <header><span>01</span><div><h2>Load the market route</h2><p>Paste the route manifest for the market you want to inspect.</p></div></header>
      <div className="trade-v3-route-grid"><label><span>Finalized RPC endpoint</span><input type="url" required value={endpoint} onChange={(event) => setEndpoint(event.target.value.trim())} /></label><label><span>Checked infrastructure manifest · base64, optional — <code>infrastructure.checked</code> from the release pipeline, {CHECKED_INFRASTRUCTURE_BYTES_V1.toLocaleString()} bytes</span><textarea value={infrastructureText} onChange={(event) => setInfrastructureText(event.target.value.trim())} /></label><label className="route-json"><span>Route manifest · JSON</span><textarea required spellCheck={false} value={routeText} onChange={(event) => setRouteText(event.target.value)} /></label></div>
    ...
    <form className="trade-v3-card" onSubmit={reviewFill}>
      <header><span>02</span><div><h2>Review a proposed trade</h2><p>Enter the buyer and seller terms, quantity and execution price.</p></div></header>
      {inspection === null ? <><p className="trade-v3-refusal">Load the market route first.</p><button disabled>Review exact arithmetic</button></> : <>
        ...
        {preview && <div className="trade-v3-preview"><div><span>Gross collateral</span><strong>{preview.grossCollateral.toString()}</strong></div><div><span>Seller credit</span><strong>{preview.sellerNetCollateralCredit.toString()}</strong></div><div><span>Buyer debit</span><strong>{preview.buyerCollateralDebit.toString()}</strong></div><div><span>Total fees</span><strong>{preview.totalFeeTransfer.toString()}</strong></div><p>Collateral is calculated as quantity × price ÷ price scale, with the market’s fees applied.</p></div>}
        <p className="direct-status" aria-live="polite">{tradeStatus}</p>
      </>}
    </form>

    <section className="trade-v3-card signing-card">
      <header><span>03</span><div><h2>Place a trade</h2><p>Use the market detail page to trade an available offer.</p></div></header>
      <p className="trade-v3-refusal">This calculator is read-only.</p>
    </section>

    <footer className="product-footer"><span>Trade calculator</span></footer>
  </PageShell>;
}
