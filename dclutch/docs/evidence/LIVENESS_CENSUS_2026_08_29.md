...
| R2 | **A market founded with a recovery policy has no terminal at all** once its provider goes silent: the failure walk refuses `recovery_policy.is_some()` and no transition advanced a recovery attempt, so neither the success capture nor the failure walk was admissible at its deadline. Both shapes were foundable. | RED → welded → **GREEN, BUILT** | Weld `12d0deb5` (LIVE-2); **build `be8cac7b0`** (RECOVERY, decision 0027). The ladder is live: `SourceResolutionStateV2::crank_recovery_ladder` is the one transition, `funded::process_funded_transition` plans it, `RelayActionV1::AdvanceRecovery` routes it, and the weld is deleted. Walked end to end on real ELFs at 216,637 / 218,163 / 227,662 CU. **What is still owed, and it is not a strand:** the honest recovery CAPTURE has a contract transition and no provider outer, so an alternative source's window can close but the source cannot yet be observed — a recovery market's ladder terminalizes, it just cannot yet be answered on its second rung |
...
| R4 | **A vanished dealer bricks the market**: `ScheduleReplacement` is the sole liveness-vault refill and is dealer-gated (`dealer-codec lib.rs:1642`); `Fill`/`Unwind` refuse under one `work_reward` (`:1756,:1873`); exhausted vault ⇒ inventory never zero ⇒ `Retire` refuses (`:1914-1918`). No force-exit in the 8-action enum; **entry has no on-chain route either** (dealer-sbf allocates nothing). Sibling veto: one LP equity share blocks obligation close (`v3_obligation.rs:204-206`). **AMENDED by DEALER 08-30: the vanishing is the mild case and this row named the wrong mechanism.** Tracing *every* vault write: the only two that INCREASE it (`schedule :1674`, `activate :1722`) both refuse outside `Phase::Open`, so after `enter_terminal` the budget is strictly decreasing with **no refill route for any party, the dealer included** — and `fill` has no reservation floor, so a market that merely traded enough before it resolved bricks with every party present. Worse, `activate` *replaces* the vault and refunds the remainder, so a one-crank replacement candidate is a free, profitable, protocol-admitted brick no quiescence timeout would catch. | RED → **BRICK WELDED** (entry + LP veto still open) | **`d49ddda3`** — exit affordability: `validate_state` and Lean's `valid` require `active_work_remaining >= work_reward × |{nonzero inventory coordinates}|`, and an `Unwind` must retire one whole coordinate so that count is exactly the walk's cost. All eight transitions inherit it (each already revalidates its post-state). **No force-exit verb, no ninth action, no new field, no ABI change, no ELF** — the bad state is unrepresentable rather than escapable. Ruling: `docs/design/DEALER_EXIT_AFFORDABILITY_V1.md` (`5bb0555c`) |
...
| R9 | **Custody DELIVERY (`activate_batch`) has no deadline and no funding**: no Clock account in the ACT frame, only exit post-`Commit` (rollback/cleanup refuse `Committed`), caller unpaid and pays a never-closed receipt PDA (`custody dealer_reservation_v1.rs:917,1080,1346-1352,1418-1450`). **Re-verified at source by DEALER 08-30, every clause holds**: the `ACT_*` frame is 20 common accounts plus four per effect (`:96-110`) and carries no Clock — the clock index and `Clock::from_account_info` exist only on the reserve/rollback legs (`:234,:297`), so no deadline is *expressible*; `create_activation_receipt_account` (`:1418-1450`) charges `ACT_PAYER` for a receipt with no close route while the escrow's recovered rent goes to `ACT_REFUND`, so the activator is net-negative on every delivery; and `require_committed_checkpoint` (`:926`, defined `:1109`) refuses any non-`Committed` checkpoint, so the only exit is forward. | RED | queue Q4 — **ruled, not built**: it is the P1 work-escrow conversion (prepay the delivery crank at `Reserve` where the reserver is already paying four rents, draw it once at activation, make the receipt rent recoverable). Deliberately not bundled with the R4 close — R9 is a Custody ABI change and R4 needed none. Est. one Custody ELF, ~200-300 lines, one new route, four hostiles. `docs/design/DEALER_EXIT_AFFORDABILITY_V1.md` |
...
| Y1 | **Systemic: every genuine expiry/cleanup route pays a creation-fixed beneficiary, never the caller** — series permit expiry (**core-sbf** `series_permit_expiry.rs:417`), controller-ledger cleanups (**trading-sbf** `projected_custody_bootstrap_v1.rs:1274-1319`), capability close (**core-sbf** `capability.rs:582`), rent sweep (**rent-sbf** `lib.rs`, zero signers on the path, beneficiary pinned by `wallet.key != state.refund_wallet()` inside `validate_sweep_frame_v2`, `rent-contract lifecycle_v2.rs` — **cited by symbol on purpose: this pin was `:837` when first written and had drifted to `:953` within the hour**, twice, once by this lane's own commit and once by another's. In a twelve-lane shared tree a bare line number is a decaying citation; name the symbol and the predicate, and let the line be a hint), dealer checkpoint cleanup (**trading-sbf**) — where the beneficiary is even *forbidden* to pay its own fee (`dealer_scenario_checkpoint_v1.rs:1747` refuses `beneficiary.is_signer`), and the codec agrees in words: *"a cleanup caller cannot redirect rent or any other lamports"* (`dealer-codec scenario_checkpoint_v1.rs:707-708`). **Two corrections by ESCROW `fdd189ac`.** (a) *The programs*: this row read as a tree-wide smear; the sites are **3 trading-sbf / 2 core-sbf / 1 rent-sbf**, so three-fifths of the pattern is behind the frozen program — which is why it survived. (b) *A fabricated quotation, struck*: this row and the `RentCredit sweep` register row both attributed to the gauntlet the phrase "a pure donation of a transaction fee" at `journey/stages.rs:748-758`. **The gauntlet does not contain that phrase** — `grep -n donation` over that file returns nothing, and the only two occurrences in the repository were these two rows quoting each other. `stages.rs:746-771` is an *assertion*, not a lament: it fails the journey unless the fee payer moves by exactly `-fee`. The finding was right; the evidence was invented. | YELLOW → **3 of 6 converted** | **pattern P1; ruled in `docs/design/FUNDED_CRANK_V1.md`.** **Landed:** rent sweep `f8ca7ab4`, series permit expiry `1cdddf27` (*with* the Lean re-proof — `seriesPermitExpiryRefund` now models the split under four theorems), capability close enabling half `bc26e8cd` + reviewed patch in `docs/design/FUNDED_CRANK_CAPABILITY_CLOSE_PATCH.md`. **Two sizing claims of this document were wrong and are struck.** (i) *"needs a `CapabilityManifestV1` ABI change"* — **no wire moves.** `FundingLedgerCloseCustodyV2` and `FundingLedgerEntryClosePlanV2` have no `encode`/`decode`/`to_bytes`; they are in-memory computed values. The claim follows only if the fee is *authored* in the manifest, which `FUNDED_CRANK_V1.md` §3 forbids — the reward is Rent-derived and the recipient is an account. (ii) *"needs a Lean re-proof"* was true and was **not** a reason to defer: the model being spec-only means program and model agreed only by inspection, which is an argument **for** doing it. The remaining three sites are trading-sbf and ride the post-cut window |
| Y2 | ~~**Systemic: retirement is 100% permissionless and 100% unfunded**~~ → **corrected by ESCROW `117356a0`: retirement is UNFUNDED throughout, but it is NOT 100% permissionless, and the headline overstated its own detail table.** Genuinely signature-free: `begin_retiring.rs:57` and `resolution.rs:576` (both real blanket refusals — `accounts.iter().any(|value| value.is_signer)`), direct root begin-retiring, and the Rent sweep. **NOT signature-free, and these are the routes that matter:** `retire_v1`'s `authenticate_direct_privileges` is a per-account enumeration, not a blanket refusal, and its checkpoint twin *requires* a signature — `\|\| !registry_admission.is_signer` (note the negation); Rent `CloseCredit` requires a Core PDA to sign; and **all three caller-negative act-points require the caller to sign**. The census's own register row already said this correctly; the headline flattened it into a false generalization. **Two evidence corrections.** (a) `retire_v1.rs:1725,1739-1740` **move no lamports** — `:1725` is `let authority = credit.refund_wallet().to_bytes();` and the others are `\|\|` conjuncts of an identity `if`. They are the *proof the destination cannot be swapped*, cited as though they were the transfer. The conclusion holds; the evidence is the wrong kind — **the same failure mode as Y1's fabricated quotation, so this is now a pattern in this document and not one slip: it cites assertions as mechanisms.** The real transfer chain is `retire_v1` `close_market` → `close_lifecycle_credit_direct` → rent-sbf, plus `claims market_closure_v1` and the custody vault/replay closes. (b) `custody …handoff:486-492` is a **function signature** containing no check at all; the mandatory-signer rule is the `let signer = matches!(index, PAYER \| CALLER_AUTHORITY)` compared with `!=`, and the payment is the `create_account` beside it. **A third caller-negative act-point** exists that the row does not count: the core-sbf twin of the same handoff, same mandatory-PAYER shape (independent reachability not traced). | YELLOW (unfunded) — **permissionless: NO** | pattern P1; shapes + sizing in `docs/design/FUNDED_CRANK_V1.md` §3.1/§9 | | YELLOW | pattern P1 |
...
| Recovery-policy market at deadline | permissionless crank, paid from the rung's own compartment | **the funded ordered ladder** — one transition per rung, advancing while the policy funds an attempt and exhausting when it does not, then the existing failure commit. Bounded by the policy's own strictly increasing deadlines and by its attempt count | no — each rung's bounty is prepaid at founding into the compartment that rung's allocation configures | **GREEN** (`be8cac7b0`) |
...
| RentCredit sweep (`rent-sbf lib.rs:356-367`; `lifecycle_v2.rs:826-850`) | **anyone** — zero signers anywhere on the path (`SweepAccountsV2::parse`, `rent-sbf lib.rs:356-367`, performs no signer check; the contract only *forbids* the credit signing, `lifecycle_v2.rs:832-837`) | surplus sits, reclaimable later — **so this is a *surplus* route, not a closing one**, the tree's only instance (`FUNDED_CRANK_V1.md` §3.1) | no — `refund_wallet` is creation-fixed, read from the credit's own bytes and pinned at `lifecycle_v2.rs:837`; there is no caller account in the frame at all. **Quotation struck by ESCROW `fdd189ac`**: this row previously cited the gauntlet saying "a pure donation of a transaction fee" at `journey/stages.rs:748-758`. That phrase does not occur in the gauntlet or anywhere else in the tree except this census. What is there is the opposite kind of statement — `stages.rs:762-771` *asserts* the payer moves by exactly `-fee`. **NOW FUNDED — `f8ca7ab4` (ESCROW):** an OPTIONAL fourth account (the frame-length idiom `CloseAccountsV2` already used) pays a crank `min(rent_floor, swept / 16)`, so every existing three-account caller — the gauntlet, the operator, the in-file test — is byte-identical and `stages.rs:762-771` stays *true* rather than being rewritten. A **share, not a floor**, because on a surplus route the amount is caller-chosen and a plain `min(floor, residual)` would let a cranker take 100% of a floor-sized sweep forever. `LifecycleSweepPlanV2::validate_post` now checks plan against *observation* for all three accounts | **YELLOW → GREEN `f8ca7ab4`** |
| RentCredit close (`rent-sbf lib.rs:492,587-597`) | Core PDA only (CPI at retire) | rent locked | no — → `refund_wallet` | YELLOW |
| **Sleeping holder** | — | zero-supply gate (`market_closure_v1.rs:669-681`) + empty-hoard gate (`custody lib.rs:950`), zero Clock *reads* on the terminal walk ⇒ **retirement blocked forever**. ~~no Clock in the whole path~~ **struck by ESCROW `117356a0` — false, and it is the sentence that oversizes Q3.** The Clock sysvar IS a required account in the `CloseFund` frame: `const CLOSE_CLOCK: usize = 17` admitted through `require_sysvar` (`core-sbf resolution.rs`), which compares the key and never deserializes — `Clock::` appears nowhere in that file. So an escheat deadline can read a slot there at **no ABI change and no account-count change**; the account is already present at the right index and simply unused. R3's own headline says "zero Clock reads", which is the weaker claim and the true one | — | **RED** (R3) |
...
| **Dealer ScheduleReplacement** (`dealer-codec lib.rs:1642`) — the sole liveness-vault refill | **`policy.dealer_id` only** | ~~liveness vault drains ⇒ Fill/Unwind die ⇒ inventory never zero ⇒ Retire refuses forever~~ — **the drain is no longer reachable** (`d49ddda3`): the vault may never fall below the cost of unwinding the inventory it is holding, so the exit is affordable in every reachable state and needs no refill | n/a | ORANGE (was RED) — dealer-gated, but absence can no longer strand |
| **Dealer AddLiquidity / RemoveLiquidity** (`dealer-codec lib.rs:1956`) — **not named by this row originally**: three of eight actions gate on `policy.dealer_id`, not one | **`policy.dealer_id` only** (enforced on chain, `dealer-sbf lib.rs:572-574`) | no dealer-directed capital adjustment; does not strand | n/a | ORANGE — deviates from the `candidate_v1` standard ("the solver signs only to own the escrow and its refund, not to be authorized"), but not a strand |
| Dealer entry (Policy/Candidate/State creation) | **no on-chain route** — dealer-sbf allocates nothing: no `create_account`, `allocate`, `assign` or `realloc` anywhere in the crate, no System program in its 23-account frame (`validate_prefix :407-450`), and its one `invoke_signed` (`:1506`) is a CPI relay. No test drives a real entry either — every campaign injects `Policy`/`Candidate`/`State` via `ProgramTest::add_account` | no dealer can be created on chain | n/a | **RED** (R4) — but a *reachability* gap, not a liveness one: nothing is stranded because nothing is founded. **Order: entry must come AFTER `d49ddda3`'s invariant**, or the free-tightening window closes and a real migration is needed |
| Dealer obligation close (`trading dealer/v3_obligation.rs:197-206`) | requires zero equity shares AND zero obligations | one sleeping LP share blocks dealer+market retirement forever | n/a | **RED** (R4) — this is R3's shape inside the Dealer family and wants the same terminal-value ruling; it should be ruled *with* R3, not separately |
...
| Q2 | Recovery-policy market has no terminal (R2) | ~~weld~~ ~~build~~ **BOTH DONE** — weld `12d0deb5`, build `be8cac7b0` | weld: landed on `CreateFund`, the action minting the state with no exit. build: **landed**, and not by resurrecting the V1 walk — that code was deleted, and decision 0027 rules the ladder as ONE transition rather than six caller-chosen ones, because a caller who could pick `Exhaust` while an attempt was still funded could skip a leg the holders paid for. `funded::process_funded_transition` is defined, `RecoveryAdvanced` and `Exhausted` have a live route, and the un-weld was exactly what its own docstring promised: deleting `recovery_walk_has_a_live_route`. The hostile beside it did go red on the day the ladder worked, and is rewritten in the new direction — it no longer cites the ladder, it walks it. |
...
| Q4 | Dealer vanish bricks market; no entry route; LP veto (R4) | ruling + build — **re-costed by DEALER 08-30; the brick half is DONE and the force-exit was declined** | **The force-exit/quiescence-timeout this entry asked for was ruled against and is not needed.** A force-exit is a bailout for a state the protocol should not have been able to reach; the reachable brick turned out to be excluded by one missing invariant, so `d49ddda3` makes the bad state unrepresentable instead — no ninth action, no new field, no ABI change, no ELF. A permissionless `FundLiveness` top-up was considered and rejected for the same reason: a bailout route for a state that can no longer occur, costing an action, a Lean regeneration, a new custody role and a dispatch arm to insure against nothing. **What remains of Q4**: (a) the on-chain entry route (build, large, and strictly *after* the invariant — see the register row); (b) the LP veto, which is R3's shape and should ride R3's terminal-value ruling rather than a Dealer-specific one; (c) the funder-is-the-authority deviation (three of eight actions gate on `policy.dealer_id`) — real, recorded, not a strand. **Not the least-live family any more; the least-*reachable* one.** Ruling and named debt: `docs/design/DEALER_EXIT_AFFORDABILITY_V1.md`. |
...
**Amended 08-30**: Q4's brick is closed (`d49ddda3`) and its force-exit was ruled
against rather than built. What is left of it is not "largest" but *least
reachable* — the Dealer family has no on-chain entry route at all, so nothing is
stranded because nothing is founded. That is also why the fix was free, and it
carries the one ordering constraint this census now imposes on another lane:
**build Dealer entry only after the exit-affordability invariant**, or the
free-tightening window closes and a genuine migration is needed.
