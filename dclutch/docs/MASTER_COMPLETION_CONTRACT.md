...
| C-07 | Series | Recurring Series found, prepare, issue/consume/expire tickets, redistribute funding once, settle occurrences, close terminal state and replay safely across more than one occurrence. |
...
| C-12 | Stranger-operable product | Static web, Wallet Standard, SDK and CLI support creation, discovery, maker/taker trade, liquidity, Series, resolution, portfolio, redemption and recovery. Inputs are typed/derived with provenance; relays and indexes remain optional untrusted projections; mobile and accessible interaction is complete. |
...
| Privacy horizon | Whether the accepted final public project includes the original FHE/MPC/energy objective | open; do not infer from the old horizon park |
...
| Recovery ontology: keep or cut | `RecoveryPolicyV2`, `RecoveryAttemptV2`, `source_recovery_policy_v2.rs` and recovery funding are all live, green and kernel-complete — with NO live on-chain route. `funded::process_funded_transition` has no definition; the per-leg `FailNext` walk sits inside `#[cfg(any())]`, so the ELF builds because the block never compiles. `SourceResolutionStateV2` has exactly three transitions and none advances a recovery attempt, so a market with a recovery policy cannot be terminalized at all | open, and `AGENTS.md` forbids parallel legacy/current authority paths, so it cannot simply sit. Compactly: **does dClutch intend markets that buy named alternative sources?** If yes the funded ordered-recovery route returns; if no the ontology gets cut. Related stale claim: the harness README documents a CU table ("funded recovery 294,002", "funded exhaustion 292,213") describing a campaign that is `#[ignore]`d and whose route no longer compiles |
