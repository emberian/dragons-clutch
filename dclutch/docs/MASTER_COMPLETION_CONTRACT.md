...
| Sustainable economics | Exact fee, upkeep, donation and opener policies after adversarial modelling | **RULED, amended by ember** — [0024](decisions/0024-sustainable-economics-and-a-governable-parameter-surface.md); the models are `docs/design/ECONOMICS_MODELS_2026_09_04.md`. The upkeep vault is **built**: contract, Lean, two Custody routes (`0x6200` band), and one producer — the Direct close-maker credits the donation remainder by CPI. Two chartered inflows have no producer yet and say so at the site (the certificate-seat prepay, whose seat has no close route; the escrow-close residue, which a landed argument already homes in the market's RentCredit). The governable record exists with its own Custody routes (`0x6100`), refuses a take before mainnet by name, and is READ at run time by exactly one consumer, the closer's carve; the fee band and the crank cap still project its genesis as constants because their frames do not carry it |
