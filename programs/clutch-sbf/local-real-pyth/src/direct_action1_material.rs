...
use crate::action_material::{
    chain_derived_direct_role_v2, direct_role_label_v1,
    decode_release_artifact, finish_chain_derived_direct_action1_material_v2,
    ActionFreshnessBoundaryV1,
    CanonicalActionMaterialErrorV1, CanonicalActionMaterialV1, DirectAccountRoleV1,
...
use clutch_product_series::{
    CompiledProductSeriesBundleV7, DirectGlobalLivenessPhaseV2,
    EvidenceOnlyRecoveryPolicyV1, FixedCodec, MarketFamilyCapabilityPolicyV1,
    MarketFamilyV1, MarketGenesisProfileV2, MarketInstancePreimageV2,
    MarketLifecyclePhaseV3, MarketLifecycleReplayPhaseV2, NativeClaimBasisV1,
    PriceMeasurePolicyV1, ProductTemplateV4, RegistryCapabilityProfileV4,
    SeriesAttachmentPlanV6, SeriesFundingQuoteV6, SeriesFundingTermsV2,
    SeriesMarketLinkPhaseV3, SeriesPlanV5,
};
use clutch_solana_layout::artifact::ArtifactKind;
use clutch_solana_layout::product_series::{
    MarketLifecycleReplayAccountV2, MarketLifecycleRootAccountV3,
    ProductDirectGlobalLivenessAccountV2, SeriesFundingAccountV5,
    SeriesMarketLinkAccountV3, SeriesRegistryAccountV4,
    PRODUCT_DIRECT_GLOBAL_LIVENESS_PDA_PREFIX_V2, SERIES_FUNDING_PDA_PREFIX_V1,
...
use clutch_source_plane_v3_runtime::{
    authenticate_source_release_account, RuntimeAccountViewV1, RuntimeDerivedPdaV1,
    RuntimeKey, SourceReleaseManifestV2,
};
...
impl<'a> DirectAction1ProductRootSnapshotV2<'a> {
    ...
    fn ordered_addresses(self) -> [Address; DIRECT_ACTION1_ACCOUNT_COUNT_V2] {
        ...
        [
            general[0].address, general[1].address, general[2].address,
            general[3].address, general[4].address, general[5].address,
            general[6].address, general[7].address, general[8].address,
            general[9].address, general[10].address, general[11].address,
            general[12].address, general[13].address, general[14].address,
            general[15].address, general[16].address, general[17].address,
            general[18].address, general[19].address, general[20].address,
            general[21].address, general[22].address, general[23].address,
            general[24].address, self.product_replay_v2.address,
            ...
            self.fresh_direct_root_v3, self.fresh_direct_replay_v1,
            self.payer.address, self.system_program.address,
            self.rent_sysvar.address, self.clock_sysvar.address,
        ]
    }
}
...
impl DirectAction1CanonicalMaterialV2 {
    pub const fn snapshot_id(&self) -> [u8; 32] { self.snapshot_id }
    pub const fn revenue_policy_digest(&self) -> [u8; 32] { self.revenue_policy_digest }
    pub const fn lookup_table(&self) -> Address { self.lookup_table }
    pub const fn direct_root_rent_lamports(&self) -> u64 { self.direct_root_rent_lamports }
    pub const fn direct_replay_rent_lamports(&self) -> u64 { self.direct_replay_rent_lamports }
...
pub fn construct_direct_action1_material_v2(
    ...
    let binding_id = binding.id()
        .map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let link = SeriesMarketLinkAccountV3::decode(
        &snapshot.general.founder_series_link_v3.data,
    ).map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let link_binding = link.state.binding_ref();
    let link_id = link.state.semantic_id()
        .map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let root_pda = pda(release.program_id, &[
        PRODUCT_ROOT_SEED_V3,
        &binding.market_instance_id.bytes(),
        &binding.generation.to_le_bytes(),
    ]);
    let link_pda = pda(release.program_id, &[
        SERIES_LINK_SEED_V3,
    ...
    let general = authenticate_general_and_revenue(
        release, snapshot.general, &root, &link,
    )?;
    let replay = authenticate_product_replay(
        release, snapshot.product_replay_v2, binding,
    )?;
    let family_policy = authenticate_family_policy(
        release, snapshot.family_capability_policy_v1, replay.state.binding(), binding,
    )?;
    ...
    authenticate_direct_liveness(release, snapshot, binding, general.product_preauthorization_id())?;

    let direct_root = pda(release.program_id, &[
        DIRECT_ROOT_SEED_V3,
        &binding.market_instance_id.bytes(),
        &binding.generation.to_le_bytes(),
    ]);
    let direct_replay = pda(release.program_id, &[
        DIRECT_REPLAY_SEED_V1,
        direct_root.0.as_ref(),
    ]);
    authenticate_absence(
        release, freshness, snapshot.fresh_direct_root_v3,
        direct_root.0, snapshot.fresh_direct_root_absence,
    )?;
    authenticate_absence(
        release, freshness, snapshot.fresh_direct_replay_v1,
        direct_replay.0, snapshot.fresh_direct_replay_absence,
    )?;
    require_system_roles(snapshot)?;

    let rent: Rent = bincode::deserialize(&snapshot.rent_sysvar.data)
        .map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let direct_root_rent_lamports = rent.minimum_balance(
        clutch_solana_layout::registry::DIRECT_MARKET_ROOT_ACCOUNT_BYTES_V3,
    );
    let direct_replay_rent_lamports = rent.minimum_balance(
        clutch_solana_layout::registry::DIRECT_ACTION_REPLAY_ACCOUNT_BYTES,
    );
    ...
    for (index, (address, role)) in addresses.into_iter().zip(roles).enumerate() {
        ...
        let signer = index == 37;
        metas.push(AccountMeta { pubkey: address, is_signer: signer, is_writable: writable });
        let projected = chain_derived_direct_role_v2(
            direct_role_label_v1(role), address, writable, signer,
        );
    ...
    let canonical = finish_chain_derived_direct_action1_material_v2(
        release, manifest, builder, workflow_id, freshness,
        snapshot.general.product_root_v3.address,
        snapshot.general.product_root_v3.provenance.slot,
        binding.generation, snapshot_id, metas, account_roles, equations, lookup_table,
    )?;
...
fn authenticate_general_and_revenue(
    ...
    for account in [
        frame.market_binding_v5, frame.market_runtime_v3, frame.series_funding_v5,
        frame.series_registry_v4, frame.source_release_v2, frame.compiler_bundle_v7,
        frame.market_instance_v2, frame.realm, frame.revenue_policy_record_v2,
        frame.revenue_policy_preimage_v2, frame.series_plan_v5, frame.funding_terms_v2,
        frame.source_template_v4, frame.native_claim_basis_v1, frame.recovery_policy_v1,
        frame.price_measure_policy_v1, frame.market_genesis_v2, frame.funding_quote_v6,
    ...
    let binding = root.state.binding_ref();
    let link_binding = link.state.binding_ref();
    let market_pda = pda(release.program_id, &[
        MARKET_BINDING_SEED_DOMAIN_V1, &relation.market_instance_v2_id.bytes(),
    ]);
    let runtime_pda = pda(release.program_id, &[
        MARKET_RUNTIME_SEED_DOMAIN_V1, frame.market_binding_v5.address.as_ref(),
    ]);
    if frame.market_binding_v5.address != market_pda.0
        ...
        || current.product_market_binding_v3_id().bytes()
            != binding.id().map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?.bytes()
        ...
        || current.series_market_link_v3_id().bytes()
            != link.state.semantic_id().map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?.bytes()
    ...
    let funding = SeriesFundingAccountV5::decode(&frame.series_funding_v5.data)
        .map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let registry_pda = pda(release.program_id, &[
        SERIES_REGISTRY_PDA_PREFIX_V1, &registry.series_plan_id.bytes(),
    ]);
    let funding_pda = pda(release.program_id, &[
        SERIES_FUNDING_PDA_PREFIX_V1, &registry.series_plan_id.bytes(),
    ]);
    ...
    let record_pda = pda(release.program_id, &[REVENUE_RECORD_SEED_V2, frame.realm.address.as_ref()]);
    let preimage_pda = pda(release.program_id, &[REVENUE_PREIMAGE_SEED_V2, frame.realm.address.as_ref()]);
    ...
    let policy_id = revenue_policy_v2_digest(&policy)
        .map_err(|_| CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let record_floor = record.terminal_payer_principal
        .checked_add(record.terminal_donation_floor)
        .ok_or(CanonicalActionMaterialErrorV1::InvalidChainState)?;
    let preimage_floor = record.policy_preimage_payer_principal
        .checked_add(record.policy_preimage_donation_floor)
        .ok_or(CanonicalActionMaterialErrorV1::InvalidChainState)?;
    if frame.realm.address != realm_pda.0 || realm.stored_bump != realm_pda.1
...
fn authenticate_artifacts(
    ...
    let template = ProductTemplateV4::decode(&frame.source_template_v4.data).map_err(|_| chain())?;
    let basis = NativeClaimBasisV1::decode(&frame.native_claim_basis_v1.data).map_err(|_| chain())?;
    let recovery = EvidenceOnlyRecoveryPolicyV1::decode(&frame.recovery_policy_v1.data).map_err(|_| chain())?;
    let price = PriceMeasurePolicyV1::decode(&frame.price_measure_policy_v1.data).map_err(|_| chain())?;
    let genesis = MarketGenesisProfileV2::decode(&frame.market_genesis_v2.data).map_err(|_| chain())?;
    let quote = SeriesFundingQuoteV6::decode(&frame.funding_quote_v6.data).map_err(|_| chain())?;
    let attachment = SeriesAttachmentPlanV6::decode(&frame.attachment_plan_v6.data).map_err(|_| chain())?;
    let bundle = CompiledProductSeriesBundleV7::decode(&frame.compiler_bundle_v7.data).map_err(|_| chain())?;
    let market = MarketInstancePreimageV2::decode(&frame.market_instance_v2.data).map_err(|_| chain())?;
    let capability = RegistryCapabilityProfileV4::decode(&frame.capability_profile_artifact.data).map_err(|_| chain())?;
    ...
    for (account, kind, id) in [
        (frame.series_plan_v5, ArtifactKind::SeriesPlanV5, series_id.bytes()),
        (frame.funding_terms_v2, ArtifactKind::SeriesFundingTermsV2, terms_id.bytes()),
        (frame.source_template_v4, ArtifactKind::ProductTemplateV4, template_id.bytes()),
        (frame.native_claim_basis_v1, ArtifactKind::NativeClaimBasisV1, basis_id.bytes()),
        (frame.recovery_policy_v1, ArtifactKind::EvidenceOnlyRecoveryPolicyV1, recovery_id.bytes()),
        (frame.price_measure_policy_v1, ArtifactKind::PriceMeasurePolicyV1, price_id.bytes()),
        (frame.market_genesis_v2, ArtifactKind::MarketGenesisProfileV2, genesis_id.bytes()),
        (frame.funding_quote_v6, ArtifactKind::SeriesFundingQuoteV6, quote_id.bytes()),
        (frame.attachment_plan_v6, ArtifactKind::SeriesAttachmentPlanV6, attachment_id.bytes()),
        (frame.compiler_bundle_v7, ArtifactKind::CompiledProductSeriesBundleV7, bundle_id.bytes()),
        (frame.market_instance_v2, ArtifactKind::MarketInstancePreimageV2, market_id.bytes()),
        (frame.capability_profile_artifact, ArtifactKind::RegistryCapabilityProfileV4, capability_id.bytes()),
...
fn authenticate_source_release(
    ...
    let recipe = PdaRecipeV3::source_release(manifest.id().map_err(|_| chain())?)
        .map_err(|_| chain())?;
    ...
    let authenticated = authenticate_source_release_account(
        runtime_key(release.program_id), account_view(account), RuntimeDerivedPdaV1 {
            program_id: runtime_key(release.program_id),
            recipe_id: recipe.id().map_err(|_| chain())?,
            address: runtime_key(derived.0), bump: derived.1,
        },
    ).map_err(|_| chain())?;
    if authenticated.id().bytes() != expected_authentication_id.bytes() { return invalid(); }
    Ok(())
}
...
fn authenticate_product_replay(
    ...
    let expected = pda(release.program_id, &[
        PRODUCT_REPLAY_SEED_V2, &binding.market_instance_id.bytes(),
    ]);
    if account.address != expected.0 || replay.stored_bump != expected.1
...
fn authenticate_family_policy(
    ...
    authenticate_artifact_address(
        release, account, ArtifactKind::MarketFamilyCapabilityPolicyV1, id.bytes(),
    )?;
...
fn authenticate_direct_liveness(
    ...
    if snapshot.product_direct_global_liveness_v2.address != expected.0
        ...
        || snapshot.product_direct_global_liveness_v2.lamports
            < manifest.rent_principal_lamports
        ...
        || manifest.state.account_id().bytes()
            != snapshot.product_direct_global_liveness_v2.address.to_bytes()
    ...
    { return invalid(); }
    let kinds = [
        RuntimeCompartmentKindV1::Source, RuntimeCompartmentKindV1::Candidate,
        RuntimeCompartmentKindV1::Clearing, RuntimeCompartmentKindV1::Settlement,
        RuntimeCompartmentKindV1::Resolution, RuntimeCompartmentKindV1::Retirement,
        RuntimeCompartmentKindV1::Recovery,
    ];
    for (index, (account, kind)) in snapshot.liveness_compartments.into_iter().zip(kinds).enumerate() {
        require_program_owned(release, account)?;
        let row = RuntimeCompartmentV1::decode(&account.data).map_err(|_| chain())?;
        let expected = pda(release.program_id, &[
            DIRECT_LIVENESS_ROW_SEED_V2, &binding.market_instance_id.bytes(),
            &binding.generation.to_le_bytes(), &[u8::try_from(index).map_err(|_| chain())?],
        ]);
        let floor = row.remaining_work_lamports.checked_add(row.rent_locked_lamports)
            .and_then(|value| value.checked_add(row.donation_remaining_lamports))
            .ok_or(CanonicalActionMaterialErrorV1::InvalidChainState)?;
        if account.address != expected.0 || row.kind != kind
            || row.identity.account_id.bytes() != account.address.to_bytes()
            || row.identity.generation != binding.generation
            || manifest.state.compartment_account(index).map(|id| id.bytes())
...
fn authenticate_absence(
    ...
    if supplied == Address::default() || supplied != derived
...
fn require_system_roles(snapshot: DirectAction1ProductRootSnapshotV2<'_>) -> Result<()> {
    ...
    if snapshot.payer.address == Address::default() || snapshot.payer.owner != system
        || snapshot.payer.executable || !snapshot.payer.data.is_empty()
        || snapshot.system_program.address != system || !snapshot.system_program.executable
        || snapshot.rent_sysvar.address != rent || snapshot.rent_sysvar.owner != sysvar_owner
        || snapshot.rent_sysvar.executable
        || snapshot.clock_sysvar.address != clock || snapshot.clock_sysvar.owner != sysvar_owner
        || snapshot.clock_sysvar.executable || snapshot.clock_sysvar.data.len() != 40
    { return invalid(); }
    let slot = u64::from_le_bytes(snapshot.clock_sysvar.data[..8].try_into().map_err(|_| chain())?);
    if slot != snapshot.clock_sysvar.provenance.slot { return invalid(); }
    Ok(())
}
...
fn authenticate_artifact_address(
    ...
    if account.address != expected.0 || account.data.len() != kind.exact_len() { return invalid(); }
    Ok(())
}

fn require_program_owned(release: &IndexedProgramRelease, account: &ObservedRpcAccount) -> Result<()> {
    if account.address == Address::default() || account.owner != release.program_id
        || account.executable || account.lamports == 0
    { return invalid(); }
    Ok(())
}
...
fn action1_roles() -> [DirectAccountRoleV1; DIRECT_ACTION1_ACCOUNT_COUNT_V2] {
    ...
    [
        R::GeneralMarketBinding, R::GeneralMarketRuntime, R::ProductRoot,
        R::WritableFounderSeriesLink, R::SeriesFunding, R::SeriesRegistry,
        R::RegistryProgram, R::RegistryProgramData, R::RegistryReleaseArtifact,
        R::CapabilityProfileArtifact, R::SourceRelease, R::CompilerBundle,
        R::MarketInstance, R::Realm, R::RevenuePolicyRecord, R::RevenuePolicy,
        R::SeriesPlan, R::FundingTerms, R::SourceTemplate, R::NativeClaimBasis,
        R::RecoveryPolicy, R::PriceMeasurePolicy, R::MarketGenesis, R::FundingQuote,
        R::AttachmentPlan, R::ProductReplay, R::FamilyCapabilityPolicy,
        R::ProductDirectGlobalLiveness, R::LivenessSource, R::LivenessCandidate,
        R::LivenessClearing, R::LivenessSettlement, R::LivenessResolution,
        R::LivenessRetirement, R::LivenessRecovery, R::DirectRoot, R::DirectReplay,
        R::ActorPayer, R::SystemProgram, R::RentSysvar, R::ClockSysvar,
    ]
}
...
fn snapshot_id(
    ...
    for (address, absence) in [
        (snapshot.fresh_direct_root_v3, snapshot.fresh_direct_root_absence),
        (snapshot.fresh_direct_replay_v1, snapshot.fresh_direct_replay_absence),
    ] {
...
fn account_view(account: &ObservedRpcAccount) -> RuntimeAccountViewV1<'_> {
    RuntimeAccountViewV1 {
        key: runtime_key(account.address), owner: runtime_key(account.owner),
        lamports: account.lamports, executable: account.executable,
        signer: false, writable: false, data: &account.data,
    }
}
...
fn invalid<T>() -> Result<T> { Err(chain()) }
