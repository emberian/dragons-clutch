...
use clutch_general_v2_contract::{
    ...
    MARKET_BINDING_ACCOUNT_VERSION_V3, MARKET_BINDING_ACCOUNT_VERSION_V4,
...
pub const CANONICAL_ACCOUNT_DECODER_SET: &str =
    "dragons-clutch/canonical-account-decoders/v10-source-work-schedule-general-v4-current-v3-historical-no-keeper-no-selected-candidate-direct-b1-v2-product-general-revenue-current";
...
fn decode_direct(account: &ObservedRpcAccount) -> Result<Option<CanonicalAccountProjection>> {
    ...
    ) && data.len() == registry::DIRECT_SELECTION_ACCOUNT_BYTES
    {
        let bytes: &[u8; registry::DIRECT_SELECTION_ACCOUNT_BYTES] = data
            .try_into()
            .map_err(|_| AccountIndexError::CanonicalDecodeRefused)?;
    ...
    ) && data.len() == registry::DIRECT_ACTION_REPLAY_ACCOUNT_BYTES
    {
        let bytes: &[u8; registry::DIRECT_ACTION_REPLAY_ACCOUNT_BYTES] = data
            .try_into()
            .map_err(|_| AccountIndexError::CanonicalDecodeRefused)?;
    ...
    ) && data.len() == registry::DIRECT_RESERVATION_ACCOUNT_BYTES
    {
        let bytes: &[u8; registry::DIRECT_RESERVATION_ACCOUNT_BYTES] = data
            .try_into()
            .map_err(|_| AccountIndexError::CanonicalDecodeRefused)?;
...
fn decode_general(data: &[u8]) -> Result<Option<CanonicalAccountProjection>> {
    let mut projection = if tag_version(
        ...
        projection.generation = Some(value.authority().product_generation());
...
mod current_decoder_tests {
    ...
    fn withdrawn_versions_do_not_enter_the_live_decoder_set() {
        ...
        assert_eq!(
            ...
            Err(AccountIndexError::CanonicalDecodeRefused),
            "a truncated current MarketBindingV4 cannot fall through to an older decoder",
        );
        assert_eq!(
        ...
        assert_eq!(
            decode_general(&[
                clutch_general_v2_contract::SELECTED_CANDIDATE_ACCOUNT_TAG,
                clutch_general_v2_contract::SELECTED_CANDIDATE_ACCOUNT_VERSION,
            ]),
            Ok(None),
            "SelectedCandidate V1 is not a live decoder coordinate",
        );
...
mod direct_decoder_tests {
    ...
    fn reservation_frame_requires_root_context_and_canonical_header() {
        let frame = DirectReservationAccountV1::new(
            7,
            [9; DIRECT_RESERVATION_BODY_BYTES_V1],
        )
        .unwrap();
