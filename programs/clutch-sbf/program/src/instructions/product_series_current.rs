...
const PRODUCT_DIRECT_FAMILY_TERMINAL_DOMAIN_V2: &[u8] =
    b"dragons-clutch/sbf/product-direct-family-terminal/v2\0";
...
/// Move-only RootV2 postwrite proving that Product consumed the sealed Direct
/// terminal receipt only after the sole Candidate allocation was retired in
/// the hostile-reopened `0xba/v2` manifest.
#[derive(Debug, Eq, PartialEq)]
pub(crate) struct AuthenticatedProductDirectFamilyTerminalV2 {
    id: ContentId,
    root_account: Pubkey,
    root_authentication_before: ContentId,
    root_authentication_after: ContentId,
...
impl AuthenticatedProductDirectFamilyTerminalV2 {
    pub(crate) const fn id(&self) -> ContentId { self.id }
    pub(crate) const fn root_account(&self) -> Pubkey { self.root_account }
    pub(crate) const fn root_authentication_before(&self) -> ContentId {
        self.root_authentication_before
...
struct ExactDirectFamilyTerminalAuthorityV2 {
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
    family_root_id: ContentId,
...
impl AuthenticatedMarketFamilyAuthorityV1 for ExactDirectFamilyTerminalAuthorityV2 {
    fn authenticate_terminal(
        &self,
        current: &MarketFamilyAggregatorV1,
        family: MarketFamilyV1,
        ...
        let current_id = current
            .semantic_id()
            .map_err(|_| clutch_product_series::Error::UnauthenticatedAuthority)?
            .content_id();
        if family != MarketFamilyV1::Direct
            || current.binding().market_instance_id != self.market_instance_id
            || current.binding().generation != self.generation
            || current_id != self.family_prestate_id
...
/// Retire Product's one live Direct family child after—and only after—the
/// sealed eighth Candidate call has advanced `0xba/v2`. The returned RootV2
/// authority remains move-only and is the immediate predecessor consumed by
/// the whole-Series retirement composer.
#[inline(never)]
pub(crate) fn consume_direct_family_terminal_postwrite_v2<'next>(
    program_id: &Pubkey,
    root_account: &AccountInfo<'_>,
    authenticated: AuthenticatedMarketLifecycleRootV2<'_>,
    direct_root_account: Pubkey,
    manifest_retirement: AuthenticatedProductDirectCandidateRetirementV2,
    ...
    let current = authenticated.state();
    let binding = current.binding();
    let families = current.product_families();
    let direct = families.family(MarketFamilyV1::Direct);
    let family_root_id = families
        .binding()
        .family_root_id(MarketFamilyV1::Direct);
    let semantic_before = current
        .semantic_id()
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    let family_prestate_id = families
        .semantic_id()
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?
        .content_id();
    let terminal_receipt_id = ContentId::from_bytes(sealed.terminal_receipt_id);
    let expected_direct_root_id = ContentId::from_bytes(direct_root_account.to_bytes());
    let manifest_retirement_id = manifest_retirement.id();
    let manifest_account = manifest_retirement.account();
    let manifest_state_before = manifest_retirement.state_before();
    let manifest_state_after = manifest_retirement.state_after();
    for id in [
        terminal_receipt_id,
        manifest_retirement_id,
    ...
    require(
        authenticated.is_writable()
            ...
            && *root_account.key == authenticated.account()
            && current.phase() == MarketLifecyclePhaseV2::Retiring
            && family_root_id == expected_direct_root_id
            && direct.status() == MarketFamilyStatusV1::Live
            && direct.counts().admitted == 1
            && direct.counts().live == 1
            && direct.counts().terminal == sealed.family_terminal_sequence
            && family_prestate_id.bytes() == sealed.product_family_prestate_id
            && manifest_retirement.lifecycle_root_account().bytes()
            ...
            && manifest_state_before != manifest_state_after,
        ClutchError::MismatchedState,
    )?;
    let authority = ExactDirectFamilyTerminalAuthorityV2 {
        market_instance_id: binding.market_instance_id,
        generation: binding.generation,
        family_root_id,
        family_prestate_id,
        family_terminal_sequence: sealed.family_terminal_sequence,
        terminal_receipt_id,
    };
    *successor_output = (*current)
    ...
    let semantic_after = rebound
        ...
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    let id = hashv(&[
        PRODUCT_DIRECT_FAMILY_TERMINAL_DOMAIN_V2,
        program_id.as_ref(),
        ...
        &sealed.family_terminal_sequence.to_le_bytes(),
        &manifest_retirement_id.bytes(),
        manifest_account.as_ref(),
        &manifest_state_before.bytes(),
        &manifest_state_after.bytes(),
    ]);
    require_live(id)?;
    Ok((
        rebound,
        AuthenticatedProductDirectFamilyTerminalV2 {
            ...
            root_semantic_before: semantic_before,
            root_semantic_after: semantic_after,
            terminal_receipt_id,
            family_terminal_sequence: sealed.family_terminal_sequence,
            manifest_retirement_id,
            manifest_account,
