...
use crate::instructions::failure_market_admission::{
    authenticate_failure_market_close_prestate_v2, AuthenticatedFailureMarketRootV2,
};
...
use crate::instructions::failure_market_replay_v2::{
    authenticate_failure_market_replay_v2, reopen_failure_market_replay_v2,
...
use clutch_failure_policy_runtime::market_replay_v2::FailureMarketReplayPhaseV2;
...
const TERMINAL_ADMISSION_CLOSE_DOMAIN_V2: &[u8] =
    b"dragons-clutch/sbf/failure-market-terminal-admission-close/v2\0";
...
/// Final physical deletion of the immutable Failure admission root.
///
/// The permanent replay remains live forever. The mutable runtime, reusable
/// cell, and append-only history must already be canonical zero-lamport System
/// accounts, so this receipt cannot be used to detach semantic authority before
/// Product has consumed the complete Failure family.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct AuthenticatedFailureMarketAdmissionCloseV2 {
    id: ContentId,
    market_terminal_id: ContentId,
    admission_state_id: ContentId,
    replay_authentication_id: ContentId,
    admission_account: Pubkey,
    rent_refund_owner: Pubkey,
    neutral_sink: Pubkey,
    refunded_principal_lamports: u64,
    neutralized_donation_lamports: u64,
...
impl AuthenticatedFailureMarketAdmissionCloseV2 {
    ...
    pub(crate) const fn admission_account(self) -> Pubkey {
        self.admission_account
    }
...
fn require_closed_failure_dependent(account: &AccountInfo<'_>) -> Outcome<()> {
    require(
        !account.is_writable
            && !account.is_signer
            && !account.executable
...
/// Close the immutable admission root only after every deletable Failure
/// dependent has physically closed and Product's exhaustive Market terminal is
/// durably readable.
///
/// The replay capitalization preimage is not terminal authority. It merely
/// reopens the private receipt whose digest is already committed by the live,
/// program-owned permanent replay body.
#[allow(clippy::too_many_arguments)]
pub(crate) fn close_failure_market_admission_root_v2<'a>(
    program_id: &Pubkey,
    market_root_account: &AccountInfo<'a>,
    admission_root_account: &AccountInfo<'a>,
    runtime_root_account: &AccountInfo<'a>,
    ...
    rent_refund_owner: &AccountInfo<'a>,
    neutral_sink: &AccountInfo<'a>,
    replay_funding_preimage: FailureMarketReplayFundingPreimageV2,
    market_root_output: &mut MarketLifecycleRootAccountV1,
) -> Outcome<AuthenticatedFailureMarketAdmissionCloseV2> {
    require_distinct(&[
        market_root_account.clone(),
        admission_root_account.clone(),
        runtime_root_account.clone(),
    ...
    require_closed_failure_dependent(runtime_root_account)?;
    require_closed_failure_dependent(interval_cell_account)?;
    require_closed_failure_dependent(interval_history_account)?;
    for recipient in [rent_refund_owner, neutral_sink] {
        require(
            recipient.is_writable
                && !recipient.is_signer
    ...
    let close = authenticate_failure_market_close_prestate_v2(
        program_id,
        admission_root_account,
        rent_refund_owner,
        neutral_sink,
    )?;
    let admission: AuthenticatedFailureMarketRootV2 = close.root();
    let policy = admission.state().binding().facts();
    let market_terminal = authenticate_market_instance_terminal_v1(
        program_id,
        market_root_account,
    ...
    let admission_state_id = ContentId::from_bytes(
        admission
            .state()
            .id()
    ...
    require(
        market_terminal.root_account() == *market_root_account.key
            && market_terminal.owner_program() == *program_id
            && market_terminal.market_instance_id() == policy.market_instance_id
            && market_terminal.generation() == policy.generation
            && !market_terminal.failure_terminal_receipt_id().is_zero()
            && replay.replay().phase() == FailureMarketReplayPhaseV2::Terminal
            && replay.replay().failure_policy_binding_id() == admission.state().binding().id()
            && replay.replay().market_instance_id() == policy.market_instance_id
            && replay.replay().generation() == policy.generation
            && replay.replay().family_aggregate_receipt_id().bytes() != [0; 32]
            && replay.replay().runtime_terminal_state_commitment().bytes() != [0; 32],
        ClutchError::MismatchedState,
    )?;
    let disposition = close.disposition();
    {
        ...
        **refund_lamports = close.refund_owner_post_balance();
        **sink_lamports = close.neutral_sink_post_balance();
    }
    admission_root_account
        .resize(0)
        .map_err(|_| Refusal::Adapter(ClutchError::AccountCreationFailed))?;
    ...
    require(
        admission_root_account.lamports() == 0
            && admission_root_account.data_len() == 0
            && *admission_root_account.owner == SYSTEM_PROGRAM_ID
            && rent_refund_owner.lamports() == close.refund_owner_post_balance()
            && neutral_sink.lamports() == close.neutral_sink_post_balance(),
        ClutchError::MismatchedState,
    )?;
    let id = ContentId::from_bytes(
        solana_sha256_hasher::hashv(&[
            TERMINAL_ADMISSION_CLOSE_DOMAIN_V2,
            &market_terminal.id().bytes(),
            ...
            rent_refund_owner.key.as_ref(),
            neutral_sink.key.as_ref(),
            &disposition.rent_refund_lamports().to_le_bytes(),
            &disposition.donation_neutral_lamports().to_le_bytes(),
        ])
        .to_bytes(),
    );
    require(!id.is_zero(), ClutchError::MismatchedState)?;
    Ok(AuthenticatedFailureMarketAdmissionCloseV2 {
        id,
        market_terminal_id: market_terminal.id(),
        admission_state_id,
        replay_authentication_id: replay.authentication_id(),
        admission_account: *admission_root_account.key,
        rent_refund_owner: *rent_refund_owner.key,
        neutral_sink: *neutral_sink.key,
        refunded_principal_lamports: disposition.rent_refund_lamports(),
        neutralized_donation_lamports: disposition.donation_neutral_lamports(),
    })
}
...
mod adversarial_retirement_tests {
    ...
    #[test]
    fn admission_close_requires_every_deletable_dependent_already_closed() {
        let source = include_str!("failure_market_retirement_v2.rs");
        let close = source
            .split("fn close_failure_market_admission_root_v2")
            .nth(1)
            .and_then(|value| value.split("#[cfg(test)]").next())
            .expect("final admission close");
        assert_eq!(close.matches("require_closed_failure_dependent(").count(), 3);
        assert!(close.contains("authenticate_market_instance_terminal_v1"));
        assert!(close.contains("reopen_failure_market_replay_v2"));
        assert!(close.contains("FailureMarketReplayPhaseV2::Terminal"));
        assert!(close.contains("admission_root_account.assign(&SYSTEM_PROGRAM_ID)"));
        assert!(!close.contains("replay_account.assign"));
    }
}
