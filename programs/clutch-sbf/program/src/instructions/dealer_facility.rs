...
use clutch_dealer_runtime_contract::{
    ...
    prepare_dealer_series_obligation_value_close_v3, prepare_dealer_terminal_replay_v2,
    ...
    DealerAssetTransferPostObservationV2, DealerEmptyAssetTransferBundleV1,
    DealerChildCountsV2,
    ...
    DealerTransitionLivenessModeV1, close_dealer_position_replay_v2,
    ...
    DealerSeriesObligationBindingV3, DealerSeriesObligationKeyV3,
    DealerSeriesObligationPhaseV1, DealerSeriesObligationValueClosePlanV3,
    DealerStateV2, DealerStateV3,
...
use clutch_product_series::{
    ...
    MarketFamilyV1, MarketLifecyclePhaseV3, MarketLifecycleRootV3,
    NativeClaimBasisV1, PriceMeasurePolicyV1, ProductTemplateV4,
    QuantizedEdgePolicyV1, SeriesLinkObligationStatusV3, SeriesLinkObligationV3,
    SeriesMarketLinkPhaseV3, SeriesMarketLinkV3, SeriesPlanV5Id,
};
...
use clutch_fractional_redemption_runtime::{
    ...
    FractionalCreditV2, FractionalLedgerV1,
...
use super::collateral_position_v3::{
    ...
    authenticate_resolution_v5, GeneralMarketValueAuthorityV2,
...
use super::product_market_replay_current::{
    authenticate_market_lifecycle_replay_v2, AuthenticatedMarketLifecycleReplayV2,
};
...
const SELECT_LEASE_BEGIN_FIXED_ACCOUNT_COUNT: usize = 53;
...
const DEALER_PRODUCT_RESOLUTION_AUTHENTICATION_DOMAIN_V3: &[u8] =
    b"dragons-clutch/sbf/dealer-product-resolution-authentication/v3\0";
...
const DEALER_FAMILY_TERMINAL_RECEIPT_DOMAIN_V3: &[u8] =
    b"dragons-clutch/sbf/dealer-family-terminal-receipt/v3\0";
const _: () = assert!(
...
const DEALER_PRODUCT_FOUNDING_PREWRITE_DOMAIN_V3: &[u8] =
    b"dragons-clutch/sbf/dealer-product-founding-prewrite/v3\0";
const DEALER_PRODUCT_FAMILY_POSTWRITE_DOMAIN_V3: &[u8] =
    b"dragons-clutch/sbf/dealer-product-family-postwrite/v3\0";
const DEALER_PRODUCT_OBLIGATION_POSTWRITE_DOMAIN_V3: &[u8] =
    b"dragons-clutch/sbf/dealer-product-obligation-postwrite/v3\0";

/// One exact prewrite shared by Product's independent RootV3-family and
/// LinkV3-obligation preparations. It is constructible only from hostile
/// current Product accounts and fresh physical Dealer coordinates.
struct AuthenticatedDealerProductFoundingPrewriteV3 {
    id: ContentId,
...
/// Move-only hostile StateV3 postwrite consumed only by Product's RootV3
/// family admission writer.
struct AuthenticatedDealerFamilyFoundingPostwriteV3 {
    id: ContentId,
    plan_id: ContentId,
    root_account: Pubkey,
    root_binding_id: ContentId,
    ...
    state_postsemantic_id: ContentId,
    obligation_binding_id: ContentId,
}

/// Move-only hostile `0xaf/v3` postwrite consumed only by Product's LinkV3
/// obligation admission writer.
struct AuthenticatedDealerObligationFoundingPostwriteV3 {
    id: ContentId,
    plan_id: ContentId,
    root_account: Pubkey,
    root_binding_id: ContentId,
    ...
    link_account: Pubkey,
    link_binding_id: ContentId,
    link_semantic_before_id: clutch_product_series::SeriesMarketLinkV3Id,
    link_semantic_after_id: clutch_product_series::SeriesMarketLinkV3Id,
...
impl AuthenticatedProductFamilyAdmissionOwnerV3
    for AuthenticatedDealerProductFoundingPrewriteV3
{
    fn family(&self) -> Outcome<MarketFamilyV1> { Ok(MarketFamilyV1::Dealer) }

    fn child_account(&self) -> Outcome<Pubkey> { Ok(self.state_account) }
    ...
    fn authenticate_product_family_admission_owner_v3(
        ...
        require(
            self.id != ContentId::ZERO
                ...
                && market_instance_id.bytes() == self.key.market_instance_v2_id.bytes()
                && generation == self.key.product_generation
                ...
                && child_account == self.state_account
                ...
                && program_id.to_bytes() != self.state_account.to_bytes(),
    ...
    for AuthenticatedDealerProductFoundingPrewriteV3
{
    fn obligation(&self) -> Outcome<SeriesLinkObligationV3> {
        Ok(SeriesLinkObligationV3::Dealer)
    }

    fn child_account(&self) -> Outcome<Pubkey> { Ok(self.obligation_account) }
    ...
    fn authenticate_product_link_obligation_admission_owner_v3(
        ...
        require(
            self.id != ContentId::ZERO
                ...
                && link_transition_sequence_before == self.link_transition_sequence
                && link_transition_sequence_after
                    == self.link_transition_sequence.checked_add(1)
                        .ok_or(ClutchError::Arithmetic)?
                && series_plan_id.bytes() == self.key.series_plan_v5_id.bytes()
                && ordinal == self.key.series_ordinal
                && market_instance_id.bytes() == self.key.market_instance_v2_id.bytes()
                && generation == self.key.product_generation
                && obligation_configuration_id == self.obligation_configuration_id
                && obligation == SeriesLinkObligationV3::Dealer
                && child_account == self.obligation_account
                && owner_prewrite_id == self.id,
            ClutchError::AuthorizationUnavailable,
        )
    }
...
impl AuthenticatedProductFamilyAdmissionPostwriteV3
    for AuthenticatedDealerFamilyFoundingPostwriteV3
{
    fn consume_product_family_admission_postwrite_v3(
        ...
        require(
            self.id != ContentId::ZERO
                ...
                && self.state_postsemantic_id != ContentId::ZERO
                && self.obligation_binding_id != ContentId::ZERO,
...
impl AuthenticatedProductLinkObligationAdmissionPostwriteV3
    for AuthenticatedDealerObligationFoundingPostwriteV3
{
    fn consume_product_link_obligation_admission_postwrite_v3(
        ...
        require(
            self.id != ContentId::ZERO
                ...
                && link_account == self.link_account
                && link_binding_id == self.link_binding_id
                && link_semantic_before_id == self.link_semantic_before_id
                && link_semantic_after_id == self.link_semantic_after_id
                && link_transition_sequence_before == self.link_transition_sequence_before
                && link_transition_sequence_after == self.link_transition_sequence_after
                && obligation == SeriesLinkObligationV3::Dealer
...
/// current Product RootV3, LinkV3, and Dealer-owned `0xaf/v3` in one call.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct AuthenticatedDealerProductResolutionV3 {
    root_account_id: Id,
    root_semantic_id: Id,
    root_data_id: Id,
    root_binding_id: Id,
...
/// the terminal Dealer Replay and hostile-decoded the finalized General
/// Resolution. Fractional consumes it by value before touching a5,
/// ClaimLedgerV3, or a6/v2; no Product receipt participates.
...
/// the same outer has accepted Dealer's complete value-terminal postwrite.
...
/// deleted from the exact live Dealer obligation and terminal State cut.
...
/// One of the two disjoint physical value-terminal postimages for a Dealer
/// family. Both variants are non-Copy and are consumed by the family sealer.
#[derive(Debug)]
enum AuthenticatedDealerTerminalValueV1 {
    ActiveFacilityCredit(AcceptedDealerFacilityCreditTerminalV1),
    UnusedFutureCredit(AuthenticatedDealerFutureCreditUnusedCloseV1),
}

/// Move-only proof that Dealer terminalized and deleted its complete local
/// family before Product mutates RootV3/LinkV3.
///
/// This receipt is intentionally not persisted and has no signer-shaped or
/// Product-supplied authority fields. Product's successor outer must consume
/// it in the same Solana instruction so any later refusal rolls every physical
/// custody, rent, State, Replay, and liveness write back atomically.
#[derive(Debug)]
pub(crate) struct AuthenticatedDealerFamilyTerminalReceiptV3 {
    receipt_id: ContentId,
    policy_id: ContentId,
    facility_id: ContentId,
    market_instance_id: ContentId,
    dealer_state_account: Pubkey,
    dealer_state_after_id: ContentId,
    dealer_obligation_account: Pubkey,
    dealer_obligation_live_id: ContentId,
    dealer_obligation_close_receipt_id: ContentId,
    terminal_state_receipt_id: ContentId,
    terminal_replay_semantic_id: ContentId,
    terminal_replay_ordinal: u64,
    ...
    obligation_configuration_id: ContentId,
    obligation_admission_projection_id: ContentId,
    obligation_admission_link_transition_sequence: u64,
    product_generation: u64,
    series_ordinal: u32,
    value_kind: u8,
    value_terminal_receipt_id: ContentId,
    resolution_semantic_id: ContentId,
    resolution_data_id: ContentId,
    action_receipt_semantic_id: ContentId,
}

impl AuthenticatedDealerFamilyTerminalReceiptV3 {
    pub(crate) const fn id(&self) -> ContentId { self.receipt_id }
    pub(crate) const fn policy_id(&self) -> ContentId { self.policy_id }
    pub(crate) const fn facility_id(&self) -> ContentId { self.facility_id }
    pub(crate) const fn market_instance_id(&self) -> ContentId { self.market_instance_id }
    pub(crate) const fn dealer_state_account(&self) -> Pubkey { self.dealer_state_account }
    pub(crate) const fn dealer_state_after_id(&self) -> ContentId {
        self.dealer_state_after_id
    }
    pub(crate) const fn dealer_obligation_account(&self) -> Pubkey {
        self.dealer_obligation_account
    }
    pub(crate) const fn dealer_obligation_live_id(&self) -> ContentId {
        self.dealer_obligation_live_id
    }
    pub(crate) const fn dealer_obligation_close_receipt_id(&self) -> ContentId {
        self.dealer_obligation_close_receipt_id
    }
    pub(crate) const fn terminal_state_receipt_id(&self) -> ContentId {
        self.terminal_state_receipt_id
    }
    pub(crate) const fn terminal_replay_semantic_id(&self) -> ContentId {
        self.terminal_replay_semantic_id
    }
    pub(crate) const fn terminal_replay_ordinal(&self) -> u64 {
        self.terminal_replay_ordinal
    }
    ...
    pub(crate) const fn series_plan_id(&self) -> ContentId { self.series_plan_id }
    ...
    pub(crate) const fn obligation_configuration_id(&self) -> ContentId {
        self.obligation_configuration_id
    }
    pub(crate) const fn obligation_admission_projection_id(&self) -> ContentId {
        self.obligation_admission_projection_id
    }
    pub(crate) const fn obligation_admission_link_transition_sequence(&self) -> u64 {
        self.obligation_admission_link_transition_sequence
    }
    ...
    pub(crate) const fn value_kind(&self) -> u8 { self.value_kind }
    pub(crate) const fn value_terminal_receipt_id(&self) -> ContentId {
        self.value_terminal_receipt_id
    }
    ...
    pub(crate) const fn action_receipt_semantic_id(&self) -> ContentId {
        self.action_receipt_semantic_id
    }
}

#[allow(clippy::too_many_arguments)]
fn seal_dealer_family_terminal_receipt_v3(
    program_id: &Pubkey,
    obligation: DealerSeriesObligationBindingV3,
    state_account: &AccountInfo<'_>,
    state_after: DealerStateV3,
    close: DealerSeriesObligationValueClosePlanV3,
    terminal_state_receipt_id: ContentId,
    terminal_replay_semantic_id: ContentId,
    terminal_replay_ordinal: u64,
    action_receipt_semantic_id: ContentId,
    value: AuthenticatedDealerTerminalValueV1,
) -> Outcome<AuthenticatedDealerFamilyTerminalReceiptV3> {
    let (value_kind, value_terminal_receipt_id, resolution_semantic_id, resolution_data_id) =
        match value {
            AuthenticatedDealerTerminalValueV1::ActiveFacilityCredit(receipt) => (
                1u8,
                ContentId::from_bytes(receipt.execution_receipt_id().bytes()),
                ContentId::from_bytes(receipt.resolution_semantic_id().bytes()),
                ContentId::from_bytes(receipt.resolution_data_id().bytes()),
            ),
            AuthenticatedDealerTerminalValueV1::UnusedFutureCredit(receipt) => {
                (2u8, ContentId::from_bytes(receipt.receipt_id().bytes()), ContentId::ZERO, ContentId::ZERO)
            }
        };
    let state_after_id = state_after.state_id().map_err(dealer_fault)?;
    let key = obligation.key;
    require(
        close.state_after == state_after
            && close.binding_account_id == key.binding_account_id
            && close.live_binding_id == obligation.binding_id().map_err(dealer_fault)?
            && close.dealer_value_terminal_receipt_id.bytes()
                == value_terminal_receipt_id.bytes()
            && state_after.series_obligation_children == 0
            && state_after.base.children.facility_positions == 0
            && state_after.base.children.facility_replays == 0
            && state_after.base.terminal_state_receipt_id.bytes()
                == terminal_state_receipt_id.bytes()
            && id(state_account.key) == key.dealer_state_account_id
            && value_terminal_receipt_id != ContentId::ZERO
            && terminal_replay_semantic_id != ContentId::ZERO
            && action_receipt_semantic_id != ContentId::ZERO,
        ClutchError::MismatchedState,
    )?;
    let receipt_id = ContentId::from_bytes(
        solana_sha256_hasher::hashv(&[
            DEALER_FAMILY_TERMINAL_RECEIPT_DOMAIN_V3,
            program_id.as_ref(),
            &key.policy_id.bytes(),
            &key.facility_id.bytes(),
            &key.market_instance_v2_id.bytes(),
            state_account.key.as_ref(),
            &state_after_id.bytes(),
            &key.binding_account_id.bytes(),
            &close.live_binding_id.bytes(),
            &close.close_receipt_id.bytes(),
            &terminal_state_receipt_id.bytes(),
            &terminal_replay_semantic_id.bytes(),
            &terminal_replay_ordinal.to_le_bytes(),
            &key.product_market_root_account_id.bytes(),
            &key.product_market_binding_id.bytes(),
            &key.series_market_link_account_id.bytes(),
            &key.series_plan_v5_id.bytes(),
            &key.compiler_bundle_v7_id.bytes(),
            &key.attachment_plan_v6_id.bytes(),
            &key.obligation_configuration_v3_id.bytes(),
            &obligation.owner_prewrite_id.bytes(),
            &obligation.family_admission_receipt_id.bytes(),
            &obligation.admission_owner_receipt_id.bytes(),
            &obligation.admission_projection_id.bytes(),
            &obligation.admission_link_transition_sequence.to_le_bytes(),
            &key.product_generation.to_le_bytes(),
            &key.series_ordinal.to_le_bytes(),
            &[value_kind],
            &value_terminal_receipt_id.bytes(),
            &resolution_semantic_id.bytes(),
            &resolution_data_id.bytes(),
            &action_receipt_semantic_id.bytes(),
        ])
        .to_bytes(),
    );
    require(receipt_id != ContentId::ZERO, ClutchError::AuthorizationUnavailable)?;
    Ok(AuthenticatedDealerFamilyTerminalReceiptV3 {
        receipt_id,
        policy_id: ContentId::from_bytes(key.policy_id.bytes()),
        facility_id: ContentId::from_bytes(key.facility_id.bytes()),
        market_instance_id: ContentId::from_bytes(key.market_instance_v2_id.bytes()),
        dealer_state_account: *state_account.key,
        dealer_state_after_id: ContentId::from_bytes(state_after_id.bytes()),
        dealer_obligation_account: Pubkey::new_from_array(key.binding_account_id.bytes()),
        dealer_obligation_live_id: ContentId::from_bytes(close.live_binding_id.bytes()),
        dealer_obligation_close_receipt_id: ContentId::from_bytes(close.close_receipt_id.bytes()),
        terminal_state_receipt_id,
        terminal_replay_semantic_id,
        terminal_replay_ordinal,
        product_market_root_account: Pubkey::new_from_array(key.product_market_root_account_id.bytes()),
        product_market_binding_id: ContentId::from_bytes(key.product_market_binding_id.bytes()),
        series_market_link_account: Pubkey::new_from_array(key.series_market_link_account_id.bytes()),
        series_plan_id: ContentId::from_bytes(key.series_plan_v5_id.bytes()),
        compiler_bundle_id: ContentId::from_bytes(key.compiler_bundle_v7_id.bytes()),
        attachment_plan_id: ContentId::from_bytes(key.attachment_plan_v6_id.bytes()),
        obligation_configuration_id: ContentId::from_bytes(
            key.obligation_configuration_v3_id.bytes(),
        ),
        obligation_admission_projection_id: ContentId::from_bytes(
            obligation.admission_projection_id.bytes(),
        ),
        obligation_admission_link_transition_sequence: obligation
            .admission_link_transition_sequence,
        product_generation: key.product_generation,
        series_ordinal: key.series_ordinal,
        value_kind,
        value_terminal_receipt_id,
        resolution_semantic_id,
        resolution_data_id,
        action_receipt_semantic_id,
    })
}

/// Hostile current Product/Dealer proof for later leases after the one-shot
/// RootV3-family and LinkV3-obligation founding transaction.
struct AuthenticatedExistingDealerSeriesAdmissionV3 {
    state: DealerStateV3,
    obligation: DealerSeriesObligationBindingV3,
}
...
#[allow(clippy::too_many_arguments)]
fn authenticate_dealer_product_founding_prewrite_v3(
    program_id: &Pubkey,
    ...
    donation_floor: u64,
) -> Outcome<AuthenticatedDealerProductFoundingPrewriteV3> {
    key.validate().map_err(dealer_fault)?;
    state.validate().map_err(dealer_fault)?;
    let base = state.base;
    let state_presemantic_id = ContentId::from_bytes(
    ...
    let family_namespace_anchor_id =
        family_policy.family_namespace_anchors()[MarketFamilyV1::Dealer.index()];
    ...
    require(
        root.is_writable()
            ...
            && !state.product_obligation_is_live()
            && base.phase == DealerPhaseV2::Trading
            && state_account.owner == program_id
            && state_account.is_writable
            && state_account.data_len() == DEALER_STATE_V3_ACCOUNT_BYTES
            && key.binding_account_id == id(obligation_account.key)
            && key.policy_id == base.policy_id
            && key.facility_id == base.facility_id
            && key.dealer_state_account_id == id(state_account.key)
            && key.facility_position_binding_id == base.facility_position_binding_id
            && key.market_instance_v2_id.bytes() == root_binding.market_instance_id.bytes()
            && key.product_market_root_account_id == id(&root.account())
            && key.product_market_binding_id == Id::from_bytes(root.binding_id().bytes())
            && key.series_plan_v5_id.bytes() == link_binding.series_plan_id.bytes()
            && key.series_market_link_account_id == id(&link.account())
            && key.compiler_bundle_v7_id.bytes() == link_binding.compiler_bundle_id.bytes()
            && key.attachment_plan_v6_id.bytes() == link_binding.attachment_plan_id.bytes()
            && key.obligation_configuration_v3_id.bytes()
                == obligation_configuration_id.bytes()
            && key.product_generation == root_binding.generation
            && key.product_generation == link_binding.generation
            ...
            && family_policy.policy_id() != family_policy.id()
            && family_namespace_anchor_id != ContentId::ZERO,
        ClutchError::AuthorizationUnavailable,
    )?;
    ...
    let id = ContentId::from_bytes(
        solana_sha256_hasher::hashv(&[
            DEALER_PRODUCT_FOUNDING_PREWRITE_DOMAIN_V3,
            program_id.as_ref(),
            state_account.key.as_ref(),
    ...
    require(id != ContentId::ZERO, ClutchError::AuthorizationUnavailable)?;
    Ok(AuthenticatedDealerProductFoundingPrewriteV3 {
        id,
        state_account: *state_account.key,
        state_presemantic_id,
        ...
        link_transition_sequence: link.state().transition_sequence(),
...
#[allow(clippy::too_many_arguments)]
fn authenticate_dealer_product_founding_postwrites_v3(
    program_id: &Pubkey,
    prewrite: AuthenticatedDealerProductFoundingPrewriteV3,
    family_plan: &AuthenticatedProductFamilyAdmissionPlanV3,
    link_plan: &AuthenticatedProductLinkObligationAdmissionPlanV3,
    state_account: &AccountInfo<'_>,
    state: &DealerStateV3,
    obligation_account: &AccountInfo<'_>,
    obligation: &DealerSeriesObligationBindingV3,
) -> Outcome<(
    AuthenticatedDealerFamilyFoundingPostwriteV3,
    AuthenticatedDealerObligationFoundingPostwriteV3,
)> {
    let observed_state = authenticate_dealer_state_v3(program_id, state_account, true)?;
    let observed_obligation =
        authenticate_dealer_series_obligation_v3(program_id, obligation_account, true)?;
    let state_postsemantic_id = ContentId::from_bytes(
        state.state_id().map_err(dealer_fault)?.bytes(),
    );
    let obligation_binding_id = ContentId::from_bytes(
        obligation.binding_id().map_err(dealer_fault)?.bytes(),
    );
    require(
        observed_state.state() == state
            && observed_obligation.binding() == obligation
            && prewrite.state_account == *state_account.key
            && prewrite.obligation_account == *obligation_account.key
            && prewrite.key == obligation.key
            && prewrite.state_presemantic_id != state_postsemantic_id
            && obligation.owner_prewrite_id.bytes() == prewrite.id.bytes()
            && obligation.family_admission_receipt_id.bytes()
                == family_plan.family_admission_receipt_id().bytes()
            && obligation.admission_owner_receipt_id.bytes()
                == link_plan.owner_admission_receipt_id().bytes()
            && obligation.admission_projection_id.bytes()
                == link_plan.product_admission_projection_id().bytes()
            && obligation.admission_link_pre_semantic_id.bytes()
                == link_plan.link_semantic_before_id().bytes()
            && obligation.admission_link_post_semantic_id.bytes()
                == link_plan.link_semantic_after_id().bytes()
            && obligation.admission_link_transition_sequence
                == link_plan.link_transition_sequence_after()
            && state.series_obligation_binding_id.bytes() == obligation_binding_id.bytes()
            && state.series_obligation_binding_account_id == prewrite.key.binding_account_id,
        ClutchError::MismatchedState,
    )?;
    let family_id = ContentId::from_bytes(
        solana_sha256_hasher::hashv(&[
            DEALER_PRODUCT_FAMILY_POSTWRITE_DOMAIN_V3,
            program_id.as_ref(),
            &prewrite.id.bytes(),
            &family_plan.id().bytes(),
            state_account.key.as_ref(),
            &state_postsemantic_id.bytes(),
            obligation_account.key.as_ref(),
            &obligation_binding_id.bytes(),
            &family_plan.family_admission_receipt_id().bytes(),
        ])
        .to_bytes(),
    );
    let obligation_id = ContentId::from_bytes(
        solana_sha256_hasher::hashv(&[
            DEALER_PRODUCT_OBLIGATION_POSTWRITE_DOMAIN_V3,
            program_id.as_ref(),
            &prewrite.id.bytes(),
            &link_plan.id().bytes(),
            state_account.key.as_ref(),
    ...
    require(
        family_id != ContentId::ZERO && obligation_id != ContentId::ZERO,
        ClutchError::AuthorizationUnavailable,
    )?;
    Ok((
        AuthenticatedDealerFamilyFoundingPostwriteV3 {
            ...
            root_account: family_plan.root_account(),
            root_binding_id: family_plan.root_binding_id(),
            root_semantic_before_id: family_plan.root_semantic_before_id(),
            root_semantic_after_id: family_plan.root_semantic_after_id(),
            root_transition_sequence_before: family_plan.root_transition_sequence_before(),
            root_transition_sequence_after: family_plan.root_transition_sequence_after(),
            ...
            state_account: *state_account.key,
            owner_prewrite_id: prewrite.id,
            state_postsemantic_id,
            obligation_binding_id,
        },
        AuthenticatedDealerObligationFoundingPostwriteV3 {
            id: obligation_id,
            plan_id: link_plan.id(),
            root_account: link_plan.root_account(),
            root_binding_id: link_plan.root_binding_id(),
            root_transition_sequence: prewrite.root_transition_sequence,
            link_account: link_plan.link_account(),
            link_binding_id: link_plan.link_binding_id(),
            link_semantic_before_id: link_plan.link_semantic_before_id(),
            link_semantic_after_id: link_plan.link_semantic_after_id(),
            link_transition_sequence_before: link_plan.link_transition_sequence_before(),
            link_transition_sequence_after: link_plan.link_transition_sequence_after(),
            owner_admission_receipt_id: link_plan.owner_admission_receipt_id(),
            product_admission_projection_id: link_plan.product_admission_projection_id(),
            obligation_account: *obligation_account.key,
            owner_prewrite_id: prewrite.id,
            state_postsemantic_id,
            obligation_binding_id,
...
fn authenticate_existing_dealer_series_admission_v3(
    root: &AuthenticatedMarketLifecycleRootV3<'_>,
    link: &AuthenticatedSeriesMarketLinkV3<'_>,
    state_account: &AccountInfo<'_>,
    state: DealerStateV3,
    obligation_account: &AccountInfo<'_>,
    obligation: DealerSeriesObligationBindingV3,
) -> Outcome<AuthenticatedExistingDealerSeriesAdmissionV3> {
    use clutch_product_series::MarketFamilyStatusV1;
    ...
    let root_binding = root.binding();
    let link_binding = link.binding();
    let dealer_family = root.state().product_families().family(MarketFamilyV1::Dealer);
    require(
        root.state().phase() == clutch_product_series::MarketLifecyclePhaseV3::Active
            && link.state().phase() == SeriesMarketLinkPhaseV3::Active
            && dealer_family.status() == MarketFamilyStatusV1::Live
            && dealer_family.counts().live != 0
            && link.state().obligation_status(SeriesLinkObligationV3::Dealer)
                == SeriesLinkObligationStatusV3::Live
            && link.state().obligation_admission_receipt_id(SeriesLinkObligationV3::Dealer)
                .bytes()
                == obligation.admission_projection_id.bytes()
            ...
            && obligation.key.product_market_root_account_id == id(&root.account())
            && obligation.key.product_market_binding_id.bytes() == root.binding_id().bytes()
            && obligation.key.series_plan_v5_id.bytes() == link_binding.series_plan_id.bytes()
            && obligation.key.series_market_link_account_id == id(&link.account())
            && obligation.key.compiler_bundle_v7_id.bytes()
                == link_binding.compiler_bundle_id.bytes()
            ...
            && obligation.key.product_generation == root_binding.generation
            && obligation.key.product_generation == link_binding.generation
            && obligation.key.series_ordinal == link_binding.ordinal
            && obligation.rent.payer.bytes() == link_binding.rent_refund_owner.bytes()
    ...
    Ok(AuthenticatedExistingDealerSeriesAdmissionV3 { state, obligation })
}
...
fn authenticate_live_series_obligation_for_state_v3(
    ...
    state_account: &AccountInfo<'_>,
    state: &DealerStateV3,
) -> Outcome<DealerSeriesObligationBindingV3> {
    let authenticated = authenticate_dealer_series_obligation_v3(program_id, account, false)?;
...
/// Authenticate the current Product RootV3/LinkV3 owner of a live Dealer
...
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn authenticate_current_product_resolution_v3(
    program_id: &Pubkey,
    ...
    link_writable: bool,
) -> Outcome<AuthenticatedDealerProductResolutionV3> {
    obligation.validate().map_err(dealer_fault)?;
    let key = obligation.key;
    ...
    let (
        ...
        let mut root_body = MarketLifecycleRootAccountV3::decode_buffer();
    ...
    let (
        ...
        let mut link_body = SeriesMarketLinkAccountV3::decode_buffer();
        ...
        require(
            link_state.phase() == SeriesMarketLinkPhaseV3::Active
                ...
                && binding.compiler_bundle_id.content_id().bytes()
                    == key.compiler_bundle_v7_id.bytes()
                && binding.attachment_plan_id.content_id().bytes()
                    == key.attachment_plan_v6_id.bytes()
                && binding.obligation_configuration_id.content_id().bytes()
                    == key.obligation_configuration_v3_id.bytes()
        ...
        (
            ...
            link_state.transition_sequence(),
            id_from_content(
                link_state.obligation_admission_receipt_id(SeriesLinkObligationV3::Dealer),
            ),
        )
    };
    ...
    let authentication_id = Id::from_bytes(
        solana_sha256_hasher::hashv(&[
            DEALER_PRODUCT_RESOLUTION_AUTHENTICATION_DOMAIN_V3,
    ...
    Ok(AuthenticatedDealerProductResolutionV3 {
...
/// action25 passes the hostile-reopened live StateV3 root and exact
/// post-Position/Replay Retiring base state before any Product terminal write.
...
fn close_unused_future_credit_funding_v1(
    program_id: &Pubkey,
    state_account: &AccountInfo<'_>,
    live_obligation_account: &AccountInfo<'_>,
    ...
    let authenticated_obligation =
        authenticate_dealer_series_obligation_v3(program_id, live_obligation_account, true)?;
    ...
    require(
        plan.funding_account_id == id(funding_account.key)
            ...
            && plan.dealer_obligation_presemantic_id
    ...
    let postwrite_receipt_id = Id::from_bytes(
        solana_sha256_hasher::hashv(&[
            ...
            &plan.dealer_obligation_presemantic_id.bytes(),
...
fn initialize_facility(
    ...
    state
        ...
        .map_err(dealer_fault)?;
    let state_v3 = DealerStateV3::founding_unadmitted(
        state,
        DeletableRentOwnerV1 {
            payer: id(accounts[0].key),
            neutral_sink: policy.neutral_sink,
            refundable_principal: state_v3_delta_principal,
            donation_floor: 0,
        },
    )
    .map_err(dealer_fault)?;
    ...
    write_dealer_body(
        ...
        state_bump,
        &state_v3.with_base(prepared.state).map_err(dealer_fault)?,
    )
}

/// Enter UnwindOnly under the exact immutable sponsor signature while
/// preserving the sole StateV3 root and its chain-derived obligation partition.
#[inline(never)]
fn sponsor_halt(
    program_id: &Pubkey,
    accounts: &[AccountInfo<'_>],
    ...
    let current_state_v3 = *authenticated.state();
    require(
        payload.existing_series_admission == current_state_v3.product_obligation_is_live(),
        ClutchError::MismatchedState,
    )?;
    let obligation = if current_state_v3.product_obligation_is_live() {
    ...
    let (state, state_v3, state_bump) = (
        current_state_v3.base,
        Some(current_state_v3),
        authenticated.bump(),
    );
    require(
    ...
    let obligation_matches = match obligation.as_ref() {
        Some(value) => authenticate_dealer_series_obligation_v3(
            program_id,
            &accounts[15],
            false,
        )?
        .binding()
            == value,
        None => !state_after_v3.product_obligation_is_live(),
    };
    let state_matches = observed_state.state() == &state_after_v3 && obligation_matches;
    require(
...
fn resolve_facility_vector(
    ...
    let authenticated_obligation =
        authenticate_dealer_series_obligation_v3(program_id, &accounts[25], false)?;
    ...
    let product_resolution = authenticate_current_product_resolution_v3(
        program_id,
        &accounts[24],
        &accounts[26],
        &obligation,
...
/// Physically terminalize Dealer's complete local family for Product RootV3.
///
/// This internal primitive has no direct instruction dispatch. It closes the
/// Position/Replay pair, exactly one disjoint value owner, the live Dealer
/// obligation, its rent, State, action receipt, and liveness postimage before
/// returning one non-Copy receipt. The Product RootV3 outer must consume that
/// receipt in the same instruction; it cannot be persisted or dropped safely.
#[inline(never)]
#[allow(clippy::too_many_lines)]
pub(crate) fn terminalize_dealer_family_for_product_v3(
    program_id: &Pubkey,
    accounts: &[AccountInfo<'_>],
    sequence: u64,
    payload_bytes: &[u8],
    ...
    let expected_count = if active_credit {
    ...
    let (policy_id, policy) = authenticate_catalog_policy(program_id, &accounts[1])?;
    let authenticated_state = authenticate_dealer_state_v3(program_id, &accounts[2], true)?;
    let state_bump = authenticated_state.bump();
    let state_v3 = *authenticated_state.state();
    let state = state_v3.base;
    require(
        state.policy_id.bytes() == policy_id
            && state.generation == payload.expected_generation
            && state.phase == DealerPhaseV2::Retiring
            && state_v3.series_obligation_children == 1,
        ClutchError::MismatchedState,
    )?;
    let authenticated_obligation =
        authenticate_dealer_series_obligation_v3(program_id, &accounts[24], true)?;
    let obligation = *authenticated_obligation.binding();
    let obligation_presemantic_id = obligation.binding_id().map_err(dealer_fault)?;
    require(
        obligation.phase == DealerSeriesObligationPhaseV1::Live
            && state_v3.series_obligation_binding_account_id == id(accounts[24].key)
            && state_v3.series_obligation_binding_id == obligation_presemantic_id
            && obligation.key.dealer_state_account_id == id(accounts[2].key)
            && obligation.key.facility_position_binding_id
                == state.facility_position_binding_id,
        ClutchError::MismatchedState,
    )?;

    let (position_binding, terminal_position, replay, replay_binding) =
        authenticate_position_and_replay(
            program_id,
            &accounts[2],
            &accounts[3],
    ...
    require(
        replay.next_transition_ordinal() == payload.expected_replay_ordinal
            && terminal_position.projection.position().lifecycle()
                == PositionLifecycleV3::CloseRequested
            && terminal_position.projection.position().cash_atoms() == 0
            && terminal_position.projection.position().reserved_cash_atoms() == 0
            && terminal_position.projection.position().native_eggs()
                == [0; clutch_retirement::MAX_OUTCOMES]
            && terminal_position
                .projection
                .position()
                .outstanding_reservations()
    ...
    let retirement_liveness =
        runtime_states[DealerLivenessCompartmentV1::Retirement.index()];
    require(
        retirement_liveness.identity.payer.bytes() == accounts[16].key.to_bytes(),
        ClutchError::MismatchedState,
    )?;

    let _current_slot = read_clock_slot(&accounts[21])?;
    let rent = read_rent(&accounts[22])?;
    require_system_program(&accounts[23])?;
    require_creatable(&accounts[15])?;
    require(
    ...
    receipt
        ...
        .map_err(dealer_fault)?;
    let authorization = receipt
        .authorization(&schedule, &runtime_binding, &retirement_liveness)
        .map_err(dealer_fault)?;
    let liveness_transition = plan_liveness_spend_absorbing_donation(
        program_id,
        &accounts[7],
    ...
    let active_resolution = if active_credit {
        Some(authenticate_resolution_v5(
            program_id,
            &accounts[35],
            value_authority.liabilities,
        )?)
    } else {
        None
    };
    let state_pre_id = state.state_content_id().map_err(dealer_fault)?;
    let replay_pre_id = replay.replay_id().map_err(dealer_fault)?;
    let empty_transfer_id = DealerEmptyAssetTransferBundleV1 {
        action: DealerRuntimeActionV1::Retire,
    }
    .bundle_id()
    .map_err(dealer_fault)?;
    let prepared_terminal = prepare_dealer_terminal_replay_v2(
        &policy,
        &position_binding,
        &state,
        id(accounts[2].key),
        ...
        DealerTransitionIntentV1 {
            ...
            replay_pre_id,
            state_pre_content_id: state_pre_id,
            state_post_content_id: state_pre_id,
            position_pre_semantic_id: terminal_position.semantic_id,
            position_post_semantic_id: terminal_position.semantic_id,
            liveness_receipt_semantic_id: authorization.receipt_semantic_id,
            fee_evidence_id: Id::ZERO,
            asset_transfer_bundle_id: empty_transfer_id,
            position_generation_before: state.generation,
    ...
    .map_err(dealer_fault)?;
    let terminal_state_receipt = prepared_terminal.receipt();
    let terminal_state_receipt_id = terminal_state_receipt
        .receipt_id()
        .map_err(dealer_fault)?;
    let terminal_replay = prepared_terminal.prepared_replay().replay_post();
    let terminal_replay_semantic_id = terminal_replay.replay_id().map_err(dealer_fault)?;

    let position_close = prepare_dealer_position_replay_close_v3(
        program_id,
        value_authority.liabilities.bound,
        policy.outcome_count,
        &accounts[3],
        &accounts[4],
        terminal_position,
        terminal_replay,
        payload.expected_replay_ordinal,
        &accounts[17],
        &accounts[18],
        &accounts[20],
    )?;
    terminal_replay
        .encode_into(&mut accounts[4].data.borrow_mut())
        .map_err(dealer_fault)?;
    let position_close_receipt_id = apply_dealer_position_replay_close_v3(
        &accounts[3],
        &accounts[4],
        &accounts[17],
        &accounts[18],
    ...
    require(
        position_close_receipt_id != ContentId::ZERO,
        ClutchError::MismatchedState,
    )?;
    let position_tombstone = PositionTombstoneV3::decode(
        &accounts[3]
            .try_borrow_data()
            .map_err(|_| Refusal::Adapter(ClutchError::AccountBorrowFailed))?,
    )
    .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    let state_after_position = close_dealer_position_replay_v2(
        &policy,
        &state,
    ...
    .map_err(dealer_fault)?;
    let state_after_position_live = state_v3.with_base(state_after_position).map_err(dealer_fault)?;
    write_dealer_body(
        &accounts[2],
        DEALER_STATE_V3_ACCOUNT_TAG,
        DEALER_STATE_V3_ACCOUNT_VERSION,
        state_bump,
        &state_after_position_live,
    )?;
    let observed_intermediate_state = authenticate_dealer_state_v3(program_id, &accounts[2], true)?;
    require(
        observed_intermediate_state.state() == &state_after_position_live
            && accounts[4].owner == &SYSTEM_PROGRAM_ID
            && accounts[4].data_is_empty()
            && accounts[4].lamports() == 0,
        ClutchError::MismatchedState,
    )?;

    let terminal_value = if active_credit {
        let resolution = active_resolution.ok_or(ClutchError::MismatchedState)?;
        let fractional_ledger = FractionalLedgerV1::decode(
            &accounts[37]
                .try_borrow_data()
                .map_err(|_| Refusal::Adapter(ClutchError::AccountBorrowFailed))?,
        )
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
        let facility_credit = FractionalCreditV2::decode(
            &accounts[38]
                .try_borrow_data()
                .map_err(|_| Refusal::Adapter(ClutchError::AccountBorrowFailed))?,
        )
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
        let ledger_before_id = fractional_ledger
            .state_id()
            .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
        let intermediate_state_id = state_after_position_live
            .state_id()
            .map_err(dealer_fault)?;
        let prestate = DealerFacilityCreditTerminalPrestateV1::new(
            retirement_id(state.facility_id)?,
            retirement_id(policy.market_instance_v2_id)?,
            obligation.key.product_generation,
            retirement_id(id(accounts[38].key))?,
            fractional_ledger.next_sequence,
            facility_credit.next_sequence,
            retirement_id(id(accounts[2].key))?,
            retirement_id(intermediate_state_id)?,
            state.generation,
            retirement_id(id(accounts[4].key))?,
            retirement_id(terminal_replay_semantic_id)?,
            terminal_replay.next_transition_ordinal(),
            retirement_id(id(accounts[24].key))?,
            retirement_id(obligation_presemantic_id)?,
            retirement_id(terminal_state_receipt_id)?,
        )?;
        let fractional_terminal = apply_dealer_facility_credit_terminal_v1(
            program_id,
            DealerFacilityCreditTerminalAccountsV1 {
                realm: &accounts[25],
                profile: &accounts[26],
                collateral_policy: &accounts[27],
                collateral_token_program: &accounts[28],
                collateral_token_programdata: &accounts[29],
                market_binding: &accounts[30],
                market_runtime: &accounts[31],
                market_instance: &accounts[32],
                hoard: &accounts[33],
                claim_ledger: &accounts[34],
                resolution: &accounts[35],
                fractional_policy: &accounts[36],
                fractional_ledger: &accounts[37],
                facility_credit: &accounts[38],
                stored_payer: &accounts[19],
                neutral_sink: &accounts[20],
                rent_sysvar: &accounts[22],
            },
            AuthenticatedDealerFacilityCreditTerminalAuthoritySbfV1 {
                prestate,
                fractional_ledger_account: id(accounts[37].key),
                fractional_ledger_before_id: Id::from_bytes(ledger_before_id.bytes()),
                resolution_semantic_id: Id::from_bytes(resolution.semantic_id.bytes()),
                resolution_data_id: Id::from_bytes(resolution.data_id.bytes()),
                stored_payer: id(accounts[19].key),
                neutral_sink: id(accounts[20].key),
            },
        )?;
        require(
                fractional_terminal.facility_id().bytes() == state.facility_id.bytes()
                && fractional_terminal.facility_credit_account().bytes()
                    == accounts[38].key.to_bytes()
                && fractional_terminal.ledger_before_id() == ledger_before_id
                && fractional_terminal.dealer_terminal_state_receipt_id().bytes()
                    == terminal_state_receipt_id.bytes()
                && fractional_terminal.execution_receipt_id().bytes() != [0; 32],
            ClutchError::MismatchedState,
        )?;
        AuthenticatedDealerTerminalValueV1::ActiveFacilityCredit(fractional_terminal)
    } else {
        let unused = close_unused_future_credit_funding_v1(
            program_id,
            &accounts[2],
            &accounts[24],
            &accounts[35],
        ...
        require(
            unused.postwrite_receipt_id != Id::ZERO
                && unused.plan.terminal_state_receipt_id == terminal_state_receipt_id
                && unused.plan.dealer_obligation_presemantic_id == obligation_presemantic_id
                && unused.refund_owner_lamports_after == accounts[19].lamports()
                && unused.neutral_sink_lamports_after == accounts[20].lamports(),
            ClutchError::MismatchedState,
        )?;
        AuthenticatedDealerTerminalValueV1::UnusedFutureCredit(unused)
    };

    let value_terminal_receipt_id = match &terminal_value {
        AuthenticatedDealerTerminalValueV1::ActiveFacilityCredit(receipt) => {
            Id::from_bytes(receipt.execution_receipt_id().bytes())
        }
        AuthenticatedDealerTerminalValueV1::UnusedFutureCredit(receipt) => receipt.receipt_id(),
    };

    let obligation_close = prepare_dealer_series_obligation_value_close_v3(
        state_after_position_live,
        &obligation,
        value_terminal_receipt_id,
        accounts[24].lamports(),
    )
    .map_err(dealer_fault)?;
    require(
        obligation_close.binding_account_id == id(accounts[24].key)
            && obligation_close.live_binding_id == obligation_presemantic_id
            && obligation_close.dealer_value_terminal_receipt_id
                == value_terminal_receipt_id
            && obligation_close.close_receipt_id != Id::ZERO
            && obligation_close.rent_payer == id(accounts[19].key)
            && obligation_close.neutral_sink == id(accounts[20].key),
        ClutchError::MismatchedState,
    )?;
    release_dealer_account(&accounts[24])?;
    credit_exact_dealer_terminal_lamports([
        (&accounts[19], obligation_close.rent_payer_credit_lamports),
        (&accounts[20], obligation_close.neutral_sink_credit_lamports),
        (&accounts[20], 0),
        (&accounts[20], 0),
    ])?;
    require_released_dealer_account(&accounts[24])?;
    write_dealer_body(
        &accounts[2],
        DEALER_STATE_V3_ACCOUNT_TAG,
        DEALER_STATE_V3_ACCOUNT_VERSION,
        state_bump,
        &obligation_close.state_after,
    )?;
    ...
    let final_state = authenticate_dealer_state_v3(program_id, &accounts[2], true)?;
    let (_, observed_receipt) = authenticate_action_receipt(program_id, &accounts[15])?;
    let retirement_data = accounts[13]
        .try_borrow_data()
        .map_err(|_| Refusal::Adapter(ClutchError::AccountBorrowFailed))?;
    require(
        final_state.state() == &obligation_close.state_after
            && final_state.state().series_obligation_children == 0
            && final_state.state().base.children.facility_positions == 0
            && final_state.state().base.children.facility_replays == 0
            && final_state.state().base.terminal_state_receipt_id
                == terminal_state_receipt_id
            && observed_receipt == receipt
            && accounts[13].lamports() == liveness_transition.account_balance_after
            && retirement_data.as_ref() == liveness_transition.post_account_data.as_slice(),
    ...
    drop(retirement_data);
    let action_receipt_semantic_id = receipt.semantic_receipt_id().map_err(dealer_fault)?;
    seal_dealer_family_terminal_receipt_v3(
        program_id,
        obligation,
        &accounts[2],
        obligation_close.state_after,
        obligation_close,
        ContentId::from_bytes(terminal_state_receipt_id.bytes()),
        ContentId::from_bytes(terminal_replay_semantic_id.bytes()),
        terminal_replay.next_transition_ordinal(),
        ContentId::from_bytes(action_receipt_semantic_id.bytes()),
        terminal_value,
    )
}
...
fn claim_terminal_allocation(
    ...
    let observed_state = authenticate_dealer_state_v3(program_id, &accounts[2], true)?;
    let observed_obligation =
        authenticate_dealer_series_obligation_v3(program_id, &accounts[34], false)?;
...
/// Before first lease admission this consumes the exact empty-obligation
/// StateV3/Position/Replay boundary; afterward it additionally preserves the
/// live Product obligation. New ticket rent is always supplied by the owner,
/// including on the optional Retirement-funded maintenance path. Hoard custody
/// is absent.
#[inline(never)]
fn queue_exit(
    program_id: &Pubkey,
...
/// Bind the next General epoch to the current StateV3 root. Repeated epochs
/// preserve its exact live Product obligation, or its canonical empty Product
/// child before first admission.
#[inline(never)]
fn bind_epoch(
    program_id: &Pubkey,
    accounts: &[AccountInfo<'_>],
...
fn select_lease_and_begin(
    ...
    let (policy_id, policy) = authenticate_catalog_policy(program_id, &accounts[1])?;
    let state_v3_before =
        *authenticate_dealer_state_v3(program_id, &accounts[2], true)?.state();
    let product_obligation_live = state_v3_before.product_obligation_is_live();
    require(
        payload.existing_series_admission == product_obligation_live,
        ClutchError::MismatchedState,
    )?;
    let state = state_v3_before.base;
    require(
        state.policy_id.bytes() == policy_id && state.generation == payload.expected_generation,
        ClutchError::MismatchedState,
    )?;
    ...
    let series_obligation_principal = if product_obligation_live {
        0
    } else {
        rent.minimum_balance(DEALER_SERIES_OBLIGATION_ACCOUNT_BYTES_V3)?
    };
    let series_obligation_donation_floor = if product_obligation_live {
        0
    } else {
        accounts[52].lamports()
    };
    ...
    .checked_add(series_obligation_principal)
    .ok_or(ClutchError::Arithmetic)?;
    ...
    let mut product_link_pre = Box::new(SeriesMarketLinkAccountV3::decode_buffer());
    {
        let data = accounts[51]
            .try_borrow_data()
            .map_err(|_| Refusal::Adapter(ClutchError::AccountBorrowFailed))?;
        SeriesMarketLinkAccountV3::decode_into(&data, &mut product_link_pre)
            .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    }
    let untrusted_product_binding = *product_link_pre.state.binding_ref();
    let mut product_root_pre = Box::new(MarketLifecycleRootAccountV3::decode_buffer());
    ...
    let product_link = authenticate_series_market_link_v3(
        program_id,
        &accounts[51],
    ...
    let root_binding = product_root.binding();
    require(
    ...
    let series_obligation_key = DealerSeriesObligationKeyV3 {
        binding_account_id: id(accounts[52].key),
        policy_id: state.policy_id,
        facility_id: state.facility_id,
        dealer_state_account_id: id(accounts[2].key),
        facility_position_binding_id: state.facility_position_binding_id,
        ...
        product_market_binding_id: Id::from_bytes(product_root.binding_id().bytes()),
        series_plan_v5_id: Id::from_bytes(untrusted_product_binding.series_plan_id.bytes()),
        series_market_link_account_id: id(accounts[51].key),
        compiler_bundle_v7_id: Id::from_bytes(untrusted_product_binding.compiler_bundle_id.bytes()),
        attachment_plan_v6_id: Id::from_bytes(untrusted_product_binding.attachment_plan_id.bytes()),
        obligation_configuration_v3_id: Id::from_bytes(
            untrusted_product_binding.obligation_configuration_id.bytes(),
        ),
        product_generation: untrusted_product_binding.generation,
        series_ordinal: untrusted_product_binding.ordinal,
    };
    let existing_series_admission = if product_obligation_live {
        let authenticated_obligation =
            authenticate_dealer_series_obligation_v3(program_id, &accounts[52], false)?;
        let existing = authenticate_existing_dealer_series_admission_v3(
            &product_root,
            &product_link,
            &accounts[2],
            state_v3_before,
    ...
    let mut product_link_successor = Box::new(SeriesMarketLinkV3::decode_buffer());
    let product_founding = if existing_series_admission.is_none() {
        require_creatable(&accounts[52])?;
        let prewrite = authenticate_dealer_product_founding_prewrite_v3(
            program_id,
            &product_root,
            &product_replay,
            &family_policy,
        ...
        let link_plan = prepare_product_link_obligation_admission_v3(
            program_id,
            &product_root,
            &product_link,
            &family_policy,
            &prewrite,
            &mut product_link_successor,
        )?;
        Some((prewrite, family_plan, link_plan))
    } else {
        None
    };
    let facility_endpoint = DealerTransferPositionV3::Facility {
    ...
    with_authenticated_complete_dealer_book_v2(
        ...
        |book, feed_data| {
            ...
            let (state_v3, series_obligation, product_founding_pending) =
                if let Some((prewrite, family_plan, link_plan)) = product_founding {
                    ...
                    let series_obligation = DealerSeriesObligationBindingV3::new_live(
                        series_obligation_key,
                        Id::from_bytes(prewrite.id.bytes()),
                        Id::from_bytes(family_plan.family_admission_receipt_id().bytes()),
                        Id::from_bytes(link_plan.owner_admission_receipt_id().bytes()),
                        Id::from_bytes(link_plan.product_admission_projection_id().bytes()),
                        Id::from_bytes(link_plan.link_semantic_before_id().bytes()),
                        Id::from_bytes(link_plan.link_semantic_after_id().bytes()),
                        link_plan.link_transition_sequence_after(),
                        prewrite.rent,
                    )
                    .map_err(dealer_fault)?;
                    let state_v3 = state_v3_before
                        .admit_product_v3(prepared.dealer.state_after, &series_obligation)
                    .map_err(dealer_fault)?;
                    (
                        state_v3,
                        series_obligation,
            ...
            if product_founding_pending.is_some() {
                write_dealer_body(
                    &accounts[52],
            ...
            let product_founding_commit = if let Some((prewrite, family_plan, link_plan)) =
                product_founding_pending
            {
                let physical = authenticate_dealer_product_founding_postwrites_v3(
                    program_id,
                    prewrite,
                    &family_plan,
                    &link_plan,
                ...
                Some((family_plan, link_plan, physical))
            } else {
                let observed_state = authenticate_dealer_state_v3(program_id, &accounts[2], true)?;
                let observed_obligation = authenticate_dealer_series_obligation_v3(
                    program_id,
                    &accounts[52],
                    false,
                )?;
                let mut product_link_post =
                    Box::new(SeriesMarketLinkAccountV3::decode_buffer());
            ...
            if let Some((family_plan, link_plan, (family_physical, link_physical))) =
                product_founding_commit
            {
                ...
                let (root_after, family_admission) = commit_product_family_admission_v3(
                    program_id,
                    &accounts[48],
                    family_plan,
                    family_physical,
                ...
                let root_after_authentication_id = root_after.authentication_id();
                let root_after_semantic_id = root_after.semantic_id();
                let mut link_before = Box::new(SeriesMarketLinkAccountV3::decode_buffer());
                let mut link_rebound = Box::new(SeriesMarketLinkAccountV3::decode_buffer());
                let (link_after, link_admission) =
                    commit_product_link_obligation_admission_v3(
                        program_id,
                        &accounts[51],
                        link_plan,
                ...
                require(
                    root_after_authentication_id
                            == family_admission.root_authentication_after_id()
                        && root_after_semantic_id == family_admission.root_semantic_after_id()
                        && family_admission.family() == MarketFamilyV1::Dealer
                        && family_admission.family_admission_receipt_id().bytes()
                            == series_obligation.family_admission_receipt_id.bytes()
                        && link_after.authentication_id()
                            == link_admission.link_authentication_after_id()
                        && link_after.semantic_id() == link_admission.link_semantic_after_id()
                        && link_admission.obligation() == SeriesLinkObligationV3::Dealer
                        && link_admission.product_admission_projection_id().bytes()
                            == series_obligation.admission_projection_id.bytes()
                        && link_admission.link_transition_sequence_after()
...
fn create_lp_page(
    ...
    let (policy_id, policy) = authenticate_catalog_policy(program_id, &accounts[1])?;
    let state_v3 = *authenticate_dealer_state_v3(program_id, &accounts[2], true)?.state();
    let state = state_v3.base;
    require(
        state.policy_id.bytes() == policy_id && state.generation == payload.expected_generation,
        ClutchError::MismatchedState,
    ...
    write_dealer_body(
        ...
        DEALER_STATE_V3_ACCOUNT_VERSION,
        state_bump,
        &state_v3.with_base(state_after).map_err(dealer_fault)?,
    )?;
    replay_after
        .encode_into(&mut accounts[4].data.borrow_mut())
        .map_err(dealer_fault)
...
fn activate_facility(
    ...
    write_dealer_body(
        ...
        DEALER_STATE_V3_ACCOUNT_VERSION,
        state_bump,
        &state_v3.with_base(prepared.state_after).map_err(dealer_fault)?,
    )?;
    prepared
        .replay
        .replay_post()
...
mod epoch_state_version_adversarial_tests {
    ...
    #[test]
    fn epoch_actions_follow_the_successor_capability_profile() {
        for action in [DealerFacilityAction::BindEpoch, DealerFacilityAction::LapseEpoch] {
            assert_eq!(
...
mod select_begin_adversarial_tests {
    ...
    fn first_lease_promotes_only_with_the_exact_product_dealer_latch() {
        ...
        assert_eq!(metas.len(), 54);
        assert_eq!(metas[48].role, DealerMetaRoleV1::ProductMarketRoot);
        assert!(metas[48].writable);
        assert_eq!(metas[49].role, DealerMetaRoleV1::ProductMarketReplay);
        assert!(!metas[49].writable);
        assert_eq!(metas[50].role, DealerMetaRoleV1::MarketFamilyCapabilityPolicy);
        assert!(!metas[50].writable);
        assert_eq!(metas[51].role, DealerMetaRoleV1::SeriesMarketLink);
        assert!(metas[51].writable);
        assert_eq!(metas[52].role, DealerMetaRoleV1::SeriesObligation);
        assert!(metas[52].writable);
        assert_eq!(metas[53].role, DealerMetaRoleV1::OrderPage);
    }
    ...
    fn product_admission_state_upgrade_and_obligation_write_are_one_outer() {
        ...
        for guard in [
            ...
            "authenticate_market_lifecycle_replay_v2",
            "authenticate_current_market_family_capability_policy_v1",
            "prepare_product_family_admission_v3",
            "prepare_product_link_obligation_admission_v3",
            "authenticate_dealer_product_founding_prewrite_v3",
            "authenticate_existing_dealer_series_admission_v3",
            "create_full_principal_pda",
            "DealerSeriesObligationBindingV3::new_live",
            ".admit_product_v3(prepared.dealer.state_after, &series_obligation)",
            "authenticate_dealer_product_founding_postwrites_v3",
    ...
    fn existing_admission_rejoins_every_persisted_product_coordinate() {
        ...
        let authenticator = source
            .split("fn authenticate_existing_dealer_series_admission_v3")
            .nth(1)
            .and_then(|value| {
                value
        ...
        for guard in [
            ...
            "obligation.key.compiler_bundle_v7_id",
            "obligation.key.attachment_plan_v6_id",
            "obligation.key.obligation_configuration_v3_id",
            "obligation.key.product_generation",
    ...
    fn dealer_admission_prewriter_owns_exact_pda_rent_and_product_receipt() {
        ...
        let prewriter = source
            .split("fn authenticate_dealer_product_founding_prewrite_v3")
            .nth(1)
        ...
        let owner = source
            .split("impl AuthenticatedProductLinkObligationAdmissionOwnerV3")
            .nth(1)
            .and_then(|value| {
                value.split("impl AuthenticatedProductFamilyAdmissionPostwriteV3")
                    .next()
            })
    ...
    #[test]
    fn complete_begin_handler_follows_the_successor_capability_profile() {
        assert_eq!(
            crate::capabilities::extension_intent_action_enabled(
                DEALER_FAMILY_TAG,
                DEALER_FAMILY_VERSION,
...
mod timed_close_adversarial_tests {
    ...
    #[test]
    fn pre_admission_unwind_uses_state_v3_without_a_fictional_product_child() {
...
mod current_terminal_cut_adversarial_tests {
    ...
    fn active_and_unused_credit_terminal_cuts_are_disjoint_exact_contracts() {
        ...
        assert_eq!(active.len(), DEALER_FAMILY_TERMINAL_ACTIVE_ACCOUNT_COUNT_V1);
        ...
        assert_eq!(active[25].role, DealerMetaRoleV1::Realm);
        assert!(!active[25].writable);
        assert_eq!(active[29].role, DealerMetaRoleV1::CollateralTokenProgramData);
        assert!(!active[29].writable);
        assert_eq!(active[34].role, DealerMetaRoleV1::ClaimLedger);
        assert!(active[34].writable);
        assert_eq!(active[37].role, DealerMetaRoleV1::FractionalLedger);
        assert!(active[37].writable);
        assert_eq!(active[38].role, DealerMetaRoleV1::FacilityCredit);
        assert_eq!(active[38].owner, DealerMetaOwnerV1::SelfProgram);
        assert!(active[38].writable);
        assert!(!active.iter().any(|meta| matches!(
            meta.role,
            DealerMetaRoleV1::ProductMarketRoot | DealerMetaRoleV1::SeriesMarketLink
        )));
        assert!(!active
            .iter()
        ...
        assert_eq!(unused.len(), DEALER_FAMILY_TERMINAL_UNUSED_ACCOUNT_COUNT_V1);
        assert_eq!(unused[34].role, DealerMetaRoleV1::ClaimLedger);
        assert!(!unused[34].writable);
        assert_eq!(unused[35].role, DealerMetaRoleV1::FutureCreditFunding);
        assert_eq!(unused[35].owner, DealerMetaOwnerV1::SelfProgram);
        assert!(unused[35].writable);
        assert!(!unused
            .iter()
    ...
    #[test]
    fn dealer_family_receipt_has_no_product_supplied_authority() {
        let source = include_str!("dealer_facility.rs");
        let receipt = source
            .split("fn seal_dealer_family_terminal_receipt_v3")
            .nth(1)
            .and_then(|value| value.split("struct AuthenticatedExistingDealerSeriesAdmissionV3").next())
            .expect("Dealer family receipt sealer");
        for guard in [
            ...
            assert!(receipt.contains(guard), "missing family receipt guard {guard}");
        }
        assert!(!receipt.contains("AuthenticatedDealerProductResolutionV2"));
        assert!(!receipt.contains("AuthenticatedSeriesDealerTerminalV2"));
    }

    #[test]
    fn dealer_terminalizer_closes_value_before_live_obligation_and_emits_receipt() {
        let source = include_str!("dealer_facility.rs");
        let handler = source
            .split("fn terminalize_dealer_family_for_product_v3")
            .nth(1)
            .and_then(|value| value.split("fn claim_terminal_allocation").next())
            .expect("Dealer family terminalizer");
        for guard in [
            "prepare_dealer_terminal_replay_v2",
            "apply_dealer_position_replay_close_v3",
            "close_dealer_position_replay_v2",
            "apply_dealer_facility_credit_terminal_v1",
            "close_unused_future_credit_funding_v1",
            "prepare_dealer_series_obligation_value_close_v3",
            "release_dealer_account(&accounts[24])",
            "apply_liveness_transition",
            "seal_dealer_family_terminal_receipt_v3",
            "series_obligation_children == 0",
        ] {
            assert!(handler.contains(guard), "missing terminal join {guard}");
        }
        assert!(!handler.contains("authenticate_current_product_resolution_v2"));
        assert!(!handler.contains("terminalize_series_dealer_obligation_v2"));
        let position = handler
            .find("apply_dealer_position_replay_close_v3")
            .expect("Position/Replay close");
        let fractional = handler
            .find("apply_dealer_facility_credit_terminal_v1")
            .expect("Fractional active-credit close");
        let obligation = handler
            .find("prepare_dealer_series_obligation_value_close_v3")
            .expect("Dealer obligation close");
        let receipt = handler
            .find("seal_dealer_family_terminal_receipt_v3")
            .expect("Dealer family receipt");
        assert!(position < fractional && fractional < obligation && obligation < receipt);
    }
...
mod resolve_vector_adversarial_tests {
    ...
    fn vector_handler_follows_the_successor_capability_profile() {
        ...
        for guard in [
            "authenticate_dealer_series_obligation_v3",
            "authenticate_current_product_resolution_v3",
