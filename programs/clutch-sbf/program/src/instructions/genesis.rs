...
//!   system-program CPI **cannot** allocate — 50,054 bytes against the
...
/// Historical host-fixture account count with a funding-ledger tail. Neither
/// the withdrawn General plane nor retired Direct V4 account shapes are
/// admitted by a current Solana constructor.
...
mod tests {
    ...
    fn the_rent_formula_matches_the_runtimes() {
         ...
         * checkpoint at 50,054 bytes (the T2-1 codec re-pin plus the T2-6b
         * owner-interning region), quoted in `SOLANA_LAYOUT.md`. */
        assert_eq!(
    ...
    fn every_created_account_fits_the_cpi_growth_ceiling() {
        for (label, len, fits) in [
             ...
             * one: the streaming checkpoint is 50,054 bytes. */
