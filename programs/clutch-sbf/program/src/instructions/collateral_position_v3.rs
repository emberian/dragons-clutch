...
/// Complete authenticated ordinary-Position and GEN1 Replay prestate under
/// the Product-authorized current MarketBinding V3 account. V2 remains a
/// distinct historical authority and cannot satisfy this capability.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct GeneralPositionReplayAuthorityV3 {
    pub(crate) position: AuthenticatedPositionV3,
    pub(crate) projection: GeneralPositionProjectionV3,
    pub(crate) replay: GeneralPositionReplayPrestateV1,
    pub(crate) market_binding: MarketBindingV3,
    pub(crate) market_runtime: MarketRuntimeV3AccountV1,
}
...
/// Exact current V5 liability authority for compact collateral family frames.
/// It retains the hostile BindingV5/Runtime identities rather than projecting
/// them through the historical V4 layout or requiring the 25-account Product
/// read graph at every value-bearing action.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct GeneralMarketLiabilityAuthorityV5 {
    pub(crate) bound: BoundCollateralProfileV2,
    pub(crate) market_binding: MarketBindingV5,
    pub(crate) market_runtime: MarketRuntimeV3AccountV1,
    pub(crate) market_instance: MarketInstancePreimageV2,
...
/// Same-instruction V5 collateral deployment proof for one value-bearing CPI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct GeneralMarketValueAuthorityV5 {
    pub(crate) liabilities: GeneralMarketLiabilityAuthorityV5,
    pub(crate) deployment:
        crate::collateral_release::AuthenticatedCollateralReleaseDeploymentV2,
    pub(crate) receipt_id: CollateralId,
}

/// Narrow common view for the sole ResolutionV5 decoder. Historical
/// consumers retain their V2 liability authority while current Fractional
/// consumers must present the exact V5 authority; no V5-to-V2 projection is
/// constructed.
pub(crate) trait ResolutionLiabilityAuthorityV5: Copy {
    fn relation_market(&self) -> &MarketBindingV1;
    fn hoard(&self) -> &HoardV2;
    fn claim_ledger(&self) -> &ClaimLedgerV3;
}

impl ResolutionLiabilityAuthorityV5 for GeneralMarketLiabilityAuthorityV2 {
    fn relation_market(&self) -> &MarketBindingV1 {
        self.market_binding.base()
    }
...
impl ResolutionLiabilityAuthorityV5 for GeneralMarketLiabilityAuthorityV5 {
    fn relation_market(&self) -> &MarketBindingV1 {
        self.market_binding.base().base()
    }
...
/// Authenticate only the Product-authorized General MarketBinding V3 and its
/// stable runtime. The historical V1/V2 bindings cannot satisfy this seam.
/// Both immutable accounts must remain read-only, must retain their typed rent
/// principal plus donation floor, and must be the exact canonical PDAs.
...
pub(crate) fn authenticate_general_market_v5_with_data_ids(
    ...
    Ok(AuthenticatedGeneralMarketV5 {
        account: *market_binding_account.key,
        binding,
        runtime_account: *market_runtime_account.key,
        runtime,
        binding_data_id,
        runtime_data_id,
        id,
...
pub(crate) fn authenticate_general_market_liabilities_v5(
    ...
    hoard_writable: bool,
    claim_ledger_writable: bool,
) -> Outcome<GeneralMarketLiabilityAuthorityV5> {
    let realm = crate::collateral_release::authenticate_realm_collateral_v2(
        program_id,
        realm_account,
        profile_account,
    ...
    let claim_ledger_semantic_id = claim_ledger
        .semantic_id(&RuntimeSha256)
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    let market_binding_data_id = authenticated_market.binding_data_id();
    let market_runtime_data_id = authenticated_market.runtime_data_id();
    let hoard_lamports = hoard_account.lamports();
    let claim_ledger_lamports = claim_ledger_account.lamports();
    require_deletable_rent_coverage_v1(hoard.rent, hoard_lamports, false)?;
    require_deletable_rent_coverage_v1(claim_ledger.rent, claim_ledger_lamports, false)?;
    ...
    receipt_id
        .require_live()
        .map_err(|_| Refusal::Adapter(ClutchError::AuthorizationUnavailable))?;
    Ok(GeneralMarketLiabilityAuthorityV5 {
        bound,
        market_binding,
        market_runtime,
        market_instance,
...
/// Authenticate the exact current V5 liability owners and Profile-selected
/// collateral ProgramData before any value-bearing CPI.
#[allow(clippy::too_many_arguments)]
pub(crate) fn authenticate_general_market_value_authority_v5(
    program_id: &Pubkey,
    realm_account: &AccountInfo<'_>,
    profile_account: &AccountInfo<'_>,
    policy_account: &AccountInfo<'_>,
    ...
    hoard_writable: bool,
    claim_ledger_writable: bool,
) -> Outcome<GeneralMarketValueAuthorityV5> {
    ...
    let receipt_id = CollateralId::from_bytes(
        solana_sha256_hasher::hashv(&[
            GENERAL_MARKET_VALUE_AUTHORITY_DOMAIN_V5,
            &liabilities.receipt_id.bytes(),
    ...
    receipt_id
        .require_live()
        .map_err(|_| Refusal::Adapter(ClutchError::AuthorizationUnavailable))?;
    Ok(GeneralMarketValueAuthorityV5 {
        liabilities,
        deployment,
        receipt_id,
    })
...
pub(crate) fn authenticate_resolution_v5<A: ResolutionLiabilityAuthorityV5>(
    ...
    let resolution = ResolutionV5::decode(&resolution_account.data.borrow())
        .map_err(|_| Refusal::Adapter(ClutchError::MismatchedState))?;
    let relation_market = liabilities.relation_market();
    let market_bytes = relation_market.market_instance_v2_id.bytes();
    expect_pda(
        resolution_account.key,
        seeds::resolution_v5_pda(program_id, &market_bytes),
...
/// raw BindingV5 facts are not accepted here.
#[allow(clippy::too_many_arguments)]
pub(crate) fn authenticate_general_position_replay_from_market_v5(
    program_id: &Pubkey,
    ...
    require(
        current.account() == *market_binding_account.key
            && current.runtime_account() == *market_runtime_account.key
            && current.binding().base().market.bytes() == market_runtime_account.key.to_bytes()
            && current.runtime().market_binding.bytes() == market_binding_account.key.to_bytes(),
        ClutchError::MismatchedState,
...
/// Replay against the Product-authorized current MarketBinding V3 account.
/// The V3 rent/PDA/Product join is reauthenticated before any Position bytes
/// are projected, so an old 572-byte V2 account cannot enter this path.
#[allow(clippy::too_many_arguments)]
fn authenticate_general_position_replay_with_access_v3(
    program_id: &Pubkey,
    bound: BoundCollateralProfileV2,
    market_binding_account: &AccountInfo<'_>,
    market_runtime_account: &AccountInfo<'_>,
    ...
    expected_sequence: u64,
    position_writable: bool,
) -> Outcome<GeneralPositionReplayAuthorityV3> {
    let (market_binding, market_runtime) =
        authenticate_general_market_v3(program_id, market_binding_account, market_runtime_account)?;
    let relation_market = market_binding.base().relation_projection();
    require_program_account(
        program_id,
        position_account,
        position_writable,
    ...
    .map_err(|_| Refusal::Adapter(ClutchError::Replay))?;
    Ok(GeneralPositionReplayAuthorityV3 {
        position: authenticated,
        projection,
        replay,
        market_binding,
...
/// Authenticate one writable ordinary Position and writable GEN1 Replay
/// against current MarketBinding V3.
#[allow(clippy::too_many_arguments)]
pub(crate) fn authenticate_general_position_replay_v3(
    program_id: &Pubkey,
    bound: BoundCollateralProfileV2,
    market_binding_account: &AccountInfo<'_>,
    market_runtime_account: &AccountInfo<'_>,
    ...
    authenticate_general_position_replay_with_access_v3(
        program_id,
        bound,
        market_binding_account,
        market_runtime_account,
...
/// against current MarketBinding V3.
#[allow(clippy::too_many_arguments)]
pub(crate) fn authenticate_general_position_replay_readonly_v3(
    program_id: &Pubkey,
    bound: BoundCollateralProfileV2,
    market_binding_account: &AccountInfo<'_>,
    market_runtime_account: &AccountInfo<'_>,
