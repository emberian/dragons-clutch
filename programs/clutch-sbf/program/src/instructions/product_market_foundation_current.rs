...
/// Consume one family-owned postwrite against the exact Product-funded debit
/// and return the sole V4 root transition projection.  Product's cursor is the
/// only constructor of the debit; callers cannot supply any projection field.
pub(crate) fn accept_product_market_foundation_postwrite_v4<P>(
    debit: AuthenticatedProductMarketFoundationDebitV4,
    postwrite: P,
) -> Outcome<MarketFoundationStepProjectionV4>
where
    P: AuthenticatedProductMarketFoundationStepPostwriteV4,
{
    ...
    let (accepted_poststate_receipt_id, observed_vault_donation_lamports) = postwrite
    ...
    require(
        accepted_poststate_receipt_id != ContentId::ZERO
            && observed_vault_donation_lamports == debit.vault_donation_after_lamports,
        ClutchError::MismatchedState,
    )?;
    Ok(MarketFoundationStepProjectionV4 {
        ...
        donation_before_lamports: debit.vault_donation_before_lamports,
        donation_after_lamports: debit.vault_donation_after_lamports,
