...
use clutch_product_series::{
    ...
    CompiledProductSeriesBundleV7, FixedCodec,
    ...
    SeriesAttachmentPlanV6, SeriesFundingQuoteV2, SeriesFundingQuoteV3,
    SeriesFundingQuoteV4, SeriesFundingQuoteV5, SeriesFundingQuoteV6,
    ...
    COMPILED_PRODUCT_SERIES_BUNDLE_V7_BYTES,
    ...
    SERIES_ATTACHMENT_PLAN_BYTES_V5, SERIES_ATTACHMENT_PLAN_BYTES_V6,
    SERIES_FUNDING_QUOTE_BYTES_V2,
    SERIES_FUNDING_QUOTE_BYTES_V3, SERIES_FUNDING_QUOTE_BYTES_V4,
...
use clutch_structured_claim_runtime_contract::{
    WrapperRecipeHashV1, WrapperRecipeSetV1, WRAPPER_RECIPE_SET_BYTES_V1,
};
...
const _: () = {
    ...
    assert!(REGISTRY_CAPABILITY_PROFILE_V3_BYTES == 816);
    ...
    assert!(SERIES_FUNDING_QUOTE_BYTES_V3 == 584);
    ...
    assert!(COMPILED_PRODUCT_SERIES_BUNDLE_V6_BYTES == 528);
    assert!(SERIES_ATTACHMENT_PLAN_BYTES_V5 == 112);
...
pub const MAX_ARTIFACT_BYTES: usize = WRAPPER_RECIPE_SET_BYTES_V1;

struct WrapperRecipeArtifactSha256V1;

impl WrapperRecipeHashV1 for WrapperRecipeArtifactSha256V1 {
...
pub enum ArtifactRegistrationStatus {
    /// New staging and sealing are admitted.
    Current,
    /// Coordinates remain decodable but no upload step may register them.
    Withdrawn,
}
...
pub enum ArtifactKind {
    ...
    /// Withdrawn historical 800-byte registry capability profile V2.
    ...
    /// Withdrawn historical 648-byte recurring-Series funding quote V2.
    SeriesFundingQuoteV2 = 48,
    /// Withdrawn compiler graph binding withdrawn QuoteV2 and AttachmentV2.
    ...
    /// Historical 46-slot recurring-Series funding quote V4.
    ...
    /// Historical operational attachment plan bound to one exact QuoteV4.
    ...
    /// Historical compiler graph binding ProfileV4 and Quote/Attachment V4.
    CompiledProductSeriesBundleV5 = 60,
    /// Historical 47-slot recurring-Series funding quote V5.
    SeriesFundingQuoteV5 = 61,
    /// Historical operational attachment plan bound to one exact QuoteV5.
    SeriesAttachmentPlanV5 = 62,
    /// Historical compiler graph binding ProfileV4 and Quote/Attachment V5.
    ...
    /// Current 50-slot recurring-Series funding quote V6.
    SeriesFundingQuoteV6 = 66,
    /// Current operational attachment plan bound to one exact QuoteV6.
...
impl ArtifactKind {
    ...
    pub const fn from_byte(byte: u8) -> Result<Self> {
        match byte {
            ...
            51 => Ok(Self::RegistryCapabilityProfileV3),
            ...
            61 => Ok(Self::SeriesFundingQuoteV5),
    ...
    pub const fn byte(self) -> u8 {
        match self {
            Self::CollateralPolicy => 1,
            Self::PriceGrid => 2,
            Self::Terms => 3,
            Self::BatchPolicy => 4,
            Self::DirectBatchPolicyV3 => 5,
            Self::NativeClaimBasisV1 => 32,
            Self::EvidenceOnlyRecoveryPolicyV1 => 33,
            Self::ProductTemplateV4 => 34,
            Self::PriceMeasurePolicyV1 => 35,
            Self::MarketGenesisProfileV2 => 36,
            Self::SeriesFundingQuoteV1 => 37,
            Self::SeriesAttachmentPlanV1 => 38,
            Self::SeriesPlanV5 => 39,
            Self::SeriesFundingTermsV2 => 40,
            Self::RegistryProgramReleaseV1 => 41,
            Self::CompiledProductSeriesBundleV1 => 42,
            Self::RegistryCapabilityProfileV2 => 43,
            Self::SourceReleaseManifestV1 => 44,
            Self::SourceWorkScheduleV1 => 45,
            Self::MarketInstancePreimageV2 => 46,
            Self::SourceReleaseManifestV2 => 47,
            Self::SeriesFundingQuoteV2 => 48,
            Self::CompiledProductSeriesBundleV2 => 49,
            Self::SeriesAttachmentPlanV2 => 50,
            Self::RegistryCapabilityProfileV3 => 51,
    ...
    pub const fn exact_len(self) -> usize {
        match self {
            ...
            Self::RegistryCapabilityProfileV3 => REGISTRY_CAPABILITY_PROFILE_V3_BYTES,
            ...
            Self::SeriesFundingQuoteV5 => SERIES_FUNDING_QUOTE_BYTES_V5,
    ...
    pub const fn registration_status(self) -> ArtifactRegistrationStatus {
        match self {
            ...
            | Self::SeriesFundingQuoteV5
            | Self::SeriesAttachmentPlanV5
            | Self::CompiledProductSeriesBundleV6 => ArtifactRegistrationStatus::Withdrawn,
...
impl ArtifactBinding {
    ...
    /// Refuse structurally valid but withdrawn coordinates for new uploads.
    pub fn validate_for_registration(&self) -> Result<()> {
        self.validate()?;
        if self.kind.registration_status() == ArtifactRegistrationStatus::Withdrawn {
...
pub fn validate_artifact(binding: ArtifactBinding, body: &[u8]) -> Result<u8> {
    ...
    match binding.kind {
        ...
        ArtifactKind::RegistryCapabilityProfileV3 => {
            let value = RegistryCapabilityProfileV3::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
        ...
        ArtifactKind::SeriesFundingQuoteV3 => {
            let value =
                SeriesFundingQuoteV3::decode(body).map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
                    .map_err(|_| CodecError::MismatchedBinding)?
        ...
        ArtifactKind::CompiledProductSeriesBundleV3 => {
            let value = CompiledProductSeriesBundleV3::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
        ...
        ArtifactKind::SeriesAttachmentPlanV3 => {
            let value =
                SeriesAttachmentPlanV3::decode(body).map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
                    .map_err(|_| CodecError::MismatchedBinding)?
        ...
        ArtifactKind::SeriesFundingQuoteV5 => {
            let value =
                SeriesFundingQuoteV5::decode(body).map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
                    .map_err(|_| CodecError::MismatchedBinding)?
        ...
        ArtifactKind::SeriesAttachmentPlanV5 => {
            let value = SeriesAttachmentPlanV5::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
        ...
        ArtifactKind::CompiledProductSeriesBundleV6 => {
            let value = CompiledProductSeriesBundleV6::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
        ...
        ArtifactKind::WrapperRecipeSetV1 => {
            let value = WrapperRecipeSetV1::decode(body, &WrapperRecipeArtifactSha256V1)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id(&WrapperRecipeArtifactSha256V1)
                    .map_err(|_| CodecError::MismatchedBinding)?,
            ) != binding.digest
            {
                return Err(CodecError::MismatchedBinding);
        ...
        ArtifactKind::SeriesFundingQuoteV6 => {
            let value =
                SeriesFundingQuoteV6::decode(body).map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
                    .map_err(|_| CodecError::MismatchedBinding)?
        ...
        ArtifactKind::SeriesAttachmentPlanV6 => {
            let value = SeriesAttachmentPlanV6::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
        ...
        ArtifactKind::CompiledProductSeriesBundleV7 => {
            let value = CompiledProductSeriesBundleV7::decode(body)
                .map_err(|_| CodecError::MismatchedBinding)?;
            if Hash32::from_bytes(
                value
                    .id()
...
mod tests {
        ...
        for (tag, expected) in (u8::MIN..=u8::MAX).map(|tag| {
            let expected = if (32..=69).contains(&tag) {
                Ok(match tag {
                    ...
                    51 => ArtifactKind::RegistryCapabilityProfileV3,
                    ...
                    61 => ArtifactKind::SeriesFundingQuoteV5,
    ...
    fn production_profiles_refuse_product_series_but_admit_central_registry_artifacts() {
        ...
        assert_eq!(
            ArtifactKind::from_byte(51),
            Ok(ArtifactKind::RegistryCapabilityProfileV3)
        );
        assert_eq!(
            ArtifactKind::from_byte(52),
            Ok(ArtifactKind::SeriesFundingQuoteV3)
        );
        assert_eq!(
            ArtifactKind::from_byte(53),
            Ok(ArtifactKind::CompiledProductSeriesBundleV3)
        );
        assert_eq!(
        ...
        assert_eq!(
            ArtifactKind::from_byte(61),
            Ok(ArtifactKind::SeriesFundingQuoteV5)
        );
        assert_eq!(
            ArtifactKind::from_byte(62),
            Ok(ArtifactKind::SeriesAttachmentPlanV5)
        );
        assert_eq!(
            ArtifactKind::from_byte(63),
            Ok(ArtifactKind::CompiledProductSeriesBundleV6)
        );
        assert_eq!(
        ...
        assert_eq!(
            ArtifactKind::from_byte(66),
            Ok(ArtifactKind::SeriesFundingQuoteV6)
        );
        assert_eq!(
            ArtifactKind::from_byte(67),
            Ok(ArtifactKind::SeriesAttachmentPlanV6)
        );
        assert_eq!(
            ArtifactKind::from_byte(68),
            Ok(ArtifactKind::CompiledProductSeriesBundleV7)
        );
    ...
    fn withdrawn_artifact_coordinates_are_decode_only_and_refuse_registration() {
        for kind in [
            ...
            assert_eq!(
                historical.validate_for_registration(),
