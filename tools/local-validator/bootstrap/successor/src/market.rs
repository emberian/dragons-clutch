...
pub(crate) const REMAINING_OPEN_SEAM: &str = "The campaign publishes the Realm/Product/ResultDomain/Portfolio/Source/RecoveryPolicy/capability-manifest graph and the canonical ProductBasisV3 the Product declares as its liability basis, creates a real lifecycle RentCreditV2, routes the oversized 31-account Found frame through a finalized address lookup table as a packet-safe v0 transaction, creates the Market, and then executes DCLTPCB1 at its own generation - the four-stage projected-Custody founding prestate in one rollback domain. Found31 is not the obstacle it once was and this string used to say otherwise: it creates the Market and costs 232,537 measured compute units, 16.6% of the per-transaction maximum. The Market is Found, not Open, and exactly one thing now stands between here and Open: this runner has no DCLTGMF1 stage. That is a runner gap and no longer a protocol gap. It was a protocol gap until dba22b5: Core's Found stage commits a Registry-owned ProductBasisV3 digest and semantic identity into the Claims request, and Claims FoundingV5 demanded an account with that same digest decoding as a legacy Core-owned LinkedBasisRecordV2, which no byte string satisfies. Claims now authenticates the basis Core actually committed, and this campaign already publishes that exact record, so nothing here has to change to make the founding outer satisfiable - it only has to be built. Two measured facts the next lane needs. DCLTPCB1 does not execute at all: it is heap-bound, not compute-bound, and no runner can fix it. Its four stages allocate from a bump allocator that never frees, and it exhausts the heap entering its third stage with most of the compute budget unspent - Custody Initialize and OpenHoard both succeed first. Requesting a 256 KiB heap frame was tried and measured to change nothing, because solana-program-entrypoint builds its BumpAllocator with the compile-time constant HEAP_LENGTH = 32 KiB and never asks the runtime how much heap it was actually granted. The fix is program-side, in projected_custody_bootstrap_v1. Second fact: the founding must run at its own generation, because every projected-Custody stage asserts the inverse of a live Market. See docs/evidence/GENERIC_FOUNDING_REACHABILITY_2026_08_26.md.";
