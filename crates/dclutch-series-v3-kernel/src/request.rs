//! Exact family request for recurring-Series V3 actions.
//!
//! This SDK-free packet owns only action selection, immutable content
//! references, replay revisions, and the occurrence Merkle path. Economic
//! amounts and program identities remain in the referenced records and current
//! Registry release set. Physical adapters authenticate finalized-record
//! provenance and account authority before consuming the admitted result.

use dclutch_core_contract::ContentId;

use crate::{
    AdmittedOccurrenceV3, AdmittedTicketV3, TemplateV3, admit_occurrence_bytes, admit_ticket,
    template_content_id,
};
...
pub enum SeriesInstructionErrorV3 {
    ...
    Proof,
    /// Exact Template, occurrence, Ticket, or their content join refused.
    ImmutableContent,
}
...
impl<'a> SeriesActionRequestV3<'a> {
    ...
    /// Complete exact header and proof bytes.
...
/// Exact immutable Series records admitted for one decoded action request.
///
/// This value owns no finalized-record provenance or account authority. A
/// physical adapter must authenticate the supplied record bytes before calling
/// [`admit_series_action_v3`].
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl<'a> AdmittedSeriesActionV3<'a> {
    /// Exact hostile-decoded action request.
    pub const fn request(self) -> SeriesActionRequestV3<'a> {
        self.request
    }
...
/// Join one canonical action request to its exact immutable record bodies.
///
...
) -> Result<AdmittedSeriesActionV3<'a>, SeriesInstructionErrorV3> {
...
mod tests {
    ...
    fn put(output: &mut [u8], offset: usize, value: &[u8]) {
        let end = offset.checked_add(value.len()).expect("fixture end");
        output
            .get_mut(offset..end)
    ...
    #[test]
    fn one_kernel_admission_joins_request_template_occurrence_and_ticket() {
        let mut occurrence = crate::generated::SERIES_EXAMPLE_OCCURRENCE_V3;
        put(
            &mut occurrence,
            crate::generated::SERIES_OCCURRENCE_INDEX_OFFSET_V3,
            &0_u32.to_le_bytes(),
        );
        put(
            &mut occurrence,
            crate::generated::SERIES_OCCURRENCE_SCHEDULED_SLOT_OFFSET_V3,
            &100_u64.to_le_bytes(),
        );
        let occurrence_id = crate::occurrence_content_id(&occurrence).expect("occurrence ID");

        let mut template = crate::generated::SERIES_EXAMPLE_TEMPLATE_V3;
        put(
            &mut template,
            crate::generated::SERIES_TEMPLATE_OCCURRENCE_COUNT_OFFSET_V3,
            &1_u32.to_le_bytes(),
        );
        put(
            &mut template,
            crate::generated::SERIES_TEMPLATE_PROJECTION_ROOT_OFFSET_V3,
            &occurrence_id.to_bytes(),
        );
        let template_id = crate::template_content_id(&template).expect("Template ID");

        let mut ticket = crate::generated::SERIES_EXAMPLE_TICKET_V3;
        put(
            &mut ticket,
            crate::generated::SERIES_TICKET_INDEX_OFFSET_V3,
            &0_u32.to_le_bytes(),
        );
        put(
            &mut ticket,
            crate::generated::SERIES_TICKET_TEMPLATE_OFFSET_V3,
            &template_id.to_bytes(),
        );
        put(
            &mut ticket,
            crate::generated::SERIES_TICKET_OCCURRENCE_ID_OFFSET_V3,
            &occurrence_id.to_bytes(),
        );
        let ticket_id = crate::admit_ticket(&ticket)
            .expect("Ticket ID")
            .content_id();
        ...
        let admitted =
            admit_series_action_v3(&request, &template, Some(&occurrence), Some(&ticket))
                .expect("admission");
        assert_eq!(admitted.request().bytes(), request.as_slice());
        assert_eq!(admitted.template_id(), template_id);
        assert_eq!(
            admitted
                .required_occurrence()
                .expect("occurrence")
        ...
        assert_eq!(
            admitted.required_ticket().expect("Ticket").content_id(),
            ticket_id
        );

        let mut substituted = ticket;
        *substituted
            .get_mut(crate::generated::SERIES_TICKET_FOUNDER_OFFSET_V3)
            .expect("founder byte") ^= 1;
        assert_eq!(
            admit_series_action_v3(&request, &template, Some(&occurrence), Some(&substituted),),
            Err(SeriesInstructionErrorV3::ImmutableContent)
        );
    }
}
