...
use dclutch_rational_representation_v2_contract::{
    OpenRepresentationHotRequestV3, REPRESENTATION_FRAME_SPEC_V2, RepresentationActionV2,
...
use crate::hot_transaction_v3::build_hot_instruction_from_claims_child_v3;
use crate::{
    Error, RATIONAL_OPEN_SELECTED_CHILD_ACCOUNTS_V3, RationalTerminalHotStateV3, Result,
...
mod tests {
    ...
    fn selected_open_builds_hot38_and_binds_complete_family_digest() {
        ...
        let built = build_rational_open_selected_hot_instruction_v3(&state, &selected)
            .expect("selected Hot");
        assert_eq!(
    ...
    fn family_or_child_substitution_refuses_before_instruction_output() {
        ...
        *hostile.family_request.last_mut().expect("family byte") ^= 1;
        assert_eq!(
            build_rational_open_selected_hot_instruction_v3(&state, &hostile),
