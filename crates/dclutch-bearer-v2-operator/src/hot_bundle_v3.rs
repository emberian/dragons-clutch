...
pub const RATIONAL_TERMINAL_DESCRIPTOR_BYTES_V3: usize = CAPABILITY_PROGRAM_V3_BYTES;
...
pub struct RationalTerminalHotBundleInputV3<'a> {
    ...
    /// Manifest-selected root derivation policy.
...
pub fn validate_rational_terminal_hot_bundle_v3(
    ...
    let strategy_id = digest(&bundle.strategy)?;
    strategy
    ...
    if descriptor.request_schema().to_bytes() != RATIONAL_TERMINAL_HOT_REQUEST_SCHEMA_ID_V3
        ...
        || effect.fixed_account_count() != RATIONAL_TERMINAL_LOGICAL_ACCOUNT_COUNT_V3
        || effect.item_account_stride() != 0
        || effect.route_count() != 1
        || effect.receipt_dependency_count() != 0
        || !geometry_matches(account, request, transition, effect)
    {
...
mod tests {
    ...
    fn complete_bundle_joins_exact_interpreted_artifacts() {
        ...
        assert_eq!(
            ...
            digest(&bundle.account_profile).expect("id")
        );
        assert_eq!(
            descriptor.effect_program(),
            digest(&bundle.effect).expect("id")
        );
    }
