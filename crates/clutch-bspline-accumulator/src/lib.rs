...
use clutch_bspline::{BasisSpec, Error as BasisError, MAX_OUTCOMES};
...
impl Summary {
    ...
    pub fn accepted(domain: BasisDomain, bucket: u64, point: u128) -> Result<Self> {
        ...
        let vector = domain.spec.evaluate(point)?;
    ...
    pub fn validate(self) -> Result<()> {
        ...
        let active = usize::from(self.domain.spec.outcome_count);
        ...
        let expected = u128::from(self.domain.spec.denominator)
    ...
    pub fn finalize(self, mode: FinalizationMode) -> Result<FinalWeights> {
        ...
        let denominator = self.domain.spec.denominator;
        ...
        match mode {
            ...
            FinalizationMode::LargestRemainderV1 => {
                if residual > u64::from(self.domain.spec.outcome_count - 1) {
                    return Err(Error::InvalidSummary);
                }
        ...
        let output = FinalWeights {
            active_len: self.domain.spec.outcome_count,
            denominator,
            weights,
        };
        output.validate()?;
