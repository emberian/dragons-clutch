...
use clutch_general_v2_runtime::{
    ...
    QuantizedWitnessBodyV1, QUANTIZED_WITNESS_BODY_MAX_BYTES_V1, RELATION_V2_POLICY_BODY_V1,
...
fn body() -> QuantizedWitnessBodyV1 {
    ...
    prices[..4].copy_from_slice(&[1, 2, 3, 4]);
...
#[test]
fn witness_codec_is_exact_active_width_and_every_binding_changes_its_identity() {
    let original = body();
    let len = original
        .encoded_len()
        .expect("canonical body has exact width");
    let mut storage = [0u8; QUANTIZED_WITNESS_BODY_MAX_BYTES_V1];
    original
        .encode(&mut storage[..len])
        .expect("canonical body encodes");
    assert_eq!(len, 149 + (4 * 8) + (2 * 24));
    assert_eq!(&storage[..32], &id(1).bytes());
    ...
    mutation = original;
    mutation.economic_domain_digest = id(9);
    assert_ne!(mutation.digest(), Ok(original_digest));
    mutation = original;
