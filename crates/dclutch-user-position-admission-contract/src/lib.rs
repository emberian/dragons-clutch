...
//! ordinary wallet authorizes occupying its unique Position coordinate, while
//! Trading supplies the release-bound caller PDA Claims already requires.
//!
...
pub enum UserPositionAdmissionErrorV1 {
    ...
    /// The embedded request selected another lifecycle action.
    InvalidAction,
...
/// One exact wallet-authorized admission request.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl UserPositionAdmissionRequestV1 {
    /// Select one canonical Claims `Admit + User + Vacant` request.
    pub fn new(claims_request: ProtocolPositionRequestV2) -> Result<Self> {
        ...
        if claims_request.action != ProtocolPositionActionV2::Admit {
        ...
        if claims_request.presence != ProtocolPositionPresenceV2::Vacant {
            return Err(UserPositionAdmissionErrorV1::InvalidPresence);
        }
...
mod tests {
    ...
    #[test]
    fn incompatible_claims_lifecycles_refuse_before_encoding() {
        ...
        hostile = claims_request();
        hostile.action = ProtocolPositionActionV2::Close;
        hostile.presence = ProtocolPositionPresenceV2::Existing;
        assert_eq!(
            UserPositionAdmissionRequestV1::new(hostile),
