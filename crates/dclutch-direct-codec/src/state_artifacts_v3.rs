...
use crate::{
    ...
    ordinary_v3::{
        ...
        SCALAR_MARKET_GENERATION_V3, SCALAR_SELLER_BUMP_OBSERVATION_V3, SCALAR_SELLER_BUMP_V3,
        SCALAR_SELLER_CREATED_V3, SCALAR_SELLER_RENT_PRINCIPAL_OBSERVATION_V3,
...
/// Exact Lifecycle V4 width for inline ordinary maker first-use.
pub const DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4: usize = HEADER_BYTES
...
pub fn encode_direct_inline_ordinary_lifecycle_v4_atomic(
    scratch: &mut [u8],
    output: &mut [u8],
) -> Result<(), DirectStateArtifactErrorV3> {
    if scratch.len() != DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4
        || output.len() != DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4
    {
...
mod tests {
    ...
    fn artifact() -> [u8; DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4] {
        let mut scratch = [0_u8; DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4];
        let mut output = [0_u8; DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4];
        encode_direct_inline_ordinary_lifecycle_v4_atomic(&mut scratch, &mut output)
            .expect("lifecycle");
        output
    }

    #[test]
    fn exact_policy_round_trips_two_mandatory_first_use_plans() {
        assert_eq!(DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4, 648);
        let bytes = artifact();
        let digest: [u8; 32] = Sha256::digest(bytes).into();
        let policy = StateLifecyclePolicyV4::decode_selected(digest, digest, &bytes)
    ...
    fn wrong_width_refuses_without_output_mutation() {
        let mut scratch = [0_u8; DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4];
        let mut output = [0x55_u8; DIRECT_INLINE_ORDINARY_LIFECYCLE_BYTES_V4 - 1];
        let before = output;
        assert_eq!(
            encode_direct_inline_ordinary_lifecycle_v4_atomic(&mut scratch, &mut output),
