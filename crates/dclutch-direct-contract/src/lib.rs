...
use dclutch_realm_contract::{MAX_OUTCOMES, MIN_OUTCOMES, PositionV1};
...
impl DirectIntentV1 {
    ...
    pub fn decode_signed_preimage(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != DIRECT_INTENT_BYTES {
            return Err(Error::InvalidLength);
        }
        if array::<8>(bytes, 0)? != DIRECT_INTENT_MAGIC {
            return Err(Error::InvalidMagic);
        }
        if u16::from_le_bytes(array(bytes, 8)?) != DIRECT_INTENT_SCHEMA_VERSION {
            return Err(Error::UnsupportedSchema);
        }
        zeros(bytes, INTENT_RESERVED, 4)?;
        zeros(bytes, INTENT_FEE_RESERVED, 6)?;
        Self::new(DirectIntentInput {
            market: array(bytes, MARKET)?,
            generation: u64::from_le_bytes(array(bytes, GENERATION)?),
            maker: array(bytes, MAKER)?,
            nonce: u64::from_le_bytes(array(bytes, NONCE)?),
            valid_from_slot: u64::from_le_bytes(array(bytes, START)?),
            valid_through_slot: u64::from_le_bytes(array(bytes, END)?),
            side: Side::decode(one(bytes, SIDE)?)?,
            outcome: one(bytes, OUTCOME)?,
            max_fill: u64::from_le_bytes(array(bytes, MAX_FILL)?),
            limit_price: u64::from_le_bytes(array(bytes, LIMIT)?),
            fee_config: array(bytes, FEE_CONFIG)?,
            fee_basis_points: u16::from_le_bytes(array(bytes, INTENT_FEE_BPS)?),
        })
    }
    ...
    pub fn signed_preimage(self) -> [u8; DIRECT_INTENT_BYTES] {
        ...
        put(&mut out, 0, &DIRECT_INTENT_MAGIC);
        put(&mut out, 8, &DIRECT_INTENT_SCHEMA_VERSION.to_le_bytes());
        out[SIDE] = self.side.byte();
        out[OUTCOME] = self.outcome;
        put(&mut out, MARKET, &self.market);
        put(&mut out, GENERATION, &self.generation.to_le_bytes());
        put(&mut out, MAKER, &self.maker);
        put(&mut out, NONCE, &self.nonce.to_le_bytes());
        put(&mut out, START, &self.from.to_le_bytes());
        put(&mut out, END, &self.through.to_le_bytes());
        put(&mut out, MAX_FILL, &self.max_fill.to_le_bytes());
        put(&mut out, LIMIT, &self.limit.to_le_bytes());
        put(&mut out, FEE_CONFIG, &self.fee_config);
        put(&mut out, INTENT_FEE_BPS, &self.fee_bps.to_le_bytes());
        out
    }
...
impl IntentStateV1 {
    ...
    pub const fn open(intent: DirectIntentV1) -> Self {
        Self {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != INTENT_STATE_BYTES {
            return Err(Error::InvalidLength);
        }
        if array::<8>(bytes, 0)? != INTENT_STATE_MAGIC {
            return Err(Error::InvalidMagic);
        }
        if u16::from_le_bytes(array(bytes, 8)?) != INTENT_STATE_SCHEMA_VERSION {
            return Err(Error::UnsupportedSchema);
        }
        zeros(bytes, STATE_RESERVED, 5)?;
        let cancelled = match one(bytes, STATE_STATUS)? {
            0 => false,
            1 => true,
            _ => return Err(Error::UnknownIntentStatus),
        };
        let state = Self {
            market: array(bytes, MARKET)?,
            generation: u64::from_le_bytes(array(bytes, GENERATION)?),
            maker: array(bytes, MAKER)?,
            nonce: u64::from_le_bytes(array(bytes, NONCE)?),
            filled: u64::from_le_bytes(array(bytes, STATE_FILLED)?),
            cancelled,
        };
        nonzero(&state.market)?;
        nonzero(&state.maker)?;
        Ok(state)
    }
    /// Encode one canonical replay-state record.
    pub fn encode(&self, output: &mut [u8]) -> Result<()> {
        if output.len() != INTENT_STATE_BYTES {
            return Err(Error::OutputLength);
        }
        nonzero(&self.market)?;
        nonzero(&self.maker)?;
        output.fill(0);
        put(output, 0, &INTENT_STATE_MAGIC);
        put(output, 8, &INTENT_STATE_SCHEMA_VERSION.to_le_bytes());
        if let Some(status) = output.get_mut(STATE_STATUS) {
            *status = if self.cancelled { 1 } else { 0 };
        }
        put(output, MARKET, &self.market);
        put(output, GENERATION, &self.generation.to_le_bytes());
        put(output, MAKER, &self.maker);
        put(output, NONCE, &self.nonce.to_le_bytes());
        put(output, STATE_FILLED, &self.filled.to_le_bytes());
        Ok(())
    }
    /// Cancel this state, requiring exact maker authorization.
    pub fn cancel(self, intent: DirectIntentV1, auth: OwnerAuthorization) -> Result<Self> {
        self.for_intent(intent)?;
        authorized(intent, auth)?;
        Ok(Self {
            cancelled: true,
            ..self
        })
    ...
    fn for_intent(&self, intent: DirectIntentV1) -> Result<()> {
        ...
        {
            return Err(Error::StateLocatorMismatch);
        }
        if self.filled > intent.max_fill {
    ...
    fn consume(self, intent: DirectIntentV1, slot: u64, fill: u64) -> Result<Self> {
        self.for_intent(intent)?;
        if self.cancelled {
            return Err(Error::IntentCancelled);
        }
        if slot < intent.from || slot > intent.through {
...
impl VenueFeePolicyV1 {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != VENUE_FEE_POLICY_BYTES {
            return Err(Error::InvalidLength);
        }
        if array::<8>(bytes, 0)? != VENUE_FEE_POLICY_MAGIC {
            return Err(Error::InvalidMagic);
        }
        if u16::from_le_bytes(array(bytes, 8)?) != VENUE_FEE_POLICY_SCHEMA_VERSION {
            return Err(Error::UnsupportedSchema);
        }
        zeros(bytes, FEE_RESERVED, 4)?;
        Self::new(
            array(bytes, FEE_RECIPIENT)?,
            u64::from_le_bytes(array(bytes, 48)?),
            array(bytes, 56)?,
            array(bytes, 88)?,
            u16::from_le_bytes(array(bytes, FEE_BPS)?),
        )
    }
    /// Encode one canonical local policy record.
    pub fn encode(&self, output: &mut [u8]) -> Result<()> {
        if output.len() != VENUE_FEE_POLICY_BYTES {
            return Err(Error::OutputLength);
        }
        output.fill(0);
        put(output, 0, &VENUE_FEE_POLICY_MAGIC);
        put(output, 8, &VENUE_FEE_POLICY_SCHEMA_VERSION.to_le_bytes());
        put(output, FEE_BPS, &self.bps.to_le_bytes());
        put(output, FEE_RECIPIENT, &self.market);
        put(output, 48, &self.generation.to_le_bytes());
        put(output, 56, &self.config);
        put(output, 88, &self.recipient);
        Ok(())
    }
...
pub fn settle_ordinary<const N: usize>(input: OrdinaryMatch<N>) -> Result<OrdinarySettlement<N>> {
    ...
    authorized(ask, input.seller_authorization)?;
    authorized(bid, input.buyer_authorization)?;
    ...
    if input.execution_price < ask.limit || input.execution_price > bid.limit {
    ...
    let fee = fee(gross, input.fee_policy)?;
    let total = gross.checked_add(fee).ok_or(Error::ArithmeticOverflow)?;
    ...
    Ok(OrdinarySettlement {
        ...
        buyer_gross_collateral_debit: gross,
        venue_fee_transfer: fee,
...
pub fn settle_merge<const N: usize>(
    input: ComplementarySellMatch<N>,
) -> Result<MergeSettlement<N>> {
    width(N)?;
    if input.fill == 0 {
        return Err(Error::ZeroQuantity);
    }
    ...
    let mut states = input.seller_states;
    let mut positions = input.seller_positions;
    let mut gross = [0_u64; N];
    let mut fees = [0_u64; N];
    let mut net = [0_u64; N];
    let mut price_sum = 0_u64;
    let mut fee_sum = 0_u64;
    let mut makers = [[0_u8; 32]; N];
    let mut maker_count = 0usize;
    for (index, ((((((intent, state), authorization), position), price), gross_slot), fee_slot)) in
        input
            .seller_intents
            .iter()
            .zip(states.iter_mut())
            .zip(input.seller_authorizations.iter())
            .zip(positions.iter_mut())
            .zip(input.execution_prices.iter())
            .zip(gross.iter_mut())
            .zip(fees.iter_mut())
            .enumerate()
    {
        ...
        if makers
            .iter()
            .take(maker_count)
            .any(|maker| maker == &intent.maker)
        {
            return Err(Error::Alias);
        }
        if let Some(slot) = makers.get_mut(maker_count) {
            *slot = intent.maker;
        } else {
        ...
        *net_slot = gross_slot
            .checked_sub(*fee_slot)
            .ok_or(Error::ArithmeticOverflow)?;
        position
            .debit_outcome(index, input.fill)
            .map_err(position_error)?;
        fee_sum = fee_sum
            .checked_add(*fee_slot)
            .ok_or(Error::ArithmeticOverflow)?;
    }
    if price_sum != PRICE_SCALE {
    ...
    if gross_sum != input.fill {
        return Err(Error::SplitFundingMismatch);
    }
    Ok(MergeSettlement {
        seller_positions: positions,
        seller_states: states,
...
pub fn settle_split<const N: usize>(input: ComplementaryBuyMatch<N>) -> Result<SplitSettlement<N>> {
    ...
    let mut states = input.buyer_states;
    let mut gross = [0; N];
    let mut fees = [0; N];
    let mut price_sum = 0_u64;
    let mut fee_sum = 0_u64;
    let mut makers = [[0_u8; 32]; N];
    let mut maker_count = 0usize;
    for (index, (((((intent, state), authorization), position), price), gross_slot)) in input
        .buyer_intents
        .iter()
        .zip(states.iter_mut())
        .zip(input.buyer_authorizations.iter())
        .zip(positions.iter_mut())
        .zip(input.execution_prices.iter())
        .zip(gross.iter_mut())
        .enumerate()
...
fn authorized(intent: DirectIntentV1, auth: OwnerAuthorization) -> Result<()> {
    if auth.owner != intent.maker {
        Err(Error::OwnerUnauthorized)
    } else {
        Ok(())
    }
}
fn venue_authorized(
    intent: DirectIntentV1,
    policy: VenueFeePolicyV1,
    authorization: MarketVenueAuthorization,
) -> Result<()> {
    nonzero(&authorization.market)?;
    nonzero(&authorization.fee_config)?;
    if policy.market != authorization.market
        || policy.generation != authorization.generation
        || policy.config != authorization.fee_config
        || intent.market != authorization.market
        || intent.generation != authorization.generation
        || intent.fee_config != authorization.fee_config
        || intent.fee_bps != policy.bps
    {
...
fn width(value: usize) -> Result<()> {
    if (MIN_OUTCOMES..=MAX_OUTCOMES).contains(&value) {
        Ok(())
    } else {
        Err(Error::InvalidOutcomeWidth)
    }
}
fn quote(quantity: u64, price: u64) -> Result<u64> {
    let product = quantity
        .checked_mul(price)
        .ok_or(Error::ArithmeticOverflow)?;
    if product % PRICE_SCALE != 0 {
        Err(Error::NonIntegralQuote)
    } else {
        Ok(product / PRICE_SCALE)
    }
}
fn fee(gross: u64, policy: VenueFeePolicyV1) -> Result<u64> {
    gross
        .checked_mul(u64::from(policy.bps))
        .ok_or(Error::ArithmeticOverflow)
        .map(|value| value / FEE_BASIS_POINTS_DENOMINATOR)
}
fn position_error(value: dclutch_realm_contract::Error) -> Error {
    match value {
...
fn zeros(bytes: &[u8], offset: usize, len: usize) -> Result<()> {
...
fn put(out: &mut [u8], offset: usize, value: &[u8]) {
    if let Some(dest) = out.get_mut(offset..offset.saturating_add(value.len())) {
...
mod tests {
    ...
    fn auth(v: u8) -> OwnerAuthorization {
        OwnerAuthorization { owner: key(v) }
    }
    fn policy(bps: u16) -> Result<VenueFeePolicyV1> {
        VenueFeePolicyV1::new(key(7), 3, key(8), key(99), bps)
    }
    fn venue() -> MarketVenueAuthorization {
        MarketVenueAuthorization {
            market: key(7),
            generation: 3,
            fee_config: key(8),
        }
    }
    fn order(maker: u8, side: Side, outcome: u8, limit: u64, nonce: u64) -> Result<DirectIntentV1> {
        order_fee(maker, side, outcome, limit, nonce, 0)
    }
    fn order_fee(
        maker: u8,
        side: Side,
        ...
        nonce: u64,
        fee_basis_points: u16,
    ) -> Result<DirectIntentV1> {
        DirectIntentV1::new(DirectIntentInput {
            market: key(7),
            generation: 3,
            maker: key(maker),
            nonce,
    ...
    fn position<const N: usize>(owner: u8, balances: [u64; N]) -> Result<PositionV1<N>> {
        PositionV1::new(key(7), key(owner), 3, balances).map_err(position_error)
    }

    #[test]
    fn canonical_preimage_binds_all_signed_facts() -> Result<()> {
        let value = order(1, Side::Buy, 1, 600_000, 4)?;
        let bytes = value.signed_preimage();
        assert_eq!(DirectIntentV1::decode_signed_preimage(&bytes)?, value);
        let mut hostile = bytes;
        hostile[12] = 1;
        assert_eq!(
            DirectIntentV1::decode_signed_preimage(&hostile),
            Err(Error::NonCanonicalReservedBytes)
        );
        hostile = bytes;
        hostile[LIMIT..LIMIT + 8].copy_from_slice(&(PRICE_SCALE + 1).to_le_bytes());
        assert_eq!(
            DirectIntentV1::decode_signed_preimage(&hostile),
            Err(Error::InvalidLimitPrice)
        );
        Ok(())
    }
    #[test]
    fn ordinary_partial_transfer_is_conservative() -> Result<()> {
        let ask = order(1, Side::Sell, 0, 400_000, 1)?;
        let bid = order(2, Side::Buy, 0, 600_000, 2)?;
        let out = settle_ordinary(OrdinaryMatch {
            slot: 12,
            seller_intent: ask,
            buyer_intent: bid,
            seller_state: IntentStateV1::open(ask),
            buyer_state: IntentStateV1::open(bid),
            seller_authorization: auth(1),
            buyer_authorization: auth(2),
            seller_position: position(1, [7, 0])?,
            buyer_position: position(2, [0, 0])?,
            fill: 5,
            execution_price: 600_000,
            fee_policy: policy(0)?,
            venue_authorization: venue(),
        })?;
        assert_eq!(out.seller_position.balances(), &[2, 0]);
        assert_eq!(out.buyer_position.balances(), &[5, 0]);
        assert_eq!(out.seller_collateral_credit, 3);
        assert_eq!(out.venue_fee_transfer, 0);
        assert_eq!(out.seller_state.filled(), 5);
        Ok(())
    }
    #[test]
    fn ordinary_refuses_expiry_replay_alias_bad_auth_and_fractional_quote() -> Result<()> {
        ...
        let base = OrdinaryMatch {
            ...
            seller_authorization: auth(1),
            buyer_authorization: auth(2),
            seller_position: position(1, [10, 0])?,
            buyer_position: position(2, [0, 0])?,
            fill: 1,
        ...
        expired.execution_price = 600_000;
        expired.slot = 21;
        assert_eq!(settle_ordinary(expired), Err(Error::IntentExpired));
        let mut bad_auth = base;
        bad_auth.execution_price = 600_000;
        bad_auth.buyer_authorization = auth(3);
        assert_eq!(settle_ordinary(bad_auth), Err(Error::OwnerUnauthorized));
        let mut replay = base;
        replay.execution_price = 600_000;
        replay.seller_state = IntentStateV1 {
            filled: 11,
            ..IntentStateV1::open(ask)
        };
        assert_eq!(settle_ordinary(replay), Err(Error::StateOverfilled));
        let self_bid = order(1, Side::Buy, 0, PRICE_SCALE, 3)?;
        let alias = OrdinaryMatch {
            buyer_intent: self_bid,
            buyer_state: IntentStateV1::open(self_bid),
            buyer_authorization: auth(1),
            buyer_position: position(1, [0, 0])?,
            ..base
        };
        assert_eq!(settle_ordinary(alias), Err(Error::Alias));
        Ok(())
    }
    #[test]
    fn cancellation_is_canonical_and_terminal() -> Result<()> {
        let bid = order(1, Side::Buy, 0, PRICE_SCALE, 1)?;
        let cancelled = IntentStateV1::open(bid).cancel(bid, auth(1))?;
        let mut bytes = [0; INTENT_STATE_BYTES];
        cancelled.encode(&mut bytes)?;
        assert_eq!(IntentStateV1::decode(&bytes)?, cancelled);
        let ask = order(2, Side::Sell, 0, PRICE_SCALE, 2)?;
        assert_eq!(
            settle_ordinary(OrdinaryMatch {
                slot: 12,
                seller_intent: ask,
                buyer_intent: bid,
                seller_state: IntentStateV1::open(ask),
                buyer_state: cancelled,
                seller_authorization: auth(2),
                buyer_authorization: auth(1),
                seller_position: position(2, [1, 0])?,
                buyer_position: position(1, [0, 0])?,
                fill: 1,
            ...
            Err(Error::IntentCancelled)
        );
        Ok(())
    }
    #[test]
    fn canonical_complementary_buys_fund_exact_complete_set() -> Result<()> {
        let a = order(1, Side::Buy, 0, 500_000, 1)?;
        let b = order(2, Side::Buy, 1, 500_000, 2)?;
        let out = settle_split(ComplementaryBuyMatch {
            slot: 12,
            buyer_intents: [a, b],
            buyer_states: [IntentStateV1::open(a), IntentStateV1::open(b)],
            buyer_authorizations: [auth(1), auth(2)],
            buyer_positions: [position(1, [0, 0])?, position(2, [0, 0])?],
            fill: 10,
            execution_prices: [500_000, 500_000],
            fee_policy: policy(0)?,
            venue_authorization: venue(),
        })?;
        assert_eq!(out.buyer_positions[0].balances(), &[10, 0]);
        assert_eq!(out.buyer_positions[1].balances(), &[0, 10]);
        assert_eq!(out.buyer_gross_collateral_debits, [5, 5]);
        assert_eq!(out.market_vault_collateral_credit, 10);
        Ok(())
    }
    #[test]
    fn canonical_complementary_sells_merge_and_release_only_vault_collateral() -> Result<()> {
        let a = order_fee(1, Side::Sell, 0, PRICE_SCALE, 1, 1_000)?;
        let b = order_fee(2, Side::Sell, 1, 0, 2, 1_000)?;
        let out = settle_merge(ComplementarySellMatch {
            slot: 12,
            seller_intents: [a, b],
            seller_states: [IntentStateV1::open(a), IntentStateV1::open(b)],
            seller_authorizations: [auth(1), auth(2)],
            seller_positions: [position(1, [10, 0])?, position(2, [0, 10])?],
            fill: 10,
            execution_prices: [PRICE_SCALE, 0],
            fee_policy: policy(1_000)?,
            venue_authorization: venue(),
        })?;
        assert_eq!(out.seller_positions[0].balances(), &[0, 0]);
        assert_eq!(out.seller_positions[1].balances(), &[0, 0]);
        assert_eq!(out.seller_gross_collateral_credits, [10, 0]);
        assert_eq!(out.seller_fee_debits, [1, 0]);
        assert_eq!(out.seller_net_collateral_credits, [9, 0]);
        assert_eq!(out.market_vault_collateral_debit, 10);
        assert_eq!(out.venue_fee_transfer, 1);
        Ok(())
    }
    #[test]
    fn merge_refuses_aliased_seller_or_underfunded_outcome_position() -> Result<()> {
        let a = order(1, Side::Sell, 0, 500_000, 1)?;
        let b = order(1, Side::Sell, 1, 500_000, 2)?;
        let base = ComplementarySellMatch {
            slot: 12,
            seller_intents: [a, b],
            seller_states: [IntentStateV1::open(a), IntentStateV1::open(b)],
            seller_authorizations: [auth(1), auth(1)],
            seller_positions: [position(1, [10, 0])?, position(1, [0, 10])?],
            fill: 10,
            execution_prices: [500_000, 500_000],
            fee_policy: policy(0)?,
            venue_authorization: venue(),
        ...
        let insufficient = ComplementarySellMatch {
            seller_intents: [a, c],
            seller_states: [IntentStateV1::open(a), IntentStateV1::open(c)],
            seller_authorizations: [auth(1), auth(2)],
            seller_positions: [position(1, [10, 0])?, position(2, [0, 9])?],
            ..base
        };
        assert_eq!(
            settle_merge(insufficient),
            Err(Error::InsufficientPositionBalance)
        );
        Ok(())
    }
    #[test]
    fn venue_config_must_be_signed_and_adapter_authenticated() -> Result<()> {
        let ask = order(1, Side::Sell, 0, PRICE_SCALE, 1)?;
        let bid = order(2, Side::Buy, 0, PRICE_SCALE, 2)?;
        let mut bad_venue = venue();
        bad_venue.fee_config = key(9);
        assert_eq!(
            settle_ordinary(OrdinaryMatch {
                slot: 12,
                seller_intent: ask,
                ...
                seller_authorization: auth(1),
                buyer_authorization: auth(2),
                seller_position: position(1, [1, 0])?,
                buyer_position: position(2, [0, 0])?,
                fill: 1,
                execution_price: PRICE_SCALE,
                fee_policy: policy(0)?,
                venue_authorization: bad_venue
            }),
    ...
    fn complement_refuses_alias_missing_outcome_and_nonintegral_funding() -> Result<()> {
        let a = order(1, Side::Buy, 0, PRICE_SCALE, 1)?;
        let b = order(1, Side::Buy, 1, PRICE_SCALE, 2)?;
        let base = ComplementaryBuyMatch {
            slot: 12,
            buyer_intents: [a, b],
            buyer_states: [IntentStateV1::open(a), IntentStateV1::open(b)],
            buyer_authorizations: [auth(1), auth(1)],
            buyer_positions: [position(1, [0, 0])?, position(1, [0, 0])?],
            fill: 10,
            execution_prices: [500_000, 500_000],
            fee_policy: policy(0)?,
            venue_authorization: venue(),
        ...
        let c = order(2, Side::Buy, 0, PRICE_SCALE, 3)?;
        let missing = ComplementaryBuyMatch {
            buyer_intents: [a, c],
            buyer_states: [IntentStateV1::open(a), IntentStateV1::open(c)],
            buyer_authorizations: [auth(1), auth(2)],
            buyer_positions: [position(1, [0, 0])?, position(2, [0, 0])?],
            ..base
        };
        assert_eq!(settle_split(missing), Err(Error::NonCanonicalComplement));
        let b2 = order(2, Side::Buy, 1, PRICE_SCALE, 2)?;
        let fractional = ComplementaryBuyMatch {
            buyer_intents: [a, b2],
            buyer_states: [IntentStateV1::open(a), IntentStateV1::open(b2)],
            buyer_authorizations: [auth(1), auth(2)],
            buyer_positions: [position(1, [0, 0])?, position(2, [0, 0])?],
            fill: 1,
            execution_prices: [400_000, 600_000],
            ..base
        };
        assert_eq!(settle_split(fractional), Err(Error::NonIntegralQuote));
        Ok(())
    }
}
