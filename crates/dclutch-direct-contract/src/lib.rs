...
pub const DIRECT_INTENT_BYTES: usize = 128;
...
pub const VENUE_FEE_POLICY_BYTES: usize = 48;
...
impl DirectIntentV1 {
    ...
    pub fn new(input: DirectIntentInput) -> Result<Self> {
        ...
        Ok(Self { market: input.market, generation: input.generation, maker: input.maker, nonce: input.nonce, from: input.valid_from_slot, through: input.valid_through_slot, side: input.side, outcome: input.outcome, max_fill: input.max_fill, limit: input.limit_price })
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct VenueFeePolicyV1 { recipient: [u8; 32], bps: u16 }
impl VenueFeePolicyV1 {
    /// Validate one local policy.
    pub fn new(recipient: [u8; 32], fee_basis_points: u16) -> Result<Self> { nonzero(&recipient)?; if u64::from(fee_basis_points) > FEE_BASIS_POINTS_DENOMINATOR { return Err(Error::InvalidFeeRate); } Ok(Self { recipient, bps: fee_basis_points }) }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        zeros(bytes, FEE_RESERVED, 4)?; Self::new(array(bytes, FEE_RECIPIENT)?, u16::from_le_bytes(array(bytes, FEE_BPS)?))
    }
    /// Encode one canonical local policy record.
    pub fn encode(&self, output: &mut [u8]) -> Result<()> {
        if output.len() != VENUE_FEE_POLICY_BYTES { return Err(Error::OutputLength); }
        output.fill(0); put(output, 0, &VENUE_FEE_POLICY_MAGIC); put(output, 8, &VENUE_FEE_POLICY_SCHEMA_VERSION.to_le_bytes()); put(output, FEE_BPS, &self.bps.to_le_bytes()); put(output, FEE_RECIPIENT, &self.recipient); Ok(())
    }
...
mod tests {
    ...
    fn policy(bps: u16) -> Result<VenueFeePolicyV1> { VenueFeePolicyV1::new(key(99), bps) }
    fn order(maker: u8, side: Side, outcome: u8, limit: u64, nonce: u64) -> Result<DirectIntentV1> { DirectIntentV1::new(DirectIntentInput { market: key(7), generation: 3, maker: key(maker), nonce, valid_from_slot: 10, valid_through_slot: 20, side, outcome, max_fill: 10, limit_price: limit }) }
    ...
    fn canonical_preimage_binds_all_signed_facts() -> Result<()> {
        ...
        hostile = bytes; hostile[LIMIT..].copy_from_slice(&(PRICE_SCALE + 1).to_le_bytes());
    ...
    fn ordinary_partial_transfer_is_conservative() -> Result<()> {
        ...
        let out = settle_ordinary(OrdinaryMatch { slot: 12, seller_intent: ask, buyer_intent: bid, seller_state: IntentStateV1::open(ask), buyer_state: IntentStateV1::open(bid), seller_authorization: auth(1), buyer_authorization: auth(2), seller_position: position(1, [7, 0])?, buyer_position: position(2, [0, 0])?, fill: 5, execution_price: 600_000, fee_policy: policy(1_000)? })?;
    ...
    fn ordinary_refuses_expiry_replay_alias_bad_auth_and_fractional_quote() -> Result<()> {
        let ask = order(1, Side::Sell, 0, 400_000, 1)?; let bid = order(2, Side::Buy, 0, 600_000, 2)?;
        let base = OrdinaryMatch { slot: 12, seller_intent: ask, buyer_intent: bid, seller_state: IntentStateV1::open(ask), buyer_state: IntentStateV1::open(bid), seller_authorization: auth(1), buyer_authorization: auth(2), seller_position: position(1, [10, 0])?, buyer_position: position(2, [0, 0])?, fill: 1, execution_price: 500_000, fee_policy: policy(0)? };
    ...
    fn cancellation_is_canonical_and_terminal() -> Result<()> {
        ...
        assert_eq!(settle_ordinary(OrdinaryMatch { slot: 12, seller_intent: ask, buyer_intent: bid, seller_state: IntentStateV1::open(ask), buyer_state: cancelled, seller_authorization: auth(2), buyer_authorization: auth(1), seller_position: position(2, [1, 0])?, buyer_position: position(1, [0, 0])?, fill: 1, execution_price: PRICE_SCALE, fee_policy: policy(0)? }), Err(Error::IntentCancelled)); Ok(())
    }
    #[test]
    fn canonical_complementary_buys_fund_exact_complete_set() -> Result<()> {
        ...
        let out = settle_split(ComplementaryBuyMatch { slot: 12, buyer_intents: [a, b], buyer_states: [IntentStateV1::open(a), IntentStateV1::open(b)], buyer_authorizations: [auth(1), auth(2)], buyer_positions: [position(1, [0, 0])?, position(2, [0, 0])?], fill: 10, execution_prices: [500_000, 500_000], fee_policy: policy(1_000)? })?;
    ...
    fn complement_refuses_alias_missing_outcome_and_nonintegral_funding() -> Result<()> {
        ...
        let base = ComplementaryBuyMatch { slot: 12, buyer_intents: [a, b], buyer_states: [IntentStateV1::open(a), IntentStateV1::open(b)], buyer_authorizations: [auth(1), auth(1)], buyer_positions: [position(1, [0, 0])?, position(1, [0, 0])?], fill: 10, execution_prices: [500_000, 500_000], fee_policy: policy(0)? };
