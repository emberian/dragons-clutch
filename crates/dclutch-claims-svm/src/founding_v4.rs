...
//! records, Core authority, PDA derivations, rent, and the already-executed
//! Custody source-to-Hoard effect before constructing this allocation-free
//! value. Custody owns token movement and replay; Claims owns liabilities.

/// Exact fixed founding request width.
pub const CLAIMS_FOUNDING_REQUEST_BYTES_V4: usize = 832;
/// Exact fixed founding receipt width.
pub const CLAIMS_FOUNDING_RECEIPT_BYTES_V4: usize = 1008;
/// Founding request wire magic.
pub const CLAIMS_FOUNDING_REQUEST_MAGIC_V4: [u8; 8] = *b"DCLFDR04";
/// Founding receipt wire magic.
pub const CLAIMS_FOUNDING_RECEIPT_MAGIC_V4: [u8; 8] = *b"DCLFDC04";
/// Implemented founding ABI version.
pub const CLAIMS_FOUNDING_WIRE_VERSION_V4: u16 = 4;
...
const CORE_PROGRAM_OFFSET: usize = 560;
...
const RECEIPT_REQUEST_DIGEST_OFFSET: usize =
    RECEIPT_REQUEST_OFFSET + CLAIMS_FOUNDING_REQUEST_BYTES_V4;
...
pub enum ClaimsFoundingErrorV4 {
...
pub type Result<T> = core::result::Result<T, ClaimsFoundingErrorV4>;

/// Construction input for one atomic Claims founding request.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClaimsFoundingRequestInputV4 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClaimsFoundingAggregateSeedsV4 {
    market: [u8; 32],
}

impl ClaimsFoundingAggregateSeedsV4 {
    ...
    pub fn as_slices(&self) -> [&[u8]; 2] {
        [CLAIMS_FOUNDING_AGGREGATE_SEED_V4, &self.market]
    }
}

/// One fully validated atomic Claims founding request.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClaimsFoundingRequestV4(ClaimsFoundingRequestInputV4);

impl ClaimsFoundingRequestV4 {
    /// Construct and fully validate one request.
    pub fn new(input: ClaimsFoundingRequestInputV4) -> Result<Self> {
        for identity in [
            input.release_set,
            input.market,
        ...
        if input.claim_count == 0 {
            return Err(ClaimsFoundingErrorV4::InvalidClaimCount);
        }
        if input.quantity == 0 || input.basis_scale == 0 {
            return Err(ClaimsFoundingErrorV4::InvalidQuantity);
        }
        let collateral = input
            .quantity
            .checked_mul(input.basis_scale)
            .ok_or(ClaimsFoundingErrorV4::InvalidCollateralTransfer)?;
        if collateral == 0
            || input
                .pre_source_amount
        ...
        {
            return Err(ClaimsFoundingErrorV4::InvalidCollateralTransfer);
        }
        ...
        {
            return Err(ClaimsFoundingErrorV4::InvalidRent);
        }
        ...
        {
            return Err(ClaimsFoundingErrorV4::InvalidRevision);
        }
        Ok(Self(input))
    }
    ...
    pub fn decode(input: &[u8]) -> Result<Self> {
        if input.len() != CLAIMS_FOUNDING_REQUEST_BYTES_V4 {
            return Err(ClaimsFoundingErrorV4::InvalidLength);
        }
        if array_at::<8>(input, 0)? != CLAIMS_FOUNDING_REQUEST_MAGIC_V4 {
            return Err(ClaimsFoundingErrorV4::InvalidMagic);
        }
        if u16_at(input, VERSION_OFFSET)? != CLAIMS_FOUNDING_WIRE_VERSION_V4 {
            return Err(ClaimsFoundingErrorV4::UnsupportedVersion);
        }
        if byte_at(input, OWNER_KIND_OFFSET)? != CLAIMS_FOUNDING_USER_OWNER_KIND_V4 {
            return Err(ClaimsFoundingErrorV4::NonCanonical);
        }
        ...
        Self::new(ClaimsFoundingRequestInputV4 {
    ...
    pub fn to_bytes(&self) -> [u8; CLAIMS_FOUNDING_REQUEST_BYTES_V4] {
        ...
        put_infallible(
            &mut output,
            VERSION_OFFSET,
            &CLAIMS_FOUNDING_WIRE_VERSION_V4.to_le_bytes(),
        );
        put_infallible(
            &mut output,
            OWNER_KIND_OFFSET,
            &[CLAIMS_FOUNDING_USER_OWNER_KIND_V4],
        );
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClaimsFoundingReceiptV4 {
...
impl ClaimsFoundingReceiptV4 {
    ...
    pub fn decode(input: &[u8]) -> Result<Self> {
        if input.len() != CLAIMS_FOUNDING_RECEIPT_BYTES_V4 {
            return Err(ClaimsFoundingErrorV4::InvalidLength);
        }
        if array_at::<8>(input, 0)? != CLAIMS_FOUNDING_RECEIPT_MAGIC_V4 {
            return Err(ClaimsFoundingErrorV4::InvalidMagic);
        }
        if u16_at(input, VERSION_OFFSET)? != CLAIMS_FOUNDING_WIRE_VERSION_V4 {
            return Err(ClaimsFoundingErrorV4::UnsupportedVersion);
        ...
        Self::new(
            ClaimsFoundingRequestV4::decode(subslice(
                input,
                RECEIPT_REQUEST_OFFSET,
                CLAIMS_FOUNDING_REQUEST_BYTES_V4,
            )?)?,
...
fn require_next_revision(before: u64, after: u64) -> Result<()> {
    if before == u64::MAX || before.checked_add(1) != Some(after) {
        Err(ClaimsFoundingErrorV4::InvalidRevision)
    } else {
        Ok(())
    }
}
...
fn require_distinct_accounts(accounts: &[[u8; 32]]) -> Result<()> {
    ...
    while outer < accounts.len() {
        let current = accounts
            .get(outer)
            .ok_or(ClaimsFoundingErrorV4::InvalidLength)?;
        let mut inner = outer
            .checked_add(1)
            .ok_or(ClaimsFoundingErrorV4::InvalidLength)?;
        while inner < accounts.len() {
            if current
                == accounts
                    ...
                    .ok_or(ClaimsFoundingErrorV4::InvalidLength)?
            {
                return Err(ClaimsFoundingErrorV4::AccountAlias);
            }
            inner = inner
                .checked_add(1)
                .ok_or(ClaimsFoundingErrorV4::InvalidLength)?;
...
fn require_nonzero(value: [u8; 32]) -> Result<()> {
    if value.iter().all(|byte| *byte == 0) {
        Err(ClaimsFoundingErrorV4::ZeroIdentity)
    } else {
        Ok(())
    }
}
...
fn require_zero(input: &[u8], offset: usize, width: usize) -> Result<()> {
        ...
        .all(|byte| *byte == 0)
    {
        ...
        Err(ClaimsFoundingErrorV4::NonCanonical)
    }
}
...
fn array_at<const N: usize>(input: &[u8], offset: usize) -> Result<[u8; N]> {
    subslice(input, offset, N)?
        .try_into()
        .map_err(|_| ClaimsFoundingErrorV4::InvalidLength)
}
...
mod tests {
    ...
    fn input() -> ClaimsFoundingRequestInputV4 {
        ClaimsFoundingRequestInputV4 {
            release_set: id(1),
            market: id(2),
    ...
    fn request() -> ClaimsFoundingRequestV4 {
        ClaimsFoundingRequestV4::new(input()).expect("valid founding")
    }
    ...
    fn exact_request_and_receipt_roundtrip_bind_atomic_founding() {
        let request = request();
        assert_eq!(
            ClaimsFoundingRequestV4::decode(&request.to_bytes()),
            Ok(request)
        );
        ...
        let receipt = ClaimsFoundingReceiptV4::new(request, id(40), id(41), id(42), id(43), id(44))
            .expect("receipt");
        assert_eq!(
        ...
        let seeds = ClaimsFoundingAggregateSeedsV4::new(id(2)).expect("seeds");
        assert_eq!(
            seeds.as_slices(),
            [CLAIMS_FOUNDING_AGGREGATE_SEED_V4, id(2).as_slice()]
        );
    }
    ...
    fn hostile_header_identity_alias_and_width_refuse() {
        ...
        assert_eq!(
            ClaimsFoundingRequestV4::decode(bytes.get(..831).expect("slice")),
            Err(ClaimsFoundingErrorV4::InvalidLength)
        );
        for (offset, expected) in [
            (0, ClaimsFoundingErrorV4::InvalidMagic),
            (VERSION_OFFSET, ClaimsFoundingErrorV4::UnsupportedVersion),
            (OWNER_KIND_OFFSET, ClaimsFoundingErrorV4::NonCanonical),
            (HEADER_RESERVED_OFFSET, ClaimsFoundingErrorV4::NonCanonical),
            (BODY_RESERVED_OFFSET, ClaimsFoundingErrorV4::NonCanonical),
            (TAIL_RESERVED_OFFSET, ClaimsFoundingErrorV4::NonCanonical),
        ] {
            let mut hostile = bytes;
            mutate(&mut hostile, offset, 99);
            assert_eq!(ClaimsFoundingRequestV4::decode(&hostile), Err(expected));
        }
        assert_eq!(
            ClaimsFoundingRequestV4::new(ClaimsFoundingRequestInputV4 {
        ...
        assert_eq!(
            ...
            Err(ClaimsFoundingErrorV4::AccountAlias)
        );
    }
    ...
    fn quantity_width_and_exact_source_to_hoard_transfer_refuse() {
        for hostile in [
            ...
            assert!(ClaimsFoundingRequestV4::new(hostile).is_err());
        }
        for hostile in [
            ClaimsFoundingRequestInputV4 {
            ...
            assert_eq!(
                ClaimsFoundingRequestV4::new(hostile),
                Err(ClaimsFoundingErrorV4::InvalidCollateralTransfer)
            );
        }
    }
    ...
    fn hostile_rent_revision_and_receipt_evidence_refuse() {
        assert_eq!(
            ...
            Err(ClaimsFoundingErrorV4::InvalidRent)
        );
        for hostile in [
            ClaimsFoundingRequestInputV4 {
        ...
        let request = request();
        assert_eq!(
            ClaimsFoundingReceiptV4::new(request, [0; 32], id(21), id(22), id(23), id(24),),
            Err(ClaimsFoundingErrorV4::ZeroIdentity)
        );
        ...
        assert_eq!(
            ClaimsFoundingReceiptV4::decode(&bytes),
            Err(ClaimsFoundingErrorV4::NonCanonical)
        );
    }
}
