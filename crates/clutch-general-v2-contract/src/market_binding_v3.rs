...
const PRODUCT_PREAUTHORIZATION_ID_OFFSET: usize = ATTACHMENT_PLAN_ID_OFFSET + 32;
...
impl MarketBindingV3 {
    ...
    /// Encode exact tag `0x79`, version `3`, and 824 canonical bytes.
...
const _: () = assert!(MARKET_BINDING_ACCOUNT_BYTES_V3 == 824);
...
mod tests {
    ...
    fn hostile_version_alias_and_product_identity_alias_refuse() {
        ...
        assert_eq!(MarketBindingV3::decode(&bytes[..823]), Err(CodecError::WrongLength));
        assert_eq!(
            MarketBindingV3::new(
                *value.base(),
