//! Shared Market lifecycle and per-Series admission links.
//!
//! `MarketLifecycleRootV1` is keyed only by the full economic Market identity.
//! It never embeds a founding Series or ordinal. `SeriesMarketLinkV1` is the
//! replay-safe `(SeriesPlanV5, ordinal)` owner which binds one compiled Source
//! occurrence and Series-scoped attachments to that shared Market. A Market is
//! inert while Founding; no trading liability may open until every bounded
//! shared slot has an authenticated zero-state receipt and the prepaid
//! FoundationVault principal is exhausted exactly.
...
/// Complete mutable shared Market root domain.
pub const MARKET_LIFECYCLE_ROOT_DOMAIN_V1: &[u8] = b"dragons-clutch/market-lifecycle-root/v1";
/// Per-Series Market link domain.
pub const SERIES_MARKET_LINK_DOMAIN_V1: &[u8] = b"dragons-clutch/series-market-link/v1";
/// Once-only Product/Failure Resolution V5 activation domain.
pub const MARKET_RESOLUTION_ACTIVATION_DOMAIN_V1: &[u8] =
    b"dragons-clutch/market-resolution-activation/v1";
/// Whole-Market terminal projection domain.
...
pub enum MarketFoundationSlotV2 {
    ...
    /// Distinct market-scoped dynamic Failure runtime root (a0/v3).
    FailureRuntimeRoot,
...
/// is nonzero and pairwise distinct, so the immutable Failure admission root,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum MarketLifecyclePhaseV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketLifecycleBindingV1 {
...
impl MarketLifecycleBindingV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        Ok(content_id(MARKET_LIFECYCLE_BINDING_DOMAIN_V1, &body))
    }

    fn identity_ids(self) -> [ContentId; MARKET_BINDING_ID_COUNT_V1] {
        [
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundationCapitalV1 {
    /// Exact founding `0xad` link.
    pub founder_link_id: SeriesMarketLinkV1Id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundationProgressV1 {
...
impl MarketFoundationProgressV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesLinkObligationV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesLinkObligationDispositionV1 {
...
impl SeriesLinkObligationDispositionV1 {
    const fn byte(self) -> u8 {
        match self {
            Self::Terminal => 1,
            Self::Absent => 2,
...
impl SeriesMarketLinkBindingV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        Ok(content_id(SERIES_MARKET_LINK_DOMAIN_V1, &body))
    }

    fn identity_ids(self) -> [ContentId; LINK_BINDING_ID_COUNT_V1] {
        [
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesMarketLinkPhaseV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLinkObligationTerminalProjectionV1 {
    ...
    pub disposition: SeriesLinkObligationDispositionV1,
...
impl SeriesLinkObligationTerminalProjectionV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        Ok(content_id(
            b"dragons-clutch/series-link-obligation-terminal/v1",
            &body,
        ))
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketLinkV1 {
    binding: SeriesMarketLinkBindingV1,
    phase: SeriesMarketLinkPhaseV1,
...
impl SeriesMarketLinkV1 {
    ...
    pub fn activate(
        ...
        if self.phase != SeriesMarketLinkPhaseV1::PendingMarket || market_admission_sequence == 0 {
            return Err(Error::WorkStateMismatch);
        }
        let next = Self {
            phase: SeriesMarketLinkPhaseV1::Active,
...
impl MarketLifecycleRootV1 {
    ...
    pub fn begin_abort_into(
        ...
        foundation.transcript_id = rolling_id(
            b"dragons-clutch/market-foundation-abort/v1",
    ...
    pub fn finalize_abort(self) -> Result<MarketFoundingAbortProjectionV1> {
        ...
        let id = content_id(b"dragons-clutch/market-founding-abort/v1", &body);
        Ok(MarketFoundingAbortProjectionV1 {
            id,
            market_instance_id: self.binding.market_instance_id,
            generation: self.binding.generation,
    ...
    fn validate(self) -> Result<()> {
        ...
        match self.phase {
            ...
            MarketLifecyclePhaseV1::Retiring => {
            ...
            MarketLifecyclePhaseV1::Aborting => {
                if !resolution_absent
                    || self.capital.principal_remaining_lamports != 0
                    || self.admitted_series_links != 1
                    || self.retired_series_links != 0
...
impl FixedCodec for MarketLifecycleRootV1 {
    const ENCODED_LEN: usize = MARKET_LIFECYCLE_ROOT_BYTES_V1;
...
impl MarketLifecycleRootV1 {
    ...
    pub fn decode_into(input: &[u8], output: &mut Self) -> Result<()> {
        ...
        let phase = MarketLifecyclePhaseV1::decode(reader.u8())?;
        reader.reserved(5)?;
        let ids = read_ids::<MARKET_BINDING_ID_COUNT_V1>(&mut reader);
        ...
        let capital = MarketFoundationCapitalV1 {
            founder_link_id: SeriesMarketLinkV1Id::from_bytes(reader.id().bytes()),
...
impl FixedCodec for SeriesMarketLinkV1 {
    const ENCODED_LEN: usize = SERIES_MARKET_LINK_BYTES_V1;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&LINK_MAGIC_V1);
        writer.u16(LINK_VERSION_V1);
        writer.u8(self.phase.byte());
        writer.reserved(5);
        for id in self.binding.identity_ids() {
            writer.id(id);
...
impl SeriesMarketLinkV1 {
    ...
    pub fn decode_into(input: &[u8], output: &mut Self) -> Result<()> {
        ...
        let phase = SeriesMarketLinkPhaseV1::decode(reader.u8())?;
        reader.reserved(5)?;
        let ids = read_ids::<LINK_BINDING_ID_COUNT_V1>(&mut reader);
...
fn binding_from_ids(
    ids: [ContentId; MARKET_BINDING_ID_COUNT_V1],
    generation: u64,
    outcome_count: u8,
...
fn link_binding_from_ids(
    ids: [ContentId; LINK_BINDING_ID_COUNT_V1],
    ordinal: u32,
...
impl SeriesMarketLinkV1 {
    ...
    pub fn pin_failure_session(self, failure_begin_receipt_id: ContentId) -> Result<Self> {
        ...
        let transcript = rolling_id(
            b"dragons-clutch/series-link-failure-session/v1",
    ...
    pub fn consume_obligation(
        self,
        projection: SeriesLinkObligationTerminalProjectionV1,
    ) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesMarketLinkPhaseV1::Active
            || projection.link_semantic_id != self.semantic_id()?
    ...
    pub fn retirement_projection(self) -> Result<SeriesMarketLinkRetirementProjectionV1> {
        self.validate()?;
        ...
        let id = content_id(b"dragons-clutch/series-market-link-retirement/v1", &body);
        Ok(SeriesMarketLinkRetirementProjectionV1 {
            id,
    ...
    pub fn mark_aborted(self, market_abort_receipt_id: ContentId) -> Result<Self> {
        ...
        if self.phase != SeriesMarketLinkPhaseV1::PendingMarket
            || self.market_admission_receipt_id != ContentId::ZERO
        {
            return Err(Error::WorkStateMismatch);
        }
        let next = Self {
            phase: SeriesMarketLinkPhaseV1::Aborted,
            transition_sequence: 1,
    ...
    pub const fn binding(self) -> SeriesMarketLinkBindingV1 {
        self.binding
    }
    /// Link phase.
    pub const fn phase(self) -> SeriesMarketLinkPhaseV1 {
        self.phase
    }
    ...
    pub fn semantic_id(self) -> Result<SeriesMarketLinkV1Id> {
        let mut body = [0u8; SERIES_MARKET_LINK_BYTES_V1];
        self.encode_into(&mut body)?;
        Ok(SeriesMarketLinkV1Id::from_bytes(
            content_id(SERIES_MARKET_LINK_DOMAIN_V1, &body).bytes(),
        ))
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketLinkRetirementProjectionV1 {
    id: ContentId,
...
impl SeriesMarketLinkRetirementProjectionV1 {
    /// Projection identity.
    pub const fn id(self) -> ContentId {
        self.id
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketAdmissionProjectionV1 {
    id: ContentId,
    market_binding_id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    link_semantic_id: SeriesMarketLinkV1Id,
...
impl SeriesMarketAdmissionProjectionV1 {
    /// Construct the deterministic root/link admission join.
    pub fn new(
        market_binding: MarketLifecycleBindingV1,
        link: SeriesMarketLinkV1,
        admission_sequence: u64,
    ) -> Result<Self> {
        ...
        let id = content_id(b"dragons-clutch/series-market-admission/v1", &body);
        Ok(Self {
            id,
            market_binding_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketResolutionActivationV1 {
    id: ContentId,
    market_binding_id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
...
impl MarketResolutionActivationV1 {
    ...
    pub fn new(
        ...
        let id = content_id(MARKET_RESOLUTION_ACTIVATION_DOMAIN_V1, &body);
        Ok(Self {
            id,
            market_binding_id,
            market_instance_id: binding.market_instance_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundingAbortProjectionV1 {
    id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
...
impl MarketFoundingAbortProjectionV1 {
    /// Abort receipt.
    pub const fn id(self) -> ContentId {
        self.id
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketLifecycleRootV1 {
    binding: MarketLifecycleBindingV1,
    phase: MarketLifecyclePhaseV1,
    transition_sequence: u64,
    capital: MarketFoundationCapitalV1,
    foundation: MarketFoundationProgressV1,
    admitted_series_links: u32,
    ...
    pub fn initialize_founder(
        ...
        let foundation = MarketFoundationProgressV1 {
            ...
            transcript_id: rolling_id(
                b"dragons-clutch/market-foundation-transcript/v1",
                ContentId::ZERO,
