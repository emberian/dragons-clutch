...
//!
//! This successor binds the exact 50-slot ScheduleV4/GraphV4 and the
//! persistent ProductReplayAnchor generation authority. Historical root/link
//! V1/V2 bytes remain decode-only and are never interpreted as these states.

use crate::codec::{Reader, Writer};
use crate::{
    content_id, AuthenticatedMarketFamilyAuthorityV1, CompiledProductSeriesBundleV7Id,
    ContentId, Error, FixedCodec,
    MarketFamilyAggregatorPhaseV1, MarketFamilyAggregatorV1, MarketFamilyV1,
    MarketFoundationAccountGraphV4, MarketFoundationAccountGraphV4Id,
    MarketFoundationScheduleV4, MarketFoundationScheduleV4Id, MarketFoundationSlotV4,
    MarketInstanceV2Id, MarketLifecycleGenerationBindingV2Id, Result,
    SeriesAttachmentPlanV6Id, SeriesFundingQuoteV6Id,
    SeriesFundingTermsV2Id,
    SeriesMarketDispositionV1, SeriesMarketLinkV3Id, SeriesPlanV5Id, SourceOccurrenceV1Id,
    MARKET_FAMILY_AGGREGATOR_BYTES_V1, MARKET_FOUNDATION_CORE_SLOT_COUNT_V4,
    MARKET_FOUNDATION_MAX_OUTCOMES_V4, MARKET_FOUNDATION_SLOT_COUNT_V4,
};

const ROOT_MAGIC_V3: [u8; 8] = *b"DCMKRTV3";
const ROOT_VERSION_V3: u16 = 3;
const LINK_MAGIC_V3: [u8; 8] = *b"DCSMLKV3";
const LINK_VERSION_V3: u16 = 3;
const MARKET_BINDING_ID_COUNT_V3: usize = 36;
const LINK_BINDING_ID_COUNT_V3: usize = 25;

/// Exact number of mandatory shared-core terminal owners outside the product-family aggregator.
pub const MARKET_SHARED_CORE_COUNT_V3: usize = 5;
/// Exact number of Series-link-scoped attachment obligations.
pub const SERIES_LINK_OBLIGATION_COUNT_V3: usize = 4;
/// Exact shared 0xaa/version3 semantic body width.
pub const MARKET_LIFECYCLE_ROOT_BYTES_V3: usize = 2_544;
/// Exact per-Series 0xad/version3 semantic body width.
pub const SERIES_MARKET_LINK_BYTES_V3: usize = 1_232;

const _: () = assert!(
    MARKET_LIFECYCLE_ROOT_BYTES_V3
        == 16
            + MARKET_BINDING_ID_COUNT_V3 * 32
            + 40
            + 5 * 32
            + 6 * 8
            + 64
            + 48
            + MARKET_FAMILY_AGGREGATOR_BYTES_V1
            + MARKET_SHARED_CORE_COUNT_V3 * 32
            + 2 * 32
            + 3 * 32
            + 8
);
const _: () = assert!(
    SERIES_MARKET_LINK_BYTES_V3
        == 16
            + LINK_BINDING_ID_COUNT_V3 * 32
            + 40
            + 16
            + 32
            + 3 * 8
            + SERIES_LINK_OBLIGATION_COUNT_V3
            + 4
            + 2 * SERIES_LINK_OBLIGATION_COUNT_V3 * 32
            + 8
            + 32
);

/// Current shared Market binding identity domain.
pub const MARKET_LIFECYCLE_BINDING_DOMAIN_V3: &[u8] = b"dragons-clutch/market-lifecycle-binding/v3";
/// Current mutable shared Market root domain.
pub const MARKET_LIFECYCLE_ROOT_DOMAIN_V3: &[u8] = b"dragons-clutch/market-lifecycle-root/v3";
/// Current per-Series Market link domain.
pub const SERIES_MARKET_LINK_DOMAIN_V3: &[u8] = b"dragons-clutch/series-market-link/v3";
/// Current once-only Product/Failure Resolution activation domain.
pub const MARKET_RESOLUTION_ACTIVATION_DOMAIN_V3: &[u8] =
    b"dragons-clutch/market-resolution-activation/v3";
/// Current whole-Market terminal projection domain.
pub const MARKET_INSTANCE_TERMINAL_PROJECTION_DOMAIN_V3: &[u8] =
    b"dragons-clutch/market-instance-terminal-projection/v3";

/// Shared Market lifecycle phase.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum MarketLifecyclePhaseV3 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketLifecycleBindingV3 {
    ...
    /// Canonical market-only persistent ProductReplayAnchor account.
    pub market_lifecycle_replay_account_id: ContentId,
    /// Immutable generation binding hostile-reconstructed from that replay.
    pub market_lifecycle_generation_binding_id: MarketLifecycleGenerationBindingV2Id,
...
impl MarketLifecycleBindingV3 {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        self.market_lifecycle_generation_binding_id.validate()?;
        if self.generation == 0
            || self.outcome_count == 0
            || usize::from(self.outcome_count) > MARKET_FOUNDATION_MAX_OUTCOMES_V4
    ...
    fn identity_ids(&self) -> [ContentId; MARKET_BINDING_ID_COUNT_V3] {
        [
            ...
            self.market_lifecycle_replay_account_id,
            self.market_lifecycle_generation_binding_id.content_id(),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundationCapitalV3 {
    /// Exact founding `0xad` link.
    pub founder_link_id: SeriesMarketLinkV3Id,
...
impl MarketFoundationCapitalV3 {
    fn validate(self, schedule: &MarketFoundationScheduleV4) -> Result<()> {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundationProgressV3 {
...
impl MarketFoundationProgressV3 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundationStepProjectionV4 {
    /// Exact Market binding.
    pub binding_id: ContentId,
    /// Exact next slot.
    pub slot: MarketFoundationSlotV4,
...
impl MarketFoundationStepProjectionV4 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        Ok(content_id(
            b"dragons-clutch/market-foundation-step/v4",
            &body,
        ))
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketSharedCoreTerminalProjectionV3 {
    id: ContentId,
    market_binding_id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
    owner: MarketSharedCoreV3,
...
impl MarketSharedCoreTerminalProjectionV3 {
    ...
    pub fn new(
        ...
        let id = content_id(b"dragons-clutch/market-shared-core-terminal/v3", &body);
        Ok(Self {
            id,
            market_binding_id,
    ...
    pub const fn owner(self) -> MarketSharedCoreV3 {
        self.owner
    }
    /// Exact owner account.
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesLinkObligationV3 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesLinkObligationDispositionV3 {
...
impl SeriesLinkObligationDispositionV3 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesLinkObligationStatusV3 {
...
impl SeriesLinkObligationStatusV3 {
    /// Stable exhaustive wire byte used by current private adapter receipts.
    pub const fn wire_byte(self) -> u8 {
        match self {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLinkObligationConfigurationV3 {
    /// Authenticated central capability profile.
    pub capability_profile_id: ContentId,
    /// Exact Series attachment plan.
    pub attachment_plan_id: ContentId,
    /// Exhaustive initial states in [`SeriesLinkObligationV3`] order.
    pub initial_statuses: [SeriesLinkObligationStatusV3; SERIES_LINK_OBLIGATION_COUNT_V3],
}
...
impl SeriesLinkObligationConfigurationV3 {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.capability_profile_id == self.attachment_plan_id
            || self.initial_statuses.iter().any(|status| {
                matches!(
                    status,
                    SeriesLinkObligationStatusV3::Live | SeriesLinkObligationStatusV3::Terminal
                )
            })
        {
            return Err(Error::InvalidParameter);
    ...
    pub fn id(self) -> Result<SeriesLinkObligationConfigurationV3Id> {
        self.validate()?;
        ...
        while index < SERIES_LINK_OBLIGATION_COUNT_V3 {
            body[64 + index] = self.initial_statuses[index].wire_byte();
            index += 1;
        }
        Ok(SeriesLinkObligationConfigurationV3Id(content_id(
            b"dragons-clutch/series-link-obligation-configuration/v3",
            &body,
        )))
    }
}
...
#[repr(transparent)]
pub struct SeriesLinkObligationConfigurationV3Id(ContentId);

impl SeriesLinkObligationConfigurationV3Id {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketLinkBindingV3 {
    /// Exact Series.
    ...
    /// Current six-compartment QuoteV6 with the exact 50-slot schedule.
    ...
    pub obligation_configuration_id: SeriesLinkObligationConfigurationV3Id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[repr(u8)]
pub enum SeriesMarketLinkPhaseV3 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLinkObligationTerminalProjectionV3 {
    /// Exact link state before consumption.
    pub link_semantic_id: SeriesMarketLinkV3Id,
    /// Exact obligation.
    pub obligation: SeriesLinkObligationV3,
    /// Terminal or authenticated absence.
    pub disposition: SeriesLinkObligationDispositionV3,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLinkObligationAdmissionProjectionV3 {
...
impl SeriesLinkObligationAdmissionProjectionV3 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        Ok(content_id(
            b"dragons-clutch/series-link-obligation-admission/v3",
            &body,
        ))
    }
}

impl SeriesLinkObligationTerminalProjectionV3 {
    /// Deterministic receipt identity.
    pub fn id(self) -> Result<ContentId> {
        self.link_semantic_id.validate()?;
        ...
        Ok(content_id(
            b"dragons-clutch/series-link-obligation-terminal/v3",
            &body,
        ))
    }
}

/// Mutable per-Series admission/link state stored under central tag `0xad/3`.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketLinkV3 {
    binding: SeriesMarketLinkBindingV3,
    phase: SeriesMarketLinkPhaseV3,
    ...
    obligation_statuses: [SeriesLinkObligationStatusV3; SERIES_LINK_OBLIGATION_COUNT_V3],
    admission_receipts: [ContentId; SERIES_LINK_OBLIGATION_COUNT_V3],
    terminal_receipts: [ContentId; SERIES_LINK_OBLIGATION_COUNT_V3],
    active_failure_sessions: u32,
    failure_sessions_started: u32,
    failure_session_transcript_id: ContentId,
}

impl SeriesMarketLinkV3 {
    ...
    pub fn decode_buffer() -> Self {
        Self {
            binding: link_binding_from_ids(
                [ContentId::ZERO; LINK_BINDING_ID_COUNT_V3],
                0,
            ...
            phase: SeriesMarketLinkPhaseV3::PendingMarket,
            ...
            obligation_statuses: [SeriesLinkObligationStatusV3::CapabilityDisabled;
                SERIES_LINK_OBLIGATION_COUNT_V3],
            admission_receipts: [ContentId::ZERO; SERIES_LINK_OBLIGATION_COUNT_V3],
            terminal_receipts: [ContentId::ZERO; SERIES_LINK_OBLIGATION_COUNT_V3],
    ...
    pub fn activate_into(
        ...
        if self.phase != SeriesMarketLinkPhaseV3::PendingMarket || market_admission_sequence == 0 {
            return Err(Error::WorkStateMismatch);
        }
        *output = Self {
            phase: SeriesMarketLinkPhaseV3::Active,
...
impl MarketLifecycleRootV3 {
    ...
    pub const fn decode_buffer() -> Self {
        Self {
            binding: MarketLifecycleBindingV3 {
                ...
                market_lifecycle_replay_account_id: ContentId::ZERO,
                market_lifecycle_generation_binding_id:
                    MarketLifecycleGenerationBindingV2Id::from_bytes([0; 32]),
                foundation_account_graph_id: MarketFoundationAccountGraphV4Id::from_bytes([0; 32]),
                foundation_schedule_id: MarketFoundationScheduleV4Id::from_bytes([0; 32]),
            ...
            phase: MarketLifecyclePhaseV3::Founding,
            transition_sequence: 0,
            capital: MarketFoundationCapitalV3 {
                founder_link_id: SeriesMarketLinkV3Id::from_bytes([0; 32]),
            ...
            foundation: MarketFoundationProgressV3 {
                expected_bitmap: 0,
            ...
            shared_core_terminal_receipts: [ContentId::ZERO; MARKET_SHARED_CORE_COUNT_V3],
    ...
    pub fn retire_series_link(
        self,
        retirement: SeriesMarketLinkRetirementProjectionV3,
    ) -> Result<Self> {
        let mut output = Self::decode_buffer();
    ...
    pub fn retire_series_link_into(
        ...
        *output = Self {
            ...
            series_link_transcript_id: rolling_id(
                b"dragons-clutch/market-series-link-transcript/v3",
    ...
    pub fn consume_shared_core_terminal(
        self,
        projection: MarketSharedCoreTerminalProjectionV3,
    ) -> Result<Self> {
        self.validate()?;
    ...
    pub fn terminal_projection(&self) -> Result<MarketInstanceTerminalProjectionV3> {
        self.validate()?;
        if self.phase != MarketLifecyclePhaseV3::Terminal {
            return Err(Error::WorkIncomplete);
        }
        ...
        let id = content_id(MARKET_INSTANCE_TERMINAL_PROJECTION_DOMAIN_V3, &body);
        Ok(MarketInstanceTerminalProjectionV3 {
            id,
            root_semantic_id,
    ...
    pub fn begin_abort_into(
        ...
        if self.phase != MarketLifecyclePhaseV3::Founding
        ...
        foundation.transcript_id = rolling_id(
            b"dragons-clutch/market-foundation-abort/v3",
            foundation.transcript_id,
            vault_refund_receipt_id,
            sequence,
        );
        *output = *self;
        output.phase = MarketLifecyclePhaseV3::Aborting;
    ...
    pub fn record_abort_close(
        ...
        if self.phase != MarketLifecyclePhaseV3::Aborting {
            return Err(Error::WorkStateMismatch);
        }
        let index = slot.index()?;
        if index == MarketFoundationSlotV4::LifecycleRoot.index()? {
            return Err(Error::InvalidParameter);
        }
        let remaining = self.foundation.initialized_bitmap
            & !self.foundation.abort_closed_bitmap
            & !slot_bit(MarketFoundationSlotV4::LifecycleRoot.index()?)?;
    ...
    pub fn finalize_abort(self) -> Result<MarketFoundingAbortProjectionV3> {
        self.validate()?;
        let root_bit = slot_bit(MarketFoundationSlotV4::LifecycleRoot.index()?)?;
        if self.phase != MarketLifecyclePhaseV3::Aborting
        ...
        let id = content_id(b"dragons-clutch/market-founding-abort/v3", &body);
        Ok(MarketFoundingAbortProjectionV3 {
            id,
            market_instance_id: self.binding.market_instance_id,
            generation: self.binding.generation,
    ...
    pub const fn binding(&self) -> MarketLifecycleBindingV3 {
        self.binding
    }
    /// Borrow the immutable Market binding without copying its full ID set.
    pub const fn binding_ref(&self) -> &MarketLifecycleBindingV3 {
        &self.binding
    }
    /// Lifecycle phase.
    pub const fn phase(&self) -> MarketLifecyclePhaseV3 {
        self.phase
    }
    /// Transition sequence.
    ...
    pub const fn capital(&self) -> MarketFoundationCapitalV3 {
        self.capital
    }
    /// Bounded founding progress.
    pub const fn foundation(&self) -> MarketFoundationProgressV3 {
        self.foundation
    }
    ...
    /// Rolling transcript of every admitted and retired Series link.
    ...
    /// Mandatory shared-core terminal receipts in [`MarketSharedCoreV3`] order.
    pub const fn shared_core_terminal_receipts(&self) -> [ContentId; MARKET_SHARED_CORE_COUNT_V3] {
        self.shared_core_terminal_receipts
    }
    /// Exact terminal receipt consumed for one mandatory shared-core owner.
    pub const fn shared_core_terminal_receipt(&self, owner: MarketSharedCoreV3) -> ContentId {
    ...
    pub const fn failure_terminal_receipt_id(&self) -> ContentId {
        self.shared_core_terminal_receipt(MarketSharedCoreV3::Failure)
    }
    ...
    pub fn semantic_id(&self) -> Result<ContentId> {
        let mut body = [0u8; MARKET_LIFECYCLE_ROOT_BYTES_V3];
        self.encode_into(&mut body)?;
        Ok(content_id(MARKET_LIFECYCLE_ROOT_DOMAIN_V3, &body))
    }

    fn validate_against_schedule(&self, schedule: &MarketFoundationScheduleV4) -> Result<()> {
        self.validate()?;
        schedule.validate()?;
    ...
    fn validate(&self) -> Result<()> {
        ...
        let mut index = 0_usize;
        while index < MARKET_SHARED_CORE_COUNT_V3 {
        ...
        let expected_aggregator_phase = match self.phase {
            MarketLifecyclePhaseV3::Founding
            | MarketLifecyclePhaseV3::Active
            | MarketLifecyclePhaseV3::Aborting => MarketFamilyAggregatorPhaseV1::Open,
            MarketLifecyclePhaseV3::Retiring => MarketFamilyAggregatorPhaseV1::Retiring,
            MarketLifecyclePhaseV3::Terminal => MarketFamilyAggregatorPhaseV1::Terminal,
        };
        ...
        match self.phase {
            ...
            MarketLifecyclePhaseV3::Aborting => {
                if !resolution_absent
                    || self.capital.principal_remaining_lamports != 0
                    || self.admitted_series_links != 1
                    || self.retired_series_links != 0
...
mod successor_tests {
    ...
    fn direct_global_liveness_binding_is_mandatory_and_identity_bearing() {
        let binding = binding_from_ids(ids::<MARKET_BINDING_ID_COUNT_V3>(1), 1, 2, 3, 4, 5, 6);
    ...
    fn link_bundle_and_attachment_substitution_change_current_binding() {
        let binding = link_binding_from_ids(
            ids::<LINK_BINDING_ID_COUNT_V3>(40),
            3,
            SeriesMarketDispositionV1::Founder,
            7,
            8,
        ...
        bundle.compiler_bundle_id = CompiledProductSeriesBundleV7Id::from_bytes([200; 32]);
        assert_ne!(bundle.id().unwrap(), original);
        let mut attachment = binding;
        attachment.attachment_plan_id = SeriesAttachmentPlanV6Id::from_bytes([201; 32]);
        assert_ne!(attachment.id().unwrap(), original);
    }

    #[test]
    fn obligation_wire_partitions_are_exhaustive_and_disjoint() {
        assert_eq!(SeriesLinkObligationStatusV3::CapabilityDisabled.wire_byte(), 0);
        assert_eq!(SeriesLinkObligationStatusV3::EnabledNeverFounded.wire_byte(), 1);
        assert_eq!(SeriesLinkObligationStatusV3::Live.wire_byte(), 2);
        assert_eq!(SeriesLinkObligationStatusV3::Terminal.wire_byte(), 3);
        assert_eq!(SeriesLinkObligationDispositionV3::Terminal.wire_byte(), 1);
        assert_eq!(SeriesLinkObligationDispositionV3::Absent.wire_byte(), 2);
    }

    #[test]
    fn wrapper_terminal_projection_refuses_owner_and_link_substitution() {
        let semantic = SeriesMarketLinkV3Id::from_bytes([211; 32]);
        let projection = SeriesLinkObligationTerminalProjectionV3 {
        ...
        semantic_splice.link_semantic_id = SeriesMarketLinkV3Id::from_bytes([214; 32]);
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketInstanceTerminalProjectionV3 {
    id: ContentId,
    ...
    shared_core_terminal_receipts: [ContentId; MARKET_SHARED_CORE_COUNT_V3],
...
impl MarketInstanceTerminalProjectionV3 {
    /// Projection identity.
    pub const fn id(self) -> ContentId {
        self.id
    }
...
impl FixedCodec for MarketLifecycleRootV3 {
    const ENCODED_LEN: usize = MARKET_LIFECYCLE_ROOT_BYTES_V3;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&ROOT_MAGIC_V3);
        writer.u16(ROOT_VERSION_V3);
...
impl MarketLifecycleRootV3 {
    ...
    /// retain a second 2,544-byte root value on its frame.
    pub fn decode_into(input: &[u8], output: &mut Self) -> Result<()> {
        let mut reader = Reader::new(input, Self::ENCODED_LEN)?;
        reader.magic(&ROOT_MAGIC_V3)?;
        if reader.u16() != ROOT_VERSION_V3 {
            return Err(Error::BadVersion);
        }
        let phase = MarketLifecyclePhaseV3::decode(reader.u8())?;
        reader.reserved(5)?;
        let ids = read_ids::<MARKET_BINDING_ID_COUNT_V3>(&mut reader);
        ...
        reader.reserved(6)?;
        let capital = MarketFoundationCapitalV3 {
            founder_link_id: SeriesMarketLinkV3Id::from_bytes(reader.id().bytes()),
        ...
        let foundation = MarketFoundationProgressV3 {
        ...
        let mut shared_core_terminal_receipts = [ContentId::ZERO; MARKET_SHARED_CORE_COUNT_V3];
...
impl FixedCodec for SeriesMarketLinkV3 {
    const ENCODED_LEN: usize = SERIES_MARKET_LINK_BYTES_V3;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&LINK_MAGIC_V3);
        writer.u16(LINK_VERSION_V3);
        writer.u8(self.phase.byte());
        writer.reserved(5);
        for id in self.binding.identity_ids() {
            writer.id(id);
...
impl SeriesMarketLinkV3 {
    ...
    pub fn decode_into(input: &[u8], output: &mut Self) -> Result<()> {
        let mut reader = Reader::new(input, Self::ENCODED_LEN)?;
        reader.magic(&LINK_MAGIC_V3)?;
        if reader.u16() != LINK_VERSION_V3 {
            return Err(Error::BadVersion);
        }
        let phase = SeriesMarketLinkPhaseV3::decode(reader.u8())?;
        reader.reserved(5)?;
        let ids = read_ids::<LINK_BINDING_ID_COUNT_V3>(&mut reader);
        ...
        let mut obligation_statuses =
            [SeriesLinkObligationStatusV3::CapabilityDisabled; SERIES_LINK_OBLIGATION_COUNT_V3];
        for status in &mut obligation_statuses {
            *status = SeriesLinkObligationStatusV3::decode(reader.u8())?;
        }
        reader.reserved(4)?;
        let mut admission_receipts = [ContentId::ZERO; SERIES_LINK_OBLIGATION_COUNT_V3];
        for receipt in &mut admission_receipts {
            *receipt = reader.id();
        }
        let mut terminal_receipts = [ContentId::ZERO; SERIES_LINK_OBLIGATION_COUNT_V3];
...
fn binding_from_ids(
    ids: [ContentId; MARKET_BINDING_ID_COUNT_V3],
    generation: u64,
    outcome_count: u8,
    ...
    MarketLifecycleBindingV3 {
        ...
        market_lifecycle_replay_account_id: ids[26],
        market_lifecycle_generation_binding_id:
            MarketLifecycleGenerationBindingV2Id::from_bytes(ids[27].bytes()),
        foundation_account_graph_id: MarketFoundationAccountGraphV4Id::from_bytes(ids[28].bytes()),
        foundation_schedule_id: MarketFoundationScheduleV4Id::from_bytes(ids[29].bytes()),
        direct_global_liveness_binding_id: ids[30],
        market_liability_founding_id: ids[31],
        claim_mint_founding_plan_id: ids[32],
        claim_issuance_binding_id: ids[33],
        general_founding_capability_id: ids[34],
        founding_abort_policy_id: ids[35],
...
fn link_binding_from_ids(
    ...
    SeriesMarketLinkBindingV3 {
        ...
        funding_quote_id: SeriesFundingQuoteV6Id::from_bytes(ids[5].bytes()),
        attachment_plan_id: SeriesAttachmentPlanV6Id::from_bytes(ids[6].bytes()),
        capability_profile_id: ids[7],
        obligation_configuration_id: SeriesLinkObligationConfigurationV3Id::from_bytes(
            ids[8].bytes(),
        ),
        compiler_bundle_id: CompiledProductSeriesBundleV7Id::from_bytes(ids[9].bytes()),
...
fn expected_foundation_bitmap(outcome_count: u8) -> Result<u64> {
    ...
    let mut bitmap = 0u64;
    let mut index = 0usize;
    while index < MARKET_FOUNDATION_CORE_SLOT_COUNT_V4 {
    ...
    index = MARKET_FOUNDATION_SLOT_COUNT_V4 - 3;
    while index < MARKET_FOUNDATION_SLOT_COUNT_V4 {
        bitmap |= slot_bit(index)?;
        index += 1;
    }
...
fn slot_bit(index: usize) -> Result<u64> {
    if index >= MARKET_FOUNDATION_SLOT_COUNT_V4 {
        return Err(Error::InvalidParameter);
    }
...
fn highest_set_index(bitmap: u64) -> Option<usize> {
    let mut index = MARKET_FOUNDATION_SLOT_COUNT_V4;
    while index > 0 {
        index -= 1;
...
fn next_uninitialized_foundation_slot(
    expected_bitmap: u64,
    initialized_bitmap: u64,
) -> Result<Option<usize>> {
    if initialized_bitmap & !expected_bitmap != 0 {
        return Err(Error::WorkStateMismatch);
    }
    let mut index = 0usize;
    while index < MARKET_FOUNDATION_SLOT_COUNT_V4 {
        let bit = slot_bit(index)?;
        if expected_bitmap & bit != 0 && initialized_bitmap & bit == 0 {
...
impl SeriesMarketLinkV3 {
    ...
    pub fn admit_obligation(
        self,
        projection: SeriesLinkObligationAdmissionProjectionV3,
    ) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesMarketLinkPhaseV3::Active
        ...
        if self.obligation_statuses[index] != SeriesLinkObligationStatusV3::EnabledNeverFounded
        ...
        obligation_statuses[index] = SeriesLinkObligationStatusV3::Live;
        let next = Self {
    ...
    pub fn pin_failure_session(self, failure_begin_receipt_id: ContentId) -> Result<Self> {
        ...
        if self.phase != SeriesMarketLinkPhaseV3::Active || self.active_failure_sessions != 0 {
            return Err(Error::WorkStateMismatch);
        }
        let sequence = self
            .transition_sequence
        ...
        let transcript = rolling_id(
            b"dragons-clutch/series-link-failure-session/v3",
    ...
    pub fn release_failure_session(self, failure_terminal_receipt_id: ContentId) -> Result<Self> {
        ...
        if self.phase != SeriesMarketLinkPhaseV3::Active || self.active_failure_sessions != 1 {
            return Err(Error::WorkStateMismatch);
        }
        let sequence = self
            .transition_sequence
    ...
    pub fn consume_obligation(
        self,
        projection: SeriesLinkObligationTerminalProjectionV3,
    ) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesMarketLinkPhaseV3::Active
            || projection.link_semantic_id != self.semantic_id()?
        ...
        let expected_disposition = match self.obligation_statuses[index] {
            ...
            | SeriesLinkObligationStatusV3::EnabledNeverFounded => {
                SeriesLinkObligationDispositionV3::Absent
            }
            SeriesLinkObligationStatusV3::Live => SeriesLinkObligationDispositionV3::Terminal,
            SeriesLinkObligationStatusV3::Terminal => return Err(Error::WorkStateMismatch),
        };
        ...
        let mut obligation_statuses = self.obligation_statuses;
        obligation_statuses[index] = SeriesLinkObligationStatusV3::Terminal;
        let next = Self {
            transition_sequence: projection.link_transition_sequence,
            obligation_statuses,
            terminal_receipts,
    ...
    pub fn begin_retirement_into(&self, output: &mut Self) -> Result<()> {
        ...
        if self.phase != SeriesMarketLinkPhaseV3::Active
            ...
            || self
                .obligation_statuses
                .iter()
                .any(|status| *status != SeriesLinkObligationStatusV3::Terminal)
        {
            return Err(Error::WorkIncomplete);
        }
        *output = Self {
            phase: SeriesMarketLinkPhaseV3::Retiring,
            transition_sequence: self
                .transition_sequence
                .checked_add(1)
                .ok_or(Error::ArithmeticOverflow)?,
    ...
    pub fn mark_retired(self, projection: SeriesMarketLinkRetirementProjectionV3) -> Result<Self> {
        let mut output = Self::decode_buffer();
    ...
    pub fn mark_retired_into(
        &self,
        projection: SeriesMarketLinkRetirementProjectionV3,
        output: &mut Self,
    ) -> Result<()> {
        self.validate()?;
        if self.phase != SeriesMarketLinkPhaseV3::Retiring
        ...
        *output = Self {
            phase: SeriesMarketLinkPhaseV3::Retired,
            transition_sequence: self
                .transition_sequence
                .checked_add(1)
                .ok_or(Error::ArithmeticOverflow)?,
    ...
    pub fn mark_aborted(self, market_abort_receipt_id: ContentId) -> Result<Self> {
        ...
        if self.phase != SeriesMarketLinkPhaseV3::PendingMarket
            || self.market_admission_receipt_id != ContentId::ZERO
        {
            return Err(Error::WorkStateMismatch);
        }
        let next = Self {
            phase: SeriesMarketLinkPhaseV3::Aborted,
            transition_sequence: 1,
    ...
    pub const fn binding(self) -> SeriesMarketLinkBindingV3 {
        self.binding
    }
    /// Borrow the immutable link binding without copying its full ID set.
    pub const fn binding_ref(&self) -> &SeriesMarketLinkBindingV3 {
        &self.binding
    }
    /// Link phase.
    ...
    pub fn semantic_id(&self) -> Result<SeriesMarketLinkV3Id> {
        ...
        Ok(SeriesMarketLinkV3Id::from_bytes(
            content_id(SERIES_MARKET_LINK_DOMAIN_V3, &body).bytes(),
        ))
    }
    ...
    fn validate(&self) -> Result<()> {
        ...
        match self.phase {
            SeriesMarketLinkPhaseV3::PendingMarket => {
            ...
            SeriesMarketLinkPhaseV3::Aborted => {
        ...
        while index < SERIES_LINK_OBLIGATION_COUNT_V3 {
            ...
            let receipts_match = match self.obligation_statuses[index] {
                ...
                SeriesLinkObligationStatusV3::Live => admission_present && !terminal_present,
                SeriesLinkObligationStatusV3::Terminal => terminal_present,
            };
            if !receipts_match {
                return Err(Error::WorkStateMismatch);
            }
        ...
        if matches!(
            self.phase,
            SeriesMarketLinkPhaseV3::Retiring | SeriesMarketLinkPhaseV3::Retired
        ) && self
            .obligation_statuses
            .iter()
            .any(|status| *status != SeriesLinkObligationStatusV3::Terminal)
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketLinkRetirementProjectionV3 {
    id: ContentId,
...
impl SeriesMarketLinkRetirementProjectionV3 {
    ...
    pub const fn link_semantic_id(self) -> SeriesMarketLinkV3Id {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesMarketAdmissionProjectionV3 {
    id: ContentId,
    market_binding_id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    link_semantic_id: SeriesMarketLinkV3Id,
...
impl SeriesMarketAdmissionProjectionV3 {
    /// Construct the deterministic root/link admission join.
    pub fn new(
        market_binding: MarketLifecycleBindingV3,
        link: SeriesMarketLinkV3,
        admission_sequence: u64,
    ) -> Result<Self> {
        Self::new_from_ref(&market_binding, &link, admission_sequence)
    }

    /// Construct the admission join without copying the full LinkV3 value.
    pub fn new_from_ref(
        market_binding: &MarketLifecycleBindingV3,
        link: &SeriesMarketLinkV3,
        admission_sequence: u64,
        ...
        link.validate()?;
        if link.phase != SeriesMarketLinkPhaseV3::PendingMarket
        ...
        let id = content_id(b"dragons-clutch/series-market-admission/v3", &body);
        Ok(Self {
            id,
            market_binding_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketResolutionActivationV3 {
    id: ContentId,
    market_binding_id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
...
impl MarketResolutionActivationV3 {
    ...
    pub fn new(
        ...
        let id = content_id(MARKET_RESOLUTION_ACTIVATION_DOMAIN_V3, &body);
        Ok(Self {
            id,
            market_binding_id,
            market_instance_id: binding.market_instance_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketFoundingAbortProjectionV3 {
    id: ContentId,
    market_instance_id: MarketInstanceV2Id,
    generation: u64,
    founder_link_id: SeriesMarketLinkV3Id,
...
impl MarketFoundingAbortProjectionV3 {
    /// Abort receipt.
    pub const fn id(self) -> ContentId {
        self.id
    }
    ...
    pub const fn founder_link_id(self) -> SeriesMarketLinkV3Id {
...
/// Shared `0xaa/3` lifecycle owner.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MarketLifecycleRootV3 {
    binding: MarketLifecycleBindingV3,
    phase: MarketLifecyclePhaseV3,
    transition_sequence: u64,
    capital: MarketFoundationCapitalV3,
    foundation: MarketFoundationProgressV3,
    admitted_series_links: u32,
...
impl MarketLifecycleRootV3 {
    ...
    pub fn initialize_founder(
        ...
        account_graph: &MarketFoundationAccountGraphV4,
        capital: MarketFoundationCapitalV3,
        product_families: &MarketFamilyAggregatorV1,
    ...
    pub fn initialize_founder_into(
        ...
        schedule: &MarketFoundationScheduleV4,
        account_graph: &MarketFoundationAccountGraphV4,
        mut capital: MarketFoundationCapitalV3,
        product_families: &MarketFamilyAggregatorV1,
        root_poststate_receipt_id: ContentId,
        output: &mut Self,
    ) -> Result<()> {
    ...
    pub fn initialize_founder_from_ref_into(
        ...
        let root_index = MarketFoundationSlotV4::LifecycleRoot.index()?;
        ...
        let foundation = MarketFoundationProgressV3 {
            ...
            transcript_id: rolling_id(
                b"dragons-clutch/market-foundation-transcript/v3",
                ContentId::ZERO,
                root_poststate_receipt_id,
                1,
            ),
    ...
    pub fn record_foundation_step(
        ...
        schedule: &MarketFoundationScheduleV4,
        account_graph: &MarketFoundationAccountGraphV4,
        step: MarketFoundationStepProjectionV4,
    ) -> Result<Self> {
        let mut output = Self::decode_buffer();
    ...
    pub fn record_foundation_step_into(
        ...
        let expected_next = next_uninitialized_foundation_slot(
            self.foundation.expected_bitmap,
            self.foundation.initialized_bitmap,
        )?
        .ok_or(Error::WorkIncomplete)?;
        if step.account_id != account_graph.account_ids[index] {
            return Err(Error::MismatchedArtifact);
        }
        let bit = slot_bit(index)?;
        if index != expected_next
            || (self.foundation.expected_bitmap & bit) == 0
    ...
    pub fn admit_series_link(self, admission: SeriesMarketAdmissionProjectionV3) -> Result<Self> {
        let mut output = Self::decode_buffer();
    ...
    pub fn admit_series_link_into(
        &self,
        admission: SeriesMarketAdmissionProjectionV3,
        output: &mut Self,
    ) -> Result<()> {
        self.validate()?;
        if !matches!(
            ...
            || (self.phase == MarketLifecyclePhaseV3::Founding
                && admission.disposition != SeriesMarketDispositionV1::Founder)
        {
            return Err(Error::UnauthenticatedAuthority);
        }
    ...
    /// Frame-bounded family admission into caller-owned RootV3 storage.
    ...
    /// Frame-bounded Fractional terminal transition into caller-owned RootV3 storage.
    ...
    pub fn activate_into(
        ...
        let foundation = MarketFoundationProgressV3 {
            transcript_id: rolling_id(
                b"dragons-clutch/market-foundation-activation/v3",
                self.foundation.transcript_id,
        ...
        *output = Self {
            phase: MarketLifecyclePhaseV3::Active,
            transition_sequence: sequence,
            foundation,
            ..*self
        };
