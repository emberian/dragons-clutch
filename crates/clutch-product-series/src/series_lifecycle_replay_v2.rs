...
use crate::{
    content_id, CompiledProductSeriesBundleV6Id, ContentId, Error, FixedCodec,
    MarketInstanceV2Id, RegistryCapabilityProfileV4Id, RegistryProgramReleaseV2Id, Result,
    SeriesAttachmentPlanV5Id, SeriesFundingQuoteV5Id, SeriesFundingTermsV2Id,
    SeriesLifecycleReplayBindingV2Id, SeriesLifecycleReplayV2Id,
    SeriesLifecycleTerminalProjectionV2Id, SeriesMarketDispositionV1, SeriesPlanV5Id,
    SourceOccurrenceV1Id,
};

const SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V2: [u8; 8] = *b"DCSLRBV2";
const SERIES_LIFECYCLE_REPLAY_MAGIC_V2: [u8; 8] = *b"DCSLRPV2";
const SERIES_LIFECYCLE_REPLAY_SCHEMA_V2: u16 = 2;
const SERIES_LIFECYCLE_ADMISSION_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-admission/v2";
const SERIES_LIFECYCLE_LAPSE_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-lapse/v2";
const SERIES_LIFECYCLE_LINK_RETIREMENT_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-link-retirement/v2";
const SERIES_LIFECYCLE_TRANSCRIPT_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-transcript/v2";

/// Exact immutable binding width.
pub const SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2: usize = 398;
/// Exact counted semantic state width.
pub const SERIES_LIFECYCLE_REPLAY_BYTES_V2: usize = 501;
/// Immutable binding identity domain.
pub const SERIES_LIFECYCLE_REPLAY_BINDING_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-replay-binding/v2";
/// Complete state identity domain.
pub const SERIES_LIFECYCLE_REPLAY_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-replay/v2";
/// Exhaustive terminal projection identity domain.
pub const SERIES_LIFECYCLE_TERMINAL_PROJECTION_DOMAIN_V2: &[u8] =
    b"dragons-clutch/series-lifecycle-terminal-projection/v2";

/// Immutable current-artifact and physical-coordinate binding for one Series.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleReplayBindingV2 {
...
impl SeriesLifecycleReplayBindingV2 {
    ...
    pub fn id(self) -> Result<SeriesLifecycleReplayBindingV2Id> {
        let mut body = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2];
        self.encode_into(&mut body)?;
        Ok(SeriesLifecycleReplayBindingV2Id::from_bytes(
            content_id(SERIES_LIFECYCLE_REPLAY_BINDING_DOMAIN_V2, &body).bytes(),
        ))
    }
...
impl FixedCodec for SeriesLifecycleReplayBindingV2 {
    const ENCODED_LEN: usize = SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V2);
        writer.u16(SERIES_LIFECYCLE_REPLAY_SCHEMA_V2);
        for id in self.ids() {
            writer.id(id);
        }
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        ...
        reader.magic(&SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V2)?;
        if reader.u16() != SERIES_LIFECYCLE_REPLAY_SCHEMA_V2 {
        ...
        let value = Self {
            ...
            funding_quote_id: SeriesFundingQuoteV5Id::from_bytes(ids[2].bytes()),
            attachment_plan_id: SeriesAttachmentPlanV5Id::from_bytes(ids[3].bytes()),
            compiler_bundle_id: CompiledProductSeriesBundleV6Id::from_bytes(ids[4].bytes()),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SeriesLifecycleReplayPhaseV2 {
...
impl SeriesLifecycleReplayPhaseV2 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleAdmissionProjectionV2 {
...
impl SeriesLifecycleAdmissionProjectionV2 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(SERIES_LIFECYCLE_ADMISSION_DOMAIN_V2, &body))
    }
}

/// Exact Funding/Clock evidence for one lapsed ordinal.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleLapseProjectionV2 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV2Id,
    /// Exact Series.
    pub series_plan_id: SeriesPlanV5Id,
...
impl SeriesLifecycleLapseProjectionV2 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(SERIES_LIFECYCLE_LAPSE_DOMAIN_V2, &body))
    }
}

/// Exact Product root/link postwrite evidence for one retired physical link.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleLinkRetirementProjectionV2 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV2Id,
    /// Exact Series.
    pub series_plan_id: SeriesPlanV5Id,
...
impl SeriesLifecycleLinkRetirementProjectionV2 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(
            SERIES_LIFECYCLE_LINK_RETIREMENT_DOMAIN_V2,
            &body,
        ))
    }
}
...
impl SeriesLifecycleTerminalEvidenceV2 {
    fn validate(self, binding: SeriesLifecycleReplayBindingV2) -> Result<()> {
        self.binding_id.validate()?;
        for id in [
            self.funding_account_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleReplayV2 {
    binding: SeriesLifecycleReplayBindingV2,
    phase: SeriesLifecycleReplayPhaseV2,
...
impl SeriesLifecycleReplayV2 {
    ...
    pub fn initialize(binding: SeriesLifecycleReplayBindingV2) -> Result<Self> {
        ...
        let value = Self {
            binding,
            phase: SeriesLifecycleReplayPhaseV2::Open,
    ...
    pub fn record_admission(self, event: SeriesLifecycleAdmissionProjectionV2) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesLifecycleReplayPhaseV2::Open
    ...
    pub fn record_lapse(self, event: SeriesLifecycleLapseProjectionV2) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesLifecycleReplayPhaseV2::Open
            || event.binding_id != self.binding.id()?
            || event.series_plan_id != self.binding.series_plan_id
    ...
    pub fn record_link_retirement(
        self,
        event: SeriesLifecycleLinkRetirementProjectionV2,
    ) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesLifecycleReplayPhaseV2::Open
            || event.binding_id != self.binding.id()?
    ...
    pub fn terminalize(
    ...
    ) -> Result<(Self, SeriesLifecycleTerminalProjectionV2)> {
        self.validate()?;
        ...
        let sequence = self.next_sequence()?;
        let projection = SeriesLifecycleTerminalProjectionV2::derive(
            self.binding,
            pre_state_id,
            sequence,
        ...
        let next = Self {
            phase: SeriesLifecycleReplayPhaseV2::Terminal,
            transition_sequence: sequence,
    ...
    /// Immutable binding.
    pub const fn binding(self) -> SeriesLifecycleReplayBindingV2 {
        self.binding
    }
    /// Current phase.
    pub const fn phase(self) -> SeriesLifecycleReplayPhaseV2 {
        self.phase
    }
    ...
    pub fn id(self) -> Result<SeriesLifecycleReplayV2Id> {
        let mut body = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V2];
        self.encode_into(&mut body)?;
        Ok(SeriesLifecycleReplayV2Id::from_bytes(
            content_id(SERIES_LIFECYCLE_REPLAY_DOMAIN_V2, &body).bytes(),
        ))
    }
    ...
    fn validate(self) -> Result<()> {
        ...
        if self.processed_ordinals > self.binding.instance_count
            ...
            || self.transition_sequence
                != u64::from(self.processed_ordinals)
                    ...
                    .and_then(|value| {
                        value.checked_add(if self.phase == SeriesLifecycleReplayPhaseV2::Terminal {
                            1
                        } else {
                            0
                        })
        ...
        match self.phase {
            SeriesLifecycleReplayPhaseV2::Open => {
                if self.terminal_projection_id != ContentId::ZERO {
                    return Err(Error::WorkStateMismatch);
                }
            }
            SeriesLifecycleReplayPhaseV2::Terminal => {
...
impl FixedCodec for SeriesLifecycleReplayV2 {
    const ENCODED_LEN: usize = SERIES_LIFECYCLE_REPLAY_BYTES_V2;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut binding = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2];
        self.binding.encode_into(&mut binding)?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&SERIES_LIFECYCLE_REPLAY_MAGIC_V2);
        writer.u16(SERIES_LIFECYCLE_REPLAY_SCHEMA_V2);
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        let mut reader = Reader::new(input, Self::ENCODED_LEN)?;
        reader.magic(&SERIES_LIFECYCLE_REPLAY_MAGIC_V2)?;
        if reader.u16() != SERIES_LIFECYCLE_REPLAY_SCHEMA_V2 {
            return Err(Error::BadVersion);
        }
        let binding_bytes = reader.bytes::<SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2>();
        let value = Self {
            binding: SeriesLifecycleReplayBindingV2::decode(&binding_bytes)?,
            phase: SeriesLifecycleReplayPhaseV2::decode(reader.u8())?,
...
/// value disposition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleTerminalProjectionV2 {
    id: SeriesLifecycleTerminalProjectionV2Id,
    binding_id: SeriesLifecycleReplayBindingV2Id,
    ...
    pre_terminal_state_id: SeriesLifecycleReplayV2Id,
...
impl SeriesLifecycleTerminalProjectionV2 {
    ...
    fn derive(
        ...
        writer.finish()?;
        let id = SeriesLifecycleTerminalProjectionV2Id::from_bytes(
            content_id(SERIES_LIFECYCLE_TERMINAL_PROJECTION_DOMAIN_V2, &body).bytes(),
        );
        id.validate()?;
        let terminal_transcript_id = roll(
    ...
    /// Projection identity.
    pub const fn id(self) -> SeriesLifecycleTerminalProjectionV2Id {
        self.id
    }
    /// Immutable binding.
    pub const fn binding_id(self) -> SeriesLifecycleReplayBindingV2Id {
        self.binding_id
    }
    /// Series.
    ...
    pub const fn pre_terminal_state_id(self) -> SeriesLifecycleReplayV2Id {
...
fn roll(previous: ContentId, event: ContentId, sequence: u64) -> ContentId {
    ...
    content_id(SERIES_LIFECYCLE_TRANSCRIPT_DOMAIN_V2, &body)
}
...
mod tests {
    ...
    fn binding() -> SeriesLifecycleReplayBindingV2 {
        SeriesLifecycleReplayBindingV2 {
    ...
    fn admission(ordinal: u32, byte: u8) -> SeriesLifecycleAdmissionProjectionV2 {
        let binding = binding();
        SeriesLifecycleAdmissionProjectionV2 {
    ...
    fn lapse(ordinal: u32) -> SeriesLifecycleLapseProjectionV2 {
        let binding = binding();
        SeriesLifecycleLapseProjectionV2 {
            binding_id: binding.id().unwrap(),
            series_plan_id: binding.series_plan_id,
            ordinal,
            funding_account_id: binding.funding_account_id,
    ...
    fn retirement(ordinal: u32, byte: u8) -> SeriesLifecycleLinkRetirementProjectionV2 {
        let binding = binding();
        SeriesLifecycleLinkRetirementProjectionV2 {
            binding_id: binding.id().unwrap(),
            series_plan_id: binding.series_plan_id,
            ordinal,
    ...
    fn counted_partition_seals_only_after_all_ordinals_and_links() {
        let state = SeriesLifecycleReplayV2::initialize(binding()).unwrap();
        ...
        assert!(state
            .terminalize(SeriesLifecycleTerminalEvidenceV2 {
                binding_id: binding().id().unwrap(),
                funding_account_id: binding().funding_account_id,
        ...
        let (terminal, projection) = state
            ...
            .unwrap();
        assert_eq!(terminal.phase(), SeriesLifecycleReplayPhaseV2::Terminal);
    ...
    fn ordinal_replay_splice_and_count_corruption_refuse() {
        ...
        let mut bytes = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V2];
        admitted.encode_into(&mut bytes).unwrap();
        bytes[417..421].copy_from_slice(&7u32.to_le_bytes());
        assert!(SeriesLifecycleReplayV2::decode(&bytes).is_err());
    }

    #[test]
    fn codecs_are_exact_and_binding_aliases_refuse() {
        let binding = binding();
        let mut binding_bytes = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V2];
        binding.encode_into(&mut binding_bytes).unwrap();
        assert_eq!(SeriesLifecycleReplayBindingV2::decode(&binding_bytes), Ok(binding));
        assert!(SeriesLifecycleReplayBindingV2::decode(&binding_bytes[..397]).is_err());
        ...
        assert_eq!(SeriesLifecycleReplayV2::decode(&bytes), Ok(state));
        let mut trailing = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V2 + 1];
        trailing[..SERIES_LIFECYCLE_REPLAY_BYTES_V2].copy_from_slice(&bytes);
        assert!(SeriesLifecycleReplayV2::decode(&trailing).is_err());
    }
}
