//! Current compiler manifest for the ProfileV4 x QuoteV4 x AttachmentV4 graph.

use crate::codec::{Reader, Writer};
use crate::{
    content_id, CompiledProductSeriesBundleV5Id, ContentId, EvidenceOnlyRecoveryPolicyId,
...
const MAGIC_V5: [u8; 8] = *b"DCCBNDV5";
const VERSION_V5: u16 = 5;

/// Semantic identity domain for the ProfileV4 compiler output.
pub const COMPILED_PRODUCT_SERIES_BUNDLE_V5_DOMAIN: &[u8] =
    b"dragons-clutch/compiled-product-series-bundle/v5";
/// Exact canonical current bundle width.
pub const COMPILED_PRODUCT_SERIES_BUNDLE_V5_BYTES: usize = 528;

/// Canonical owners consumed by the untrusted V5 bundle assembler.
#[derive(Clone, Copy, Debug)]
pub struct ProductSeriesBundleInputsV5<'a> {
...
/// Assemble a deterministic V5 proposal from the complete owning bodies.
pub fn assemble_compiled_product_series_bundle_v5(
    inputs: ProductSeriesBundleInputsV5<'_>,
) -> Result<CompiledProductSeriesBundleV5> {
    ...
    let value = CompiledProductSeriesBundleV5 {
...
pub struct CompiledProductSeriesBundleV5 {
...
impl CompiledProductSeriesBundleV5 {
    ...
    /// Typed identity of the complete exact V5 graph.
    pub fn id(&self) -> Result<CompiledProductSeriesBundleV5Id> {
        let mut body = [0u8; COMPILED_PRODUCT_SERIES_BUNDLE_V5_BYTES];
        self.encode_into(&mut body)?;
        Ok(CompiledProductSeriesBundleV5Id::from_bytes(
            content_id(COMPILED_PRODUCT_SERIES_BUNDLE_V5_DOMAIN, &body).bytes(),
        ))
    }
}

impl FixedCodec for CompiledProductSeriesBundleV5 {
    const ENCODED_LEN: usize = COMPILED_PRODUCT_SERIES_BUNDLE_V5_BYTES;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&MAGIC_V5);
        writer.u16(VERSION_V5);
        writer.reserved(6);
        for id in [
            self.registry_release_id,
            self.capability_profile_id.content_id(),
...
mod tests {
    ...
    #[test]
    fn v4_bytes_do_not_cross_decode_as_v5() {
        let mut bytes = [0u8; COMPILED_PRODUCT_SERIES_BUNDLE_V5_BYTES];
        bytes[..8].copy_from_slice(b"DCCBNDV4");
        bytes[8..10].copy_from_slice(&4u16.to_le_bytes());
        assert_eq!(
            CompiledProductSeriesBundleV5::decode(&bytes),
