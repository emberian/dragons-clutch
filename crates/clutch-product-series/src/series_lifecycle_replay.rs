...
use crate::{
    content_id, CompiledProductSeriesBundleV5Id, ContentId, Error, FixedCodec,
    MarketInstanceV2Id, RegistryCapabilityProfileV4Id, RegistryProgramReleaseV2Id, Result,
    SeriesAttachmentPlanV4Id, SeriesFundingQuoteV4Id, SeriesFundingTermsV2Id,
    SeriesLifecycleReplayBindingV1Id, SeriesLifecycleReplayV1Id,
    SeriesLifecycleTerminalProjectionV1Id, SeriesMarketDispositionV1, SeriesPlanV5Id,
    SourceOccurrenceV1Id,
};

const SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V1: [u8; 8] = *b"DCSLRBV1";
const SERIES_LIFECYCLE_REPLAY_MAGIC_V1: [u8; 8] = *b"DCSLRPV1";
const SERIES_LIFECYCLE_REPLAY_SCHEMA_V1: u16 = 1;
const SERIES_LIFECYCLE_ADMISSION_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-admission/v1";
const SERIES_LIFECYCLE_LAPSE_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-lapse/v1";
const SERIES_LIFECYCLE_LINK_RETIREMENT_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-link-retirement/v1";
const SERIES_LIFECYCLE_TRANSCRIPT_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-transcript/v1";

/// Exact immutable binding width.
pub const SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1: usize = 398;
/// Exact counted semantic state width.
pub const SERIES_LIFECYCLE_REPLAY_BYTES_V1: usize = 501;
/// Immutable binding identity domain.
pub const SERIES_LIFECYCLE_REPLAY_BINDING_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-replay-binding/v1";
/// Complete state identity domain.
pub const SERIES_LIFECYCLE_REPLAY_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-replay/v1";
/// Exhaustive terminal projection identity domain.
pub const SERIES_LIFECYCLE_TERMINAL_PROJECTION_DOMAIN_V1: &[u8] =
    b"dragons-clutch/series-lifecycle-terminal-projection/v1";

/// Immutable current-artifact and physical-coordinate binding for one Series.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleReplayBindingV1 {
...
impl SeriesLifecycleReplayBindingV1 {
    ...
    pub fn id(self) -> Result<SeriesLifecycleReplayBindingV1Id> {
        let mut body = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1];
        self.encode_into(&mut body)?;
        Ok(SeriesLifecycleReplayBindingV1Id::from_bytes(
            content_id(SERIES_LIFECYCLE_REPLAY_BINDING_DOMAIN_V1, &body).bytes(),
        ))
    }
...
impl FixedCodec for SeriesLifecycleReplayBindingV1 {
    const ENCODED_LEN: usize = SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V1);
        writer.u16(SERIES_LIFECYCLE_REPLAY_SCHEMA_V1);
        for id in self.ids() {
            writer.id(id);
        }
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        ...
        reader.magic(&SERIES_LIFECYCLE_REPLAY_BINDING_MAGIC_V1)?;
        if reader.u16() != SERIES_LIFECYCLE_REPLAY_SCHEMA_V1 {
        ...
        let value = Self {
            ...
            funding_quote_id: SeriesFundingQuoteV4Id::from_bytes(ids[2].bytes()),
            attachment_plan_id: SeriesAttachmentPlanV4Id::from_bytes(ids[3].bytes()),
            compiler_bundle_id: CompiledProductSeriesBundleV5Id::from_bytes(ids[4].bytes()),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SeriesLifecycleReplayPhaseV1 {
...
impl SeriesLifecycleReplayPhaseV1 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleAdmissionProjectionV1 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV1Id,
    /// Exact Series.
    pub series_plan_id: SeriesPlanV5Id,
    /// Sequential ordinal.
    pub ordinal: u32,
    /// Exact FundingV2 account.
    pub funding_account_id: ContentId,
    /// FundingV2 semantic prestate.
    pub funding_state_before_id: ContentId,
    /// FundingV2 semantic poststate.
...
impl SeriesLifecycleAdmissionProjectionV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(SERIES_LIFECYCLE_ADMISSION_DOMAIN_V1, &body))
    }
}

/// Exact Funding/Clock evidence for one lapsed ordinal.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleLapseProjectionV1 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV1Id,
    /// Exact Series.
    pub series_plan_id: SeriesPlanV5Id,
...
impl SeriesLifecycleLapseProjectionV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(SERIES_LIFECYCLE_LAPSE_DOMAIN_V1, &body))
    }
}

/// Exact Product root/link postwrite evidence for one retired physical link.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleLinkRetirementProjectionV1 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV1Id,
    /// Exact Series.
    pub series_plan_id: SeriesPlanV5Id,
...
impl SeriesLifecycleLinkRetirementProjectionV1 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(
            SERIES_LIFECYCLE_LINK_RETIREMENT_DOMAIN_V1,
            &body,
        ))
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleTerminalEvidenceV1 {
    /// Immutable aggregate binding.
    pub binding_id: SeriesLifecycleReplayBindingV1Id,
    /// Exact FundingV2 account.
    pub funding_account_id: ContentId,
    /// Exact hostile-authenticated Closed FundingV2 semantic state.
    pub funding_state_id: ContentId,
    /// Exact FundingV2 terminal principal/donation projection.
    pub funding_terminal_projection_id: ContentId,
    /// Permanent RegistryV2 account.
    pub registry_account_id: ContentId,
    /// Full current RegistryV2 authentication receipt.
...
impl SeriesLifecycleTerminalEvidenceV1 {
    fn validate(self, binding: SeriesLifecycleReplayBindingV1) -> Result<()> {
        self.binding_id.validate()?;
        for id in [
            self.funding_account_id,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesLifecycleReplayV1 {
    binding: SeriesLifecycleReplayBindingV1,
    phase: SeriesLifecycleReplayPhaseV1,
...
impl SeriesLifecycleReplayV1 {
    /// Initialize the sole empty replay owner at FundingV2 activation.
    pub fn initialize(binding: SeriesLifecycleReplayBindingV1) -> Result<Self> {
        binding.validate()?;
        let value = Self {
            binding,
            phase: SeriesLifecycleReplayPhaseV1::Open,
    ...
    pub fn record_admission(self, event: SeriesLifecycleAdmissionProjectionV1) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesLifecycleReplayPhaseV1::Open
    ...
    pub fn record_lapse(self, event: SeriesLifecycleLapseProjectionV1) -> Result<Self> {
        self.validate()?;
        if self.phase != SeriesLifecycleReplayPhaseV1::Open
            || event.binding_id != self.binding.id()?
            || event.series_plan_id != self.binding.series_plan_id
    ...
    /// Seal exhaustive Series terminality from the current Closed FundingV2.
    pub fn terminalize(
        self,
        evidence: SeriesLifecycleTerminalEvidenceV1,
    ) -> Result<(Self, SeriesLifecycleTerminalProjectionV1)> {
        self.validate()?;
        ...
        let sequence = self.next_sequence()?;
        let projection = SeriesLifecycleTerminalProjectionV1::derive(
            self.binding,
            pre_state_id,
            sequence,
        ...
        let next = Self {
            phase: SeriesLifecycleReplayPhaseV1::Terminal,
            transition_sequence: sequence,
    ...
    /// Immutable binding.
    pub const fn binding(self) -> SeriesLifecycleReplayBindingV1 {
        self.binding
    }
    /// Current phase.
    pub const fn phase(self) -> SeriesLifecycleReplayPhaseV1 {
        self.phase
    }
    ...
    pub fn id(self) -> Result<SeriesLifecycleReplayV1Id> {
        let mut body = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V1];
        self.encode_into(&mut body)?;
        Ok(SeriesLifecycleReplayV1Id::from_bytes(
            content_id(SERIES_LIFECYCLE_REPLAY_DOMAIN_V1, &body).bytes(),
        ))
    }
    ...
    fn validate(self) -> Result<()> {
        ...
        if self.processed_ordinals > self.binding.instance_count
            ...
            || self.transition_sequence
                != u64::from(self.processed_ordinals)
                    ...
                    .and_then(|value| {
                        value.checked_add(if self.phase == SeriesLifecycleReplayPhaseV1::Terminal {
                            1
                        } else {
                            0
                        })
        ...
        match self.phase {
            SeriesLifecycleReplayPhaseV1::Open => {
                if self.terminal_projection_id != ContentId::ZERO {
                    return Err(Error::WorkStateMismatch);
                }
            }
            SeriesLifecycleReplayPhaseV1::Terminal => {
...
impl FixedCodec for SeriesLifecycleReplayV1 {
    const ENCODED_LEN: usize = SERIES_LIFECYCLE_REPLAY_BYTES_V1;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut binding = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1];
        self.binding.encode_into(&mut binding)?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&SERIES_LIFECYCLE_REPLAY_MAGIC_V1);
        writer.u16(SERIES_LIFECYCLE_REPLAY_SCHEMA_V1);
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        let mut reader = Reader::new(input, Self::ENCODED_LEN)?;
        reader.magic(&SERIES_LIFECYCLE_REPLAY_MAGIC_V1)?;
        if reader.u16() != SERIES_LIFECYCLE_REPLAY_SCHEMA_V1 {
            return Err(Error::BadVersion);
        }
        let binding_bytes = reader.bytes::<SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1>();
        let value = Self {
            binding: SeriesLifecycleReplayBindingV1::decode(&binding_bytes)?,
            phase: SeriesLifecycleReplayPhaseV1::decode(reader.u8())?,
...
pub struct SeriesLifecycleTerminalProjectionV1 {
    ...
    pre_terminal_state_id: SeriesLifecycleReplayV1Id,
...
impl SeriesLifecycleTerminalProjectionV1 {
    ...
    fn derive(
        ...
        writer.finish()?;
        let id = SeriesLifecycleTerminalProjectionV1Id::from_bytes(
            content_id(SERIES_LIFECYCLE_TERMINAL_PROJECTION_DOMAIN_V1, &body).bytes(),
        );
        id.validate()?;
        let terminal_transcript_id = roll(
    ...
    /// Projection identity.
    pub const fn id(self) -> SeriesLifecycleTerminalProjectionV1Id {
        self.id
    }
    /// Immutable binding.
    pub const fn binding_id(self) -> SeriesLifecycleReplayBindingV1Id {
        self.binding_id
    }
    /// Series.
    ...
    pub const fn pre_terminal_state_id(self) -> SeriesLifecycleReplayV1Id {
...
fn roll(previous: ContentId, event: ContentId, sequence: u64) -> ContentId {
    ...
    content_id(SERIES_LIFECYCLE_TRANSCRIPT_DOMAIN_V1, &body)
}
...
mod tests {
    ...
    fn binding() -> SeriesLifecycleReplayBindingV1 {
    ...
    fn admission(ordinal: u32, byte: u8) -> SeriesLifecycleAdmissionProjectionV1 {
        let binding = binding();
        SeriesLifecycleAdmissionProjectionV1 {
    ...
    fn lapse(ordinal: u32) -> SeriesLifecycleLapseProjectionV1 {
        let binding = binding();
        SeriesLifecycleLapseProjectionV1 {
            binding_id: binding.id().unwrap(),
            series_plan_id: binding.series_plan_id,
            ordinal,
            funding_account_id: binding.funding_account_id,
    ...
    fn retirement(ordinal: u32, byte: u8) -> SeriesLifecycleLinkRetirementProjectionV1 {
        let binding = binding();
    ...
    fn counted_partition_seals_only_after_all_ordinals_and_links() {
        ...
        assert!(state
            .terminalize(SeriesLifecycleTerminalEvidenceV1 {
                binding_id: binding().id().unwrap(),
                funding_account_id: binding().funding_account_id,
        ...
        let (terminal, projection) = state
            ...
            .unwrap();
        assert_eq!(terminal.phase(), SeriesLifecycleReplayPhaseV1::Terminal);
    ...
    fn ordinal_replay_splice_and_count_corruption_refuse() {
        ...
        let mut bytes = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V1];
        admitted.encode_into(&mut bytes).unwrap();
        bytes[417..421].copy_from_slice(&7u32.to_le_bytes());
        assert!(SeriesLifecycleReplayV1::decode(&bytes).is_err());
    }

    #[test]
    fn codecs_are_exact_and_binding_aliases_refuse() {
        let binding = binding();
        let mut binding_bytes = [0u8; SERIES_LIFECYCLE_REPLAY_BINDING_BYTES_V1];
        binding.encode_into(&mut binding_bytes).unwrap();
        assert_eq!(SeriesLifecycleReplayBindingV1::decode(&binding_bytes), Ok(binding));
        ...
        assert_eq!(SeriesLifecycleReplayV1::decode(&bytes), Ok(state));
        let mut trailing = [0u8; SERIES_LIFECYCLE_REPLAY_BYTES_V1 + 1];
        trailing[..SERIES_LIFECYCLE_REPLAY_BYTES_V1].copy_from_slice(&bytes);
        assert!(SeriesLifecycleReplayV1::decode(&trailing).is_err());
    }
}
