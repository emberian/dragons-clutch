...
//!
//! This is a fresh owner for QuoteV6/AttachmentV6/BundleV7. Historical V1-V4
//! bytes are never decoded as this state. Pending commits an acyclic pre-Source
//! reservation binding, never the final LinkV3 semantic ID: Source must first
...
use crate::{
    content_id, CompiledProductSeriesBundleV7Id, ComponentDebitV1, ContentId, Error, FixedCodec,
    MarketInstanceV2Id, Result, SeriesAttachmentPlanV6, SeriesAttachmentPlanV6Id,
    SeriesFundingComponentV2, SeriesFundingQuoteV6, SeriesFundingQuoteV6Id,
    SeriesFundingAbortBindingV5Id, SeriesFundingCompletionAuthorizationV5Id,
    SeriesFundingCompletionBindingV5Id,
    SeriesFundingReservationBindingV5Id,
    SeriesFundingStateV5Id, SeriesFundingTermsV2Id, SeriesMarketDispositionV1, SeriesPlanV5,
    SeriesPlanV5Id, SourceOccurrenceV1Id, SERIES_FUNDING_COMPONENT_COUNT_V2,
};

const MAGIC_V5: [u8; 8] = *b"DCSFSTV5";
const SCHEMA_V5: u16 = 5;

/// Semantic identity domain of the exact current state.
pub const SERIES_FUNDING_STATE_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-state/v5";
/// Semantic identity of the exact current terminal principal/donation view.
pub const SERIES_FUNDING_TERMINAL_PROJECTION_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-terminal-projection/v5";
/// Acyclic Active-prestate reservation identity.
pub const SERIES_FUNDING_RESERVATION_BINDING_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-reservation-binding/v5";
/// Acyclic pre-Replay completion authorization identity.
pub const SERIES_FUNDING_COMPLETION_AUTHORIZATION_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-completion-authorization/v5";
/// Final Source/Root/Link/replay completion identity.
pub const SERIES_FUNDING_COMPLETION_BINDING_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-completion-binding/v5";
/// Exact inert/retired Source proof required before a Pending debit is restored.
pub const SERIES_FUNDING_ABORT_BINDING_V5_DOMAIN: &[u8] =
    b"dragons-clutch/series-funding-abort-binding/v5";
/// Exact bytes per separately accounted component.
pub const SERIES_COMPONENT_CAPITAL_BYTES_V5: usize = 40;
/// Exact current state width with no unnamed authority-bearing padding.
pub const SERIES_FUNDING_STATE_BYTES_V5: usize = 16
    + 5 * 32
    + 3 * 4
    + 8
    + 3 * 32
    ...
    + SERIES_FUNDING_COMPONENT_COUNT_V2 * SERIES_COMPONENT_CAPITAL_BYTES_V5;

const _: () = assert!(SERIES_FUNDING_STATE_BYTES_V5 == 704);

/// Exhaustive successor funding phase.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SeriesFundingPhaseV5 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesComponentCapitalV5 {
...
pub struct SeriesFundingReservationBindingV5 {
    /// Exact writable FundingV5 account coordinate.
    ...
    pub funding_state_before_id: SeriesFundingStateV5Id,
    /// Exact finite Series.
    ...
    /// Current QuoteV6-bound attachment.
    ...
    /// Final RootV3 immutable binding derivable before Source publication.
    pub market_binding_id: ContentId,
    /// Canonical RootV3 PDA coordinate.
    pub market_root_account_id: ContentId,
    /// Canonical future LinkV3 PDA coordinate.
...
impl SeriesFundingReservationBindingV5 {
    /// Hash every exact reservation fact under the fresh V5 domain.
    pub fn id(&self) -> Result<SeriesFundingReservationBindingV5Id> {
        ...
        writer.finish()?;
        Ok(SeriesFundingReservationBindingV5Id::from_bytes(
            content_id(SERIES_FUNDING_RESERVATION_BINDING_V5_DOMAIN, &body).bytes(),
        ))
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SeriesFundingCompletionAuthorizationV5 {
    /// Exact reservation persisted in Pending.
    pub reservation_binding_id: SeriesFundingReservationBindingV5Id,
    /// Exact writable FundingV5 account coordinate.
    pub funding_account_id: ContentId,
    /// Hostile authentication of FundingV5 Pending poststate.
    pub funding_account_authentication_pending_id: ContentId,
    /// Typed semantic identity of the exact Pending poststate.
    pub funding_pending_state_id: SeriesFundingStateV5Id,
    ...
    /// Canonical RootV3 account.
    pub market_root_account_id: ContentId,
    /// Exact immutable RootV3 binding.
    pub market_binding_id: ContentId,
    /// RootV3 semantic before the final activation step.
    pub root_semantic_before_id: ContentId,
    /// RootV3 semantic after the final activation step.
    pub root_semantic_after_id: ContentId,
    /// Canonical LinkV3 account.
    pub series_market_link_account_id: ContentId,
    /// LinkV3 semantic before activation.
    pub link_semantic_before_id: ContentId,
    /// LinkV3 semantic after activation.
    ...
    pub funding_projected_state_after_id: SeriesFundingStateV5Id,
}

impl SeriesFundingCompletionAuthorizationV5 {
    /// Validate and hash the acyclic pre-Replay authority.
    pub fn id(&self) -> Result<SeriesFundingCompletionAuthorizationV5Id> {
        ...
        writer.finish()?;
        Ok(SeriesFundingCompletionAuthorizationV5Id::from_bytes(
            content_id(SERIES_FUNDING_COMPLETION_AUTHORIZATION_V5_DOMAIN, &body).bytes(),
        ))
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SeriesFundingCompletionBindingV5 {
    /// Non-detachable acyclic pre-Replay completion authority.
    pub completion_authorization_id: SeriesFundingCompletionAuthorizationV5Id,
    /// Canonical permanent current Series lifecycle replay account.
...
impl SeriesFundingCompletionBindingV5 {
    /// Hash the final authorization/Replay join.
    pub fn id(&self) -> Result<SeriesFundingCompletionBindingV5Id> {
        ...
        for id in ids { writer.id(id); }
        writer.finish()?;
        Ok(SeriesFundingCompletionBindingV5Id::from_bytes(
            content_id(SERIES_FUNDING_COMPLETION_BINDING_V5_DOMAIN, &body).bytes(),
        ))
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SeriesFundingAbortDispositionV5 {
...
impl SeriesFundingAbortDispositionV5 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SeriesFundingAbortBindingV5 {
    /// Exact reservation persisted in Pending.
    pub reservation_binding_id: SeriesFundingReservationBindingV5Id,
    /// Exact writable FundingV5 account coordinate.
    pub funding_account_id: ContentId,
    /// Hostile authentication of the FundingV5 Pending poststate.
    pub funding_account_authentication_pending_id: ContentId,
    /// Typed semantic identity of that Pending poststate.
    pub funding_pending_state_id: SeriesFundingStateV5Id,
    /// Exhaustive Source-side state accepted by this abort.
    pub disposition: SeriesFundingAbortDispositionV5,
...
impl SeriesFundingAbortBindingV5 {
    /// Hash the exhaustive Source absence/retirement proof.
    pub fn id(&self) -> Result<SeriesFundingAbortBindingV5Id> {
        self.reservation_binding_id.validate()?;
        self.funding_pending_state_id.validate()?;
        for id in [
            self.funding_account_id,
        ...
        let absence = self.disposition == SeriesFundingAbortDispositionV5::SourceNeverPublished;
        if absence {
        ...
        writer.finish()?;
        Ok(SeriesFundingAbortBindingV5Id::from_bytes(
            content_id(SERIES_FUNDING_ABORT_BINDING_V5_DOMAIN, &body).bytes(),
        ))
    }
}
...
pub trait AuthenticatedSeriesFundingAuthorityV5 {
    ...
    fn authenticate_pending_completion(
        &self,
        state: &SeriesFundingStateV5,
        binding: &SeriesFundingCompletionBindingV5,
    ...
    fn authenticate_pending_abort(
        &self,
        state: &SeriesFundingStateV5,
        binding: &SeriesFundingAbortBindingV5,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesFundingStateV5 {
    /// Registered finite Series.
    pub series_plan_id: SeriesPlanV5Id,
    /// Immutable refund/sink/collateral ownership.
    pub funding_terms_id: SeriesFundingTermsV2Id,
    /// Current 50-slot quote.
    pub funding_quote_id: SeriesFundingQuoteV6Id,
    /// Current QuoteV6-bound attachment.
    pub attachment_plan_id: SeriesAttachmentPlanV6Id,
    /// Exact compiler/Source/capability graph retained by SeriesRegistry V5.
    pub compiler_bundle_id: CompiledProductSeriesBundleV7Id,
    ...
    pub phase: SeriesFundingPhaseV5,
    ...
    pub components: [SeriesComponentCapitalV5; SERIES_FUNDING_COMPONENT_COUNT_V2],
}

impl SeriesFundingStateV5 {
    /// Activate current state from exact V5/V6 artifacts and physical deposits.
    #[allow(clippy::too_many_arguments)]
    pub fn activate<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        authority: &A,
        series: &SeriesPlanV5,
        funding_terms_id: SeriesFundingTermsV2Id,
        compiler_bundle_id: CompiledProductSeriesBundleV7Id,
        ...
        let mut components = [SeriesComponentCapitalV5::ZERO; SERIES_FUNDING_COMPONENT_COUNT_V2];
        ...
        while index < SERIES_FUNDING_COMPONENT_COUNT_V2 {
            ...
            components[index] = SeriesComponentCapitalV5 {
        ...
        let value = Self {
            ...
            phase: SeriesFundingPhaseV5::Active,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn reserve_created<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &mut self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
        ...
        if self.phase != SeriesFundingPhaseV5::Active || self.next_ordinal >= self.instance_count {
        ...
        next.phase = SeriesFundingPhaseV5::Pending;
    ...
    pub fn complete_pending<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &mut self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
        ...
        self.validate_against(series, quote, attachment)?;
        if self.phase != SeriesFundingPhaseV5::Pending {
            return Err(Error::WorkStateMismatch);
        }
        ...
        next.phase = if next.next_ordinal == next.instance_count {
            SeriesFundingPhaseV5::Closed
        } else {
            SeriesFundingPhaseV5::Active
        };
        next.transition_sequence = increment(next.transition_sequence)?;
        next.validate_against(series, quote, attachment)?;
        *self = next;
    ...
    pub fn abort_pending<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &mut self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
        ...
        next.clear_pending();
        next.phase = SeriesFundingPhaseV5::Active;
        next.transition_sequence = increment(next.transition_sequence)?;
        next.validate_against(series, quote, attachment)?;
        *self = next;
        Ok(ordinal)
    ...
    pub fn lapse<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &mut self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
    ...
    pub fn add_donation<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &mut self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
    ...
    pub fn close<A: AuthenticatedSeriesFundingAuthorityV5 + ?Sized>(
        &self,
        authority: &A,
        series: &SeriesPlanV5,
        quote: &SeriesFundingQuoteV6,
        attachment: &SeriesAttachmentPlanV6,
        terminal_receipt_id: ContentId,
    ) -> Result<SeriesFundingTerminalProjectionV5> {
        self.validate_against(series, quote, attachment)?;
        if self.phase != SeriesFundingPhaseV5::Closed {
        ...
        Ok(SeriesFundingTerminalProjectionV5 {
    ...
    pub fn validate_against(
        ...
        while index < SERIES_FUNDING_COMPONENT_COUNT_V2 {
            ...
            let pending_delta = if self.phase == SeriesFundingPhaseV5::Pending
    ...
    pub fn id(&self) -> Result<SeriesFundingStateV5Id> {
        let mut body = [0u8; SERIES_FUNDING_STATE_BYTES_V5];
        self.encode_into(&mut body)?;
        Ok(SeriesFundingStateV5Id::from_bytes(
            content_id(SERIES_FUNDING_STATE_V5_DOMAIN, &body).bytes(),
        ))
    }
    ...
    fn validate(&self) -> Result<()> {
        ...
        let pending = self.phase == SeriesFundingPhaseV5::Pending;
        if pending {
        ...
        match self.phase {
            SeriesFundingPhaseV5::Active if self.next_ordinal < self.instance_count => {}
            SeriesFundingPhaseV5::Pending if self.next_ordinal < self.instance_count => {}
            SeriesFundingPhaseV5::Closed if self.next_ordinal == self.instance_count => {}
    ...
    fn admitted_created_count(&self) -> Result<u32> {
        self.next_ordinal
            .checked_sub(self.lapsed_count)
            .and_then(|created| {
                created.checked_add(if self.phase == SeriesFundingPhaseV5::Pending {
                    1
                } else {
                    0
                })
...
impl FixedCodec for SeriesFundingStateV5 {
    const ENCODED_LEN: usize = SERIES_FUNDING_STATE_BYTES_V5;

    fn encode_into(&self, output: &mut [u8]) -> Result<()> {
        self.validate()?;
        let mut writer = Writer::new(output, Self::ENCODED_LEN)?;
        writer.bytes(&MAGIC_V5);
        writer.u16(SCHEMA_V5);
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        ...
        reader.magic(&MAGIC_V5)?;
        if reader.u16() != SCHEMA_V5 {
            return Err(Error::BadVersion);
        }
        let phase = SeriesFundingPhaseV5::decode(reader.u8())?;
        ...
        let funding_quote_id = SeriesFundingQuoteV6Id::from_bytes(reader.id().bytes());
        let attachment_plan_id = SeriesAttachmentPlanV6Id::from_bytes(reader.id().bytes());
        let compiler_bundle_id = CompiledProductSeriesBundleV7Id::from_bytes(reader.id().bytes());
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SeriesFundingTerminalProjectionV5 {
    /// Exact closed Series.
    pub series_plan_id: SeriesPlanV5Id,
...
impl SeriesFundingTerminalProjectionV5 {
    ...
    pub fn id(self) -> Result<ContentId> {
        ...
        writer.finish()?;
        Ok(content_id(
            SERIES_FUNDING_TERMINAL_PROJECTION_V5_DOMAIN,
            &body,
        ))
    }
}
...
mod tests {
    ...
    impl AuthenticatedSeriesFundingAuthorityV5 for DonationOnlyAuthority {
        fn authenticate_activation(
            &self,
    ...
    fn active_state() -> SeriesFundingStateV5 {
        SeriesFundingStateV5 {
            ...
            pending_clock_bucket: 0,
            pending_debits: [ComponentDebitV1::ZERO; SERIES_FUNDING_COMPONENT_COUNT_V2],
            components: [SeriesComponentCapitalV5::ZERO; SERIES_FUNDING_COMPONENT_COUNT_V2],
        }
    }

    fn joined_fixtures() -> (SeriesPlanV5, SeriesFundingQuoteV6, SeriesAttachmentPlanV6) {
        let mut slot_principal_lamports = [0u64; crate::MARKET_FOUNDATION_SLOT_COUNT_V4];
        for principal in &mut slot_principal_lamports
            [..crate::MARKET_FOUNDATION_CORE_SLOT_COUNT_V4 + 2]
        {
            *principal = 10;
        }
        let custody_start = crate::MARKET_FOUNDATION_CORE_SLOT_COUNT_V4
            + crate::MARKET_FOUNDATION_MAX_OUTCOMES_V4;
        for principal in &mut slot_principal_lamports[custody_start..custody_start + 2] {
            *principal = 10;
        }
        for principal in &mut slot_principal_lamports[47..50] {
            *principal = 10;
        }
        let foundation = crate::MarketFoundationScheduleV4 {
            outcome_count: 2,
        ...
        let quote = SeriesFundingQuoteV6 {
        ...
        let attachment = SeriesAttachmentPlanV6 {
    ...
    fn pending_state() -> (SeriesFundingStateV5, SeriesPlanV5, SeriesFundingQuoteV6, SeriesAttachmentPlanV6) {
        ...
        let state = SeriesFundingStateV5 {
            ...
            compiler_bundle_id: CompiledProductSeriesBundleV7Id::from_bytes([19; 32]),
            instance_count: 2,
            next_ordinal: 0,
            lapsed_count: 0,
            transition_sequence: 1,
            phase: SeriesFundingPhaseV5::Pending,
    ...
    fn reservation_binding(state: &SeriesFundingStateV5) -> SeriesFundingReservationBindingV5 {
        SeriesFundingReservationBindingV5 {
    ...
    fn current_width_is_exact_and_not_the_historical_state_width() {
        assert_eq!(SERIES_FUNDING_STATE_BYTES_V5, 704);
        assert_ne!(
            SERIES_FUNDING_STATE_BYTES_V5,
    ...
    fn codec_round_trips_and_refuses_a_caller_shaped_pending_phase() {
        ...
        let mut body = [0; SERIES_FUNDING_STATE_BYTES_V5];
        value.encode_into(&mut body).unwrap();
        assert_eq!(SeriesFundingStateV5::decode(&body), Ok(value));
        body[10] = SeriesFundingPhaseV5::Pending.byte();
        body[11] = 1;
        assert_eq!(
            SeriesFundingStateV5::decode(&body),
    ...
    fn completion_preview_is_deterministic_but_does_not_mutate_pending() {
        ...
        let projected = state
            ...
            .unwrap();
        assert_eq!(state.phase, SeriesFundingPhaseV5::Pending);
        assert_eq!(projected.phase, SeriesFundingPhaseV5::Active);
    ...
    fn completion_authorization_binds_preview_and_pre_replay_postwrites() {
        ...
        let projected = state
            .project_pending_completion_poststate(&series, &quote, &attachment)
            .unwrap();
        let base = SeriesFundingCompletionAuthorizationV5 {
        ...
        hostile.funding_projected_state_after_id =
            SeriesFundingStateV5Id::from_bytes([74; 32]);
    ...
    fn abort_partition_requires_zero_absence_receipts_or_both_retirement_receipts() {
        ...
        let base = SeriesFundingAbortBindingV5 {
            reservation_binding_id,
            funding_account_id: ContentId::from_bytes([60; 32]),
            funding_account_authentication_pending_id: ContentId::from_bytes([61; 32]),
            funding_pending_state_id: state.id().unwrap(),
            disposition: SeriesFundingAbortDispositionV5::SourceNeverPublished,
        ...
        hostile = base;
        hostile.disposition = SeriesFundingAbortDispositionV5::SourceRetired;
    ...
    fn pending_debits_refuse_wrong_disposition_geometry() {
        ...
        let quote = SeriesFundingQuoteV6 {
            ...
            failure_recovery_quote_schedule_id: ContentId::from_bytes([13; 32]),
            components,
            foundation: crate::MarketFoundationScheduleV4 {
                outcome_count: 2,
                slot_principal_lamports: [0; crate::MARKET_FOUNDATION_SLOT_COUNT_V4],
    ...
    fn terminal_projection_identity_binds_every_value_and_authority() {
        ...
        let projection = SeriesFundingTerminalProjectionV5 {
            series_plan_id: SeriesPlanV5Id::from_bytes([1; 32]),
            funding_terms_id: SeriesFundingTermsV2Id::from_bytes([2; 32]),
            compiler_bundle_id: CompiledProductSeriesBundleV7Id::from_bytes([3; 32]),
