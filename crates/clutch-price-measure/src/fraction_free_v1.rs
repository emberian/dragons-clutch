...
//! that arithmetic separate from search policy. Its eight-limb magnitude has
//! 512 bits: enough for Bareiss intermediates, 3-by-3 determinants, and the
//! later determinant-times-atom reconstruction bound. Every operation is
//! checked, and exact division refuses a remainder.

use core::cmp::Ordering;

const WIDE_LIMBS_V1: usize = 8;
...
pub(crate) enum FractionFreeErrorV1 {
    /// A result exceeded the fixed 512-bit magnitude.
    ArithmeticOverflow,
...
/// Unsigned 512-bit magnitude in little-endian `u64` limbs.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl WideUnsignedV1 {
    ...
    pub(crate) fn checked_mul(self, other: Self) -> Option<Self> {
        ...
        while left_limb < WIDE_LIMBS_V1 {
            let mut right_limb = 0usize;
            while right_limb < WIDE_LIMBS_V1 {
...
/// Signed 512-bit value with canonical positive zero.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
mod tests {
    ...
    fn wide_exact_division_and_overflow_are_explicit() {
        ...
        let top_bit = WideUnsignedV1::from_u64(1).checked_shl(511).unwrap();
