...
control flow. The complete-set projection cases are evaluated by Lean from
`projectCapSets`, so Rust is graded against the one formal rounding boundary.
-/
...
def emitCapProjection (case : Nat × Nat) : IO Unit := do
  IO.println "    PrincipalCapProjectionCaseV1 {"
  IO.println s!"        cap_atoms: {case.1},"
  IO.println s!"        basis_scale: {case.2},"
  match projectCapSets case.1 case.2 with
  | some projected => IO.println s!"        projected_sets: Some({projected}),"
  | none => IO.println "        projected_sets: None,"
  IO.println "    },"
...
def main : IO Unit := do
  ...
  IO.println s!"pub const MARKET_PRINCIPAL_CAP_SETS_UNBOUNDED_V1: u64 = {capSetsUnbounded};"
  ...
  IO.println "];"
  IO.println s!"pub const PRINCIPAL_CAP_PROJECTION_CASES_V1: [PrincipalCapProjectionCaseV1; {capProjectionCases.length}] = ["
  for case in capProjectionCases do
    emitCapProjection case
  IO.println "];"
  IO.println "#[cfg(test)]"
  IO.println "#[rustfmt::skip]"
  IO.println
