...
production-inert ELF (one unreachable fixture release; no production release)
three ways with identical 2,160,072-byte SHA-256
...
root, deduplicates the idle ClearWork image, and removes the CreateMarket
decoder's redundant encode round trip. At runtime-input commit `169a1ba`, the
129-file source closure rebuilt three ways to the byte-identical 2,082,320-byte
ELF `193c08723eaefeff9a1c2aa53c9e3feb58960a919fb0bbb7ca5da3bd817aa95b`.
The dependency/syscall, loader-shape, and final-LTO stack gates passed; the
source-scope digest is
`5ec6c61c2134c7e50c220ff0325d454a98b16c3bd0830acd33d28e05d8f29e03`.
This is reproducible artifact evidence, not a seal or release. Bank and
signed-validator results are artifact-specific and do not themselves promote
the artifact. The complete default production-inert SVM profile has now passed
165 tests with zero failures against
that exact ELF. The separately compiled `non-production-mock-source` profile
passed 168 tests with zero failures against its distinct 2,110,240-byte ELF
`342fdfcb0e6b0836ec9ecd492d9a8577c87f493b49fd8c35e3cb47c448d06112`.
That second result is laboratory source/value evidence, never production source
evidence. The keeper gate against that mock profile completed 24 permissionless
actions plus one owner-signed action, survived a deliberate kill after four
actions by resuming from chain state, removed every currently safe leaf, and
reached the same fail-closed `Blocked` state across another fresh restart while
retaining the CLEARED Epoch, Window, and epoch funding ledger. Conservation was
exact and `CloseGeneralEpoch` was never attempted. This remains local
engineering evidence, not a release or global liveness claim.
...
| Pooled custody and cash exit | exits **SBF-EXECUTED**; production value admission **STOP** | Endow is the sole inbound token boundary; Split/Merge/internal redemption are pooled-accounting reclassifications; exact unreserved `WithdrawCash` performs Hoard-to-owner Token-2022 transfer. Since `cfea8e8`, default Endow authenticates Terms/SourceSpec and unregistered production-shaped specs refuse `SourceReleaseUnavailable` (`0x79`) before owner allocation or Token-2022 CPI. The default artifact's sole registered row is a fabricated off-curve fixture that cannot be deployed on a real cluster. | Mock-source Endow success requires a distinct `non-production-mock-source` ELF. No production source release or value-bearing market is admitted by the sealed default artifact. Full venue settlement remains open. |
...
| Source admission and archive | **HISTORICAL PRE-R2 ROW; superseded by §4** | A 292-byte content-addressed SourceSpec and 2,560-byte one-window archive bind provider/parser/deployment/spec/grid/window/lineage and a sealed receipt. Resolve derives and authenticates their canonical PDAs and requires the compatibility projection to equal the sealed archive. The later R2 V2 path now has runtime codecs, loader/Instructions/Clock authentication, public create/append/seal routes, and the local-real provider-to-resolution campaign summarized in §4. | Production identities and the semantic feed profile remain unfrozen. The default artifact contains one unreachable fixture release and no production release; mock and captured-Pyth success use distinct non-production profiles. |
...
1. **Native mode binding — repaired at runtime; joined evidence landed;
   ...
   **SBF-EXECUTED** in local in-process banks under both ELF campaigns:
   ...
   constructs); the default production-inert ELF campaign asserts the exact
