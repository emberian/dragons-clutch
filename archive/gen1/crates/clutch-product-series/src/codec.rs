...
pub(crate) const LEGACY_PAYOUT_V3_MAGIC: [u8; 8] = *b"DCPAYTV3";
pub(crate) const LEGACY_TEMPLATE_V3_MAGIC: [u8; 8] = *b"DCTMPLV3";

pub(crate) fn reject_legacy_numeric_fallback(input: &[u8]) -> Result<()> {
    if input.starts_with(&LEGACY_PAYOUT_V3_MAGIC) || input.starts_with(&LEGACY_TEMPLATE_V3_MAGIC) {
