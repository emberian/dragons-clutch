...
ProfileV4/BundleV7 input graph, and an optional bounded exact-market search. It
...
```json
{
  ...
  "bundleInputs": {
    ...
    "seriesFundingQuoteV6BytesHex": "<624 canonical bytes>",
    "seriesAttachmentPlanV6BytesHex": "<112 canonical bytes>",
...
BundleV7 artifact PDA is derived.
...
- reopens the SeriesPlanV5, FundingTermsV2, QuoteV6, and AttachmentV6 joins;
  and
- assembles the sole current `CompiledProductSeriesBundleV7` graph. The graph
...
`compiledProductSeriesBundleV7.identities` contains exactly:
...
the recomputed BundleV7 identity under the ReleaseV2-owned Product program.
...
- `claims` with `uniquePrice`, `fairValue`, and `optimalClearing` all exactly
  false;
- exact Market, Product Terms, native basis, price, and BundleV7 bindings;
...
- a 176-byte sidecar bound to the kind-68, global-context BundleV7 identity.
...
program address, the sixteen exposed identities, the native-basis/BundleV7
...
Neither a compiler response, a BundleV7 PDA string, nor an operatord index row
...
- the recomputed BundleV7 body, typed identity, kind-68 PDA, and bump; and
