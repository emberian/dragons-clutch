  ...
  const renderKeeper = (snapshot) => {
    ...
    for (const action of actions) {
      ...
      facts.append(
        ...
        definition("Current material contract", action.directContract ? `${action.directContract.schema} · ${action.directContract.branch}` : action.fractionalContract ? `${action.fractionalContract.schema} · ${action.fractionalContract.geometry}` : action.failureContract ? `${action.failureContract.schema} · ${action.failureContract.geometry}` : action.payloadVariant ? `exact Dealer target-${action.payloadVariant.payloadDiscriminator}` : "release-bound family contract"),
