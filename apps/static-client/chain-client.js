...
(function (root) {
  ...
  const DECODER_SET = "dragons-clutch/canonical-account-decoders/v8-dealer-v3-terminal";
  ...
  const FAILURE_COORDINATE = Object.freeze({ familyTag: "78", familyVersion: "1", family: "recovery", flow: "failure-recovery", messageVersion: "v0", lookupTables: 1 });
  const GENERAL_ACTION39_COORDINATE = Object.freeze({ familyTag: "74", familyVersion: "1", family: "general", flow: "general-v2-settlement", messageVersion: "v0", lookupTables: 1 });
  const FAILURE_ACTION11_ROLES = Object.freeze([
    "market-lifecycle-root", "series-market-link", "series-funding-v5", "failure-admission-root", "failure-runtime-root", "failure-interval-cell", "failure-interval-history", "series-registry-v4",
    "registry-program", "registry-program-data", "registry-release-v2", "capability-profile-v4", "compiler-bundle-v7", "funding-quote-v6", "series-plan-v5", "series-funding-terms-v2",
    "product-template-v4", "native-claim-basis-v1", "recovery-policy-v1", "price-measure-policy-v1", "market-genesis-v2", "attachment-plan-v6", "market-instance-v2", "source-release-v2",
    "source-adapter-program", "source-adapter-program-data", "source-parser-program", "source-parser-program-data", "source-parser-config", "source-spec", "source-work-schedule", "source-occurrence",
    "source-window", "source-statistic-key", "source-summary", "source-window-seal", "source-statistic-result", "source-result-lineage", "source-handoff-receipt", "source-work-receipt",
    "failure-liveness-policy", "failure-recovery-compartment", "keeper", "recovery-refund-owner"
  ]);
  const FAILURE_ACTION11_WRITABLE_ROLES = Object.freeze(new Set(["failure-runtime-root", "failure-interval-cell", "failure-recovery-compartment", "keeper"]));
  const FAILURE_ACTION13_ROLES = Object.freeze([
    "market-lifecycle-root", "series-market-link", "series-funding-v5", "failure-admission-root", "failure-runtime-root", "failure-interval-cell", "failure-interval-history", "series-registry-v4",
    "registry-program", "registry-program-data", "registry-release-v2", "capability-profile-v4", "compiler-bundle-v7", "funding-quote-v6", "failure-liveness-policy", "failure-recovery-compartment"
  ]);
  const FAILURE_ACTION13_WRITABLE_ROLES = Object.freeze(new Set(["series-market-link", "failure-runtime-root", "failure-interval-cell", "failure-interval-history"]));
  ...
  const GENERAL_ACTION39_FIXED_ROLE_NAMES = Object.freeze([
  ...
  const DEALER_COMMON_ROLES = Object.freeze([
    ...
    dealerRole("liveness-payer", false, true), dealerRole("position-rent-payer", false, true), dealerRole("replay-rent-payer", false, true), dealerRole("obligation-rent-payer", false, true), dealerRole("neutral-lamport-sink", false, true), dealerRole("clock-sysvar", false, false), dealerRole("rent-sysvar", false, false), dealerRole("system-program", false, false), dealerRole("dealer-series-obligation-v3", false, true), dealerRole("realm", false, false), dealerRole("collateral-profile-v2", false, false), dealerRole("collateral-policy-v2", false, false), dealerRole("collateral-token-program", false, false), dealerRole("collateral-token-programdata", false, false), dealerRole("market-binding-v2", false, false), dealerRole("market-runtime-v3", false, false), dealerRole("market-instance-v2", false, false), dealerRole("hoard-v2", false, false)
  ]);
  const DEALER_ACTIVE_TAIL = Object.freeze([dealerRole("claim-ledger-v3", false, true), dealerRole("resolution-v5", false, false), dealerRole("fractional-policy-v3", false, false), dealerRole("fractional-ledger-v1", false, true), dealerRole("facility-credit-v2", false, true), dealerRole("product-market-root-v3", false, true), dealerRole("series-market-link-v3", false, true)]);
  const DEALER_UNUSED_TAIL = Object.freeze([dealerRole("claim-ledger-v3", false, false), dealerRole("dealer-future-credit-funding-v1", false, true), dealerRole("product-market-root-v3", false, true), dealerRole("series-market-link-v3", false, true)]);
  ...
  const validateGeneralAction39MaterialContract = (row, coordinate, accountRoles, stateSelection, transactionDraft, signerRequirements, configuration) => {
    if (coordinate.action !== "initialize-general-settlement-root-v5"
        || row.semanticOwnerConstructor !== "dragons-clutch/operator/general-action39-material/v1"
        || accountRoles.length < 50
        || accountRoles.length > 53) {
      throw new Error("General action 39 differs from its current bounded semantic-owner material contract.");
    }
    const exactRoles = [
      ...GENERAL_ACTION39_FIXED_ROLES,
      ...Array.from({ length: accountRoles.length - GENERAL_ACTION39_FIXED_ROLES.length }, (_, page) => directRole(`order-page-v5-${page}`, false, false))
    ];
    if (!directRolesMatch(accountRoles, exactRoles)
        || new Set(accountRoles.map((role) => role.address)).size !== accountRoles.length) {
      throw new Error("General action 39 does not have 49 distinct fixed roles followed by one to four exact retained-feed pages.");
    }
    if (stateSelection.action !== coordinate.action
        || stateSelection.account !== accountRoles[2].address
        || transactionDraft.driverAccount !== accountRoles[2].address
        || stateSelection.cursor.lane !== "general-settlement"
        || stateSelection.cursor.phase !== "39"
        || BigInt(stateSelection.cursor.item) === 0n) {
      throw new Error("General action 39 does not retain its selected AdmissionNode driver and positive Candidate ordinal cursor.");
    }
    const expectedDependencies = accountRoles
      .filter((_, roleIndex) => roleIndex !== 2)
    ...
    if (stateSelection.dependencies.length !== expectedDependencies.length
      ...
      throw new Error("General action 39 selection differs from its complete distinct semantic-owner role tuple.");
    }
    if (transactionDraft.messageVersion !== "v0"
      ...
      throw new Error("General action 39 is not one current v0 transaction with one finalized lookup table.");
    }
    const owner = transactionDraft.semanticOwners[0];
    if (owner.package !== "clutch-general-v2-contract+clutch-general-v2-runtime+clutch-product-series"
        || owner.schema !== "dragons-clutch/operator/general-action39-material/v1"
        || owner.releaseSha256 !== configuration.release.releaseManifestSha256) {
      throw new Error("General action 39 differs from its exact current semantic owner.");
    }
    const binding = transactionDraft.registryBindings[0];
    if (decimal(binding.centralAction, "General action-39 central action", 255n).toString() !== "39") throw new Error("General action 39 does not retain its frozen central allocation.");
    if (transactionDraft.exactEquations.length !== 1
        || transactionDraft.exactEquations[0].name !== "finalized absence proves zero preexisting fresh-account rent principal"
        || transactionDraft.exactEquations[0].unit.kind !== "lamports"
        || transactionDraft.exactEquations[0].left !== "0"
        || transactionDraft.exactEquations[0].right !== "0") {
      throw new Error("General action 39 differs from its exact finalized-absence rent-principal equation.");
    }
    if (transactionDraft.feePayer !== accountRoles[45].address
        || signerRequirements.length !== 1
        || signerRequirements[0].address !== accountRoles[45].address
        || signerRequirements[0].semanticRoles.length !== 2
        || signerRequirements[0].semanticRoles[0] !== "rent-payer"
        || signerRequirements[0].semanticRoles[1] !== "transaction-fee-payer") {
      throw new Error("General action 39 does not bind its sole signer and transaction payer to role 45.");
    }
    if (row.symbolicPostcondition !== null) throw new Error("General action 39 unexpectedly carries a Direct execution-Clock postcondition.");
    return Object.freeze({
      schema: "dragons-clutch/browser/general-action39-chain-material-contract/v1",
      geometry: `49-fixed-plus-${accountRoles.length - 49}-retained-feed-pages`,
      freshRoleAddresses: Object.freeze(GENERAL_ACTION39_FRESH_INDEXES.map((roleIndex) => accountRoles[roleIndex].address)),
      absenceDisposition: "must-remain-unselectable-until-all-nine-finalized-absence-observations-are-attached",
      holderChoice: null
    });
  };
  ...
  const validateFailureAction11MaterialContract = (row, coordinate, accountRoles, stateSelection, transactionDraft, signerRequirements, configuration) => {
    if (coordinate.familyTag !== FAILURE_COORDINATE.familyTag
        ...
        || coordinate.family !== FAILURE_COORDINATE.family
        || coordinate.localAction !== "11"
        || coordinate.action !== "advance-failure-interval-consensus"
        || row.semanticOwnerConstructor !== "clutch-failure-policy-runtime/current-action11-chain-state-v1") {
      throw new Error("Failure verdict is not the current chain-derived action-11 constructor.");
    }
    const exactRoles = FAILURE_ACTION11_ROLES.map((role) => directRole(role, false, FAILURE_ACTION11_WRITABLE_ROLES.has(role)));
    if (!directRolesMatch(accountRoles, exactRoles)) throw new Error("Failure action 11 differs from its exact 44-role Product/Source/Failure account contract.");
    if (new Set(accountRoles.map((role) => role.address)).size !== accountRoles.length) throw new Error("Failure action 11 contains an unowned account-role alias.");
    if (stateSelection.action !== coordinate.action
        || stateSelection.cursor.lane !== "failure-recovery"
        || stateSelection.cursor.phase !== "1"
        || stateSelection.account !== accountRoles[5].address
        || transactionDraft.driverAccount !== accountRoles[5].address
        || stateSelection.dependencies.some((dependency) => !accountRoles.some((role) => role.address === dependency))) {
      throw new Error("Failure action-11 finalized cell cursor and exact dependency tuple do not agree.");
    }
    if (transactionDraft.messageVersion !== FAILURE_COORDINATE.messageVersion
        || transactionDraft.addressLookupTables.length !== FAILURE_COORDINATE.lookupTables
        || transactionDraft.flows[0] !== FAILURE_COORDINATE.flow
        || transactionDraft.actions[0] !== "advance-failure-interval-consensus-v1") {
      throw new Error("Failure action 11 is not one current v0 lookup-table transaction.");
    }
    const owner = transactionDraft.semanticOwners[0];
    if (owner.package !== "clutch-failure-policy-runtime+clutch-product-series+clutch-source-plane-v3-runtime"
        || owner.schema !== "dragons-clutch/operator/failure-action11-material/v1"
        || owner.releaseSha256 !== configuration.release.elfSha256) {
      throw new Error("Failure action 11 differs from its checked cross-owner semantic material.");
    }
    const binding = transactionDraft.registryBindings[0];
    if (decimal(binding.centralAction, "Failure action-11 central action", 255n).toString() !== "11") throw new Error("Failure action 11 does not retain its exact allocated central action.");
    if (transactionDraft.exactEquations.length !== 1
        || transactionDraft.exactEquations[0].name !== "Recovery work principal funds the exact action-11 keeper reward"
        || transactionDraft.exactEquations[0].unit.kind !== "lamports") {
      throw new Error("Failure action 11 differs from its exact funded-reward conservation equation.");
    }
    if (signerRequirements.length !== 1
      ...
      throw new Error("Failure action-11 fee payer is not the sole disjoint transaction signer.");
    }
    if (row.symbolicPostcondition !== null) throw new Error("Failure action 11 unexpectedly carries a Direct execution-Clock postcondition.");
    return Object.freeze({
      schema: "dragons-clutch/browser/failure-action11-chain-material-contract/v1",
      geometry: "44-chain-derived-roles-plus-disjoint-fee-payer-and-one-finalized-lookup-table",
      provenance: "all-payload-progress-reward-account-and-privilege-facts-derived-from-finalized-product-source-and-failure-state",
      holderChoice: null
    });
  };

  const validateFailureAction13MaterialContract = (row, coordinate, accountRoles, stateSelection, transactionDraft, signerRequirements, configuration) => {
    if (coordinate.familyTag !== FAILURE_COORDINATE.familyTag
        || coordinate.familyVersion !== FAILURE_COORDINATE.familyVersion
        || coordinate.family !== FAILURE_COORDINATE.family
        || coordinate.localAction !== "13"
        || coordinate.action !== "close-failure-interval-consensus-work"
        || row.semanticOwnerConstructor !== "clutch-failure-policy-runtime/current-action13-finite-session-archive-v1") {
      throw new Error("Failure verdict is not the current chain-derived action-13 archive constructor.");
    }
    const exactRoles = FAILURE_ACTION13_ROLES.map((role) => directRole(role, false, FAILURE_ACTION13_WRITABLE_ROLES.has(role)));
    if (!directRolesMatch(accountRoles, exactRoles)) throw new Error("Failure action 13 differs from its exact 16-role finite-session archive contract.");
    if (new Set(accountRoles.map((role) => role.address)).size !== accountRoles.length) throw new Error("Failure action 13 contains an unowned account-role alias.");
    if (stateSelection.action !== coordinate.action
        || stateSelection.cursor.lane !== "failure-recovery"
        || stateSelection.cursor.phase !== "13"
        || stateSelection.account !== accountRoles[5].address
        || transactionDraft.driverAccount !== accountRoles[5].address
        || stateSelection.dependencies.some((dependency) => !accountRoles.some((role) => role.address === dependency))) {
      throw new Error("Failure action-13 finalized cell cursor and exact archive dependency tuple do not agree.");
    }
    if (transactionDraft.messageVersion !== FAILURE_COORDINATE.messageVersion
        || transactionDraft.addressLookupTables.length !== FAILURE_COORDINATE.lookupTables
        || transactionDraft.flows[0] !== FAILURE_COORDINATE.flow
        || transactionDraft.actions[0] !== "close-failure-interval-consensus-work-current-v3") {
      throw new Error("Failure action 13 is not one current v0 finite-session archive transaction.");
    }
    const owner = transactionDraft.semanticOwners[0];
    if (owner.package !== "clutch-failure-policy-runtime+clutch-product-series"
        || owner.schema !== "dragons-clutch/operator/failure-action13-finite-session-archive/v1"
        || owner.releaseSha256 !== configuration.release.elfSha256) {
      throw new Error("Failure action 13 differs from its checked current archive semantic owner.");
    }
    const binding = transactionDraft.registryBindings[0];
    if (decimal(binding.centralAction, "Failure action-13 central action", 255n).toString() !== "13") throw new Error("Failure action 13 does not retain its exact allocated central action.");
    if (transactionDraft.exactEquations.length !== 1
        || transactionDraft.exactEquations[0].name !== "Finite-session archive preserves Recovery custody work principal"
        || transactionDraft.exactEquations[0].unit.kind !== "lamports") {
      throw new Error("Failure action 13 differs from its readonly Recovery-principal equation.");
    }
    if (signerRequirements.length !== 1
        || signerRequirements[0].address !== transactionDraft.feePayer
        || signerRequirements[0].semanticRoles.length !== 1
        || signerRequirements[0].semanticRoles[0] !== "transaction-fee-payer"
        || accountRoles.some((role) => role.signer || role.address === transactionDraft.feePayer)) {
      throw new Error("Failure action-13 fee payer is not the sole disjoint transaction signer.");
    }
    if (row.symbolicPostcondition !== null) throw new Error("Failure action 13 unexpectedly carries a Direct execution-Clock postcondition.");
    return Object.freeze({
      schema: "dragons-clutch/browser/failure-action13-chain-material-contract/v1",
      geometry: "16-chain-derived-archive-roles-plus-disjoint-fee-payer-and-one-finalized-lookup-table",
      provenance: "empty-payload-finite-exhaustion-archive-derived-from-finalized-product-failure-and-liveness-state",
      holderChoice: null
    });
  };
  ...
  const validateActionCapabilities = (raw, configuration, session) => {
    ...
    const actions = raw.actions.map((row, index) => {
      ...
      const failureContract = failureAction11
        ...
        : failureAction13
          ? validateFailureAction13MaterialContract(row, coordinate, accountRoles, stateSelection, transactionDraft, signerRequirements, configuration)
          : null;
      requirePlain(row.freshnessDisposition, `actions[${index}].freshnessDisposition`);
      const observedSlot = positiveDecimal(row.freshnessDisposition.observedSlot, `actions[${index}].freshnessDisposition.observedSlot`);
      const validBeforeSlot = positiveDecimal(row.freshnessDisposition.validBeforeSlot, `actions[${index}].freshnessDisposition.validBeforeSlot`);
  ...
  const validateCanonicalTransactionDraft = (raw, configuration, releaseAdmission, coordinate, selection, roles, signers, index) => {
    ...
    const expectedAction = failureAction11
      ? "advance-failure-interval-consensus-v1"
      : failureAction13
        ? "close-failure-interval-consensus-work-current-v3"
        : coordinate.action;
    if (!Array.isArray(raw.actions) || raw.actions.length !== 1 || raw.actions[0] !== expectedAction || !Array.isArray(raw.flows) || raw.flows.length !== 1 || !Array.isArray(raw.semanticOwners) || raw.semanticOwners.length !== 1 || !Array.isArray(raw.registryBindings) || raw.registryBindings.length !== 1 || !Array.isArray(raw.runtimeAdmissions) || raw.runtimeAdmissions.length !== 1 || raw.runtimeAdmissions[0] !== "release-bound-enabled" || !Array.isArray(raw.exactEquations) || raw.exactEquations.length === 0 || !Array.isArray(raw.addressLookupTables)) throw new Error("callable draft is not one exact release-admitted semantic-owner action.");
    const flowContract = [SOURCE_COORDINATE, STRUCTURED_COORDINATE, DIRECT_COORDINATE, FRACTIONAL_COORDINATE, FAILURE_COORDINATE, GENERAL_ACTION39_COORDINATE].find((contract) => contract.familyTag === coordinate.familyTag && contract.familyVersion === coordinate.familyVersion && contract.family === coordinate.family) || null;
    ...
    if ((coordinate.family === "structured-claim" && binding.centralAction !== null) || ((coordinate.family === "source" || coordinate.family === "direct" || coordinate.family === "fractional" || coordinate.family === "recovery" || coordinate.family === "general") && decimal(binding.centralAction, "draft central action", 255n).toString() !== coordinate.localAction)) throw new Error("draft central action binding differs from its family-owned current contract.");
  ...
  const actionInspectionDisposition = (action, accountByAddress, tipSlot, requestedCommitment, maximumSlotLag) => {
    ...
    if (action.generalContract) {
      ...
      if (freshPresent.length !== 0) {
        return Object.freeze({
          eligible: false,
          kind: "general-action39-fresh-pda-now-present",
          reason: `${freshPresent.length} action-39 fresh PDA role(s) are now present; discard the material and derive the successor state instead.`,
          observedAccounts: String(namedAddresses.filter((identity) => accountByAddress.has(identity)).length),
          staleAccounts: "0",
          validBeforeSlot: action.freshnessDisposition.validBeforeSlot
        });
      }
      const fresh = new Set(action.generalContract.freshRoleAddresses);
      const missingPresentRoles = namedAddresses.filter((identity) => !fresh.has(identity) && !accountByAddress.has(identity));
      if (missingPresentRoles.length === 0) {
        return Object.freeze({
          eligible: false,
          kind: "general-action39-finalized-absence-observations-unavailable",
          reason: "The exact action-39 accounts, cursor, semantic owner, payer, equation, and unsigned v0 material agree, but the browser has no session-bound finalized absence observations for the nine fresh PDA roles and will not infer absence from omission.",
          observedAccounts: String(namedAddresses.length - action.generalContract.freshRoleAddresses.length),
          staleAccounts: "0",
          validBeforeSlot: action.freshnessDisposition.validBeforeSlot
        });
      }
    ...
    if (action.failureContract) {
      const archive = action.coordinate.localAction === "13";
      return Object.freeze({
        eligible: true,
        kind: archive ? "finalized-exact-failure-action13-inspectable" : "finalized-exact-failure-action11-inspectable",
        reason: archive
          ? "Failure action 13 binds the exact 16-role finite exhausted-session archive tuple, empty caller-neutral payload, readonly Recovery-principal equation, disjoint fee payer, finalized lookup table, checked release, and blockhash-free unsigned bytes; it does not pretend to retire the whole Product."
          : "Failure action 11 binds the exact 44-role Product/Source/Failure tuple, finite interval-cell cursor, funded keeper-reward equation, disjoint fee payer, finalized lookup table, checked release, and blockhash-free unsigned bytes; the browser contributes no progress or account authority.",
        observedAccounts: String(observations.length),
        staleAccounts: "0",
        validBeforeSlot: action.freshnessDisposition.validBeforeSlot
      });
