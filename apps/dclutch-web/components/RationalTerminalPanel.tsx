...
export default function RationalTerminalPanel() {
  ...
  async function connectWallet() {
    try {
      ...
      if (payer === '') setPayer(identity.address); if (actor === '') setActor(identity.address);
      setWalletStatus(`${identity.address} · identity only; no transaction or signature request`);
  ...
  return <>
    ...
    <form className="trade-v3-card route-card" onSubmit={(event) => void inspect(event)}>
      ...
      <div className="direct-actions"><button type="button" onClick={() => void connectWallet()}>Connect payer / actor identity</button><button disabled={state.kind === 'loading'}>{state.kind === 'loading' ? 'Reading terminal semantics…' : 'Evaluate chain-derived terminal payout'}</button></div>
