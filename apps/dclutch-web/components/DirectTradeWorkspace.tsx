...
export default function DirectTradeWorkspace() {
  ...
  function buildTransaction(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPlan(next);
      setTradeStatus(`Unsigned v0 transaction constructed from the finalized route: ${next.wireBytes.length} / 1232 bytes, ${next.loadedAddresses} ALT addresses. Its ${next.nativeEvidenceBytes.length}-byte Ed25519 instruction references Trading instruction ${next.tradingInstructionIndex} at exact offsets ${next.nativeMessageOffsets.join(' / ')}.`);
  ...
  return <main className="product-shell trade-v3-shell">
    <ConsoleHeader path="/trade" title="Direct trade" purpose="For traders testing routes: check one trade against live chain state and build its unsigned transactions." />

    <section className="trade-v3-hero"><div><h1>Direct<br /><em>trade.</em></h1><p>You name a market and a trade; the console reads the market, its programs, its route, and its release evidence from the chain, then builds one unsigned transaction pair for that trade. If any part of that state disagrees, nothing is built and the reason is named.</p></div><aside><span>Execution boundary</span><strong>{inspection?.checkedOuter.status === 'checked' ? 'recognized V4-capable release' : 'fail closed'}</strong><p>No checked immutable V4-capable Trading manifest, no executable transaction. Static account lists are never authority.</p></aside></section>
    ...
    <form className="trade-v3-card" onSubmit={buildTransaction}>
      <header><span>02</span><div><h2>Price and authorize one atomic fill</h2><p>Both 172-byte messages carry runtime-u32 outcome coordinates inside the authenticated Trading instruction. The adjacent 222-byte Ed25519 evidence references those bytes by exact offset and assembler-derived instruction index; it never duplicates them. The transaction payer is separate.</p></div></header>
