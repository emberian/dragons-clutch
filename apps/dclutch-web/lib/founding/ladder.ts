...
export const FOUNDING_LADDER_V1: ReadonlyArray<FoundingRungV1> = Object.freeze([
  ...
  Object.freeze({
    ...
    reason: 'Each body is compiled by a first-party Rust encoder — ProductCompilerV2, SourceSpecV1, WindowSpecV1, StatisticSpecV1, RecoveryPolicyV2, SourceMaterialV3, the capacity and manipulation-floor records, and the liability-basis V3 linker. A finalized record lives at an address derived from the hash of its body, so an encoder that is one byte off names a record nobody can publish. The honest browser path is an emitter per encoder, not a second handwritten implementation.',
    lookupTable: false,
  }),
  Object.freeze({
    id: 'rent-credit',
  ...
  Object.freeze({
    ...
    reason: 'The whole 37-account projection, every record authentication, and the exact rent debit are derived in the browser from finalized RPC. It does not fit a packet inline: with the ComputeBudget limit it cannot execute without, the message is 1,242 bytes against a 1,232-byte bound, so it rides the routing table below.',
    lookupTable: true,
  }),
  Object.freeze({
    id: 'custody-bootstrap',
    title: 'DCLTPCB2 — the projected-Custody prestate',
    ...
    transactions: '1 (v0 on a 256 KiB heap frame, 90 account references and 62 distinct locks), plus its lookup table and prefunding',
    status: 'tooling-only',
    builder: 'market.rs · build_projected_custody_bootstrap_v1',
    reason: 'Its coordinates are Custody and Resolution PDAs the browser does not yet emit, and its FundingLedgerV2 plan comes from the protocol operator. It becomes browser-reachable only after those seed and ledger plans have one generated client emitter.',
    lookupTable: true,
  }),
  Object.freeze({
