...
function descriptorFixture(): Uint8Array {
  ...
  bytes.set(Abi.DIRECT_INLINE_ORDINARY_LIFECYCLE_ID_V4, Abi.CAPABILITY_PROGRAM_V4_DERIVATION_POLICY_OFFSET);
  ...
  bytes.set(Abi.LIFECYCLE_SUCCESSOR_SCHEMA_RELEASE_ID, Abi.CAPABILITY_PROGRAM_V4_LIFECYCLE_SCHEMA_OFFSET);
  bytes.set(Abi.DIRECT_INLINE_ORDINARY_LIFECYCLE_ID_V4, Abi.CAPABILITY_PROGRAM_V4_LIFECYCLE_PROGRAM_OFFSET);
...
describe('Direct V3 chain-selected artifacts', () => {
  ...
  it('hostile-decodes the schema-bound V4 successor descriptor', () => {
    ...
    expect(decoded.lifecycle.program).toEqual(Abi.DIRECT_INLINE_ORDINARY_LIFECYCLE_ID_V4);
