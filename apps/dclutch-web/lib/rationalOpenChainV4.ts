...
function canonicalOpenCore(account: RpcAccount): void {
  if (account.data[10] !== 1 || account.data[11] !== 2 || u32(account.data, 12) !== 0 || !isZero(slice(account.data, 320, 32))) {
    throw new Error('Rational open requires an Open, readiness-consumed Core Market without terminal state');
  }
}

function decodeRepresentationGraph(bytes: Uint8Array, descriptor: ReturnType<typeof decodeRationalRepresentationDescriptorV3>): void {
  ...
  const outcomes = u32(bytes, 80); const nodes = u32(bytes, 84); const edges = u32(bytes, 88);
...
type ClaimsAggregate = Readonly<{ revision: bigint; basis: Uint8Array }>;
function decodeClaimsAggregate(bytes: Uint8Array, input: Readonly<{
  ...
  requireNonzero(slice(bytes, 216, 32), 'Claims custody context');
  return Object.freeze({ revision: u64(bytes, 16), basis });
}

function decodePosition(bytes: Uint8Array, aggregate: string, owner: string, basis: Uint8Array, outcomes: number): bigint {
...
function decodeReplay(account: RpcAccount, claims: string, descriptor: Uint8Array, actor: string): bigint {
...
function profile11Physical(profile: Uint8Array, tailCount: number, injected: ReadonlyArray<Meta>, child: ReadonlyArray<Meta>, accounts: ReadonlyMap<string, RpcAccount | null>): ReadonlyArray<Meta> {
...
export async function inspectRationalOpenChainV4(
  ...
  const market = decodeRationalHotCoreV2(marketAddress, marketAccount, coreProgram); canonicalOpenCore(marketAccount);
  if (market.phase !== 1 || market.registry !== registry || fixed[Hot.HOT_RENT_SYSVAR_ACCOUNT_V3]?.address !== RENT_SYSVAR_ID
      || fixed[Hot.HOT_INSTRUCTIONS_SYSVAR_ACCOUNT_V3]?.address !== INSTRUCTIONS_SYSVAR) throw new Error('Hot fixed frame differs from the Open Market lifecycle or runtime sysvars');
  const payerAccount = required(first.accounts, payer, 'payer'); const actorAccount = required(first.accounts, actor, 'actor');
  for (const [account, field] of [[payerAccount, 'payer'], [actorAccount, 'actor']] as const) {
  ...
  const setRaw = await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
    ...
    PROGRAM_SET_SCHEMA_V2, selection.programSet, 'Rational open ProgramSetV2');
  const capabilitySelection = selectRationalHotCapabilityV4(setRaw.data, actionSelector(input.action));
  if (!same(capabilitySelection.schema, CAPABILITY_PROGRAM_V4_SCHEMA)) throw new Error('Rational open selector does not choose CapabilityProgramV4');
  const capabilityRaw = await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
    ...
    capabilitySelection.schema, capabilitySelection.digest, 'Rational open CapabilityProgramV4');
  ...
  if (!same(capability.kind, selection.kind) || !same(capability.configSchema, TOKEN_BEHAVIOR_SELECTION_SCHEMA_ID_V2)
      || !same(capability.requestSchema, OPEN_REPRESENTATION_HOT_REQUEST_SCHEMA_ID_V3)
      || !same(capability.rootSchema, manifest.rootSchema) || !same(capability.derivation, manifest.derivation)
      || !same(capability.capacity, manifest.capacity) || rootAccount.data.length !== 232 + capability.rootStateBytes) {
    throw new Error('Rational open CapabilityV4 does not join the selected manifest/root/config/request family');
  }
  ...
  for (let index = 0; index < capability.artifacts.length; index += 1) {
    ...
    artifacts.push(await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
      fixed[raw]?.address ?? '', fixed[raw + 1]?.address ?? '', artifact.schema, artifact.digest, `Rational open artifact ${index}`));
  }
  ...
  decodeRepresentationGraph(graphRaw.data, descriptor);
  ...
  const productRaw = await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
  ...
  const domainRaw = await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
  ...
  const portfolioRaw = await authenticateFinalizedRationalHotRecordV4(client, first.accounts, registry,
  ...
  const dynamicObservation = await acquireRationalHotAccountsV4(client, dynamic, graphObservation.slot);
  const accounts = new Map([...first.accounts, ...descriptorObservation.accounts, ...graphObservation.accounts, ...dynamicObservation.accounts]);
  ...
  const claims = decodeClaimsAggregate(aggregateAccount.data, { market: marketAddress, releaseSet: market.releaseSet, registry,
  ...
  const replayRevision = decodeReplay(replayAccount, activation.claims, descriptorId, actor);
  ...
  if (actorPosition !== null) {
    ...
    actorPositionRevision = decodePosition(account.data, aggregate.toBase58(), actor, claims.basis, product.outcomeCount);
  }
  ...
  for (const row of derived) {
    ...
    const positionRevision = decodePosition(positionAccount.data, aggregate.toBase58(), row.owner.toBase58(), claims.basis, product.outcomeCount);
  ...
  const physical = profile11Physical(artifacts[0]?.data ?? new Uint8Array(), selectedAction(input.action) ? 0 : product.outcomeCount, injected as ReadonlyArray<Meta>, child, accounts);
  ...
  return Object.freeze({ observedSlot: dynamicObservation.slot, action: input.action, payer, actor, market: marketAddress,
    ...
    displayDecimals: receiptMint.displayDecimals, descriptorId, tokenBehaviorDigest: selection.config,
    capabilityDigest: capabilitySelection.digest, rootDigest: await sha256(rootAccount.data), family, fixedAccounts: fixed,
    physicalClaimsAccounts: physical, lookupTable, executionStatus: 'blocked',
