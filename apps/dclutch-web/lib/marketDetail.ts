...
import { type MarketPhase, type RequiredBackingBasis } from './decoders';
 ...
 */
export const MARKET_PHASE_MEANING_V1: Readonly<Record<MarketPhase, string>> = Object.freeze({
  Founding: 'The Market root and its immutable identities exist. No claim has been issued, no collateral is held, and no settlement can be written yet.',
  Open: 'The Market admits complete-set split and merge against its Hoard. No terminal settlement is written, so no claim can be redeemed.',
  Resolved: 'Exactly one outcome is frozen as winning. Split and merge are closed; redemption of resolved claims is open at one collateral atom per winning claim atom.',
  Retiring: 'The Market is winding down. It admits no new issuance; a resolved Market still admits redemption, and an unresolved one holds no economic state at all.',
  Retired: 'No outstanding direct child and no economic state remain. The Market admits no further action.',
});

/** What the exact required backing is measured against at this phase. */
export const REQUIRED_BACKING_MEANING_V1: Readonly<Record<RequiredBackingBasis, string>> = Object.freeze({
  'maximum-outcome-supply': 'largest outcome supply — while no outcome is settled, every outcome could still be the one that pays',
  'winning-outcome-supply': 'winning outcome supply — settlement is terminal, so only winning claims can still be paid',
});

export function marketPhaseMeaningV1(phase: MarketPhase): string {
  return MARKET_PHASE_MEANING_V1[phase];
}

export function requiredBackingMeaningV1(basis: RequiredBackingBasis): string {
